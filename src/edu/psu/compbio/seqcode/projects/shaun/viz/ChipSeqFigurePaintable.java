...
public class ChipSeqFigurePaintable extends FigurePaintable{
	...
	private HashMap<String, AbstractPaintable> exptPainters = new HashMap<String, AbstractPaintable>();
	...
	public ChipSeqFigurePaintable(FigureOptions opts){
		...
		for(String t : exptNames){
			...
			if(options.experiments.get(t).preFormattedDataFile != null){
				...
				DiffSeqExptPaintable dp = new DiffSeqExptPaintable(options.genome, options.gRegion, options.experiments.get(t).preFormattedDataFile, options.experiments.get(t).diffWinWidth, options.experiments.get(t).diffWinStep, options.experiments.get(t).scaling );
				dp.setSigPerBaseMax(options.experiments.get(t).pbMax);
				dp.setCtrlPerBaseMax(options.experiments.get(t).pbMax);
				...
				if(options.experiments.get(t).isDiff && options.experiments.get(t).baseExpt != null){
					...
					//Get the reads
					List<Region> reads = getReads(loc, options.gRegion, options.experiments.get(t).pbMax, options.experiments.get(t).pairedReads);
					List<Region> pairs = options.experiments.get(t).pairedReads ? getPairs(loc, options.gRegion, options.experiments.get(t).pbMax, options.experiments.get(t).pairedReads) : null;
					
					//Initialize the ThinOverlapPaintable
					ThinOverlapPaintable tp = new ThinOverlapPaintable(options.gRegion, sites, reads.iterator(), pairs, inters);
	...
	public void paintItem (Graphics g, int x1, int y1, int x2, int y2){
		...
		for(String t : exptNames){
			...
			p.paintItem(g, leftBound, baseLine-offset-options.experiments.get(t).exptTrackHeight, rightBound, baseLine-offset);
		...
		for(int i=0; i<geneSetNames.size(); i++){
			offset += drawGenes(g2d, x1, baseLine+offset+options.geneHeight, x2, geneSets.get(i), options.drawGeneLabels, geneSetNames.get(i));
		}
	...
	private void drawMotifs(Graphics2D g2d, int currLine, WeightMatrix wm, double thres){
		...
		for(int z=0; z<options.gRegion.getWidth(); z++){
			...
			if(currScore>=thres){
				motifLocs.add(new StrandedRegion(options.genome, chr, rstart+z, rstart+z+wm.length(), profiler.getMaxStrand(z)));
			}
		}
