...
public class EMStepPlotter {
	...
	public static void execute(String outName, Region reg, int [][] mu, double [][] pi, double[][] condPrior, int numConditions, int numComps, int iteration, int trimLeft, int trimRight){
	    ...
	    if(condPrior!=null){
		    ...
		    for(int c=0; c<numConditions; c++)
		    	for(int x=0; x<reg.getWidth(); x++)
		    		if(condPrior[c][x]>maxPrior)
		    			maxPrior = condPrior[c][x];
		    if(maxPrior>1)
		    	priorMax = maxPrior;
		    ...
		    for(int c=0; c<numConditions; c++){
		    	...
		    	for(int x=0; x<rWidth; x++){
		    		int pos = x+trimLeft;
		    		if(condPrior[c][pos]>0){
		    			float roffset = x;
		    			float toffset = (roffset/rWidth)*trackWidth;
		    			double priorHeight =(condPrior[c][pos]/priorMax)*trackHeight;  
