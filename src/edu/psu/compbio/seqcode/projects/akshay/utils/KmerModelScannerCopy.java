...
public class KmerModelScannerCopy {
	...
	protected int Kmin; // Length of the smallest kmer in the model
	protected int Kmax; // Length of the largest kmer in the model
	...
	public void setKmin(int K){Kmin=K;}
	public void setKmax(int K){Kmax=K;}
	...
	public static void main(String[] args) throws IOException{
		...
		int Kmax = ap.hasKey("kmax") ? Args.parseInteger(args, "kmax", 7) : 4;
		...
		if(Kmin != Kmax){
			...
			ws = new double[(int)Math.pow(4, Kmin)];
		}
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if(!isPair || !words[0].contains("-")){
            	...
            	int ind = 0;
            	for(int k=Kmin; k<words[0].length(); k++){
            		ind += Math.pow(4, k);
            	}
            	ws[ind + kind] = Double.parseDouble(words[1]);
            	...
            	pws = new double[(int) Math.pow(4, Kmin)][(int) Math.pow(4, Kmin)];
        ...
        scanner.setKmin(Kmin);
        scanner.setKmax(Kmax);
