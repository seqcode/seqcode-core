...
/**
 * Simple package testing class to cache a set of experiments and print total
 * hit counts.
 * 
 * Unit testing: java -Xmx2G
 * org.seqcode.deepseq.utils.SeqExperimentLoadingExample --species "Mus
 * musculus;mm9" --rdbexptC1 "ES2MN Day0(iCdx2.V5+Dox) iCdx2.V5
 * Ainv15_iCdx2.V5;1;bowtie_unique" --fixedpb 100000 (--nocache) Result: C1:rep1
 * 4700200.0 4700200.0
 * 
 * @author mahony
 *
...
public class SeqExperimentLoadingExample {
	...
	public static void main(String[] args) {
		// GenomeConfig and ExptConfig read various command-line options in
		// order to load requested genome and sequencing datasets (resp.)
		...
		// Everything required to get the data requested from the command-line
		// args is now in the *Config objects
	...
	/**
	 * The functionality here is trivial - just counts the number of hits in the
	 * experiment. The point is to demonstrate iteration over chromosomes and to
	 * test read loading per chromosome.
	 */
	public void execute() {
		// ExperimentManager initializes the data structures associated with the
		// experimental data pointed to on cmd line
		...
		for (ControlledExperiment expt : manager.getReplicates()) {
			...
			while (chroms.hasNext()) {
				...
				for (int x = currentRegion.getStart(); x <= currentRegion.getEnd(); x += MAXSECTION) {
					...
					// Get hits in this subregion from the current experiment's
					// signal track
	...
	protected void close() {
		// Always close database connections (if applicable) via a call to
