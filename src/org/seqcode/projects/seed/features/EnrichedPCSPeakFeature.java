...
public class EnrichedPCSPeakFeature extends EnrichedPeakFeature {
	...
	protected float[] tagCountsByBase; // Per condition tag counts in enriched
										// region, indexed by ACGT
	protected float[] bubbleSequenceCounts; // Counts of the 4 bases in the
											// bubble region, indexed by ACGT

	public EnrichedPCSPeakFeature(Region c, Point p, float[] perSamplePosCounts, float[] perSampleNegCounts,
			float sigCount, float ctrlCount, double score, float peakScore, float[] bubbleTags, float[] bubbleBases)
			throws Exception {
	...
	public String toString() {
		return new String(peak.toString() + "\t" + coords.toString() + "\t" + String.format("%.1f", signalCount) + "\t"
				+ String.format("%.1f", controlCount) + "\t" + String.format("%.5e", score) + "\t"
				+ String.format("%.3f", peakScore) + "\t" + String.format("%.5f", bubbleIndex()));
	}

	public String toGFF() {
		return new String(peak.getChrom() + "\tSEEDS\tfeature\t" + peak.getLocation() + "\t" + peak.getLocation() + 1
				+ "\t.\t" + coords.getStrand() + "\t.\t" + "Note=" + "Score:" + String.format("%.5e", score)
				+ ",PeakScore:" + String.format("%.3f", peakScore) + ",Signal=" + signalCount + ",Control="
				+ controlCount + ",BubbleIndex=" + bubbleIndex());
	}
