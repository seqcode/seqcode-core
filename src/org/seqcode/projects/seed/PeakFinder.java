...
public class PeakFinder extends DomainFinder {
	...
	public Map<ExperimentCondition, List<Feature>> postProcess() {
		...
		// Summarize
		for (ExperimentCondition cond : manager.getConditions())
			System.err.println(cond.getName() + "\t" + features.get(cond).size() + " peaks\t"
					+ signifFeatures.get(cond).size() + " peaks below threshold.");
	...
	public class PeakFinderThread extends DomainFinderThread {
		 ...
		 * comparing tag counts to a background model, and find the peaks of
		 * enrichment within these domains using a choice of methods.
		 * 
		 * @param subRegion
		...
		public Map<ExperimentCondition, List<Feature>> findFeatures(Region subRegion) {
			// Get the enriched domains using the functionality from the
			// superclass (DomainFinderThread)
		 ...
		 * overlapping hits. - Counts hits in each feature, per sample - Finds
		 * the peak position in the domain according to a choice of methods.
		 * 
		 * Specifically, the procedure characterizes enriched domains using the
		 * functionality in the superclass (DomainFinder), and then finds peaks
		 * using either: - Point of maximum density - Point of balance between
		 * positive and negative strand densities - Point of maximum likelihood,
		 * given an empirical read distribution (stranded)
		 * 
		 * @param currFeatures
		 * @param current
		 *            region
		...
		protected Map<ExperimentCondition, List<EnrichedFeature>> processDomains(
			...
			for (ExperimentCondition currCondition : manager.getConditions()) {
				for (EnrichedFeature currDomain : currFeatures.get(currCondition)) {
					...
					// else if(sconfig.getPeakFindingApproach() ==
					// PeakFindingMethod.TAGDISTRIBUTION &&
					// sconfig.getEventModel()!=null)
					// peak = findPeakWithBindingModel(fHitsPos, fHitsNeg,
					// currDomain, currCondition);
						...
						System.err.println("PeakFindingMethod " + sconfig.getPeakFindingApproach()
								+ " not defined. Using max density.");
						peak = findPeakMaxHit(fHitsPos, fHitsNeg, (EnrichedFeature) currDomain, currCondition);
					}
					if (peak != null)
		 ...
		 */
		protected EnrichedPeakFeature findPeakMaxHit(Map<Sample, List<StrandedBaseCount>> fHitsPos,
				Map<Sample, List<StrandedBaseCount>> fHitsNeg, EnrichedFeature domain,
				ExperimentCondition currCondition) {
			...
			try {
				epf = new EnrichedPeakFeature(domain.getCoords(), p, domain.getSampleCountsPos(),
						domain.getSampleCountsNeg(), domain.getSignalCount(), domain.getControlCount(),
						domain.getScore(), max);
			} catch (Exception e) {
			}
			return (epf);
		...
		/**
		 * Find the peak locations based on left-right balance of
		 * forward/reverse reads. Motivated by SISSRS.
		 * 
		 * @param hits
		 * @param coords
		 * @return
		 */
		protected EnrichedPeakFeature findPeakLRBalance(Map<Sample, List<StrandedBaseCount>> fHitsPos,
				Map<Sample, List<StrandedBaseCount>> fHitsNeg, EnrichedFeature domain,
				ExperimentCondition currCondition) {
			...
			for (int s = 0; s <= domain.getCoords().getWidth(); s++) {
				...
				for (int r = domain.getCoords().getWidth(); r > s; r--) {
					right += reverse[r];
				}
		...
		/*
		 * protected EnrichedFeature findPeakWithBindingModel(Map<Sample,
		 * List<StrandedBaseCount>> fHitsPos, Map<Sample,
		 * List<StrandedBaseCount>> fHitsNeg, EnrichedFeature domain,
		 * ExperimentCondition currCondition, BindingModel model){ float [] p =
		 * new float[domain.getCoords().getWidth()+1]; for(int s=0;
		 * s<=domain.getCoords().getWidth(); s++){p[s]=0; }
		 * 
		 * int maxPos=0; float maxScore=0; //Populate the probability landscape
		 * - need to correct for strandedness of events here. for(Sample s :
		 * currCondition.getSignalSamples()){ for(StrandedBaseCount h :
		 * fHitsPos.get(s)){ int fivePrime =
		 * getShifted5Prime(h)-domain.getCoords().getStart(); if(fivePrime>=0 &&
		 * fivePrime<p.length){ for(int i=Math.max(model.getMin()+fivePrime, 0);
		 * i<=Math.min(domain.getCoords().getWidth(), fivePrime+model.getMax());
		 * i++) p[i]+=model.probability(i-fivePrime); } } for(StrandedBaseCount
		 * h : fHitsNeg.get(s)){ int fivePrime =
		 * getShifted5Prime(h)-domain.getCoords().getStart(); if(fivePrime>=0 &&
		 * fivePrime<p.length){ for(int i=Math.max(fivePrime-model.getMax(), 0);
		 * i<=Math.min(domain.getCoords().getWidth(), fivePrime-model.getMin());
		 * i++) p[i]+=model.probability(fivePrime-i); } } } for(int k=0;
		 * k<=domain.getCoords().getWidth(); k++)
		 * if(p[k]>maxScore){maxScore=p[k]; maxPos=k;}
		 * 
		 * Point pt = new Point(gen, domain.getCoords().getChrom(),
		 * domain.getCoords().getStart()+maxPos); EnrichedPeakFeature epf=null;
		 * try { epf = new EnrichedPeakFeature(domain.getCoords(), pt,
		 * domain.getSampleCountsPos(), domain.getSampleCountsNeg(),
		 * domain.getSignalCount(), domain.getControlCount(), domain.getScore(),
		 * maxScore); } catch (Exception e) {} return(epf); }
		 */
	}
