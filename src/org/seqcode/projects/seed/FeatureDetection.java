...
/**
 * FeatureDetection is the parent class for all enrichment-based peak/domain
...
public abstract class FeatureDetection {
	...
	protected Map<ExperimentCondition, List<Feature>> features; // The
																// discovered
																// features
																// (defined per
																// condition)

	protected boolean strandedEventDetection = false; // Strand aware event
														// detection?
	...
	/**
	 * Post-process the features, and print output. Obviously, assume that all
	...
	/**
	 * The execute method should instantiate the FeatureDetection threads to
	 * perform per-region analysis, and should then collect the Features
	 * discovered in each thread for any necessary post-processing. This method
	 * should also take care of any output printing, etc.
	 * 
	...
	public Map<ExperimentCondition, List<Feature>> execute() {
		...
		// Threading divides analysis over entire chromosomes. This approach is
		// not compatible with file caching.
	...
	/*
	 * Java doesn't allow me to impose this, but it would also be good practise
	 * to include in each subclass a static method that returns a String
	 * describing the command-line arguments. The main method in the subclass
	 * can then draw on that String if no args are provided.
	 */
	...
	/**
	 * Print the features to output files. Prefix is defined by SEEDConfig.
	 * Suffix is defined depending on what the lists are.
	 * 
	 * @param f
	 */
	protected void printEventsFile(Map<ExperimentCondition, List<Feature>> fLists, String suffix) {
		try {
			for (ExperimentCondition cond : manager.getConditions()) {
				...
				String filename = sconfig.getOutputParentDir() + File.separator + sconfig.getOutBase() + condName
						+ suffix;
	 ...
	 * @param threshold
	 * @param below
	 *            : accept only features below threshold
	 * @return
	 */
	protected Map<ExperimentCondition, List<Feature>> filter(Map<ExperimentCondition, List<Feature>> features,
			double threshold, boolean below) {
	...
	public abstract class FeatureDetectionThread implements Runnable {
		...
		// Hits maintained in separate lists per strands - it's easier to do
		// feature trimming & quantification this way
		protected Map<Sample, List<StrandedBaseCount>> hitsPos; // Lists of
																// positive
																// strand tags
																// in the
																// current
																...
																// Indexed by
																// Sample.
		protected Map<Sample, List<StrandedBaseCount>> hitsNeg; // Lists of
																// negative
																// strand tags
																// in the
																// current
																...
																// Indexed by
																// Sample.
		protected float[][][] landscape = null; // Binned tag density in the
												// current region after shifting
												// and extending. Indexed by
												// Sample, base, strand
		protected Map<ExperimentCondition, BackgroundCollection> conditionBackgrounds; // Backgrounds
																						// are
																						// in
																						// here
																						// for
		...
		protected int shift = sconfig.getTagShift(), hit3Extend = sconfig.getTag3PrimeExtension(),
				hit5Extend = sconfig.getTag5PrimeExtension();
		 ...
		 * 
		 * @param regs
		 *            : regions that this thread examines
		 */
		public void run() { // declare as final if not overloaded (guarantees
							// that landscapes is correctly set)
			...
			for (Region currentRegion : runRegions) {
				...
				for (int x = currentRegion.getStart(); x <= currentRegion.getEnd(); x += sconfig.MAXSECTION) {
					...
					for (Sample samp : manager.getSamples()) {
						synchronized (manager) {// hitCache requires thread
												// safety
							...
							Collections.sort(sampHitsP);
							Collections.sort(sampHitsN); // This might be
															// pointless - the
															// hits should be
															// sorted in the
															// cache already
					...
					// makeHitLandscape & make GaussianLandscape populate the
					// landscape data structure
					// execute can therefore assume that these structures are
					// updated, unless run() is overloaded
					if (sconfig.getBinWidth() == 1 && sconfig.getTagGaussSigma() > 0)
						makeGaussianLandscape(hitsPos, hitsNeg, currSubRegion, sconfig.getTagGaussSigma(),
					...
					// TODO: Progress print (replace with some kind of
					// percentage update)
					System.err.print(".");
				}
			}
		...
		/**
		 * The core functionality in any event finder should be implemented in
		 * this method. Assumes hits, landscape has been initialized
		 * 
		 * @param r
		 *            : Region to analyze
		 * @return : Lists of Features in each ExperimentCondition in the
		 *         current region
		 */
		public abstract Map<ExperimentCondition, List<Feature>> findFeatures(Region r);

		/**
		 * Makes integer arrays corresponding to the read landscape over the
		 * current region. Tags are semi-extended by half the binWidth to
		 * account for the step, and may also be shifted or extended here,
		 * depending on the event detection strategy No needlefiltering here as
		 * that is taken care of during tag loading (i.e. in Sample)
		 * 
		 * @param hits
		 *            : Lists of StrandedBaseCounts, indexed by Sample (sorted
		 *            within each sample)
		 * @param currReg
		 * @param binWidth
		 * @param binStep
		 */
		protected void makeHitLandscape(Map<Sample, List<StrandedBaseCount>> hitsPos,
				Map<Sample, List<StrandedBaseCount>> hitsNeg, Region currReg, int binWidth, int binStep) {
		...
		/**
		 * Makes integer arrays corresponding to the Gaussian-smoothed read
		 * landscape over the current region. This only operates at single-bp
		 * resolution. Tags are gaussian smoothed over the landscape, and may
		 * also be shifted, depending on the event detection strategy No
		 * needlefiltering here as that is taken care of during tag loading
		 * (i.e. in Sample)
		 * 
		 * @param hits
		 *            : Lists of StrandedBaseCounts, indexed by Sample
		 * @param currReg
		 * @param gaussSigma:
		 *            Gaussian sigma (std dev)
		 * @param gaussWidth:
		 *            width over which to 'extend' each tag
		 */
		protected void makeGaussianLandscape(Map<Sample, List<StrandedBaseCount>> hitsPos,
				Map<Sample, List<StrandedBaseCount>> hitsNeg, Region currReg, float gaussSigma, int gaussWidth) {
			...
			for (Sample samp : manager.getSamples()) {
				for (int i = 0; i <= length; i++)
					for (int s = 0; s <= 1; s++) {
						landscape[samp.getIndex()][i][s] = 0;
						fivePrimes[samp.getIndex()][i][s] = 0;
					}
				for (int strand = 0; strand <= 1; strand++) {
					List<StrandedBaseCount> currHits = strand == 0 ? hitsPos.get(samp) : hitsNeg.get(samp);
					...
					for (int s = 0; s <= 1; s++) {
						for (int i = 0; i < length; i++) {
							float v = kernel[0] * fivePrimes[samp.getIndex()][i][s] + Float.MIN_VALUE; // init
																										// with
																										// very
																										// small
																										// number
		...
		protected final int getShifted5Prime(StrandedBaseCount h) {
			return (h.getStrand() == '+' ? h.getCoordinate() + shift : h.getCoordinate() - shift);
		}

		protected final int getLeft(StrandedBaseCount h) {
			return (h.getStrand() == '+' ? h.getCoordinate() + shift - hit5Extend
					: h.getCoordinate() - shift - hit3Extend);
		}

		protected final int getRight(StrandedBaseCount h) {
			return (h.getStrand() == '+' ? h.getCoordinate() + shift + hit3Extend
		 ...
		 * 
		 * @param cond
		 *            : ExperimentCondition of interest
		 * @param data
		 *            : data structure to parse. Should be landscape. Assumes
		 *            indexed by Sample, base, strand
		 * @param strand
		 *            : +/-/.
		 * @param signal
		 *            : true to count condition's signal samples, false to count
		 *            condition's control samples.
		 * @return
		 */
		...
		/**
		 * Uses a rough, inexact binary search to find StrandedBaseCounts that
		 * overlap a given feature in each Sample
		 * 
		 * @param sameStrHits
		 *            : Lists of StrandedBaseCounts, all from same strand,
		 *            indexed by Sample - assumes sorted
		 * @param f
		 *            : Feature
		 * @return : Lists of StrandedBaseCounts that overlap the feature
		 *         coordinates
		 */
		protected Map<Sample, List<StrandedBaseCount>> overlappingHits(Map<Sample, List<StrandedBaseCount>> sameStrHits,
				Feature f) {
			...
			for (Sample samp : manager.getSamples()) {
				...
				while (l < sameStrHits.get(samp).size() && getLeft(sameStrHits.get(samp).get(l)) <= featureEnd) {
					...
					if (f.getCoords().overlaps(hitL, hitR) && (str == '.' || str == hit.getStrand())) {
						sub.add(hit);
					}
					l++;
				}
		 ...
		 * 
		 * By default, the background collections contain only a global Poisson
		 * model. If the use of local backgrounds is requested, the following
		 * occurs for each CONDITION backgorund model: If there is a control and
		 * the control is scaled, a local threshold based on expectation from
		 * CONTROL is added to the signal If there is no control or the control
		 * is not scaled, a local threshold based on expectation from SIGNAL is
		 * added to the signal This makes the behavior of local background
		 * thresholds similar to that used by MACS.
		 */
		protected void initializeBackgrounds() {
			...
			if (!strandedEventDetection) {
				...
				for (Sample s : manager.getSamples())
					sampleBackgrounds.get(s)
							.addBackgroundModel(new PoissonBackgroundModel(-1, sconfig.getPerBinPoissonLogPThres(),
									s.getHitCount(), gen.getGenomeLength(), econfig.getMappableGenomeProp(),
									(double) sconfig.getBinWidth(), '.', 1, true));
				// Condition-level genomic backgrounds
				for (ExperimentCondition c : manager.getConditions())
					conditionBackgrounds.get(c)
							.addBackgroundModel(new PoissonBackgroundModel(-1, sconfig.getPerBinPoissonLogPThres(),
									c.getTotalSignalCount(), gen.getGenomeLength(), econfig.getMappableGenomeProp(),
									(double) sconfig.getBinWidth(), '.', 1, true));
					...
					for (ExperimentCondition c : manager.getConditions()) {
						if (c.getControlSamples().size() > 0) {
							conditionBackgrounds.get(c).addBackgroundModel(new PoissonBackgroundModel(i.intValue(),
									sconfig.getPerBinPoissonLogPThres(), c.getTotalControlCount(),
									gen.getGenomeLength(), econfig.getMappableGenomeProp(),
									(double) sconfig.getBinWidth(), '.', c.getPooledSampleControlScaling(), false));
						} else {
							// local signal high -- this may bias against
							// locally enriched signal regions, and so should
							// only be used if there is no control or if the
							// control is not yet scaled
							if (i.intValue() >= 5000) // we don't want the
														// window too small in
														// this case
								conditionBackgrounds.get(c).addBackgroundModel(new PoissonBackgroundModel(i.intValue(),
										sconfig.getPerBinPoissonLogPThres(), c.getTotalSignalCount(),
										gen.getGenomeLength(), econfig.getMappableGenomeProp(),
										(double) sconfig.getBinWidth(), '.', c.getPooledSampleControlScaling(), true));
						}
					}
				}
			} else { // Stranded
				...
				for (Sample s : manager.getSamples()) {
					sampleBackgrounds.get(s)
							.addBackgroundModel(new PoissonBackgroundModel(-1, sconfig.getPerBinPoissonLogPThres(),
									s.getStrandedHitCount('+'), gen.getGenomeLength(), econfig.getMappableGenomeProp(),
									(double) sconfig.getBinWidth(), '+', 1, true));
					sampleBackgrounds.get(s)
							.addBackgroundModel(new PoissonBackgroundModel(-1, sconfig.getPerBinPoissonLogPThres(),
									s.getStrandedHitCount('-'), gen.getGenomeLength(), econfig.getMappableGenomeProp(),
				...
				for (ExperimentCondition c : manager.getConditions()) {
					conditionBackgrounds.get(c)
							.addBackgroundModel(new PoissonBackgroundModel(-1, sconfig.getPerBinPoissonLogPThres(),
									c.getStrandedTotalSignalCount('+'), gen.getGenomeLength(),
									econfig.getMappableGenomeProp(), (double) sconfig.getBinWidth(), '+', 1, true));
					conditionBackgrounds.get(c)
							.addBackgroundModel(new PoissonBackgroundModel(-1, sconfig.getPerBinPoissonLogPThres(),
									c.getStrandedTotalSignalCount('-'), gen.getGenomeLength(),
									econfig.getMappableGenomeProp(), (double) sconfig.getBinWidth(), '-', 1, true));
				...
				for (Integer i : sconfig.getLocalBackWins()) {
					for (ExperimentCondition c : manager.getConditions()) {
						if (c.getControlSamples().size() > 0) {
							conditionBackgrounds.get(c).addBackgroundModel(new PoissonBackgroundModel(i.intValue(),
									sconfig.getPerBinPoissonLogPThres(), c.getStrandedTotalControlCount('+'),
									gen.getGenomeLength(), econfig.getMappableGenomeProp(),
									(double) sconfig.getBinWidth(), '+', c.getPooledSampleControlScaling(), false));
							conditionBackgrounds.get(c).addBackgroundModel(new PoissonBackgroundModel(i.intValue(),
									sconfig.getPerBinPoissonLogPThres(), c.getStrandedTotalControlCount('-'),
							...
							if (i.intValue() >= 5000) { // we don't want the
														...
														// this case
								conditionBackgrounds.get(c).addBackgroundModel(new PoissonBackgroundModel(i.intValue(),
										sconfig.getPerBinPoissonLogPThres(), c.getStrandedTotalSignalCount('+'),
										gen.getGenomeLength(), econfig.getMappableGenomeProp(),
										(double) sconfig.getBinWidth(), '+', c.getPooledSampleControlScaling(), true));
								conditionBackgrounds.get(c).addBackgroundModel(new PoissonBackgroundModel(i.intValue(),
										sconfig.getPerBinPoissonLogPThres(), c.getStrandedTotalSignalCount('-'),
		...
		/**
		 * Filter out pre-defined regions to ignore (e.g. tower / blacklist
		 * regions)
		 * 
		 * @param testRegions
		 * @return
		 */
		protected Map<ExperimentCondition, List<Feature>> filterExcluded(
				Map<ExperimentCondition, List<Feature>> testFeatures) {
