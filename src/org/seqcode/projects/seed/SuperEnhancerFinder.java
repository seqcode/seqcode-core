...
public class SuperEnhancerFinder extends DomainFinder {
	...
	// pseudocount to calculate fold-change while finding inflectioin point for
	// super enhancers
	public final int PSEUDOCOUINT = 1;

	// At the moment only one TSS is alowed to overlap identified super
	// enahncers
	...
	// Minimum length of an enriched feature to be considered for stitching to
	// get a super enhancer
	...
	public SuperEnhancerFinder(GenomeConfig gcon, ExptConfig econ, SEEDConfig scon, ExperimentManager man) {
	...
	public Map<ExperimentCondition, List<Feature>> postProcess() {
		...
		for (ExperimentCondition ec : manager.getConditions()) {
			...
			for (int i = 0; i < signal_sorted_features.get(ec).size(); i++) {
				...
				if (y > signal_sorted_features.get(ec).size()) {
					y = signal_sorted_features.get(ec).size();
				}
	...
	public static String getDomainFinderArgs() {
		return (new String("" + "SuperEnhancerFinder arguments:\n"
	...
	public static void main(String[] args) {
		...
		String SuperEnhancerFinderHelp = new String("" + "SuperEnhancerFinder arguments:\n"
				+ "\t--supStitchWin <Window to stitch typical enhancers into super enhancers (default: supStitchWin=12500bp)>\n"
				+ "\t--distalDistance <Minmum distnace from TSSs for calling domains (default: distalDistance= 2000bp)>\n"
				+ "\t--refTSSs <TSS annotations, eg :- chr2:45667-45767:+ >\n" + "");
		...
		if (scon.helpWanted()) {
			...
			System.err.println(gcon.getArgsList() + econ.getArgsList() + scon.getArgsList() + SuperEnhancerFinderHelp);
		} else {
	...
	public class SuperEnhancerFinderThread extends DomainFinderThread {
		 ...
		 * comparing tag counts to a background model, and then finds super
		 * enhancers as described in ...et.al
		 */
		...
		protected Map<ExperimentCondition, List<EnrichedFeature>> processDomains(
			...
			// Remove enriched domains less than 100bp. Playing around with the
			// parameter at the moment (Needed especially when extending reads)
			// Also removes enriched feature that are have a p-value <
			// 0.001(PER_ENRICHED_FEATURE_THRES). Playing around with the
			...
			for (ExperimentCondition ec : manager.getConditions()) {
				...
				while (it.hasNext()) {
					...
					if (ef.getCoords().getWidth() < MIN_ENRICHED_FEATURE_LENGHT
							|| ef.getScore() >= PER_ENRICHED_FEATURE_THRES) {
						it.remove();
					}
				}
			}
		...
		protected void keepOnlyDistalFeature(List<EnrichedFeature> enrichedFeatures) {
			...
			for (StrandedPoint sp : refTSSs) {
				if (refTSSsbyChrom.containsKey(sp.getChrom())) {
					refTSSsbyChrom.get(sp.getChrom()).add(sp);
				} else {
					refTSSsbyChrom.put(sp.getChrom(), new ArrayList<StrandedPoint>());
					refTSSsbyChrom.get(sp.getChrom()).add(sp);
				}
			}
			...
			while (it.hasNext()) {
				...
				if (refTSSsbyChrom.containsKey(ef.getCoords().getChrom())) {
					for (StrandedPoint sp : refTSSsbyChrom.get(ef.getCoords().getChrom())) {
						int dis = Math.min(sp.distance(ef.getCoords().startPoint()),
								sp.distance(ef.getCoords().endPoint()));
						if (dis < minDistance) {
							minDistance = dis;
						}
					}
			...
			try {
				for (EnrichedFeature ef : enhancers) {
					if (lastadded == null) {
						...
						if (lastadded.getLastTEF().getCoords().getMidpoint()
								.distance(ef.getCoords().getMidpoint()) < superStitchWin) {
