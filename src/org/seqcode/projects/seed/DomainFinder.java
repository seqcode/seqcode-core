...
/**
 * DomainFinder: A FeatureDetection implementation that scans the genome for
 * statistically enriched blocks (domains)
 * 
 * @author mahony
 *
 */
public class DomainFinder extends FeatureDetection {
	...
	public String getProgramName() {
		return "org.seqcode.projects.seed.DomainFinder";
	}
	...
	public static void main(String[] args) {
		...
		if (scon.helpWanted()) {
			...
			System.err.println(gcon.getArgsList() + econ.getArgsList() + scon.getArgsList());
		} else {
	...
	/**
	 * Multiple-hypothesis correction. Print the output files. Assumes that all
	...
	public Map<ExperimentCondition, List<Feature>> postProcess() {
		...
		Map<ExperimentCondition, List<Feature>> signifFeatures = this.filter(features, sconfig.perBinBinomialPThres,
				true);
		...
		// Summarize
		for (ExperimentCondition cond : manager.getConditions())
			System.err.println(cond.getName() + "\t" + features.get(cond).size() + " domains\t"
					+ signifFeatures.get(cond).size() + " domains below threshold.");
	...
	public class DomainFinderThread extends FeatureDetectionThread {
		...
		/**
		 * findFeatures: find potentially enriched domains on the genome by
		 * comparing tag counts to a background model
		 * 
		 * Specifically, the procedure requires that the signal count in a bin
		 * passes all PoissonBackground thresholds, and the signal count vs
		 * control count (if exists) Binomial test passes an uncorrected p-value
		 * threshold. Neighboring bins that pass the thresholds are merged.
		 * 
		 * This method can assume that the hits and landscape data structures
		 * have been initialized for a given region - for each Sample these data
		 * structures contain, respectively: the StrandedBaseCounts hits
		 * (sorted), and the binned tag density (after any shifting and
		 * extending).
		 * 
		 * @param subRegion
		 *            : region to run analysis on
		 * @return Map of Lists of Features in the subRegion, Indexed by
		 *         ExperimentCondition
		 */
		public Map<ExperimentCondition, List<Feature>> findFeatures(Region subRegion) {
			...
			for (ExperimentCondition cond : manager.getConditions()) {
				...
				for (int stranditer = 1; stranditer <= numStrandIter; stranditer++) {
					...
					// If stranded peak-finding, run over both strands
					// separately
					char str = !strandedEventDetection ? '.' : (stranditer == 1 ? '+' : '-');

					float[] condSigCounts = getConditionCounts(cond, landscape, str, true);
					float[] condCtrlCounts = cond.getControlSamples().size() > 0 ? null
							: getConditionCounts(cond, landscape, str, false);

					// Scan regions
					int currBin = 0;
					for (int i = subRegion.getStart(); i < subRegion.getEnd() - sconfig.getBinWidth(); i += sconfig
							.getBinStep()) {
						float sigCounts = condSigCounts[currBin];
						// If there's no control, use the expected bin count
						// given random distribution
						float ctrlCounts = condCtrlCounts == null
								? conditionBackgrounds.get(cond).getGenomicExpectedCount()
								: condCtrlCounts[currBin] * (float) (cond.getPooledSampleControlScaling());

						// First Test: is the read count above the genome-wide
						// thresholds?
						if (conditionBackgrounds.get(cond).passesGenomicThreshold((int) sigCounts, str)) {
							// Second Test: refresh all thresholds & test again
							conditionBackgrounds.get(cond).updateModels(subRegion, i - subRegion.getStart(),
									condSigCounts, condCtrlCounts, sconfig.getBinStep());
							if (conditionBackgrounds.get(cond).passesAllThresholds((int) sigCounts, str)) {
								// Third Test: Binomial test between signal &
								// (scaled) control
								double pval = stats.binomialPValue(ctrlCounts, (sigCounts + ctrlCounts),
										sconfig.getMinSigCtrlFoldDifference());
								if (pval < sconfig.getPerBinBinomialPThres()) {
									// Add new event or append to the last one.
									Region currWin = strandedEventDetection
											? new StrandedRegion(gen, subRegion.getChrom(), i,
													i + sconfig.getBinWidth() - 1, str)
											: new Region(gen, subRegion.getChrom(), i, i + sconfig.getBinWidth() - 1);
									lastFeature = addEnrichedDomain(currFeatures.get(cond), lastFeature, currWin,
											sigCounts, condCtrlCounts == null ? 0 : ctrlCounts, pval);
								}
							}
						}
						currBin++;
			...
			// Trim, quantify, & properly score currFeatures before adding them
			// to the results
		 ...
		 * @return
		 */
		protected EnrichedFeature addEnrichedDomain(List<EnrichedFeature> currResults, EnrichedFeature lastFeat,
				Region currWin, float sigWinHits, float ctrlWinHits, double score) {
			EnrichedFeature resFeat = null;

			// Is this hit close to the previously added one? If so, merge
			if (lastFeat != null && currWin.getStrand() == lastFeat.getCoords().getStrand()
					&& currWin.distance(lastFeat.getCoords()) <= sconfig.getFeatureMergeWindow()) {
				lastFeat.setCoords(strandedEventDetection
						? new StrandedRegion(gen, lastFeat.getCoords().getChrom(), lastFeat.getCoords().getStart(),
								currWin.getEnd(), lastFeat.getCoords().getStrand())
						: new Region(gen, lastFeat.getCoords().getChrom(), lastFeat.getCoords().getStart(),
								currWin.getEnd()));
		...
		/**
		 * ProcessDomains: - Trims feature coordinates back to agree with
		 * overlapping hits. - Counts hits in each feature, per sample
		 * 
		 * @param currFeatures
		 * @param current
		 *            region
		 ...
		 */
		protected Map<ExperimentCondition, List<EnrichedFeature>> processDomains(
				Map<ExperimentCondition, List<EnrichedFeature>> currFeatures, Region currSubRegion) {
			for (ExperimentCondition currCondition : manager.getConditions()) {
				for (EnrichedFeature f : currFeatures.get(currCondition)) {
					...
					// Quantify the feature in each Sample and in the condition
		...
		/**
		 * Trim a feature back to the coordinates of the first and last
		 * overlapping hit, if necessary. Only trims - does not allow extension
		 ...
		 * @param featureCond
		 */
		protected void trimFeature(Feature f, Map<Sample, List<StrandedBaseCount>> featureHitsPos,
				Map<Sample, List<StrandedBaseCount>> featureHitsNeg, ExperimentCondition featureCond) {
			...
			for (Sample s : featureCond.getSignalSamples()) {
				for (int strand = 0; strand <= 1; strand++) {
					List<StrandedBaseCount> featureHitsCurr = strand == 0 ? featureHitsPos.get(s)
							: featureHitsNeg.get(s);
			...
			maxCoord = Math.min(maxCoord + 1, f.getCoords().getEnd()); // Plus
																		// one
																		// to
																		// avoid
																		// minCoord
		...
		/**
		 * Count the number of tags that overlap this enriched feature in every
		 * sample and in the condition in which it was discovered. In place
		 * update of counts.
		 * 
		 * @param f
		 ...
		 * @param featureCond
		 */
		protected void quantifyFeature(EnrichedFeature f, Map<Sample, List<StrandedBaseCount>> featureHitsPos,
				Map<Sample, List<StrandedBaseCount>> featureHitsNeg, ExperimentCondition featureCond) {
			...
			// Sample-pooled count for the condition that this feature was
			// discovered in
			...
			// If there is no control, use the expected number of tags in this
			// region as the control count for the purposes of significance
			// testing
			float binomialTestCtrlCount = (float) (featureCond.getControlSamples().size() == 0
					? (featureCond.getTotalSignalCount() * f.getCoords().getWidth())
							/ (econfig.getMappableGenomeLength())
			...
			f.setScore(stats.binomialPValue(binomialTestCtrlCount, (pooledSigCount + binomialTestCtrlCount),
					sconfig.getMinSigCtrlFoldDifference()));
		}
