...
/**
 * Utility to quantify enrichment and assess significance of a set of peaks
 * (from GFF file). If multiple conditions are specified, differential binding
 * analysis is performed via edgeR. Meant to be used in the galaxy-exo pipeline.
 * 
 * Input: - Genome - GFF file of potential peak locations - Window around peaks
 * in which to count reads Output: - Annotated GFF file (all peaks) - Annotated
 * GFF file (peaks passing threshold) - Replicate count file
 * 
 * SignificanceTester performs no cleaning, merging, or shifting of input
 * locations.
 * 
 * @author mahony
 *
...
public class SignificanceTester {
	...
	// Constructor
	public SignificanceTester(ExptConfig econ, EventsConfig config, ExperimentManager manager, BindingManager bman,
			String gffFileName, int win, double q, double minfold, boolean rankByQ, String outDirName) {
		this.econfig = econ;
		this.config = config;
		...
		this.outDirName = outDirName == null
				? gffDir + "signif_w" + searchRegionWin + "_q" + String.format("%.2e", qThres) + "_minfold"
						+ String.format("%.1f", minFold)
				: outDirName + File.separator + "signif_w" + searchRegionWin + "_q" + String.format("%.2e", qThres)
						+ "_minfold" + String.format("%.1f", minFold);
	...
	public void execute() {
		...
		PointsToEvents p2e = new PointsToEvents(config, manager, bindingManager, potentialSites, searchRegionWin,
				!simpleReadAssignment);
		...
		EnrichmentSignificance tester = new EnrichmentSignificance(config, manager, bindingManager,
				config.getMinEventFoldChange(), econfig.getMappableGenomeLength());
		...
		if (manager.getNumConditions() > 1 && config.getRunDiffTests()) {
			...
			for (int ref = 0; ref < manager.getNumConditions(); ref++) {
				...
				data.savePairwiseConditionMAPlots(config.getDiffPMinThres(),
						outImagesDir.getAbsolutePath() + File.separator, true);
				...
				data.savePairwiseConditionXYPlots(manager, bindingManager, config.getDiffPMinThres(),
						outImagesDir.getAbsolutePath() + File.separator, true);
			}
		}
			...
			try {
				...
				for (ExperimentCondition cond : manager.getConditions()) {
					...
					for (BindingEvent e : events) {
						...
						String attrib = origGFF.getAttribString() + String.format(
								";sig_tags=%.1f;ctrl_tags=%.1f;log2_fold_sigctrl=%.3f;log2_qval_sigctrl=%.3f", sigHits,
								ctrlHits, logF, logP);
						...
						if (e.isFoundInCondition(cond) && P <= config.getQMinThres()) {
							signifFout.write(
									"chr" + origGFF.getChr() + "\t" + origGFF.getSource() + "\t" + origGFF.getFeature()
											+ "\t" + origGFF.getStart() + "\t" + origGFF.getEnd() + "\t" + score + "\t"
											+ origGFF.getStrand() + "\t" + origGFF.getFrame() + "\t" + attrib + "\n");
						}
						allFout.write(
								"chr" + origGFF.getChr() + "\t" + origGFF.getSource() + "\t" + origGFF.getFeature()
										+ "\t" + origGFF.getStart() + "\t" + origGFF.getEnd() + "\t" + score + "\t"
	...
	public static void main(String[] args) {
		// Hack to set a default fixedpb limit. Need a better way to set
		...
		if (config.helpWanted()) {
			...
			SignificanceTester tester = new SignificanceTester(econ, config, manager, bman, siteFile, win, qThres,
					minFold, rankbyq, outDirName);
