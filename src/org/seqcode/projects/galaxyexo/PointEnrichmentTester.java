...
/**
 * Utility to access peak enrichment at a set of genomic regions. Statistical
 * significance of peak enrichment over the set of sites is accessed using
 * Poisson model. The null distribution comes from randomly placed peaks thought
 * a genome.
 * 
 * Input: - Genome - GFF file of peak locations - A set of genomic regions to
 * test peak enrichment Output: - A text file indicating the number of overlap
 * and Poisson p-value.
 * 
 * @author naomi yamada
 */
...
public class PointEnrichmentTester {
	...
	protected int pseudocounts; // noise added to prevent calling significance
								// in telomere regions
	protected boolean printRandOverlap = false; // flag to print number of
												// random overlap
	...
	public void printRandOverlap() {
		printRandOverlap = true;
	}
	...
	public void execute() throws FileNotFoundException {
		...
		for (Region reg : regions) {
			totalRegionSize += reg.getWidth();
		} // total size of regions
		// expand regions for 20bp so that I don't double counts the peaks
		// nearby
		...
		writer.println(
				"number of overlap between gff points and regions with size " + totalRegionSize + " : " + totalOverlap);
		...
		double maxPval = 0;
		// Determined p-val based on Poisson distributions for numItr times and
		// return the max p-val
		for (int i = 0; i < numItr; i++) {
			...
			if (i == 1) {
				writer.println("number of overlap with random regions for iteration one : " + numRandOverlaps);
			}
			if (w != null) {
			...
			if (pValuePoisson > maxPval) {
				maxPval = pValuePoisson;
			}
		}
