...
public class FeatureCountsLoader {
	...
	protected int edge = 0; // Because shift will shorten the array, edge will
							// ensure that windows are covered with reads
	...
	// setters
	public void setStrandedRegions(List<StrandedRegion> reg) {
		strandedRegions = reg;
	}

	public void setFivePrimeShift(int s) {
		fivePrimeShift = s;
	}
	...
	// getters
	public List<StrandedRegion> getStrandedRegions() {
		return strandedRegions;
	}

	public Map<StrandedRegion, double[][]> getControlStrandedRegionCounts() {
		return controlRegionCounts;
	}

	public double[] controlComposite() {
		return controlComposite;
	}

	public Map<StrandedRegion, double[][]> strandedRegionSampleCounts(ControlledExperiment rep) {

		// if there tag shift is needed, set the edge to 40 to obtain counts
		// from extra region
		if (fivePrimeShift > 0) {
			edge = 40;
		}
		...
		// StrandedBasedCount object contains positive and negative strand
		// separately
		...
		for (StrandedRegion reg : sampleCountsMap.keySet()) {
			...
			if (reg.getStrand() == '+') { // regions(features) are positive
											// strand
				for (StrandedBaseCount hits : sampleCountsMap.get(reg)) {
					if (hits.getStrand() == '+') {
						sampleCounts[hits.getCoordinate() - reg.getMidpoint().getLocation()
								+ (windowSize + edge) / 2][0] = hits.getCount();
					} else {
						sampleCounts[hits.getCoordinate() - reg.getMidpoint().getLocation()
								+ (windowSize + edge) / 2][1] = hits.getCount();
					}
				}
			} else { // if regions (features) are reverse strand, I need to flip
						// the strands and locations
				for (StrandedBaseCount hits : sampleCountsMap.get(reg)) {
					if (hits.getStrand() == '+') {
						sampleCounts[reg.getMidpoint().getLocation() - hits.getCoordinate()
								+ (windowSize + edge) / 2][1] = hits.getCount();
					} else {
						sampleCounts[reg.getMidpoint().getLocation() - hits.getCoordinate()
								+ (windowSize + edge) / 2][0] = hits.getCount();
			...
			if (rep.hasControl()) {
				if (reg.getStrand() == '+') {
					for (StrandedBaseCount hits : controlCountsMap.get(reg)) {
						if (hits.getStrand() == '+') {
							controlCounts[hits.getCoordinate() - reg.getMidpoint().getLocation()
									+ (windowSize + edge) / 2][0] = hits.getCount();
						} else {
							controlCounts[hits.getCoordinate() - reg.getMidpoint().getLocation()
									+ (windowSize + edge) / 2][1] = hits.getCount();
					...
					for (StrandedBaseCount hits : controlCountsMap.get(reg)) {
						if (hits.getStrand() == '+') {
							controlCounts[reg.getMidpoint().getLocation() - hits.getCoordinate()
									+ (windowSize + edge) / 2][1] = hits.getCount();
						} else {
							controlCounts[reg.getMidpoint().getLocation() - hits.getCoordinate()
									+ (windowSize + edge) / 2][0] = hits.getCount();
	...
	public double[] sampleComposite(ControlledExperiment rep) {
		...
		// nonstrandedComposite is a shifted and strand merged composite to be
