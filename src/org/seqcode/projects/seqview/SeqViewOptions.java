...
public class SeqViewOptions {
	...
	public static final int DEFAULT_MAX_SEQUENCE_VIEW = 1000; // Don't call
																// sequence
																// generator for
																// paintables if
																// Region is
	...
	public int maxSequenceQuery = DEFAULT_MAX_SEQUENCE_VIEW;// Don't call
															// sequence
															// generator for
															// paintables if
															// Region is greater
	...
	public boolean hash = true, seqletters = true, polya = false, gccontent = false, pyrpurcontent = false, cpg = false,
			regexmatcher = false;
	...
	// filename to label mappings. These are loaded from a file and the data
	// held statically
	 ...
	 * (this must be set by the creator, currently done in RegionPanel). This
	 * information is useful for removing a painter because it lets you figure
	 * out which field of the SeqViewOptions class should be modified
	 */
	public static final int BINDINGSCAN = 1, GENES = 2, NCRNAS = 3, OTHERANNOTS = 4, AGILENTDATA = 5, MOTIFS = 11,
			PEAKS = 12, SEQLETTERS = 13, GCCONTENT = 14, EXPRESSION = 15, CPG = 16, MOTIFSCANS = 17, REGEXMATCHER = 18,
			REGIONTRACKS = 19, PYRPURCONTENT = 20, CHIPSEQANALYSES = 21, SEQDATA = 22;
	...
	/*
	 * adds options from this into union. For lists, it generates the union. For
	 * other settings, this takes priority
	 */
	public void mergeInto(SeqViewOptions union) throws IllegalArgumentException {
		...
		if (genome == null) {
			throw new NullPointerException("Tried to call mergeInto when genome is null");
		}
		if (chrom != null) {
			union.chrom = chrom;
			union.start = start;
	...
	/*
	 * deletes options from this that are also present in other. Doesn't mess
	 * with chrom, start, stop, or gene
	 */
	public void differenceOf(SeqViewOptions other) {
		if (genome != null && other.genome != null && !other.genome.equals(genome)) {
		...
		try {
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (args[i].equals("--seqexpt")) {
					// This is a really awkward (and possibly bug-filled) way to
					// load experiments,
				...
				/*
				 * if (args[i].equals("--wm")) { String[] pieces =
				 * args[++i].split(";"); Species thisorg = organism; if
				 * (pieces.length == 3) { thisorg = new Species(pieces[2]); }
				 * WeightMatrix matrix =
				 * wmloader.query(thisorg.getDBID(),pieces[0],pieces[1]);
	 ...
	 * 
	 * @param is
	 *            the input stream from which to read the options
	 * @throws IOException
	 ...
	 * 
	 * @param os
	 *            the output stream to which to write the options
	 * @throws IOException
	...
	/**
	 * Checks whether the specified width is within the allowable range for the
	 * SeqView Main Frame Width
	 * 
	 * @param testWidth
	 *            the width to test
	 */
	...
	/**
	 * Checks whether the specified height is within the allowable range for the
	 * SeqView Main Frame Height
	 * 
	 * @param testHeight
	 *            the height to test
	 */
	 ...
	 * 
	 * @param newWidth
	 *            the preferred width for the SeqView Main Frame
	 */
	 ...
	 * 
	 * @param newHeight
	 *            the preferred height for the SeqView Main Frame
	 */
	 ...
	 * 
	 * @param isCentered
	 *            true causes the SeqView Main Frame to be centered on the
	 *            screen
	 */
	...
	/**
	 * Sets the x-coord of the top left corner of the preferred location for the
	 * SeqView Main Frame
	 * 
	 * @param newX
	 *            the x-coord of the top left corner of the preferred location
	 */
	...
	/**
	 * Sets the y-coord of the top left corner of the preferred location for the
	 * SeqView Main Frame
	 * 
	 * @param newY
	 *            the y-coord of the top left corner of the preferred location
	 */
