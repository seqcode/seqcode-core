...
public class SeqAlignmentSelectPanel extends GenericSelectPanel<SeqLocatorMatchedAligns> {
	...
	public JPanel getInputsPanel() {
		...
		try {
			...
			regexLab.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					filter();
				}
			});
			regexCond = new JTextField();
			regexCond.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					filter();
				}
			});
			regexTarget = new JTextField();
			regexTarget.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					filter();
				}
			});
			regexCell = new JTextField();
			regexCell.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					filter();
				}
			});
			regexAlign = new JTextField();
			regexAlign.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					filter();
				}
			});
			regexRep = new JTextField();
			regexRep.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					filter();
				}
			});
			...
			labelAlign.setFont(labelFont);
			labelAlign.setBorder(paddingBorder);
	...
	public Collection<SeqLocatorMatchedAligns> getFilteredForSelected() {
		...
		for (Pair<String, String> nv : reps.keySet()) {
			try {
				output.add(
						new SeqLocatorMatchedAligns(aligns.get(nv), new SeqLocator(nv.car(), reps.get(nv), nv.cdr())));
		...
		try {
			synchronized (lmes) {
				...
				for (SeqAlignment align : alignments) {
					...
					a.add(align);
					lmes.add(new SeqLocatorMatchedAligns(a, new SeqLocator(align.getExpt().getName(),
							align.getExpt().getReplicate(), align.getName())));
				}
			}
		} catch (SQLException e) {
	...
	public void filter() {
		Pattern pattType = null, pattLab = null, pattCond = null, pattTarget = null, pattCell = null, pattAlign = null,
				pattRep = null;
		...
		synchronized (lmes) {
			...
			for (SeqAlignment align : alignments) {
				if ((pattType == null || pattType.matcher(align.getExpt().getExptType().getName().toLowerCase()).find())
						...
						&& (pattCond == null
								|| pattCond.matcher(align.getExpt().getExptCondition().getName().toLowerCase()).find())
						&& (pattTarget == null
								|| pattTarget.matcher(align.getExpt().getExptTarget().getName().toLowerCase()).find())
						&& (pattCell == null
								|| pattCell.matcher(align.getExpt().getCellLine().getName().toLowerCase()).find())
	...
	/*
	 * public static Collection<SeqLocator>
	 * collapseLocatorsByName(Collection<SeqLocator> locs) {
	 * LinkedHashMap<String,Map<String,Set<String>>> map = new
	 * LinkedHashMap<String,Map<String,Set<String>>>();
	 * 
	 * for(SeqLocator loc : locs) { String exptName = loc.getExptName(); String
	 * alignName = loc.getAlignName(); if(!map.containsKey(exptName)) {
	 ...
	 * 
	 * for(String exptName : map.keySet()) { for(String alignName :
	 * map.get(exptName).keySet()) { SeqLocator newloc = new
	 * SeqLocator(exptName, map.get(exptName).get(alignName), alignName);
