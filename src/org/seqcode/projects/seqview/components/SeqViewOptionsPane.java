...
public class SeqViewOptionsPane extends JTabbedPane implements ItemListener, ActionListener, Closeable {
	// Current genome settings
	Genome currGen = null;
	Species currSpecies = null;
	// regexes get special handling at the moment because there's no gui
	// component for them,
	...
	private JPanel speciesLocationPanel, annotationsPanel, seqPanel, optionsPanel;
	...
	private void reloadGenome() {
		...
		if (orgName != null && genName != null
				&& (!orgName.equals(currSpecies.getName()) || !genName.equals(currGen.getVersion()))) {
			Species org;
			try {
	...
	public void init(SeqViewOptions opts) throws NotFoundException {
		...
		genes.setVisibleRowCount(7);
		genes.setLayoutOrientation(JList.VERTICAL);
		otherfeats.setVisibleRowCount(7);
		otherfeats.setLayoutOrientation(JList.VERTICAL);
		ncrnas.setVisibleRowCount(7);
		ncrnas.setLayoutOrientation(JList.VERTICAL);

		seqlimitlabel = new JLabel(
				"Sequence features (visible only below " + SeqViewOptions.DEFAULT_MAX_SEQUENCE_VIEW + "bp):");
		...
		dummy = new JPanel();
		dummy.add(optionsPanel);
		dummy.add(new JPanel());
		addTab("Display Options", new JScrollPane(dummy));

		if (opts != null) {
			if (opts.gene != null && !opts.gene.equals("")) {
			...
			if (opts.position != null && !opts.position.equals("")) {
				position.setText(opts.position);
			} else {
				if (opts.chrom != null && !opts.chrom.equals("")) {
	...
	public void setSpeciesGenomeDefaults() {
		...
		try {
			...
			// don't do anything. If it fails, then we just fall back to the
			// defaults.
		}
	...
	/*
	 * fills in and returns a SeqViewOptions object based on the current
	 * selections
	 */
	...
	public SeqViewOptions parseAndDiff() {
		...
		// options.
		if (createdFrom != null && these.getGenome().equals(createdFrom.getGenome())) {
	...
	/*
	 * updates the choice of experiments based on the currently selected genome
	 * and species
	 */
