...
public class SeqAlignmentTableModel extends ObjectTableModel<SeqLocatorMatchedAligns> {
	...
	public Object getValueAt(int rowIndex, int c) {
		if (c == 0) {
			return getObject(rowIndex).expts.get(0).getExptType().getName();
		}
		if (c == 1) {
			return getObject(rowIndex).expts.get(0).getLab().getName();
		}
		if (c == 2) {
			return getObject(rowIndex).expts.get(0).getExptCondition().getName();
		}
		if (c == 3) {
			return getObject(rowIndex).expts.get(0).getExptTarget().getName();
		}
		if (c == 4) {
			return getObject(rowIndex).expts.get(0).getCellLine().getName();
		}
		if (c == 5) {
			return getObject(rowIndex).locator.getAlignName();
		}
		if (c == 6) {
			return getObject(rowIndex).locator.getReplicateString();
		}
		return null;
	}
