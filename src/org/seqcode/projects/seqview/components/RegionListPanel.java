...
public class RegionListPanel extends JPanel
		implements ActionListener, KeyListener, MouseListener, RegionList, Runnable {
	...
	public RegionListPanel(RegionPanel panel, java.util.List<? extends Region> regions) {
		init();
		regionpanel = panel;
		if (regions != null) {
	...
	public Region regionAt(int i) {
		return (Region) listmodel.getElementAt(i);
	}
	...
	public void go() {
		...
		if (Math.abs(r.getStart() - r.getEnd()) < 200) {
			...
			r = new Region(r.getGenome(), r.getChrom(), center - size / 2, center + size / 2);
		}
