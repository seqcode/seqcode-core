...
public class SeqAnalysisPainter extends RegionPaintable {
	...
	public SeqAnalysisProperties getProperties() {
		return props;
	}
	...
	public void paintItem(Graphics2D g, int ulx, int uly, int lrx, int lry) {
		if (!canPaint()) {
			return;
		}
		...
		for (SeqAnalysisResult r : results) {
			int x1 = getXPos(r.getStart(), region.getStart(), region.getEnd(), ulx, lrx);
			int x2 = getXPos(r.getEnd(), region.getStart(), region.getEnd(), ulx, lrx);
			if (x2 == x1) {
				x2 = x1 + 1;
			}
			...
			addLabel(x1, y1, x2 - x1, trackHeight - spacing,
					String.format("%.1f/%.1f p=%.2f", r.foregroundReadCount, r.backgroundReadCount, r.pvalue));
		}
