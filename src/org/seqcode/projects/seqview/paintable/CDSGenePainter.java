	...
	public void paintItem(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		for (Gene gene : genes) {
			if (!painted.contains(gene)) {
				...
				int nx = Math.max(x1 + 3, gx1 + 3); // gotta do the Math.max(),
				...
				for (String s : aliases) {
					if (!s.endsWith("Rik")) {
						...
						if (drawallgenenames && todraw != null && fontmetrics.charsWidth((newtodraw).toCharArray(), 0,
								newtodraw.length()) < rectwidth) {
							todraw = newtodraw;
						}
