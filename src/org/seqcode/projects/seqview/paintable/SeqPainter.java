...
public abstract class SeqPainter extends RegionPaintable {
	...
	public SeqProperties getProperties() {
		return props;
	}

	public void setProperties(SeqProperties p) {
		props = p;
	}
	...
	public SeqScaleModel getScaleModel() {
		return scale;
	}
	...
	protected void drawStep(Graphics2D g, int px, int py, int pover, int x, int y, int over) {
		if (over > pover) {
			drawStepUp(g, px, py, x, y);
		}
		if (over < pover) {
			drawStepDown(g, px, py, x, y);
		}
		if (over == pover) {
	...
	public synchronized void eventRegistered(EventObject e) {
		if ((e.getSource() == model)
				|| (e.getSource() == scale) && model.isReady() && ((scale == null) || scale.isReady())) {
			setCanPaint(true);
			setWantsPaint(true);
	...
	protected abstract void paintOverlapping(Graphics2D g, int x1, int y1, int x2, int y2);

	protected abstract void paintNonOverlapping(Graphics2D g, int x1, int y1, int x2, int y2);
