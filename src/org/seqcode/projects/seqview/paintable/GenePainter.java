...
public class GenePainter extends RegionPaintable {
	...
	public void setProperties(GeneProperties p) {
		props = p;
	}
	...
	public void paintItem(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		for (Gene gene : genes) {
			...
			if (gx1 >= x1) {
				g.drawLine(gx1, gtop, gx1, gbottom);
			}
			if (gx2 <= x2) {
				g.drawLine(gx2, gtop, gx2, gbottom);
			}

			// Somewhat prettier gene-name output
			// g.drawString(gene.getName(), gx1 + (gxw/3), my);
			int nx = Math.max(x1 + 3, gx1 + 3); // gotta do the Math.max(), to
												// make sure the name is on the
												// screen.
			...
			if (strand == '+') {
				...
				// int a2 = (int) Math.round(startX + (arrowWd * 6)); //(int)
				// Math.round((gx1 + 25) / (gx1+depth));
				// int a3 = (int) Math.round(startX + (arrowWd * 8)); //(int)
				// Math.round((gx1 + 35) / (gx1+depth));
			...
			/*
			 * int b1 = (int) Math.round(my); int b2 = (int) Math.round(my -
			 * (arrowHt * 13)); int b3 = (int) Math.round(my - (arrowHt * 10));
