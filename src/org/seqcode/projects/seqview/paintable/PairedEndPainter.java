...
public class PairedEndPainter extends RegionPaintable {
	...
	public void setProperties(PairedEndProperties p) {
		props = p;
	}
	...
	public void paintItem(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		// int alphastep = Math.min(255, Math.max(255 / (height / (hits.size() *
		// linewidth)), 4));
		int alphastep = 255;
		int h = height;
		...
		for (int i = 0; i < hits.size(); i++) {
			...
			// For the case when "left" should mean "first read off the
			// machine", the color of the edge should not depend on
			// leftx2<rightx1.
			// For the case when "left" means lower coordinate, it should depend
			// on leftx2<rightx1, but this is enforced by the leftAlwaysLesser
			// option and it's call to the flip method.
			g.setColor(hit.leftStrand ? (hit.rightStrand ? plusplus : plusminus)
					: (hit.rightStrand ? minusplus : minusminus));

			if (leftx2 < rightx1) {
				// g.setColor(hit.leftStrand ? (hit.rightStrand ? plusplus :
				// plusminus) : (hit.rightStrand ? minusplus : minusminus));
				g.drawLine(leftx2, y1 + h, rightx1, y1 + h);
			} else {
				// g.setColor(hit.rightStrand ? (hit.leftStrand ? plusplus :
				// plusminus) : (hit.leftStrand ? minusplus : minusminus));
		...
		if (getProperties().DrawOtherChromHits) {
			...
			for (int i = 0; i < otherc.size(); i++) {
				...
				if (hit.leftChrom == thischrom) {
					...
					onlyx2 = getXPos(hit.leftStrand ? hit.leftPos + hit.leftLength : hit.leftPos - hit.leftLength,
							regionStart, regionEnd, x1, x2);
				} else {
					onlyx1 = getXPos(hit.rightPos, regionStart, regionEnd, x1, x2);
					onlyx2 = getXPos(hit.rightStrand ? hit.rightPos + hit.rightLength : hit.rightPos - hit.rightLength,
							regionStart, regionEnd, x1, x2);
				}
				if (onlyx2 > onlyx1) {
					int t = onlyx2;
