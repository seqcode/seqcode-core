...
public class SeqHistogramPainter extends RegionPaintable {
	...
	public SeqHistogramProperties getProperties() {
		return props;
	}

	public void setProperties(SeqHistogramProperties p) {
		props = p;
	}
	...
	private Map<Integer, Float> convertKernelDensity(Map<Integer, Float> data) {
		...
		double[] profile = new double[max - min + 1 + 100]; // add 50bp padding
															// to the ends
		for (int pos : data.keySet()) {
	...
	public void paintItem(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		if (stranded) {
			...
			// Plus strand : first screen out overlapping rects (take max), then
			// plot
			g.setColor(getProperties().getMinusColor());
			...
			int step = Math.max(1, (int) Math.round(maxhits / 1)); // y-axis
																	// scale
			...
			// Plot density : first screen out overlapping rects (take max),
			// then plot
			HashMap<Integer, Double> plotXVals = new HashMap<Integer, Double>();
			g.setColor(getProperties().getUnstrandedColor());
			for (int pos : plus.keySet()) {
				if (true/*
						 * !model.getProperties().ShowInteractionHistogram ||
						 * !pvals.containsKey(pos)
						 */) {
					double val = plus.get(pos);
		...
		if (props.DrawPairedCurves && alignPaired) {
			...
			for (PairedHit pair : pairs) {
				if (pair.leftChrom == pair.rightChrom && pair.leftPos >= arcmodel.getRegion().getStart()
						&& pair.leftPos <= arcmodel.getRegion().getEnd()
						&& pair.rightPos >= arcmodel.getRegion().getStart()
						&& pair.rightPos <= arcmodel.getRegion().getEnd()) {
					...
					if (pair.pairCode == 1) {
						...
						// and it looks better if they go from the end of one
						// block to the start of the other
		...
		if (getProperties().DrawBinSize) {
			...
			g.drawString(binString, x2 - g.getFontMetrics().stringWidth(binString) - g.getFont().getSize() * 2,
					y1 + g.getFont().getSize());
		}
	}
	 ...
	 * each bin and so they'll be drawn overlapping. This method resizes the
	 * bins into bins of width binWidth by combining skinny bins that map to the
	 * same fat bin
	 */
