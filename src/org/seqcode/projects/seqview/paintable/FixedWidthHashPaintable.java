...
public class FixedWidthHashPaintable extends RegionPaintable {
	...
	public void setLabelAbove(boolean b) {
		labelAbove = b;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.viz.paintable.Paintable#paintItem(java.awt.Graphics,
	 * int, int, int, int)
	 */
	public void paintItem(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		/*
		 * while(spaceUsed > spaceAvail) { tickSize *= 10; tickDigits =
		 * (int)Math.ceil(Math.log(re / tickSize) / log10) + 1; spaceUsed =
		 * fontmetrics.charsWidth(nines,0,tickDigits); spaceAvail = (int)
		 * (multiplier * w / (rw / ((double)tickSize))); }
		 * 
		...
		Region r = getRegion();
		String axisLabel = String.format("%s: %s-%s,   %s bp/segment", r.getChrom(), nf.format(r.getStart()),
				nf.format(r.getEnd()), nf.format(tickSize));
		...
		for (int bp = bpStart; bp <= re; bp += tickSize) {
			...
			if (paintBlack) {
				// g.drawString(String.valueOf(bp / tickSize), xOffset+2, my +
				// hh - 1);
			}
