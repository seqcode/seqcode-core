...
public class SeqAboveBelowStrandPainter extends SeqPainter {
	...
	public SeqAboveBelowStrandPainter(SeqDataModel model) {
		super(model);
	...
	protected void setLayoutHits() {
		if (canPaint()) {
			if (getProperties().Overlapping) {
				/*
				 * don't do anything. The model can provide us with the
				 * RunningOverlapSums
				 */
			} else {
	...
	protected void paintOverlapping(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		/*
		 * maxoverlap is the maximum height of the reads scale If the property
		 * is negative, use the observed maximum (ie, dynamic scaling).
		 * Otherwise, use the value from the property and clip peaks higher than
		 * that value
		 */
		double maxoverlap;
		if (propmaxoverlap > 0) {
	...
	protected void paintNonOverlapping(Graphics2D g, int x1, int y1, int x2, int y2) {
		...
		/*
		 * if the user has asked for a maxcount greater than the maximum number
		 * of tracks into which NonOverlappingLayout would have done the layout,
		 * tell NOL to use more tracks
		 */
		...
		for (SeqHit watsonHit : watsonLayoutHits) {
			...
			if (getProperties().DrawNonUnique || watsonHit.getWeight() >= 1.0) {
				...
				Color c = new Color(plusHitColor.getRed(), plusHitColor.getGreen(), plusHitColor.getBlue(),
						(int) (watsonHit.getWeight() * 255));
				g.setColor(c);
