...
public class NamedStrandedPainter extends RegionPaintable {
	...
	public void paintItem(Graphics2D g, int ulx, int uly, int lrx, int lry) {
		...
		for (int j = 0; j < regions.size(); j++) {
			...
			int min = f.getStart();
			int max = f.getEnd();
			int x1 = getXPos(min, getRegion().getStart(), getRegion().getEnd(), ulx, lrx);
			int x2 = getXPos(max, getRegion().getStart(), getRegion().getEnd(), ulx, lrx);
			...
			if (((center - texthalf > lastx[trackIndex[j]]) && drawgenenames) || alwaysDrawNames) {
				g.setColor(Color.black);
