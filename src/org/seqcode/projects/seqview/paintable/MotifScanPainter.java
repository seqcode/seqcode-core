...
public class MotifScanPainter extends RegionPaintable {
	...
	public void paintItem(Graphics2D g, int ulx, int uly, int lrx, int lry) {
		...
		g.setColor(new Color(107, 66, 38)); // brown. don't use red because
											// that's not a match in the
											// per-base scanner
		...
		while (iter.hasNext()) {
			...
			int x1 = getXPos(r.getStart(), model.getRegion().getStart(), model.getRegion().getEnd(), ulx, lrx);
			int x2 = getXPos(r.getEnd(), model.getRegion().getStart(), model.getRegion().getEnd(), ulx, lrx);
