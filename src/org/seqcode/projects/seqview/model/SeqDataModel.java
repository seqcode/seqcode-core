...
/**
 * SeqDataModel: SeqView data model for SeqHits. If an extension is specified,
 * then each return hit will be extended by that many bp.
 */
public class SeqDataModel extends SeqViewModel implements RegionModel, Runnable {
	...
	public SeqDataProperties getProperties() {
		return props;
	}
	...
	public double getTotalMaxOverlap() {
		return totalSum.getMaxOverlap();
	}

	public double getWatsonMaxOverlap() {
		return watsonSum.getMaxOverlap();
	}

	public double getCrickMaxOverlap() {
		return crickSum.getMaxOverlap();
	}
				...
				try {
					...
					if (doSums) {
						if (props.ShowType1Reads) {
							mapToSum(totalSum,
									client.getWeightHistogram(alignids,
											region.getGenome().getChromID(region.getChrom()), false, // type1
											false, // paired
											extension, 1, // binsize
											props.DeDuplicate, region.getStart(), region.getEnd(), null, null));
							mapToSum(watsonSum,
									client.getWeightHistogram(alignids,
											region.getGenome().getChromID(region.getChrom()), false, // type1
											false, // paired
											extension, 1, // binsize
											props.DeDuplicate, region.getStart(), region.getEnd(), null, true));
							mapToSum(crickSum,
									client.getWeightHistogram(alignids,
											region.getGenome().getChromID(region.getChrom()), false, // type1
											false, // paired
											extension, 1, // binsize
											props.DeDuplicate, region.getStart(), region.getEnd(), null, false));
						}
						...
						for (String aid : alignids) {
							if (props.ShowType1Reads) {
								for (SingleHit hit : client.getSingleHits(aid,
										region.getGenome().getChromID(region.getChrom()), false, region.getStart(),
	...
	public void setDoSums(boolean b) {
		doSums = b;
	}

	public void setDoHits(boolean b) {
		doHits = b;
	}
	...
	private SeqHit convert(SingleHit hit) {
		...
		SeqHit out = new SeqHit(region.getGenome(), region.getChrom(), hit.strand ? hit.pos : hit.pos - hit.length + 1,
				hit.strand ? hit.pos + hit.length + 1 : hit.pos, hit.strand ? '+' : '-', hit.weight);
		return out;
	}
