...
/**
 * An ExpanderModel wraps an ewok Expander (something that maps an input of type
 * A to an iterator over type B). The expander's execute method is called in
 * response to new inputs and the results are cached such that the getResults()
 * method can be called multiple times.
 *
 * This caching is critical to the visualizer- we don't want to rerun the
 * database query every time the screen is redrawn (eg when the window is
 * resized or uncovered) but the input hasn't changed
 */
