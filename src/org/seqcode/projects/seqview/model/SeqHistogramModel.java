...
/**
 * Data model for chipseq histogram. Separate methods for retrieving plus and
...
public class SeqHistogramModel extends SeqViewModel implements RegionModel, Runnable {
	...
	public SeqHistogramModel(SeqAlignment a) throws IOException, ClientException {
		...
		if (!client.exists(Integer.toString(a.getDBID()))) {
			System.err.println("SeqHistogramModel: Error: " + a.getExpt().getName() + ";" + a.getExpt().getReplicate()
	...
	public SeqHistogramModel(Collection<SeqAlignment> a) throws IOException, ClientException {
		...
		for (SeqAlignment align : alignments) {
			...
			if (!client.exists(Integer.toString(align.getDBID()))) {
				System.err.println(
						"SeqHistogramModel: Error: " + align.getExpt().getName() + ";" + align.getExpt().getReplicate()
								+ ";" + align.getName() + "\tRDBID:" + align.getDBID() + " does not exist in ReadDB.");
				dataError = true;
			}
		}
	}

	public SeqHistogramModelProperties getProperties() {
		return props;
	}
	...
	// Forces getting data for region again, even if same region (good for model
	// properties updates).
					...
					if (props.UseWeights) {
						if (!props.ShowPairedReads || props.ShowSingleReads) {
							try {
								if (props.ShowType1Reads) {
									resultsPlus = Aggregator.mergeHistogramsFF(resultsPlus,
											client.getWeightHistogram(ids,
													region.getGenome().getChromID(region.getChrom()), false, // type1
													false, extension, width, (int) props.DeDuplicate, region.getStart(),
													region.getEnd(), null, true));

									resultsMinus = Aggregator.mergeHistogramsFF(resultsMinus,
											client.getWeightHistogram(ids,
													region.getGenome().getChromID(region.getChrom()), false, // type1
													false, extension, width, (int) props.DeDuplicate, region.getStart(),
													region.getEnd(), null, false));
								}
								if (props.ShowType2Reads) {
									resultsPlus = Aggregator.mergeHistogramsFF(resultsPlus,
											client.getWeightHistogram(ids,
													region.getGenome().getChromID(region.getChrom()), true, // type2
													false, extension, width, (int) props.DeDuplicate, region.getStart(),
													region.getEnd(), null, true));
								...
								// Fail silently if there are no single read
								// alignments
							}
						}
						if (props.ShowPairedReads) {
							try {
								resultsPlus = Aggregator.mergeHistogramsFF(resultsPlus,
										client.getWeightHistogram(ids, region.getGenome().getChromID(region.getChrom()),
												false, true, extension, width, (int) props.DeDuplicate,
												region.getStart(), region.getEnd(), null, true));

								resultsMinus = Aggregator.mergeHistogramsFF(resultsMinus,
										client.getWeightHistogram(ids, region.getGenome().getChromID(region.getChrom()),
												false, true, extension, width, (int) props.DeDuplicate,
												region.getStart(), region.getEnd(), null, false));
							} catch (Exception ex) {
								// Fail silently if there are no paired read
								// alignments
							}
						}
							...
							try {
								if (props.ShowType1Reads) {
									resultsPlus = Aggregator.mergeHistogramsIF(client.getHistogram(ids,
											region.getGenome().getChromID(region.getChrom()), false, // type1
											false, extension, width, (int) props.DeDuplicate, region.getStart(),
											region.getEnd(), null, true), resultsPlus);

									resultsMinus = Aggregator.mergeHistogramsIF(client.getHistogram(ids,
											region.getGenome().getChromID(region.getChrom()), false, // type1
											false, extension, width, (int) props.DeDuplicate, region.getStart(),
											region.getEnd(), null, false), resultsMinus);
								}
								if (props.ShowType2Reads) {
									resultsPlus = Aggregator.mergeHistogramsIF(client.getHistogram(ids,
											region.getGenome().getChromID(region.getChrom()), true, // type2
						...
						if (props.ShowPairedReads) {
							try {
								resultsPlus = Aggregator.mergeHistogramsIF(client.getHistogram(ids,
										region.getGenome().getChromID(region.getChrom()), false, true, extension, width,
										(int) props.DeDuplicate, region.getStart(), region.getEnd(), null, true),
										resultsPlus);

								resultsMinus = Aggregator.mergeHistogramsIF(client.getHistogram(ids,
										region.getGenome().getChromID(region.getChrom()), false, true, extension, width,
										(int) props.DeDuplicate, region.getStart(), region.getEnd(), null, false),
										resultsMinus);
							} catch (Exception ex) {
								// Fail silently if there are no paired read
								// alignments
