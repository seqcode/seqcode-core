...
import org.seqcode.projects.seqview.components.SeqViewOptionsFrame;
 ...
 * 
 * It aims to be the top-level class, running the others, whereas
 * WarpOptionsFrame was the top-level class in WarpDrive. This class assumes you
 * will view one genome version at a time (RegionFrame did not). If you switch
 * genomes, the current session should be wiped out.
 * 
 * @author mahony
 *
 */
public class SeqView extends JFrame {
	...
	public SeqViewOptions getOptions() {
		return options;
	}
	...
	public void updateOptions(SeqViewOptions opts) {
		if (regPanel == null) {
			// regPanel should have been created in the constructor, so if this
			// is true, something is wrong.
			// Nevertheless:
	...
	private JMenuBar createDefaultJMenuBar() {
		 ...
		 * actionPerformed(ActionEvent e) { RegionListPanel rlp = new
		 * RegionListPanel(thisviewer.getRegionPanel(),null);
		 * RegionListPanel.makeFrame(rlp); new
		 * BindingScanSelectFrame(thisviewer.getRegionPanel().getGenome(),rlp);
		 * } });
		 */
		...
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (v == JFileChooser.APPROVE_OPTION) {
					...
					java.util.List<Region> regions = RegionPanel.readRegionsFromFile(regPanel.getGenome(),
							f.getAbsolutePath(), false);
					RegionListPanel p = new RegionListPanel(regPanel, regions);
		...
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RegionListPanel p = new RegionListPanel(regPanel, new ArrayList<Region>());
				RegionListPanel.makeFrame(p);
			}
		});
	...
	class AboutMessage extends JFrame implements ActionListener {
		...
		public AboutMessage(String version) {
			...
			try {
				...
				String text = "<html><center>" + "<p>" + version + "<p>" + "<p>Written by Shaun Mahony" + "<p>"
						+ "<p>Based on the GSE library," + "<p>by Alex Rolfe & Tim Danford"
						+ "<p>(Gifford Lab, CSAIL, MIT)" + "</html>";
				...
				if (iconlabel != null)
					...
					javax.swing.JLabel svlabel = new javax.swing.JLabel(
							new String("<html><center><font size=\"+2\" color=\"blue\">SeqView</font></html>"));
