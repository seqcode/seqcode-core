...
/**
 * Base class for SeqView (paintable and model) properties. Uses Tim's JSON
 * model stuff to handle serializing and deserializing.
 */
public abstract class SeqViewProperties extends Model {
	 ...
	 * Used to prevent multiple windows from working on the same properties at
	 * once.
	 */
	...
	static class DoneConfiguring implements Runnable {
		...
		public DoneConfiguring(Collection<SeqViewProperties> p, MultiModelPrefs mmp) {
			this.p = p;
			this.mmp = mmp;
		}
	...
	public static void configure(Collection<? extends SeqViewProperties> props, RegionPanel regionpanel,
			boolean batchUpdate) {
	...
	/**
	 * Since both PaintableProperties and ModelProperties inherit from here, we
	 * don't want to confuse the two types of properties. So each subclass needs
	 * to define the suffix that its files use.
	 */
	...
	/**
	 * Displays a JFileChooser to select the name of the file to which you want
	...
	/**
	 * Displays a JFileChooser to select the file from which you want to load
	 * the properties
	 */
	 ...
	 * FOO.defaults.svpp (seq view paintable properties) where FOO is the
	 * runtime class name in - current directory - home directory - the whole
	 * classpath
	 */
