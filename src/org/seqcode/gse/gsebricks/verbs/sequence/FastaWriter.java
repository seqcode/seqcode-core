...
import org.seqcode.gse.gsebricks.verbs.FileLineExpander;
import org.seqcode.gse.gsebricks.verbs.Sink;
import org.seqcode.gse.utils.Closeable;
    ...
    public void consume(X r) {
        ...
        if (r instanceof StrandedRegion) {
            if (((StrandedRegion)r).getStrand() == '-') {
                s = org.seqcode.gse.utils.sequence.SequenceUtils.reverseComplement(s);
            }
        }
        int start = 0;
        int len = s.length();
