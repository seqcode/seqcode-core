package org.seqcode.gse.projects.gps.discovery;
...
import org.seqcode.gse.gsebricks.verbs.chipseq.MACSParser;
import org.seqcode.gse.gsebricks.verbs.chipseq.MACSPeakRegion;
import org.seqcode.gse.projects.gps.BindingModel;
import org.seqcode.gse.projects.gps.DeepSeqExpt;
import org.seqcode.gse.projects.gps.PairedCountData;
import org.seqcode.gse.projects.gps.StrandedBase;
import org.seqcode.gse.projects.gps.analysis.MultiIndependentMixtureCounts;
import org.seqcode.gse.projects.gps.features.*;
import org.seqcode.gse.projects.gps.utilities.CommonUtils;
import org.seqcode.gse.projects.gps.utilities.ReadCache;
import org.seqcode.gse.tools.utils.Args;
import org.seqcode.gse.utils.Pair;
import org.seqcode.gse.utils.Utils;
...
//							if ((ratio>=fold && cf.getAverageIpCtrlLogKL()>kl_ic) || (cf.getAverageIpCtrlLogKL()<kl_ic && ratio>=fold*2)){
							if (ratio>=fold && cf.getAverageIpCtrlLogKL()>kl_ic){
								notFiltered = true;
								break;
							}
						}						
...
//	}//end of normExpts method
	...
	private double calcSlope(List<PairedCountData> scalePairs) {
        ...
        DataFrame df = new DataFrame(org.seqcode.gse.projects.gps.PairedCountData.class, scalePairs.iterator());
			...
			for(String chrom:chrom_comp_pair.keySet()) {
				...
				for(int c = 0; c < numConditions; c++) {
					...
					for(int i:chrom_comp_pair.get(chrom)) {
						...
						Region expandedRegion = new Region(gen, chrom, Math.max(1, cf.getPosition().getLocation()-third_lambda_region_width), Math.min(chromLen, cf.getPosition().getLocation()+third_lambda_region_width));
...
//		int left_peak           = (int)Math.min(chromLen-1, peakRegion.getStart() + shift_size);
		...
		left_third_region   = (int)Math.max(         1, pos.getLocation() - third_lambda_region_width/2);
		left_second_region  = (int)Math.max(         1, pos.getLocation() - second_lambda_region_width/2);
		left_first_region   = (int)Math.max(         1, pos.getLocation() - first_lambda_region_width/2);

//		int right_peak          = (int)Math.max(         0, peakRegion.getEnd()   - shift_size);
		right_peak          = (int)Math.max(         1, peakRegion.getEnd());
		right_third_region  = (int)Math.min(chromLen, pos.getLocation() + third_lambda_region_width/2);
		right_second_region = (int)Math.min(chromLen, pos.getLocation() + second_lambda_region_width/2);
		right_first_region  = (int)Math.min(chromLen, pos.getLocation() + first_lambda_region_width/2);
