package org.seqcode.gse.tools.seqdata.editor;

import org.seqcode.gse.datasets.seqdata.SeqExpt;
import org.seqcode.gse.viz.components.ObjectTableModel;
...
public class SeqExptTableModel extends ObjectTableModel<SeqExpt> {
    ...
    public String getColumnName(int i) {
        ...
        if(i==5) { return "Replicate"; }
        return null;
    }

    public Object getValueAt(int rowIndex, int c) {
        if(c==0) { return getObject(rowIndex).getExptType().getName(); }
        if(c==1) { return getObject(rowIndex).getLab().getName(); }
        if(c==2) { return getObject(rowIndex).getExptCondition().getName(); }
        if(c==3) { return getObject(rowIndex).getExptTarget().getName(); }
        if(c==4) { return getObject(rowIndex).getCellLine().getName(); }
        if(c==5) { return getObject(rowIndex).getReplicate(); }
        return null;
    }
