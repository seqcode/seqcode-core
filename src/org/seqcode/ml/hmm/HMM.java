 ...
 * Forward-Backward, Viterbi, K-means, Baum-Welch, and Kullback-Leibler distance
 * measure.
 *
 * @author Kanav Kahol
...
public class HMM {
	 ...
	 * sequence o, it will train this HMM using o to increase the probability of
	 * this HMM generating o.
	 *
	 * @param o
	...
	public void baumWelch(int[] o, int steps) {
		...
		for (int s = 0; s < steps; s++) {
			// calculation of Forward and Backward Variables from the current
			// model
			fwd = forwardProc(o);
			bwd = backwardProc(o);
	...
	/**
	 * Calculation of Forward variables f(i,t) for state i at time t for
	...
	/**
	 * Calculation of Backward variables b(i,t) for state i at time t for
	 * sequence o with the current HMM parameters.
	 *
	 * @param o
	 *            the observation sequence
	...
	/**
	 * Calculation of xi_t(i, j), which is the probability P(i_t = s_i, i_t+1 =
	 * s_j | o, hmm), that is, the probability of being in state i at time t and
	 * state j at time t+1 given observation sequence o and this HMM.
	 *
	 * @param t
	 *            the time
	 * @param i
	...
	/**
	 * Calculation of gamma_t(i), which is the probability P(i_t = s_i | o,
	 * hmm), that is, the probability of being in state i at time given
	...
	/**
	 * Viterbi algorithm for hidden Markov models. Given an observation sequence
	 * o, Viterbi finds the best possible state sequence for o on this HMM,
	 * along with the state optimized probability.
	 *
	 * @param o
	 *            the observation sequence
	...
	/**
	 * K-means algorithm for hidden Markov models. Given training data, this
	 * algorithm will classify the data into a specified number of states, then
	 * using the final classification, calculate initial, transition, and bias
	 * probabilities.
	 *
	 * @param data
	 *            the filename of training data file
	 * @param N
	 *            the number of states
	 * @param steps
	 *            the number of iterations
	 * @throws IOException
	 *             If training data file is invalid
	 */
	...
	/*
	 * Calculates and sets the initial means for the K-means algorithm.
	 */
	private static void setInitialMeans(double[][] means, double[][][] trainingData, int D, int w, int T, int N) {
		double[][] sortedarray = new double[w * T][3];
		int baseIndex;
		Random rand = new Random();

		/*
		 * Construct the initial sortedarray (currently unsorted) by storing
		 * each training data point as a square root of a sum of its elements
		 * squared. Note that its position within the file is stored.
		 */
	...
	/*
	 * Calculates the initial, transition, and bias probabilities for the
	 * K-means algorithm.
	 */
	private static void probabilitiesFromKmeans(HMM newHmm, int[][] classification, double[][][] trainingData, int N,
			int D, int w, int T) {
	 ...
	 */
	private static int getSymbols(double[][] symbolSet, double[][][] trainingData, int D, int w, int T) {
	...
	/**
	 * Calculates the distance between two hidden Markov models using the
	 * Kullback-Leibler distance measure. The models should have the same size
	 * vocabulary.
	 *
	 * @param hmm2
	 *            the hidden Markov model that will be compared against this HMM
	...
	public double hmmDistance(HMM hmm2) {
		...
		/*
		 * Generate observation sequences from both HMMs. Note that the length
		 * is 4000 since research has found that the distance stabilizes at a
		 * length of 4000.
		 */
	...
	/*
	 * Calculates the Euclidean distance between two means: the current mean and
	 * mean k.
	 */
	 ...
	 */
	private static boolean symbolCheck(double[] symbol, double[][] symbolSet, int D, int w, int T) {
		boolean inSet = false;
		boolean match;
	...
	/**
	 * Converts a sequence of actually values into their corresponding indices
	 * based on the vocabulary of this HMM. Any time a hidden Markov model
	 * algorithm whose input involves an observation sequence is used, the
	 * observation sequence should be converted to indices if it's not already.
	 *
	 * @param sequence
	 *            the observation sequence (actual values)
	 * @param D
	 *            the number of dimensions of sequence
	 * @param T
	...
	/**
	 * Loads an HMM from a file. A new HMM is created and the read values are
	 * stored as the HMM parameters. Returned is the new HMM.
	 *
	 * @param filename
	 *            name of file to read the HMM from
	...
	/**
	 * Writes this HMM to a file. Information written includes the number of
	 * states, output vocabulary size, number of dimensions (-1 if not used),
	 * initials, transitions, biases, and, if it was used, the output
	 * vocabulary.
	 *
	 * @param filename
	 *            name of file to write this HMM to
	 * @throws IOException
	 *             if a problem occurred while writing to the file
	 */
