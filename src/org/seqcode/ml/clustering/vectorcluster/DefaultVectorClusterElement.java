...
/**
 * A default implementation of VectorClusterElement which implements a vector of
 * Doubles for each index, with null's indicating missing values.
 * 
 * @author tdanford
 */
public class DefaultVectorClusterElement implements VectorClusterElement {
	...
	public boolean equals(Object o) {
		if (!(o instanceof DefaultVectorClusterElement)) {
			return false;
		}
		DefaultVectorClusterElement e = (DefaultVectorClusterElement) o;
		if (array.length != e.array.length) {
			return false;
		}
		for (int i = 0; i < array.length; i++) {
			if (!equal(array[i], e.array[i])) {
				return false;
			}
		}
		return true;
