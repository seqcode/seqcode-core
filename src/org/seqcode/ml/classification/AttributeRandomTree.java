 ...
 */
public class AttributeRandomTree extends AbstractClassifier
		implements OptionHandler, WeightedInstancesHandler, Randomizable, Drawable, PartitionGenerator {

	public int[] nodeNumInstances() {
		if (m_zeroR != null)
			throw new Error("not yet implemented");

		int[] weights = new int[m_Info.numAttributes()];
		m_Tree.nodeNumInstances(weights);
		// for (int i = 0; i < weights.length; i++)
		// if (weights[i] != 0)
		// System.out.println(i + " " + m_Info.attribute(i).name() + " " +
		// weights[i]);
		return weights;
	}

	public String[] getAttributes() {
		String[] a = new String[m_Info.numAttributes()];
		for (int i = 0; i < a.length; i++)
			a[i] = m_Info.attribute(i).name();
		return a;
	}
	...
	public Enumeration<Option> listOptions() {
		...
		newVector.addElement(
				new Option("\tNumber of folds for backfitting " + "(default 0, no backfitting).", "N", 1, "-N <num>"));
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		if (data.classAttribute().isNumeric()) {
			trainVariance = AttributeRandomTree.singleVariance(classProbs[0], totalSumSquared, totalWeight)
					/ totalWeight;
	...
	protected class Tree implements Serializable {
		...
		protected int nodeNumInstances(int[] weights) {
			if (m_Successors == null || m_Successors.length == 0)
				throw new Error("not a node, has no successors!");
			int sum = 0;
			for (Tree t : m_Successors) {
				if (t.m_Attribute == -1)
					sum += t.leafNumInstances();
				else
					sum += t.nodeNumInstances(weights);
			}
			weights[m_Attribute] += sum;
			return sum;
		}
		...
		public int toGraph(StringBuffer text, int num) throws Exception {
			...
			String classValue = m_Info.classAttribute().isNominal() ? m_Info.classAttribute().value(maxIndex)
			...
			try {
				...
				if (m_Attribute == -1) {
					...
					for (int i = 0; i < m_Successors.length; i++) {
						...
						text.append(
								m_Info.attribute(m_Attribute).name() + " = " + m_Info.attribute(m_Attribute).value(i));
		...
		protected void backfitData(Instances data, double[] classProbs, double totalWeight) throws Exception {
			...
			if (data.classAttribute().isNumeric()) {
				...
				priorVar = AttributeRandomTree.singleVariance(totalSum, totalSumSquared, totalSumOfWeights);
			}
			...
			if (m_Attribute > -1) {
				...
				// If we have a least two non-empty successors, we should keep
				// this tree
				/*
				 * int nonEmptySuccessors = 0; for (int i = 0; i <
				 * subsets.length; i++) { if
				 * (m_Successors[i].m_ClassDistribution != null) {
		...
		protected void buildTree(Instances data, double[] classProbs, int[] attIndicesWindow, double totalWeight,
			...
			if (Utils.gr(val, 0)) {
				...
				// If all successors are non-empty, we don't need to store the
				// class
				// distribution
			...
			if (attribute.isNominal()) {
				...
				for (int i = 0; i < indexOfFirstMissingValue; i++) {
					...
					if (inst.value(att) > currSplit) {
						currVal = AttributeRandomTree.variance(currSums, currSumSquared, currSumOfWeights);
						if (currVal < bestVal) {
							bestVal = currVal;
	...
	public static void main(String[] argv) {
		runClassifier(new AttributeRandomTree(), argv);
	}
}
