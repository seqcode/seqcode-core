...
/**
 * DataFrame holds a collection of Model objects -- intuitively, a DataFrame is
 * a matrix of data, although the matrix data is not necessarily numerical. Each
 * internal Model is a row in the matrix, and each field in the Model is a
 * column. The DataFrame gives accessor methods for getting data by either row
 * or column, for reading and writing tables to files, for assembling matrices
 * of numerical data, etc.
 * 
 * @author tdanford
 * @param <T>
 *            The particular subclass of Model whose objects make up the rows of
 *            the frame.
 */
public class DataFrame<T extends Model> {
	 ...
	 * 
	 * @param pred
	 *            The indicator for which elements are to be removed.
	...
	/**
	 * A Transformation<T,S> object turns objects of type T into objects of type
	 * S. The transform() method takes a transformation, where T is the
	 * model-type of this DataFrame, and returns a new DataFrame of S objects,
	 * where each object corresponds to a transformed version of the original T
	 * from this frame.
	 * 
	 * @param <S>
	 * @param trans
	...
	public <R extends Model, S extends Model> DataFrame<R> join(Class<R> rClass, DataFrame<S> outerFrame,
			String fieldName, String innerName, String outerName) {
		...
		for (int i = 0; i < outerFrame.size(); i++) {
			...
			try {
				...
				e.printStackTrace();
				throw new IllegalArgumentException(String.format("Field %s " + "was illegally accessed in Model %s",
						fieldName, outerFrame.getModelClass().getSimpleName()));
			}
		}
		...
		for (int i = 0; i < size(); i++) {
			...
			try {
				...
				e.printStackTrace();
				throw new IllegalArgumentException(String.format("Field %s " + "was illegally accessed in Model %s",
						fieldName, getModelClass().getSimpleName()));
			} catch (InstantiationException e) {
				e.printStackTrace();
				throw new IllegalArgumentException(
						String.format("Couldn't instantiate Model class %s", rClass.getSimpleName()));
			}
		}
	...
	/**
	 * Saves the data back out to a particular file -- this file then becomes
	 * the DataFrame's "default" file, and is the target when future calls are
	 * made to the save() method (no parameters).
	 * 
	 * @param f
	 * @throws IOException
	 */
	...
	/**
	 * Saves the data back out to the "default" file (either the file from which
	 * the data was loaded, or the file that was given as a parameter to the
	 * last call to save(File).)
	 * 
	 * @throws IOException
	 */
	...
	public Vector<String> getFields() {
		return fieldAnalysis.getFieldNames();
	}
	...
	/**
	 * For a particular named field in the model class, this returns a Set of
	 * all distinct values of that field among the rows of this DataFrame.
	 * 
	 * @param <FT>
	 * @param fieldName
	 * @return
	...
	public Double pearsonCorrelation(String xCol, String yCol) {
		...
		/**
		 * (n \sum xy - \sum x \sum y) r_xy =
		 * ------------------------------------------------------------- sqrt(n
		 * \sum x^2 - (\sum x)^2) * sqrt(n \sum y^2 - (\sum y)^2)
		 */
			...
			for (int i = 0; i < fieldOrder.size(); i++) {
				...
				try {
					...
					if (Model.isSubclass(type, Double.class)) {
						...
						System.err.println(
								String.format("Field %s has unsupported parsing type %s", f.getName(), type.getName()));
					}
