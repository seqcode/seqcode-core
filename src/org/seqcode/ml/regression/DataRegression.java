...
public class DataRegression<M extends Model> {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (String title : coeffs.keySet()) {
				...
				System.out.println(String.format("%s \t%.3f\t(%.3f, %.3f)", title, coeffs.get(title), b[0], b[1]));
			}
	...
	public DataRegression(DataFrame<M> f, String stmt) {
		...
		if (vs == null) {
			throw new IllegalArgumentException(String.format("Couldn't parse statement \"%s\"", stmt));
		}
