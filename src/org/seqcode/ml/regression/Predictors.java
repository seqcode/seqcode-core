...
public class Predictors<M extends Model> {
	...
	private Map<Field, Vector<Object>> factorCodes; // the unique values that
													// can be taken by each
													// factor variable.

	// Each interaction is the product of one or more variables, some of which
	// may be factors and the others 'numeric'.
	...
	public Predictors(DataFrame<M> f, String... fs) {
		...
		for (int i = 0; i < fs.length; i++) {
			if (seenFields.contains(fs[i])) {
				throw new IllegalArgumentException(String.format("Duplicate field name: %s", fs[i]));
			}
					...
					if (mf != null) {
						...
						if (Model.isSubclass(t, Number.class)) {
							...
							throw new IllegalArgumentException(
									String.format("Field %s is not a regression-ready predictor", fs[i]));
						}
						...
						throw new IllegalArgumentException(String.format("Unknown field name: %s", fs[i]));
					}
	...
	public void addInteraction(String... fns) throws NoSuchFieldException {
		...
		for (int i = 0; i < fields.length; i++) {
			if (fields[i] == null) {
				throw new NoSuchFieldException(fns[i]);
			}
		}
	...
	public void addPredictor(String fn) throws NoSuchFieldException {
		...
		if (Model.isSubclass(t, Double.class)) {
			...
			throw new NoSuchFieldException(String.format("%s is not a numeric field (%s)", fn, t.getName()));
		}
		cols += 1;
		columnNames.add(fn);
	}
	...
	public Matrix createMatrix(BitVector selector, Map<String, Transformation<Double, Double>> transforms) {
		...
		for (Field f : numeric) {
			Transformation<Double, Double> transform = transforms != null && transforms.containsKey(f.getName())
				...
				if (selector == null || selector.isOn(j)) {
					...
					try {
						...
						throw new IllegalStateException(
								String.format("Couldn't access field %s: %s", f.getName(), e.getMessage()));
					}
					i += 1;
				}
			}
	...
	private class Interaction {
		...
		public boolean equals(Object o) {
			if (!(o instanceof Predictors.Interaction)) {
				return false;
			}
			Interaction in = (Interaction) o;
			if (fields.size() != in.fields.size()) {
				return false;
			}
			for (Field f : fields) {
		...
		public Vector<InteractionValue> allInteractionValues(String[] names) {
			...
			// Analogous to taking out the very first value of a set of factor
			// values.
			vv.remove(0);
	...
	private class InteractionValue {
		...
		public boolean equals(Object o) {
			if (!(o instanceof Predictors.InteractionValue)) {
				return false;
			}
			InteractionValue iv = (InteractionValue) o;
			if (iv.values.size() != values.size()) {
				return false;
			}
			for (int i = 0; i < values.size(); i++) {
				if (!values.get(i).equals(iv.values.get(i))) {
					return false;
				}
			}
			return true;
