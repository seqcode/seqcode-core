...
/**
 * SAMFileHitLoader: A FileHitLoader for SAM and BAM files. Accounts for
 * uniqueness of hits according to user-specified option. Ignores secondary &
 * supplementary (i.e. chimeric) alignments.
 * 
 * @author mahony
 *
 */
public class SAMFileHitLoader extends FileHitLoader {
	...
	private boolean useChimericReads = false; // Ignore chimeric mappings for
												// now.

	public SAMFileHitLoader(File f, boolean nonUnique, boolean loadT1Reads, boolean loadT2Reads, boolean loadRead2,
			boolean loadPairs) {
		super(f, nonUnique, true, false, loadRead2, loadPairs);
		if (!loadT1Reads || loadT2Reads)
			System.err.println(
					"SAMFileHitLoader: You asked to load only Type1 or Type2 reads, we do not yet load this information from SAM format.");
	}

	/**
	 * Get the reads from the appropriate source (implementation-specific).
	 * Loads data to the fivePrimesList and hitsCountList Loads pairs to
	 * hitPairsList
	 */
	public void sourceAllHits() {
		...
		while (iter.hasNext()) {
			...
			if (record.isSecondaryOrSupplementary() && !useChimericReads) {
				continue;
			}
			if (record.getReadPairedFlag() && record.getSecondOfPairFlag() && !loadRead2) {
				continue;
			}
			...
			byRead.add(record); // Filter by first or second of pair here if
								// loading by type1/2?
			...
			if (loadPairs && record.getFirstOfPairFlag() && record.getProperPairFlag()) {
				...
				HitPair hp = new HitPair((neg ? record.getAlignmentEnd() : record.getAlignmentStart()),
						record.getMateReferenceName().replaceFirst("^chromosome", "").replaceFirst("^chrom", "")
								.replaceFirst("^chr", ""),
						(mateneg ? record.getMateAlignmentStart() + record.getReadLength() - 1
								: record.getMateAlignmentStart()),
						mateneg ? 1 : 0, 1);
				addPair(record.getReferenceName().replaceFirst("^chromosome", "").replaceFirst("^chrom", "")
						.replaceFirst("^chr", ""), neg ? '-' : '+', hp);
			}
		}
	...
	protected void processRead(Collection<SAMRecord> records) {
		...
		for (SAMRecord record : records) {
			...
			ReadHit currHit = new ReadHit(record.getReferenceName().replaceFirst("^chromosome", "")
					.replaceFirst("^chrom", "").replaceFirst("^chr", ""), start, end,
					record.getReadNegativeStrandFlag() ? '-' : '+', weight);
