...
/**
 * HitLoaders load alignment hits & pairs from various sources, including ReadDB
 * and various files. Five-prime positions and associated weight sums are loaded
 * into ArrayLists. Pairing information is loaded if requested and if it exits.
 * Where/how those hits & pairs are sourced is implementation-specific.
 * 
 * Five prime positions and weights are loaded into two collections of
 * ArrayLists, where the collections are indexed by chromosome name. Within each
 * chromosome's set, a 2D array of ArrayLists collects data for each strand.
 * However, the ArrayLists are temporary -- once a Sample loads the hits into
 * primitive arrays, the ArrayLists are reset and the garbage collector is
 * called.
 * 
 * @author mahony This class combines functionality from ReadLoaders,
 *         AlignmentFileReaders, and ReadCache in the old setup.
 */
public abstract class HitLoader {
	...
	protected boolean hasPairs = false; // Flag to say there are pairs in the
										// sample
	 ...
	 * Dimension in the array of ArrayLists represents the strand. 0 for '+', 1
	 * for '-'
	 */
	private HashMap<String, ArrayList<Integer>[]> fivePrimePosList = null;
	/**
	 * Sum of read hit weights that corresponds to the 5' position HashMap is
	 * indexed by chromosome name. <br>
	 * Dimension in the array of ArrayLists represents the strand. 0 for '+', 1
	 * for '-' Ordering of each ArrayList is the same as fivePrimePosList
	 */
	 ...
	 * Dimension in the array of ArrayLists represents the R1 read strand. 0 for
	 * '+', 1 for '-' Ordering of each ArrayList is the same as
	 * fivePrimePosList.
	 * 
	 */
	...
	public HashMap<String, ArrayList<Integer>[]> getFivePrimePositions() {
		return fivePrimePosList;
	}

	public HashMap<String, ArrayList<Float>[]> getFivePrimeCounts() {
		return fivePrimeCountsList;
	}

	public HashMap<String, ArrayList<HitPair>[]> getPairs() {
		return hitPairsList;
	}

	// Abstract methods
	/**
	 * Get all hits from the appropriate source (implementation-specific). Loads
	 * single end data to the fivePrimePosList and fivePrimeCountsList.
	 * Enforcing which reads to load (Type1 and/or Type2) is also
	 * implementation-specific. Loads pairs to hitPairsList (if requested & if
	 * they exist).
	 * 
	 */
	...
	/**
	 * Add hits to the list data structures. It may be called multiple times to
	 * retrieve all the data, then populateArrays() is called
	 */
