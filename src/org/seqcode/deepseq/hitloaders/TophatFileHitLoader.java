...
/**
 * TophatFileHitLoader: A FileHitLoader for Tophat SAM/BAM output. Each
 * alignment block is loaded as a separate hit. Ignores uniqueness.
 * 
 * @author mahony
 *
 */
public class TophatFileHitLoader extends FileHitLoader {
	...
	public TophatFileHitLoader(File f, boolean nonUnique, boolean loadT1Reads, boolean loadT2Reads, boolean loadRead2,
			boolean loadPairs) {
		super(f, nonUnique, true, false, loadRead2, loadPairs);
		if (!loadT1Reads || loadT2Reads)
			System.err.println(
					"TophatFileHitLoader: You asked to load only Type1 or Type2 reads, we do not load this information from Tophat SAM format.");
	}
		...
		while (iter.hasNext()) {
			...
			for (int a = 0; a < blocks.size(); a++) { // Iterate over alignment
				...
				ReadHit currHit = new ReadHit(
						record.getReferenceName().replaceFirst("^chromosome", "").replaceFirst("^chrom", "")
								.replaceFirst("^chr", ""),
						aStart, aEnd, record.getReadNegativeStrandFlag() ? '-' : '+', weight);
