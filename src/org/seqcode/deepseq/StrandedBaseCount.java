...
/**
 * StrandedBaseCount represents the sum of hit weights at a stranded base
 * position in the genome. The same class was called StrandedBase previously
 * (renaming to reduce confusion) Coordinate is the 5' end of the read. We do
 * not store chromomosome info here because this class is always used in the
 * context of a chromosome or a region, no need to differentiate. It records the
 * number of reads mapped to this base position.
 * 
 * @author yguo
 *
 */
public class StrandedBaseCount implements Comparable<StrandedBaseCount> {
	...
	public int compareTo(StrandedBaseCount b) {
		...
		return diff == 0 ? (strand == b.strand ? 0 : strand == '+' ? 1 : -1) : // same
																				// coord,
																				// compare
																				// strand
