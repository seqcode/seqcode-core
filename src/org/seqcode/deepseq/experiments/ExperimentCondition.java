...
/**
 * ExperimentCondition is a collection of ControlledExperiments representing a
 * set of experimental replicates. Scaling replicate read counts against each
 * other is NOT necessary under the current design philosophy.
 * 
 * @author Shaun Mahony
 * @version %I%, %G%
 */
public class ExperimentCondition {
	...
	public ExperimentCondition(ExptConfig c, int idx, String n, List<ControlledExperiment> reps,
			boolean estimateScalingFactors) {
		econfig = c;
		index = idx;
		name = n;
		replicates = reps;
	...
	public List<Sample> getSignalSamples() {
	...
	public double getPooledSampleControlScaling() {
		return pooledSampleControlScaling;
	}
	...
	/**
	 * Runs the chosen scaling method for all underlying replicates (signal vs
	 * control) and for the pooled signal in this condition vs the pooled
	 * control.
	 * 
	 * @param scalingWindowSize
	 *            : size of scaling window in bp
	 */
	protected void calculateControlScalingFactors(int scalingWindowSize) {
		// No point in doing any of this unless some of the replicates have
		// controls
			...
			for (ControlledExperiment expt : getReplicates()) {
				if (expt.hasControl()) {
					...
					if (econfig.getScalingBySES())
						expt.setScaling(scaler.scalingRatioBySES(sampleWindowCounts.get(expt.getSignal()),
								sampleWindowCounts.get(expt.getControl())));
					else if (econfig.getScalingByRegression())
						expt.setScaling(scaler.scalingRatioByRegression(sampleWindowCounts.get(expt.getSignal()),
								sampleWindowCounts.get(expt.getControl())));
					else if (econfig.getScalingByMedian())
						expt.setScaling(scaler.scalingRatioByMedian(sampleWindowCounts.get(expt.getSignal()),
								sampleWindowCounts.get(expt.getControl())));
					else
						expt.setScaling(scaler.scalingRatioByNCIS(sampleWindowCounts.get(expt.getSignal()),
								sampleWindowCounts.get(expt.getControl()), scalingPlotFilename));
				}
			}

			// Calculate scaling factor for pooled signal vs pooled control for
			// this condition
