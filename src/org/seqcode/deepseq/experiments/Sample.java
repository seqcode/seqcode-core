...
/**
 * Sample represents a single experimental sample whose hits are sourced from
 * one or more HitLoaders.
 * 
 * @author mahony
 *
 */
public class Sample {
	...
	protected String sourceName = ""; // String describing the source files or
										// DBIDs
	protected double totalHits; // totalHits is the sum of alignment weights
	protected double totalHitsPos; // totalHitsPos is the sum of alignment
									// weights on the plus strand
	protected double totalHitsNeg; // totalHitsNeg is the sum of alignment
									// weights on the minus strand
	protected double uniqueHits; // count of unique mapped positions (just
									// counts the number of bases with non-zero
									// counts - does not treat non-uniquely
									// mapped positions differently)
	 ...
	 * 
	 * @param g
	 *            Genome (can be null to estimate from data)
	 * @param name
	 *            String
	 */
	...
	public double getStrandedHitCount(char strand) {
		return (strand == '+' ? totalHitsPos : totalHitsNeg);
	}
	 ...
	 * 
	 * @param cacheEntireGenome
	 *            : boolean to keep the full set of hits cached
	 * @param initialCachedRegions
	 *            : list of regions to keep cached at the start (can be null)
	 */
	public void initializeCache(boolean cacheEntireGenome, List<Region> initialCachedRegions) {
		cache = new HitCache(econfig.getLoadPairs(), econfig, loaders, maxReadsPerBP, cacheEntireGenome,
				initialCachedRegions);
	...
	/**
	 * Load all base counts in a region, regardless of strand. If caching in
	 * local files, group calls to this method by same chromosome.
	 * 
	 * @param r
	 *            Region
	...
	/**
	 * Loads hits from a given strand in the region. If caching in local files,
	 * group calls to this method by same chromosome.
	 * 
	 * @param r
	 *            Region
	 * @return List of StrandedBaseCounts
	...
	/**
	 * Load all paired hits that have an R1 read in a region. If caching in
	 * local files, group calls to this method by same chromosome.
	 * 
	 * @param r
	 *            Region
	...
	/**
	 * Sum of all hit weights in a region. If caching in local files, group
	 * calls to this method by same chromosome.
	 * 
	 * @param r
	 *            Region
	 * @return float
	...
	/**
	 * Sum of hit weights in one strand of a region. If caching in local files,
	 * group calls to this method by same chromosome.
	 * 
	 * @param r
	 *            Region
