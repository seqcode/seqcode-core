...
/**
 * ExperimentScaler: calculate a scaling transformation between all Sample pairs
 * in an ExperimentCondition This is performed on the condition level so that a
 * scaling can also be defined between pooled hits from all signals & controls
 * 
 * @author Shaun Mahony
 * @version %I%, %G%
 */
public class ExperimentScaler {
	...
	/**
	 * Find the scaling ratio according to the SES method from Diaz, et al. Stat
	 * Appl Genet Mol Biol. 2012. Also sets a background proportion estimate for
	 * the signal channel.
	 * 
	 * @return
	 */
	 ...
	 * (BMC Bioinf 2012). Also sets a background proportion estimate for the
	 * signal channel. Should be run using *all* genomic windows in the Lists.
	 ...
	 *            : control list
	 * @param outputFile
	 *            : optional file that will contain the data
	 * @return
	 */
	public double scalingRatioByNCIS(List<Float> setA, List<Float> setB, String outputFile) {
		...
		for (PairedCounts pc : counts) {
			...
			i++;
			if (i / numPairs > 0.75 && cumulA > 0 && cumulB > 0) { // NCIS
																	// estimates
																	// begin
																	// using the
																	// lower 3
		...
		if (outputFile != null) {
			...
			plotter.saveXYplot(dataToPlot, totalAtScaling, scalingRatio, "Binned Total Tag Count",
					"Cumulative Count Scaling Ratio", outputFile + ".NCIS_scaling-ccr.png", true);
			ScalingPlotter plotter2 = new ScalingPlotter(outputFile + " NCIS plot");
			plotter2.saveXYplot(dataToPlot2, totalAtScaling, scalingRatio, "Binned Total Tag Count",
					"Marginal Signal/Control Ratio", outputFile + ".NCIS_scaling-marginal.png", true);
		}
	...
	/**
	 * Calculate the background proportion of an IP experiment by correcting the
	 * scaling ratio by the read count ratio. Be careful with this method, there
	 * are a couple of assumptions: - The scaling ratio was calculated between
	 * IP and control experiments - The method used to calculate the scaling
	 * ratio attempted to normalize to background and not all regions (e.g. SES
	 * method attempts background normalization)
	 * 
	 * @return
	 */
		...
		if (gconfig.helpWanted()) {
			...
			for (ExperimentCondition c : exptMan.getConditions()) {
				for (ControlledExperiment r : c.getReplicates()) {
					...
					if (r.getControl() == null)
						...
						System.err.println("\tSignal:\t" + r.getSignal().getHitCount() + "\tControl:\t"
								+ r.getControl().getHitCount());
				}
			}
			...
			for (Sample samp : exptMan.getSamples()) {
				...
				for (String chrom : genome.getChromList()) {
					...
					for (int start = 1; start < chrlen - econfig.getScalingSlidingWindow(); start += econfig
							.getScalingSlidingWindow()) {
			...
			System.out.println("\tNumbers of windows:\tAll="
					+ sampleWindowCounts.get(exptMan.getSamples().get(0)).size() + "\tnoPotenials="
					+ noPotSampleWindowCounts.get(exptMan.getSamples().get(0)).size() + "\n");
				...
				if (sampA.isSignal()) {
					for (Sample sampB : exptMan.getSamples())
						if (sampA != null && sampB != null && sampA.getIndex() != sampB.getIndex()) {
							...
							System.out.println(
									"HitRatio\t" + sampA.getName() + " vs " + sampB.getName() + "\t" + hitRatio);
						}
				}
			}
			...
			for (Sample sampA : exptMan.getSamples()) {
				if (sampA.isSignal()) {
					for (Sample sampB : exptMan.getSamples())
						if (sampA != null && sampB != null && sampA.getIndex() != sampB.getIndex())
							System.out.println("Median\t" + sampA.getName() + " vs " + sampB.getName() + "\t"
									+ scaler.scalingRatioByMedian(sampleWindowCounts.get(sampA),
											sampleWindowCounts.get(sampB)));
				}
			}
			...
			for (Sample sampA : exptMan.getSamples()) {
				if (sampA.isSignal()) {
					for (Sample sampB : exptMan.getSamples())
						if (sampA != null && sampB != null && sampA.getIndex() != sampB.getIndex())
							System.out.println("Regression\t" + sampA.getName() + " vs " + sampB.getName() + "\t"
									+ scaler.scalingRatioByRegression(sampleWindowCounts.get(sampA),
											sampleWindowCounts.get(sampB)));
				}
			}
			...
			for (Sample sampA : exptMan.getSamples()) {
				if (sampA.isSignal()) {
					for (Sample sampB : exptMan.getSamples())
						if (sampA != null && sampB != null && sampA.getIndex() != sampB.getIndex())
							System.out.println("SES\t" + sampA.getName() + " vs " + sampB.getName() + "\t" + scaler
									.scalingRatioBySES(sampleWindowCounts.get(sampA), sampleWindowCounts.get(sampB)));
				}
			}
			...
			for (Sample sampA : exptMan.getSamples()) {
				if (sampA.isSignal()) {
					for (Sample sampB : exptMan.getSamples())
						if (sampA != null && sampB != null && sampA.getIndex() != sampB.getIndex())
							System.out.println("NCIS\t" + sampA.getName() + " vs " + sampB.getName() + "\t"
									+ scaler.scalingRatioByNCIS(sampleWindowCounts.get(sampA),
											sampleWindowCounts.get(sampB), null));
				}
			}

			// Regression after filtering out potential regions (i.e. PeakSeq
			// using Pf=1)
			for (Sample sampA : exptMan.getSamples()) {
				if (sampA.isSignal()) {
					for (Sample sampB : exptMan.getSamples())
						if (sampA != null && sampB != null && sampA.getIndex() != sampB.getIndex())
							System.out.println("PeakSeq\t" + sampA.getName() + " vs " + sampB.getName() + "\t"
									+ scaler.scalingRatioByRegression(noPotSampleWindowCounts.get(sampA),
											noPotSampleWindowCounts.get(sampB)));
				}
			}
	...
	/**
	 * Simple class for storing paired counts that are sortable in first
	 * dimension
	 * 
	 * @author mahony
	 *
	...
	public class ScalingPlotter extends ScatterPlot {
		...
		/**
		 * Make an XY scatter plot from a 2-D dataset and save image. This
		 * configuration is used by deepseq.stats.Normalization classes
		 * 
		 * @param datapoints
		 *            - 2D dataset (colored grey)
		 * @param datapoints_highlight
		 *            - 2D dataset (colored blue), can be null
		 * @param yLine
		 *            Double - data coordinates of line to be drawn parallel to
		 *            x axis (used to show scaling line)
		 * @param outFilename
		 *            - String
		 * @param rasterImage
		 *            - boolean
		 */
		private void saveXYplot(Matrix datapoints, double scalingTotal, double scalingRatio, String xName, String yName,
				String outFilename, boolean rasterImage) {
			this.setWidth(800);
			this.setHeight(800);
