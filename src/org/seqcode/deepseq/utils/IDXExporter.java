...
public class IDXExporter {
	...
	protected char baseLimit = '.'; // Only use tags with this character at
									// baseLimitRelPosition relative to 5' end
									// (. = use all tags)
						...
						for (int x = currentRegion.getStart(); x <= currentRegion.getEnd(); x += 100000000) {
							...
							for (int i = currSubRegion.getStart(); i < currSubRegion.getEnd(); i++) {
								...
								if (posHits > 0 || negHits > 0) {
									fw.write("chr" + currSubRegion.getChrom() + "\t" + i + "\t"
											+ String.format("%.0f\t%.0f\t%.0f", posHits, negHits, sum) + "\n");
								}
								// Print out progress
								basesDone++;
