...
/**
 * This class aims to test the sequencing read coverage at a set of points (e.g.
 * coverage overlapping a set of SNPs).
 * 
 * @author mahony
 *
 */
			...
			for (ControlledExperiment rep : c.getReplicates()) {
				...
				try {
					FileWriter fw = new FileWriter(
							outName + "." + c.getName() + "." + rep.getName() + ".point-coverage.txt");
					...
					while (chroms.hasNext()) {
						...
						for (int x = currentRegion.getStart(); x <= currentRegion.getEnd(); x += 100000000) {
							...
							// Get coverage of points that lie within the
							// current region
							for (Point pt : testSites) {
								if (currSubRegion.contains(pt)) {
									...
									fw.write("chr" + pt.getChrom() + ":" + pt.getLocation() + "\t"
		...
		for (StrandedBaseCount r : hits) {
			if (r.getCoordinate() >= currReg.getStart() && r.getCoordinate() <= currReg.getEnd()) {
				if (r.getStrand() == '+') {
					...
					int offsetStart = inBounds(r.getCoordinate() - readLen + 1 - currReg.getStart(), 0,
							currReg.getWidth());
