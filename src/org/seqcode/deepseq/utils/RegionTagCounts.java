...
public class RegionTagCounts {
	...
	private boolean scaleToPercentile = false; // scale to Xth percentile of
												// observed counts
	...
	public void setTotalTagNorm(boolean n) {
		totalTagNorm = n;
	}

	public void setSigPropNorm(boolean n) {
		sigPropNorm = n;
	}
			...
			for (ControlledExperiment rep : c.getReplicates()) {
				...
				while (chroms.hasNext()) {
					...
					for (int x = currentRegion.getStart(); x <= currentRegion.getEnd(); x += 100000000) {
						...
						// Get coverage of points that lie within the current
						// region
						for (Region r : testRegs) {
							if (currSubRegion.contains(r)) {

								int offsetStart = inBounds(r.getStart() - currSubRegion.getStart(), 0,
										currSubRegion.getWidth() - 1);
								int offsetEnd = inBounds(r.getEnd() - currSubRegion.getStart(), 0,
										currSubRegion.getWidth() - 1);
								double sum = 0;
	...
	public static void main(String[] args) {
		...
		if (args.length == 0 || ap.hasKey("h")) {
			...
			System.err.println("Genome:" + "\t--species <Species;Genome>\n" + "\tOR\n"
			...
			rct.setSigPropNorm(signorm);
			rct.setTotalTagNorm(totnorm);
			rct.setPercentileScale(scalePercentile);
			rct.execute();
			rct.close();
		}
