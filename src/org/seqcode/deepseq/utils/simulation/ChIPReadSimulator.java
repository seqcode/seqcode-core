...
/**
 * Simulates single or double condition fragments and reads using BindingModels.
 * <br>
 * 
 * In any given simulated replicate experiment, reads are simulated according to
 * a BindingModel. Read counts for each binding event in each condition are
 * determined from a CountDataSimulator, which allows for differential binding
 * between two conditions, and negative-binomial sampled read counts in each
 * replicate. Noise reads are distributed uniformly across the genome or sampled
 * from an input tag file.
 * 
 * 
 * @author Shaun Mahony
 *
 *         Extended from
 *         org.seqcode.projects.multigps.utilities.MultiConditionReadSimulator
 */
public class ChIPReadSimulator {
	...
	public ChIPReadSimulator(BindingModel m, Genome g, List<SimCounts> counts, int numCond, int numRep,
			double noiseProb, double jointRate, int jointSpacing, String outPath) {
		model = m;
	...
	/**
	 * Make binding positions corresponding to all simulated counts. By default,
	 * shared events are in chr1, and diff events are in chrX If there is no
	 * chrX (e.g. yeast), shared and diff events are interspersed along all
	 * chroms
	 */
	private void setBindingPositions() {
		...
		if (chromOffsets.containsKey("X")) {
			sharedOffset = chromOffsets.get("1");
			diffOffset = chromOffsets.get("X");
		}
	...
	/**
	 * Simulate a set of binding event reads for each replicate in each
	 * condition. Number of reads, number of events, and strengths of events are
	 * all pre-determined.
	 */
	private void simulateReads() {
		// Initialize the probability landscape
		int eventWidth = 1000;
		int evoff = eventWidth / 2;
		double[] forProbLand = new double[eventWidth];
						...
						for (int x = 0; x < readCount; x++) {
							...
							if (forwardStrand) {
								...
								if (fivePrimeEnd < chrLen) {
									...
									if (paired) {
										int len = Math.max(rLen, (int) fragLengthDistrib.sample());
										rhp = new ReadHit(chr, Math.max(1, fivePrimeEnd - len),
												Math.max(1, fivePrimeEnd - len + rLen - 1), '+');
									}
								}
							}
				...
				if (noiseSource == null) // Poisson
					for (int i = 0; i < noiseFrags; i++) {
						...
						if (strandrand < 0.5) {
							...
							if (paired) {
								int len = Math.max(rLen, (int) fragLengthDistrib.sample());
								rhp = new ReadHit(chr, Math.max(1, (int) start - len),
										Math.max(1, (int) start - len + rLen - 1), '+');
							}
						}
						frags.add(rh);
						if (paired)
					...
					// If the noise source is a control experiment, we shouldn't
					// sample fragments with replacement.
					// Rather, we treat each control experiment read as a
					// distinct fragment in the initial library
				...
				try {
					...
					for (int x = 0; x < numReads; x++) {
						...
						if (noiseSource == null || !subsampleControl) {
							...
							// Here, the options tell us to just *subsample* the
							// control reads directly without replacement.
							// This is easy here, as frags should contain a
							// randomly ordered subset of control reads already
							rh = frags.get(x);
						}
						writers[co][r].write(rh.getChrom() + "\t" + rh.getStart() + "\t" + rh.getEnd() + "\tU\t0\t"
								+ rh.getStrand() + "\n");
						if (paired)
							writers[co][r].write(rhp.getChrom() + "\t" + rhp.getStart() + "\t" + rhp.getEnd()
									+ "\tU\t0\t" + rhp.getStrand() + "\n");
					}
				} catch (IOException e) {
					e.printStackTrace();
	...
	/**
	 * Write an output file of event positions & expected read counts per
	 * replicate
	 */
	...
	public static void main(String[] args) {
		...
		if (args.length == 0 || ap.hasKey("h") || !ap.hasKey("emp")) {
			...
			ChIPReadSimulator sim = new ChIPReadSimulator(bm, gcon.getGenome(), counts, c, r, noiseProb, jointRate,
					jointSpacing, outFile);
			if (noiseProb == 1.0)
