...
/**
 * PointsToEvents: this class converts a list of points into pseudo-events with
 * tag counts.
 * 
 * @author mahony
 *
...
public class PointsToEvents {
	 ...
	 * @param win
	 */
	public PointsToEvents(EventsConfig c, ExperimentManager man, BindingManager bindMan, List<Point> p, int win,
			boolean assignWithModel) {
		config = c;
		manager = man;
	...
	public List<BindingEvent> execute() {
		...
		Collections.sort(points); // Sort for efficient experiment file cache
									// loading
		// For each point
		for (Point p : points) {
			...
			for (ExperimentCondition c : manager.getConditions()) {
				...
				for (ControlledExperiment r : c.getReplicates()) {
					...
					if (assignReadsWithModel) {
						// Scan region with binding distribution to find ML
						// position
						Point maxSigPoint = findMaxWithBindingModel(sigHits, potentialReg,
								bindingManager.getBindingModel(r));
						Point maxCtrlPoint = ctrlHits == null ? null
								: findMaxWithBindingModel(ctrlHits, potentialReg, bindingManager.getBindingModel(r));

						// ML assign reads (single binding event)
						sigResp = assignReadsSingleEvent(sigHits, maxSigPoint, bindingManager.getBindingModel(r));
						ctrlResp = ctrlHits == null ? 0
								: assignReadsSingleEvent(ctrlHits, maxCtrlPoint, bindingManager.getBindingModel(r));
					} else {
						sigResp = simpleAssignReadsToEvent(sigHits, e.getPoint(), potentialReg);
						ctrlResp = ctrlHits == null ? 0
								: simpleAssignReadsToEvent(ctrlHits, e.getPoint(), potentialReg);
					}
	...
	protected Point findMaxWithBindingModel(List<StrandedBaseCount> hits, Region coords, BindingModel model) {
		...
		for (StrandedBaseCount x : hits) {
			...
			if (readStart >= coords.getStart() - model.getMax() && readStart <= coords.getEnd() + model.getMax()) {
				...
				if (x.getStrand() == '+')
					for (int i = Math.max(model.getMin() + offset, 0); i <= Math.min(coords.getWidth(),
							offset + model.getMax()); i++)
						p[i] += model.probability(i - offset) * x.getCount();
				else
					for (int i = Math.max(offset - model.getMax(), 0); i <= Math.min(coords.getWidth(),
							offset - model.getMin()); i++)
	...
	/**
	 * Assign all reads in the list that are within the binding model boundaries
	 * to the event at point
	 * 
	 * @param hits
	 * @param point
	 * @param model
	...
	protected double assignReadsSingleEvent(List<StrandedBaseCount> hits, Point point, BindingModel model) {
		...
		for (StrandedBaseCount x : hits) {
			int readStart = x.getCoordinate();
			if (readStart >= point.getLocation() - model.getMax()
					&& readStart <= point.getLocation() + model.getMax()) {
				count += x.getCount();
			}
		}
		return count;
	...
	/**
	 * Assign all reads in the list that are within the region to the event at
	 * point
	 * 
	 * @param hits
	 * @param point
