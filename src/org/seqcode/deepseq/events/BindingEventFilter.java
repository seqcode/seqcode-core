...
public class BindingEventFilter {
	...
	public void execute() {
		/*
		 * //TEST try{ String testName = "TEST.events"; FileWriter fw1 = new
		 * FileWriter(testName); fw1.write(BindingEvent.headString()+"\n");
		 * for(BindingEvent e : events){ fw1.write(e.toString()+"\n"); }
		 * fw1.close(); } catch (IOException e) { e.printStackTrace(); }
		 */
		...
		for (ExperimentCondition c1 : experiments.getConditions()) {
			for (ExperimentCondition c2 : experiments.getConditions()) {
				if (c1 != c2) {
					try {
						// 1) List of sites enriched in one condition more than
						// another
						...
						// 3) List of sites enriched similarly in both
						// conditions
						...
						for (BindingEvent e : events) {
							if (e.getCondSigVCtrlP(c1) <= sigThres && e.getCondSigVCtrlP(c2) <= sigThres
									&& e.getInterCondP(c1, c2) > sigThres && e.getInterCondFold(c1, c2) < 2)
								fw.write(e.toString() + "\n");
						}
						fw.close();
						...
						for (BindingEvent e : events) {
							if (e.getCondSigVCtrlP(c1) <= sigThres || e.getCondSigVCtrlP(c2) <= sigThres)
								fwAll.write(e.getPoint().getLocationString() + "\t" + e.getCondSigHits(c1) + "\t"
										+ e.getCondSigHits(c2) + "\n");
							if (e.getCondSigVCtrlP(c1) <= sigThres && e.getInterCondP(c1, c2) <= sigThres)
								fwOut.write(e.getPoint().getLocationString() + "\t" + e.getCondSigHits(c1) + "\t"
										+ e.getCondSigHits(c2) + "\n");
						...
						for (BindingEvent e : events) {
							double logFold = e.getInterCondFold(c1, c2) == 0 ? minLogFold
									: Math.log(e.getInterCondFold(c1, c2)) / Math.log(2);
							if (logFold < minLogFold)
								logFold = minLogFold;
							if (logFold > maxLogFold)
								logFold = maxLogFold;
							if (e.getCondSigVCtrlP(c1) <= sigThres || e.getCondSigVCtrlP(c2) <= sigThres)
								fwAll.write(e.getPoint().getLocationString() + "\t" + e.getInterCondScMean(c1, c2)
										+ "\t" + logFold + "\n");
							if (e.getCondSigVCtrlP(c1) <= sigThres && e.getInterCondP(c1, c2) <= sigThres)
								fwOut.write(e.getPoint().getLocationString() + "\t" + e.getInterCondScMean(c1, c2)
										+ "\t" + logFold + "\n");
							if (e.getCondSigVCtrlP(c2) <= sigThres && e.getInterCondP(c2, c1) <= sigThres)
								fwOut.write(e.getPoint().getLocationString() + "\t" + e.getInterCondScMean(c1, c2)
										+ "\t" + logFold + "\n");
