...
public class BindingModelPerBase extends BindingModel {
	...
	protected List<Pair<Integer, Double[]>> empiricalDistributionPerBase; // Per
																			// base
																			// empirical
																			// distrib
			...
			while ((line = reader.readLine()) != null) {
				...
				if (words.length >= 5) {
					...
					if ((dist.intValue() >= minDist) && (dist.intValue() <= maxDist)) {
						...
						Pair<Integer, Double> p = new Pair<Integer, Double>(dist,
	...
	public BindingModelPerBase(List<Pair<Integer, Double[]>> bindingDist) {
		...
		for (Pair<Integer, Double[]> ps : bindingDist) {
			Pair<Integer, Double> p = new Pair<Integer, Double>(ps.car(),
					ps.cdr()[0] + ps.cdr()[1] + ps.cdr()[2] + ps.cdr()[3]);
			empiricalDistribution.add(p);
		}
		loadData(empiricalDistribution);
		loadDataPB(empiricalDistributionPerBase);
	...
	// Accessors
	public double[][] getProbabilitiesPerBase() {
		return probs_pb.clone();
	}

	public double[][] getLogProbabilitiesPerBase() {
		return logProbs_pb.clone();
	}
	...
	protected void loadDataPB(List<Pair<Integer, Double[]>> bindingDist) {
		...
		for (int i = 0; i <= (max - min); i++) {
			for (int j = 0; j < 4; j++) {
				data_pb[i][j] = 0;
				probs_pb[i][j] = 0;
				logProbs_pb[i][j] = Double.NEGATIVE_INFINITY;
			}
		}
	...
	protected void makeProbabilities() {
		super.makeProbabilities();// Call parent method to do everything for the
									// combined empirical distribution
		double[] totalVals = { 0, 0, 0, 0 },
				minProbs = { Double.MAX_VALUE, Double.MAX_VALUE, Double.MAX_VALUE, Double.MAX_VALUE };
	 ...
	 * 
	 * @param distance
	 *            : Distance should be defined as (Read position - Peak
	 *            position)
	 * @param base
	 *            : base that defines which component of the read distribution
	 *            to use
	 * @return
	 */
	...
	public void printToFile(String filename) {
		try {
			...
			for (int i = min; i <= max; i++) {
				fout.write(i + "\t" + probability(i, 'A') + "\t" + probability(i, 'C') + "\t" + probability(i, 'G')
						+ "\t" + probability(i, 'T') + "\n");
			}
			fout.close();
		} catch (IOException e) {
	...
	/**
	 * Generated using MetaMaker pile-ups of human TFIIB permanganate ChIP-seq
	 * (Pugh39735hg19) over initial binding event calls generated by Will Lai.
	 */
	@SuppressWarnings("unchecked")
	public static final List<Pair<Integer, Double[]>> defaultPermChipSeqEmpiricalDistribution = Arrays.asList(
