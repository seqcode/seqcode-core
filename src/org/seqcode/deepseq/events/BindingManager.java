...
/**
 * BindingManager stores lists of binding events and motifs associated with
 * experiment conditions, and binding models associated with replicates. These
 * data structures used to be under the relevant experiment components, but we
 * moved them here to allow experiment loading to be independent.
 * 
 * @author mahony
 *
 */
public class BindingManager {
	...
	public List<BindingEvent> getBindingEvents() {
		return events;
	}

	public List<BindingEvent> getConditionBindingEvents(ExperimentCondition ec) {
		return conditionEvents.get(ec);
	}

	public WeightMatrix getMotif(ExperimentCondition ec) {
		return motifs.get(ec);
	}

	public WeightMatrix getFreqMatrix(ExperimentCondition ec) {
		return freqMatrices.get(ec);
	}

	public Integer getMotifOffset(ExperimentCondition ec) {
		return motifOffsets.get(ec);
	}

	public BindingModel getBindingModel(ControlledExperiment ce) {
		return models.get(ce);
	}

	public Integer getMaxInfluenceRange(ExperimentCondition ec) {
		return maxInfluenceRange.get(ec);
	}

	public void setBindingEvents(List<BindingEvent> e) {
		events = e;
	}

	public void setConditionBindingEvents(ExperimentCondition ec, List<BindingEvent> e) {
		conditionEvents.put(ec, e);
	}

	public void setMotif(ExperimentCondition ec, WeightMatrix m) {
		motifs.put(ec, m);
	}

	public void setFreqMatrix(ExperimentCondition ec, WeightMatrix m) {
		freqMatrices.put(ec, m);
	}

	public void setMotifOffset(ExperimentCondition ec, Integer i) {
		motifOffsets.put(ec, i);
	}

	public void setBindingModel(ControlledExperiment ce, BindingModel mod) {
		models.put(ce, mod);
	}
	...
	/**
	 * For each controlled experiment, simply calculate the proportion of reads
	 * in the provided list of binding events to everything else.
	 * 
	 * @param regs
	 */
	public void estimateSignalVsNoiseFractions(List<BindingEvent> signalEvents) {
		for (ExperimentCondition c : manager.getConditions()) {
			for (ControlledExperiment r : c.getReplicates()) {
				...
				System.err.println(r.getName() + "\t" + r.getIndex() + "\tsignal-noise ratio:\t"
						+ String.format("%.4f", r.getSignalVsNoiseFraction()));
			}
		}
	}
	...
	/**
	 * Count the differential binding events present in a given pair of
	 * conditions
	 * 
	 * @param cond
	 * @return
	...
	public int countDiffEventsBetweenConditions(ExperimentCondition cond, ExperimentCondition othercond,
			double qMinThres, double diffPMinThres) {
		int count = 0;
		for (BindingEvent e : events) {
			if (e.isFoundInCondition(cond) && e.getCondSigVCtrlP(cond) <= qMinThres)
	 ...
	 */
	public void writeBindingEventFiles(String filePrefix, double qMinThres, boolean runDiffTests,
			double diffPMinThres) {
		if (events.size() > 0) {
			...
			try {
				...
				for (ExperimentCondition cond : manager.getConditions()) {
					...
					Collections.sort(events, new Comparator<BindingEvent>() {
						public int compare(BindingEvent o1, BindingEvent o2) {
							return o1.compareBySigCtrlPvalue(o2);
						}
					});
					...
					for (BindingEvent e : events) {
						...
						// Because of the ML step and component sharing, I think
						// that an event could be assigned a significant number
						// of reads without being "present" in the condition's
						// EM model.
					...
					if (config.getPrintBED()) {
						...
						for (BindingEvent e : events) {
							double Q = e.getCondSigVCtrlP(cond);
							// Because of the ML step and component sharing, I
							// think that an event could be assigned a
							// significant number of reads without being
							// "present" in the condition's EM model.
							if (e.isFoundInCondition(cond) && Q <= qMinThres)
					...
					for (ExperimentCondition cond : manager.getConditions()) {
						...
						for (ExperimentCondition othercond : manager.getConditions()) {
							if (!cond.equals(othercond)) {
								...
								for (BindingEvent e : events) {
									double Q = e.getCondSigVCtrlP(cond);
									// Because of the ML step and component
									// sharing, I think that an event could be
									// assigned a significant number of reads
									// without being "present" in the
									// condition's EM model.
									if (e.isFoundInCondition(cond) && Q <= qMinThres) {
										if (e.getInterCondP(cond, othercond) <= diffPMinThres
												&& e.getInterCondFold(cond, othercond) > 0) {
											fout.write(e.getConditionString(cond) + "\n");
										}
									}
								}
								...
								if (config.getPrintBED()) {
									...
									for (BindingEvent e : events) {
										double Q = e.getCondSigVCtrlP(cond);
										// Because of the ML step and component
										// sharing, I think that an event could
										// be assigned a significant number of
										// reads without being "present" in the
										// condition's EM model.
										if (e.isFoundInCondition(cond) && Q <= qMinThres) {
											if (e.getInterCondP(cond, othercond) <= diffPMinThres
													&& e.getInterCondFold(cond, othercond) > 0) {
				...
				// If necessary, print per-replicate event base composition
				// files
				if (config.getCalcEventBaseCompositions()) {
					for (ExperimentCondition cond : manager.getConditions()) {
						for (ControlledExperiment rep : cond.getReplicates()) {
							...
							fout = new FileWriter(filename);
							fout.write(
									"#Position\tEventRegA\tEventRegC\tEventRegG\tEventRegT\tEventTagA\tEventTagC\tEventTagG\tEventTagT\tBubbleRegA\tBubbleRegC\tBubbleRegG\tBubbleRegT\tBubbleTagA\tBubbleTagC\tBubbleTagG\tBubbleTagT\tBubbleIndex\n");
							for (BindingEvent e : events) {
								double Q = e.getCondSigVCtrlP(cond);
								if (e.isFoundInCondition(cond) && Q <= qMinThres) {
									...
									if (bubbleBases[3] > 0) {
										...
										if (baseACG > 0)
											expectedACG = (bubbleTagBases[0] + bubbleTagBases[1] + bubbleTagBases[2])
													/ baseACG;
										if (expectedACG > 0)
											bubbleIndex = expectedT / expectedACG;
									}
									fout.write(e.getPoint() + "\t" + eventBases[0] + "\t" + eventBases[1] + "\t"
											+ eventBases[2] + "\t" + eventBases[3] + "\t" + eventTagBases[0] + "\t"
											+ eventTagBases[1] + "\t" + eventTagBases[2] + "\t" + eventTagBases[3]
											+ "\t" + bubbleBases[0] + "\t" + bubbleBases[1] + "\t" + bubbleBases[2]
											+ "\t" + bubbleBases[3] + "\t" + bubbleTagBases[0] + "\t"
											+ bubbleTagBases[1] + "\t" + bubbleTagBases[2] + "\t" + bubbleTagBases[3]
											+ "\t" + bubbleIndex + "\n");
	...
	/**
	 * Print all binding events to screen TESTING ONLY
	 */
