 ...
 * 
 * Model Types: Genome-wide = -1 CurrentRegion = 0 Local window size = positive
 * integer
 * 
 * @author Shaun Mahony
 * @version %I%, %G%
...
public abstract class BackgroundModel {
	protected int modelType; // -1 for genome-wide, 0 for current region,
								// positive integer for local window size
	...
	public BackgroundModel(int mtype, double lc, double r, double rl, double mr, double bw, char str) {
		this(mtype, lc, r, rl, mr, bw, str, 1, true);
	}

	public BackgroundModel(int mtype, double lc, double r, double rl, double mr, double bw, char str, double sc,
			boolean ute) {
		modelType = mtype;
		logConfidence = lc;
	...
	public void updateModel(Region currReg, int currOffset, float[] thisExptHitCounts, float[] otherExptHitCounts,
			float hitCountBinStep) {
		...
		if (hitCounts != null) {
			if (modelType == -1) {// Genome-wide
				...
				int istop = currOffset + (win / 2) >= currReg.getWidth() ? currReg.getWidth() - 1
						: currOffset + (win / 2);
