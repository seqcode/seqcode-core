...
public class ROC {
	...
	/*
	 * 2D ROC. second dimension must have size 2. Takes two comparators. The
	 * comparator will be called on the value and the threshold and should
	 * return 1 iff the value meets the threshold
	 */
	public ROC(double[][] positive, double[][] negative, Comparator c1, Comparator c2) {
		...
		maxtwo = positive[0][1];
		for (int i = 0; i < positive.length; i++) {
			if (positive[i][0] > maxone) {
				maxone = positive[i][0];
			}
			if (positive[i][0] < minone) {
				minone = positive[i][0];
			}
			if (positive[i][1] > maxtwo) {
				maxtwo = positive[i][1];
			}
			if (positive[i][1] < mintwo) {
				mintwo = positive[i][1];
			}
		}
		for (int i = 0; i < negative.length; i++) {
			if (negative[i][0] > maxone) {
				maxone = negative[i][0];
			}
			if (negative[i][0] < minone) {
				minone = negative[i][0];
			}
			if (negative[i][1] > maxtwo) {
				maxtwo = negative[i][1];
			}
			if (negative[i][1] < mintwo) {
				mintwo = negative[i][1];
			}
		}
		...
		for (double t1 = minone; t1 <= maxone; t1 += step1) {
			for (double t2 = mintwo; t2 <= maxtwo; t2 += step2) {
				int tp = 0, tn = 0, fp = 0, fn = 0;
				for (int j = 0; j < positive.length; j++) {
					if (c1.compare(positive[j][0], t1) == 1 && c2.compare(positive[j][1], t2) == 1) {
						tp++;
					} else {
						fn++;
					}
				...
				for (int j = 0; j < negative.length; j++) {
					if (c1.compare(negative[j][0], t1) == 1 && c2.compare(negative[j][1], t2) == 1) {
						fp++;
					} else {
						tn++;
					}
				...
				n.sens = tp / (tp + fn + .000001);
				n.spec = tn / (tn + fp + .000001);
				// System.err.println("At t1=" + t1 + " and t2=" + t2 + " sens="
				// + n.sens + " and spec="+ n.spec);
			}
		}
	}
