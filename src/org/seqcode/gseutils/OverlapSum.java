 ...
 * of a ChIP-Seq experiment, but since adapted to several more uses.
 * 
 ...
 * region have been "covered" by on or more intervals.
 * 
...
public class OverlapSum {
	...
	/*
	 * public void addRegion(Region r) { if(!genome.equals(r.getGenome())) {
	 * throw new IllegalArgumentException(r.getGenome().toString()); }
	 * if(!chrom.equals(r.getChrom())) { throw new
	 * IllegalArgumentException(r.getChrom()); }
	 * 
	 * int start = r.getStart(); int end = r.getEnd();
	 * if(changes.containsKey(start)) { changes.put(start, changes.get(start) +
	 * 1); } else { changes.put(start, 1); }
	 * 
	 ...
	 * if(changes.containsKey(end + 1)) { changes.put(end + 1, changes.get(end +
	 * 1) - 1); } else { changes.put(end + 1, -1); } }
	 */
	...
	public boolean hasOverlap(OverlapSum sum, int thresh) {
		Iterator<Integer> chs = new MergingIterator<Integer>(changes.keySet().iterator(),
