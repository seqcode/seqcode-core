...
public class ShortBitVector implements Comparable<ShortBitVector>, BitVector {
	 ...
	 *            The length of the blank vector.
	 * @param value
	 *            The value of each bit in the new vector.
	 */
	 ...
	 * 
	 * @param _array
	 *            The array of values for the vector: 0 values become OFF,
	...
	/**
	 * Creates a BitVector from a raw string representation, with length equal
	 * to the character array supplied. If _array[i] == maker, then
	 * bit_vector[i] == value.
	 * 
	 * @param _array
	 *            The character array, whose values dictate the contents of the
	 *            bit vector.
	 * @param marker
	 *            The character that indicates which bits have 'value.'
	 * @param value
	 *            The value of all bits that correspond to "marker" character.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#toHexString()
	 */
	...
	/**
	 * expandString(base, offInsert) takes a string, which should have a length
	 * equal to the number of ON bits in the vector. It then returns the same
	 * string, but with the substring offInsert inserted repeatedly for each
	 * occurrence of an OFF bit. For instance, the bit vector 1101101 will
	 * expand the string "aabbc" (with offInsert set to "-") into the string
	 * "aa-bb-c".
	 * 
	 * @param base
	 *            The "base" string which will be inserted.
	 * @param offInsert
	 *            The substring which we will insert for each occurrence of an
	 *            OFF bit in the bit-vector.
	 * @return Returns the expanded String.
	 */
	public String expandString(String base, String offInsert) {
		if (countOnBits() != base.length()) {
			throw new IllegalArgumentException("length " + base.length() + " != " + countOnBits());
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#isOn(int)
	 */
	public boolean isOn(int index) {
		return array[index] == ON;
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#isOff(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#length()
	 */
	public int length() {
		return array.length;
	}
	...
	/**
	 * @param offset
	 *            The offset up to which we should count (inclusive).
	...
	/**
	 * @param onBitIndex
	 *            The number of the "on" bit which we want the index of.
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#countOnBits()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#turnOnBit(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.seqcode.gseutils.BitVector#turnOffBit(int)
	 */
	...
	public void bitwiseOR(ShortBitVector bv) {
		if (array.length != bv.array.length) {
			throw new IllegalArgumentException();
		}
		for (int i = 0; i < array.length; i++) {
	...
	public boolean equals(Object o) {
		if (!(o instanceof ShortBitVector)) {
			return false;
		}
