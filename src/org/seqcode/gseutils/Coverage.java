...
/**
 * 
 * Collects a series of "UnitCoverage" objects, keyed by arbitrary strings. This
 * is most useful when we're looking at the coverage of an entire genome, in
 * which case each UnitCoverage corresponds to a Chromosome and the Coverage
...
public class Coverage {
	...
	public boolean hasCoverage(String key, int start, int end) {
		return covermap.containsKey(key) ? covermap.get(key).hasOverlap(start, end) : false;
	}

	public boolean isContained(String key, int start, int end) {
		return covermap.containsKey(key) ? covermap.get(key).isContained(start, end) : false;
	}
	...
	public Iterator<Integer[]> units() {
		...
		for (String k : covermap.keySet()) {
			units.add(covermap.get(k).covered());
		}
	...
	public void addInterval(String key, int start, int end) {
		if (!covermap.containsKey(key)) {
			covermap.put(key, new UnitCoverage());
		}
	...
	public Collection<Integer[]> covered(String key, int start, int end) {
		return covermap.containsKey(key) ? covermap.get(key).covered(start, end) : new ArrayList<Integer[]>();
	}
	...
	public Collection<Pair<Integer[], Integer[]>> findRightPairs(String key, int maxDist, Coverage coverage) {
	...
	public Collection<Pair<Integer[], Integer[]>> findLeftPairs(String key, int maxDist, Coverage coverage) {
