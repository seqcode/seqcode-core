...
public class WeightedAlgorithms extends Algorithms {
	...
	/**
	 * Tests the Bellman-Ford implementation using the example from CLRS page
	 * 589 (Figure 24.4).
	 * 
	 * @param args
	 */
	...
	/**
	 * Runs, as a test of the dijkstra implementation, the example from CLRS on
	 * page 596 of the 2nd edition.
	 * 
	 * @param args
	 */
	 ...
	 * 
	 * For a given tree and a given "target" node, this method will follow links
	 * in the path tree back to the root -- this gives the shortest path (in
	 * reverse order, of course) of the path from root --> target in the
	 * original graph.
	 * 
	 * This method also returns the ultimate weight of that (shortest) path.
	 * 
	 * @param g
	 *            The path-tree, which must have been calculated with respect to
	 *            a given source.
	 * @param target
	 *            The ultimate target in the graph.
	 * @return
	 */
	...
	/**
	 * Performs the Floyd-Warshall all-pairs shortest-paths algorithm. Returns
	 * an array of path tree matrices.
	 * 
	 * @return A path-tree matrix, where matrix[i] is the path-tree for paths
	 *         originating from vertices[i].
	 */
	public WeightedGraph[] floydWarshall() {

		DirectedWeightedGraph[][] karray = new DirectedWeightedGraph[][] { createFloydWarshallMatrix(),
	 ...
	 * 
	 * For a given v2 \in G = dijkstra(v1), the weight(v2) in G is the weight of
	 * the shortest path v1 -> v2.
	 * 
	 ...
	 * weight of the edge v3->v2 in the *original* graph which is on that
	 * shortest path.
	 * 
	 * dijkstra() throws an IllegalArgumentException if it encounters a
	 * negative-weight edge.
	 * 
	 * @param source
	 *            The source node in the original graph.
	...
	public WeightedGraph dijkstra(String source) {
		...
		while (Q.size() > 0) {
			...
			if (u.weight == null) {
				// The rest of the graph is unconnected -- so we just return the
				// pathtree.
				return pathtree;
			} else {
				for (String v : graph.getNeighbors(u.node)) {
					...
					if (w < 0.0) {
						// Dijkstra's algorithm doesn't work, when there are
						// negative weight edges.
						throw new IllegalArgumentException(
								String.format("%s->%s has negative weight %f", u.node, v, w));
					}
	 ...
	 * 
	 * The RELAX() method described in CLRS is actually tricky -- it requires an
	 * (implicit) update of the Q heap, and that requires knowing where each old
	 * key was in the heap so we can call increaseKey() on it ... so I do that
	 * here with a linear search through the heap (implemented in the increase()
	 * method, in Heap.java), but this is probably sub-optimal... sigh.
	 * 
	 * @param Q
	 *            See the note above.
	 * @param pathtree
	 *            Stores the 'd' and 'pi' variables from CLRS, implicitly.
	 * @param u
	 * @param v
	 * @param w
	 */
	private void relax(Heap<WeightedNode> Q, DirectedWeightedGraph pathtree, WeightedNode u, String v, Double w) {
		...
		if (newvn.compareTo(oldvn) == -1) {
			...
			// System.out.println(String.format("%s => %s", oldvn.toString(),
			// newvn.toString()));
