...
/**
 * <code>Args</code> is a utility class for parsing command line arguments. It
 * can parse different types of values (eg, strings, integers, Genomes,
 * WeightMatrixScans).
 *
 * <code>Args</code> provides some internal caching of previously parsed objects
 * based on the String[] object such that it returns the same Genome object no
 * matter how many times parseGenome is called.
 *
...
public class Args {
	 ...
	 * 
	 * @param args
	 *            The command line options of the form <tt>--foo</tt>
	 * @return
	 */
	...
	/**
	 * parses flags. These are command line options of the form --foo followed
	 * by another option (eg, --foo --bar quux) or the end of the command line.
	 * They take no value after the name of the argument
	 *
	 * @returns the Set of flags present in args[]. The Strings returned do not
	 *          include the leading --
	 */
	public static Set<String> parseFlags(String args[]) {
		...
		HashSet<String> output = new HashSet<String>();
		for (int i = 0; i < args.length; i++) {
			if (args[i].matches("^--.*") && ((i == args.length - 1) || args[i + 1].matches("^--.*"))) {
				output.add(args[i].substring(2));
			}
		}
	...
	/**
	 * Parses the integer value of the argument named by <code>key</code> from
	 * the specified command line. If no value is present, returns
	 * <code>defaultValue</code>. If the key is present multiple times on the
	 * command line, the first instance is returned. Example:
	 * parseInteger(args,"foo",10); where args={"--minimum","1.3", "--foo","50"}
	 * returns 50.
	 */
	 ...
	 * Example: parseIntegers(args, "foo"); where args ={"--foo", "3", "--min",
	 * "2.5", "--foo", "4"} returns [3, 4].
	 * 
	 * @param args
	 *            arguments of the command line
	 * @param key
	 *            the argument named by <tt>key</tt>
	 * @return
	 */
	 ...
	 * Example: parseDoubles(args, "foo"); where args ={"--foo", "3.2", "--min",
	 * "2.5", "--foo", "4.3"} returns [3.2, 4.3].
	 * 
	 * @param args
	 *            arguments of the command line
	 * @param key
	 ...
	 * immediately preceded by <tt>--file</tt> or is any argument(s) that
	 * come(s) after <tt>--</tt> and then follows the end of the command line.
	 * <br>
	 * For example: <tt>--max 10 --min 3 -- foo.txt bar.txt baz.txt</tt>
	 */
	...
	/**
	 * Parses a list of files of the argument by the name <tt>key</tt> from the
	 * command line and returns file handles
	 * 
	 */
	...
	/**
	 * This method returns a list containing all values preceded by the
	 * specified key. for example:
	 * <tt>--input foo.txt --input bar.txt --input baz.txt</tt> would return the
	 * list "foo.txt","bar.txt","baz.txt" if key="input"
	 */
	 ...
	 * Also parses <tt>--genome mm8</tt> or <tt>--gen mm8</tt> into a Genome and
	 * inferred Species
	 * 
	...
	public static Pair<Species, Genome> parseGenome(String args[]) throws NotFoundException {
		if (orgs.containsKey(args) && genomes.containsKey(args)) {
			return new Pair<Species, Genome>(orgs.get(args), genomes.get(args));
		}
	...
	/**
	 * parses SeqLocators from the <tt>--seqexpt</tt> parameters. Takes either
	 * "name;alignment" or "name;replicate;alignment"
	 */
	...
	/**
	 * parses SeqLocators from the <tt>argname</tt> parameters. Takes either
	 * "name;alignment" or "name;replicate;alignment"
	 * 
		...
		try {
			...
			if (a == null) {
				throw new RuntimeException(
						"Couldn't parse or find a ChipSeqAnalysis from " + base + " or " + aname + "," + aversion);
			}
		} catch (SQLException e) {
			throw new DatabaseException(e.toString(), e);
		} catch (Exception e) {
	...
	/**
	 * parses <tt>--scan</tt> or <tt>wmscan</tt> options into a list of
	 * WeightMatrixScans. --scan takes 3 or 4 semicolon separated values: -
	 * matrix name - matrix version - scan name - organism for the matrix if not
	 * the same as what was specified in --species
	 * 
	...
	public static Collection<WeightMatrix> filterMatrices(Collection<String> accepts, Collection<String> rejects,
			Collection<String> acceptvers, Collection<String> rejectvers, Collection<String> accepttypes,
			Collection<String> rejecttypes, Collection<WeightMatrix> matrices) {
		...
		for (WeightMatrix wm : matrices) {
			if (matchesAny(wm.name, rejectp) || matchesAny(wm.version, rejectversp)
					|| matchesAny(wm.type, rejecttypep)) {
				continue;
			}
			if ((matchesAny(wm.name, acceptp) || matchesAny(wm.version, acceptversp)
	...
	public static Collection<WeightMatrix> parseWeightMatrices(String args[]) throws NotFoundException {
		...
		if (awm.size() > 0) {
			System.err.println("Acceping wmnames " + awm);
		}
		if (awmv.size() > 0) {
			System.err.println("Acceping wmvers " + awmv);
		}
		if (awmt.size() > 0) {
			System.err.println("Acceping wmtypes " + awmt);
		}
		if (rwm.size() > 0) {
			System.err.println("Rejecting wmnames " + rwm);
		}
		if (rwmv.size() > 0) {
			System.err.println("Rejecting wmvers " + rwmv);
		}
		if (rwmt.size() > 0) {
			System.err.println("Rejecting wmtypes " + rwmt);
		}

		Collection<WeightMatrix> out = new ArrayList<WeightMatrix>();
		if (awm.size() > 0 || rwm.size() > 0 || awmv.size() > 0 || rwmv.size() > 0 || awmt.size() > 0
				|| rwmt.size() > 0) {
		...
		if (namevers.size() > 0) {
			...
			loader.close();
		} else if (awm.size() == 0 && rwm.size() == 0 && awmv.size() == 0 && rwmv.size() == 0 && awmt.size() == 0
				&& rwmt.size() == 0) {
	...
	/**
	 * returns a list of RefGeneGenerator. If --genes is specified, uses the
	 * value as the type rather than the default type of refGene. The type is
	 * the name of the table that the RefGeneGenerator will query.
	 */
	public static List<RefGeneGenerator> parseGenes(String args[]) throws NotFoundException {
		...
		boolean flipstrands = parseFlags(args).contains("flipgenestrands")
				|| parseFlags(args).contains("flipgenestrand");
		for (int i = 0; i < args.length; i++) {
	 ...
	 * For example, if key is <tt>quux</tt>, then looks for <tt>--quux</tt>. The
	 * value after each <tt>--quux</tt> is parsed as specifying a filename that
	 * should be opened and read. Each line is parsed as a region and those
	 * regions are returned, sorted.
	 * 
	 * @see org.seqcode.genome.Genome
	...
	/**
	 * Parses the regions from the command line as specified by
	 * <tt>--region</tt>. The command line must also contain a
	 * <tt>--species</tt> option somewhere
	 */
	...
	/**
	 * Parses the <tt>--region</tt> options from the command line. If none are
	 * specified, returns regions corresponding to the chromosomes in the genome
