...
public class UkkonenSuffixTree {
	...
	private UkkonenState extState; // this is a field, because TreeEdge depends
									// on it.
	...
	public UkkonenSuffixTree() {
		logger = Logger.getLogger("org.seqcode.gse.projects.chipseq.assembler.UkkonenSuffixTree");
		isLogging = true;
	...
	private EdgeMatch findEdge(TreeNode currentNode, char[] array, int start, int end, boolean skipcount) {
		EdgeMatch em = new EdgeMatch(array, start, end);
		em.matchFrom(currentNode, skipcount);
		return em;
	}

	private EdgeMatch findEdge(TreeNode currentNode, TreeString string, int start, int end, boolean skipcount) {
	...
	private void ukkonenExtendSuffixTree(int arrayIdx) {
		...
		logger.log(Level.FINEST, String.format("Ukkonen: (%d,%d)", extState.nextPhaseStart, extState.string.length()));
	...
	/*
	 * ukkonenSPA(i) performs phase i of Ukkonen's algorithm. This means that
	 * we're making sure that array[0,i] (note the inclusivity!) is a part of
	 * the current suffix tree.
	 * 
	...
	private void ukkonenSPA(int i) {
		...
		/*
		 * SPA Step 1: "Increment index e to i+1"
		 * 
		 * The equivalent of Gusfield's i+1 is, in our situation, just i.
		 * However, the coordinates are inclusive in Gusfield, and exclusive in
		 * our case (along the tree edges). Therefore, lastE should be updated
		 * to be i+1, exactly.
		 */
		extState.lastE = i + 1;
		...
		/*
		 * SPA Step 2: "Explicitly compute successive extensions, using the SEA
		 * algorithm, starting at j_i + 1 until reaching the first extension j*
		 * where rule3 applies or until all extensions are done in this phase."
		 * 
		 ...
		 * ukkonenSEA returns false (ukkonenSEA returns a true if rule 1 or rule
		 * 2 applies in its extension).
		 * 
		 * We extend until j==i, because the last extension of each phase is the
		 * extension that *just* adds the new character into the tree.
		 */
		...
		/*
		 * SPA Step 3: "Set j_{i+1} to j*-1, to prepare for the next phase."
		 */
	...
	private boolean ukkonenSEA(int i, int j) {
		...
		/*
		 * SEA Step 1: "Find the first node v at or above the end of S[j-1,i]
		 * that either has a suffix link from it or is the root. This requires
		 * walking up at most one edge from the end of S[j-1,i] in the current
		 * tree. Let \gamma (possibly empty) denote the string between v and the
		 * end of S[j-1,i]."
		 */

		/*
		 * SEA Step 2: "If v is not the root, traverse the suffix link from v to
		 * node s(v) and then walk down from s(v) following the path for string
		...
		/*
		 * SEA Step 3: "Using the extension rules, ensure that the string
		 * S[j,i]S(i+1) is in the tree."
		 * 
		 * In our coordinates, this is array[j,i)+array[i] \beta = array[j,i)
		 * 
		 ...
		 * ends either at a node (in which case, no child of the node starts
		 * with array[i]), or in an edge (in which case, the edge doesn't
		 * continue with array[i]). Either way, we create a new edge that is
		 * labeled with array[i] (coordinates: [i,i+1) ). Rule 3: \beta+array[i]
		 * is already in the tree -- either \beta ends in an edge that continues
		 * with array[i], or at a node that has a child under array[i]. Either
		 * way, return false (break!).
		 */
		...
		if (m.lastEdge == null) {
			...
			boolean foundLastChar = !lastCharIsTerminal ? root.childEdges.containsKey(lastChar)
			...
			boolean foundLastChar = !lastCharIsTerminal ? m.lastEdge.getChar(offset) == lastChar
					: (m.lastEdge.string.getIndex() == extState.string.getIndex() && offset == m.lastEdge.length() - 1);
			...
			if (extState.nextNode.suffixLink == null && !extState.nextNode.isRoot()) {
				logger.log(Level.FINEST, String.format("Walking up edge: %d", extState.nextNode.parentEdge.length()));
			...
			boolean foundLastChar = !lastCharIsTerminal ? m.lastEdge.tailNode.childEdges.containsKey(lastChar)
		...
		/*
		 * SEA Step 4: "If a new internal node w was created in extension j-1
		 * (by extension rule 2) then by Lemma 6.1.1 string alpha must end at
		 * node s(w), the end node for the suffix link from w. Create the suffix
		 * link (w, s(w)) from w to s(w)."
		 * 
		 * This wording is confusing -- is there a typo in Gusfield? I'm not
		 * sure where the 'w' comes from.
		 */
	...
	public class TreeString {
		...
		public char getChar(int i) {
			return i < array.length ? array[i] : terminal;
		}
	...
	public class StringSuffix implements Comparable<StringSuffix> {
		...
		public boolean equals(Object o) {
			if (!(o instanceof StringSuffix)) {
				return false;
			}
			StringSuffix ss = (StringSuffix) o;
			if (!ss.string.equals(string)) {
				return false;
			}
	 ...
	 * extension and from phase to phase, of the Ukkonen extension algo- rithm.
	 * Particularly important is the "lastE" field, which is globally referenced
	 * by a certain subset of the TreeEdges (the 'leaf' edges) during the
	 * execution of the Ukkonen Algorithm. As a result, there is a global field
	 * ('extState') that is maintained during any run of the algorithm.
	 */
	private class UkkonenState {
		...
		public UkkonenState(TreeString str) {
			...
			if (string.getIndex() > 0) {
				...
				logger.log(Level.FINEST, String.format("String %s can start at phase %d (E:%d)", string.toString(),
						nextPhaseStart, lastE));
			} else {
	...
	/*
	 * We can't just match into the tree in a naive way -- we need to store some
	 * state about how *far* we matched into the tree, where we ended up, etc.
	 * As a result, this EdgeMatch class implements tree descent -- it walks as
	 * far down into the tree as possible and then (upon return from the
	 * matchFrom() method) contains all the state we need about where that match
	 * ended.
	 */
	private class EdgeMatch {
		...
		public String toString() {
			return String.format("EdgeMatch (%d,%d-->%d,%d) '%s' in '%s'", start, matchingFrom, matchedTo, end,
					matchedString(), matchingString());
		}
		...
		public void matchFrom(TreeNode startNode, boolean skipcount) {
			...
			while (keepMatching) {
				...
				int matching = skipcount ? Math.min(lastEdge.length(), remaining)
				...
				if (matching < lastEdge.length()) {
					...
					if (matching < remaining) {
						...
						if (lastEdge.tailNode.childEdges.containsKey(nextChar)) {
							...
							String err = String.format("[%s] node:'%s' (next: %c)", toString(), startNode.pathLabel(),
									nextChar);
		...
		public int lastMatchLength() {
			return matchedTo - matchingFrom;
		}

		public int matchLength() {
			return matchedTo - start;
		}
		...
		public boolean completedMatch() {
			return lastEdge != null && matchedTo == end;
		}
	}

	private class TreeEdge {

		// This is a pointer to the overall string of which this edge is a
		// substring.
		public TreeString string;
		...
		public TreeEdge split(int offset) {
			if (offset <= 0 || offset >= length()) {
				throw new IllegalArgumentException(
						String.format("Illegal edge split offset %d (length %d)", offset, length()));
			}
		...
		public int end() {
			return end != null ? end : extState.lastE;
		}
	...
	private class LoggingHandler extends Handler {
		...
		public void setCloseStream(boolean cs) {
			closeStream = cs;
		}
	...
	/*
	 * The Level.intValue() method apparently doesn't do what we want? So I'll
	 * write my own. This lets us filter by "greater than" relations on Level's,
	 * with the natural (to me) ordering.
	 */
