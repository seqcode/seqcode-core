...
public abstract class Utils {
	...
	public static Map<String, String> readMap(String filename) throws FileNotFoundException, IOException {
	...
	/**
	 * Saves a map to a file with the given name -- by default, calls the
	 * corresponding saveMap() function with a "reverse" parameter of
	 * <i>false</i>.
	 * 
	 * @param filename
	 *            The name of the file to which to save.
	 * @param map
	 *            The String-to-String map to save.
	 * @throws IOException
	 */
	 ...
	 *            The String-to-String map to save.
	 * @param reverse
	 *            If true, saves a map not of key:value, but of value:key.
	 * @throws IOException
	 */
	...
	public static String transformDuration(long msec_duration) {
		return transformDuration(msec_duration, "dhms");
	}
	...
	public static String transformDuration(long msec_duration, String format) {
		...
		if (!format.matches("^[DdHhMmSs]?[DdHhMmSs]?[DdHhMmSs]?[DdHhMmSs]?$"))
			throw new IllegalArgumentException("The format has to be a set of: dhms. "
					+ "d: for days, h: for hours, m: for minutes, s: for seconds." + "E.g.: dh, ds, hms and so on.");
		...
		if (format.matches("[Dd]+")) {
			return String.format("%.1f d", (double) sec_duration_long / 86400);
		...
		if (format.matches("[Hh]+")) {
			return sb.append(String.format("%.1f h", (double) sec_duration_long / 3600)).toString();
		...
		if (format.matches("[Mm]+")) {
			return sb.append(String.format("%.1f m", (double) sec_duration_long / 60)).toString();
