...
public class FieldAccessor<T extends Model> implements Accessor<T> {
	...
	public FieldAccessor(Class cls, String fieldName) {
		try {
			...
			e.printStackTrace();
			throw new IllegalArgumentException(String.format("No field %s in class %s", fieldName, cls.getSimpleName()),
					e);
		}
		if ((field.getModifiers() | Modifier.PUBLIC) == 0) {
			throw new IllegalArgumentException(String.format("Class %s has field %s, " + "but the field isn't public.",
