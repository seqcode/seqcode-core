...
import org.seqcode.gseutils.json.JSONArray;
import org.seqcode.gseutils.json.JSONException;
import org.seqcode.gseutils.json.JSONObject;
...
public class Model {
	...
	public static Object unjsonify(Class type, Object value) {
		if (value == JSONObject.NULL) {
			...
			return new Color(Integer.parseInt(pieces[0]), Integer.parseInt(pieces[1]), Integer.parseInt(pieces[2]),
	...
	public JSONObject asJSON() {
		...
		for (int i = 0; i < fs.length; i++) {
			...
			if (!isStatic) {
				...
				} catch (org.seqcode.gseutils.json.JSONException e) {
					System.err.println(String.format("Model Class: %s", getClass().getName()));
					System.err
							.println(String.format("Field Name: %s (%s)", fs[i].getName(), fs[i].getType().getName()));
			...
			if (!isStatic) {
				try {
					...
					if (obj.has(fname)) {
						...
						// fs[i].getName(), unjsoned, fs[i].get(this)));
					} else {
						/*
						 * this used to set object fields to null if they
						 * weren't present in the json representation. I've
						 * changed it to ignore those fields. The old behavior
						 * was a problem if you were reading property objects
						 * back from disk and the file was missing fields that
						 * were recently added to an object. Those fields would
						 * be set to null even if the object's constructor had
						 * initialized them to something else.
						 */
						// fs[i].set(this, null);
					}
				} catch (IllegalAccessException e) {
				...
				if (fs[i].getType().isArray()) {
					...
					if (array != null) {
						...
						for (int j = 0; j < len; j++) {
							...
							String valueStr = value instanceof Double ? String.format("%.3f", (Double) value)
