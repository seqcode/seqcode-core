...
public class ModelFieldAnalysis<T extends Model> {
	...
	/**
	 * @param cls
	 *            A Class object that represents a java class which is a
	 *            subclass of Model.
	 */
	...
	public Object get(String compositeFieldName, Object value) {
		...
		for (int i = 0; i < fieldNameArray.length; i++) {
			...
			if (!Model.isSubclass(valueClass, Model.class)) {
				return null;
			}
			try {
	...
	public boolean getStaticSwitch(String name, boolean defaultValue) {
		...
		if (!Model.isSubclass(staticFields.get(name).getType(), Boolean.class)) {
			return defaultValue;
		}
		try {
			...
			throw new IllegalArgumentException(
					String.format("Can't access field %s in class %s", name, modelClass.getSimpleName()));
		}
	}
