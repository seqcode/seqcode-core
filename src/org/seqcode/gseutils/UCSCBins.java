...
/**
 * Performs calculations for the UCSC bin indexing scheme. See
...
public class UCSCBins {
	...
	public static final int[] binSizes = { 512 * 1024 * 1024, 64 * 1024 * 1024, 8 * 1024 * 1024, 1024 * 1024,
			128 * 1024 };
	public static final int binFirstShift = 17; /*
												 * How much to shift to get to
												 * finest bin.
												 */
	public static final int binNextShift = 3; /*
												 * How much to shift to get to
												 * next larger bin.
												 */

	/*
	 * Given start,end in chromosome coordinates assign it a bin. There's a bin
	 * for each 128k segment, for each 1M segment, for each 8M segment, for each
	 * 64M segment, and for each chromosome (which is assumed to be less than
	...
	public static int rangeToBin(int start, int end) throws IllegalArgumentException {
		...
		if (start > binSizes[0]) {
			throw new IllegalArgumentException("Start > 512M");
		}
		if (end > binSizes[0]) {
			throw new IllegalArgumentException("End > 512M");
		}
		...
		throw new IllegalArgumentException(
				String.format("start %d, end %d out of range in findBin (max is 512M)", start, end));
	}
