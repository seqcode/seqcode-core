 ...
 * @author tdanford
 * 
 *         BinarySplayTree is an implementation of binary search trees with the
 *         self-adjusting "splay" maneuver -- these are commonly known as "splay
 *         trees."
 * 
 *         This was originally outlined in the paper: Sleator and Tarjan,
 *         "Self-Adjusting Binary Search Trees" (JACM V.32 no.3, July 1985)
 * 
 *         All "quoted" comments to the methods in this class are direct quotes
 *         taken from that original paper.
 *
 * @param <X>
 */
public class BinarySplayTree<X extends Comparable<X>> {
	...
	/**
	 * "To perform access(i, t), we search down from the root of t, looking for
	 * i. If the search reaches node x containing i, we complete the access by
	 * splaying at x and returning a pointer to x. If the search reaches a null
	 * node, indicating that i is not in the tree, we complete the access by
	 * splaying at the last nonnull node reached during the search (the node
	 * from which the search ran off the bottom of the tree) and returning a
	 * pointer to null. If the tree is empty, we omit the splaying operation."
	 * 
	 * @param i
	 * @return
	 */
	...
	/**
	 * "To carry out insert(i, t), we perform split(i, t) and then replace t by
	 * a tree consisting of a new root node containing i, whose left and right
	 ...
	 * i, in t1. After the access, the root of t1 contains i and thus has a null
	 * right child. We complete the join by making t2 the right subtree of this
	 * root and returning the resulting tree."
	 * 
	...
	/**
	 * "To carry out split(i, t) we perform acces(i, t) and then return the two
	 * trees formed by breaking either the left link or the right link from the
	 * new root of t, depending on whether the root contains an item greater
	...
	/**
	 * "Splaying step: Case 1 (zig): If p(x), the parent of x, is the tree root,
	 * rotate the edge joining x with p(x). (This case is terminal.)
	 * 
	 * Case 2 (zig-zig): If p(x) is not the root and x and p(x) are both left or
	 * right children, rotate the edge joining p(x) with its grandparent g(x)
	 * and then rotate the edge joining x with p(x).
	 * 
	 * Case 3 (zig-zag): If p(x) is not the root and x is the left child and
	 * p(x) is the right child (or vice-versa), rotate the edge joining x with
	 * p(x), and then rotate the edge joining x with the new p(x)."
	 * 
	 * @return
	 */
