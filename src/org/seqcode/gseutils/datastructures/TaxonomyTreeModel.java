...
class TaxonomyAddr<X> {
	...
	public int indexOfAddr(TaxonomyAddr next) {
		for (int i = 0; i < nextAddrs.size(); i++) {
			if (nextAddrs.get(i).equals(next)) {
				return i;
			}
		}
		return -1;
	...
	public boolean equals(Object o) {
		if (!(o instanceof TaxonomyAddr)) {
			return false;
		}
		TaxonomyAddr ta = (TaxonomyAddr) o;
		if (addr.size() != ta.addr.size()) {
			return false;
		}
		for (int i = 0; i < addr.size(); i++) {
			if (!(addr.get(i).equals(ta.addr.get(i)))) {
				return false;
			}
		}
		return true;
