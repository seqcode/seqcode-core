...
/**
 * An implementation of a Heap/priority-queue, which is basically derived from
 * the pseudocode in: "Heapsort", Chapter 6 (pg. 127) of Cormen, Leiserson,
 * Rivest, and Stein (ed. 2)
 * 
 * @author tdanford
 *
 * @param <Key>
...
public class Heap<Key extends Comparable> {
	...
	private int polarity; // == -1 or 1. polarity == -1 -> min-heap, == 1 ->
							// max-heap
	...
	private void increaseKey(int i, Key k) {
		...
		if (newWrapper.compareTo(old) == -polarity) {
			throw new IllegalArgumentException(String.format("Can't increase/decrease key %s over %s",
	...
	private class KeyWrapper implements Comparable<KeyWrapper> {
		...
		public boolean equals(Object o) {
			if (!(o instanceof Heap.KeyWrapper)) {
				return false;
			}
			KeyWrapper w = (KeyWrapper) o;
			return special == w.special && (key == null || w.key == null ? key == w.key : key.equals(w.key));
		}
