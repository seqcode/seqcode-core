 ...
 * @author tdanford
 *
 *         <code>GeneFactoryLoader</code> is basically a Singleton. It reads a
 *         properties file in the classpath and creates GeneFactory objects.
 *         Creating a GeneFactory with <code>getFactory</code> requires a Genome
 *         and a type; the type generally indicates the set of gene annotations
 *         to be accessed.
 *
 *         The indirection through GeneFactoryLoader and GeneFactory allows the
 *         properties file to specify more parameters than just the type
 */
public class GeneFactoryLoader {
	...
	public GeneFactory getFactory(Genome g, String type) {
		if (!factories.containsKey(g.getVersion())) {
			return null;
		}
		if (!factories.get(g.getVersion()).containsKey(type)) {
			return null;
		}
	...
	public Set<String> getTypes(String gversion) {
		if (!factories.containsKey(gversion)) {
			return new HashSet<String>();
		}
	...
	private void addFactory(String genome, String type, String pathName) {
		...
		try {
			...
			if (!factories.containsKey(genome)) {
				factories.put(genome, new HashMap<String, GeneFactory>());
			}
