...
/**
 * DistanceGenerator takes two Expander<Region,Region>, a source and a sink.
 * When execute() is presented with a Region, DistanceGenerator applies the two
 * expanders to that region to generate a set of sources and a set of sinks. It
 * then tries to map each source to the nearest sink without reusing any sink.
 * There is a maximum distance (distanceLimit) for the matching.
 *
 * execute() doesn't allow you to determine the distance for any given
 * source-sink; instead, it returns an iterator over all the distances with
 * negative values indicating a source that couldn't be mapped (no sinks left)
 * or was too far.
 *
 * This class is useful, for example, for mapping binding events to motifs.
 */
public class DistanceGenerator implements Expander<Region, Integer> {
	...
	/*
	 * how much should we expand the input Region when presenting it to the
	 * sinkmapper
	 */
	private int expandInputRegion;

	public DistanceGenerator(Expander<Region, Region> sourcemapper, Expander<Region, Region> sinkmapper) {
	...
	public DistanceGenerator(Expander<Region, Region> sourcemapper, Expander<Region, Region> sinkmapper,
			int expandInputRegion, int distanceLimit) {
		this.sourcemapper = sourcemapper;
		this.sinkmapper = sinkmapper;
	...
	/**
	 * returns a distance of -1 for any source that can't be matched to a sink
	 * and -2 for anything that's too far
	 */
	public Iterator<Integer> execute(Region region) {
		...
		while ((todo > 0) && (snkleft > 0)) {
			...
			for (int i = 0; i < sources.length; i++) {
				...
				while ((after <= sinks.length - 1) && ((sinks[after] < sources[i]) || (sinks[after] == -1))) {
					after++;
				}
				before = after - 1;
				while ((before > 0) && (sinks[before] == -1)) {
					before--;
				}
				int d1 = Integer.MAX_VALUE, d2 = Integer.MAX_VALUE, d3 = Integer.MAX_VALUE, d4 = Integer.MAX_VALUE;
				int prevsrc = i - 1, nextsrc = i + 1;
				while ((prevsrc > 0) && (sources[prevsrc] == -1)) {
					prevsrc--;
				}
				while ((nextsrc < sources.length - 1) && (sources[nextsrc] == -1)) {
					nextsrc++;
				}
				if ((before >= 0) && (prevsrc >= 0) && (prevsrc != i) && (sources[prevsrc] != -1)
						&& (sinks[before] != -1)) {
					d1 = Math.abs(sources[prevsrc] - sinks[before]);
				}
				if ((before >= 0) && (sinks[before] != -1)) {
					d2 = Math.abs(sources[i] - sinks[before]);
				}
				if ((after < sinks.length) && (sinks[after] != -1)) {
					d3 = Math.abs(sources[i] - sinks[after]);
				}
				if ((after < sinks.length) && (sinks[after] != -1) && (nextsrc < sources.length) && (nextsrc != i)
						&& (sources[nextsrc] != -1)) {
					d4 = Math.abs(sources[nextsrc] - sinks[after]);
				}
				// System.err.println("sources[i] = " + sources[i] + " sinks are
				// " + Arrays.toString(sinks));
				// System.err.println("d1=" + d1 + " d2=" + d2 + " d3="+ d3 + "
				// d4=" + d4);
				if (d2 < d3) {
					if (d2 < d1) {
						if (d2 < distanceLimit) {
							// System.out.println(d2 + "\t" + region.getChrom()
							// + "\t" + sources[i] + "\t" + sinks[before]);
							distances.add(d2);
						} else {
							distances.add(-2);
						}
					...
					if (d3 < d4) {
						if (d3 < distanceLimit) {
							distances.add(d3);
							// System.out.println(d3 + "\t" + region.getChrom()
							// + "\t" + sources[i] + "\t" + sinks[after]);
						} else {
							distances.add(-2);
						}
						sources[i] = -1;
