...
public class ExpanderIterator<A, B> implements Iterator<B>, Closeable {
	...
	private void getNext() {
		 ...
		 * if (curiter != null && curiter.hasNext()) {
		 * 
		 * nextval = curiter.next(); // System.err.println(" nextval="+nextval);
		 * return; } else if (curiter != null && curiter instanceof Closeable) {
		 * ((Closeable)curiter).close(); }
		 * 
		 * if (input.hasNext()) { A a = input.next(); curiter =
		 * mapper.execute(a); //
		 * System.err.println("ExpanderIterator just got its next input: " + a +
		 * " and curiter is " + curiter); //
		 * System.err.println("  input.hasNext is " + input.hasNext() +
		 * " and curiter.hasNext is " + curiter.hasNext()); getNext(); } else {
		 * nextval = null; //
	...
	public void close() {
		...
		/*
		 * if(mapper instanceof Closeable) { Closeable c = (Closeable)mapper;
