...
public class GPSParser {
		...
		try {
			...
			if (pair == null) {
				...
				if (ap.hasKey("geninfo")) {
					...
					System.err.println(
							"No genome provided; provide a Gifford lab DB genome name or a file containing chromosome name/length pairs.");
					;
					System.exit(1);
				}
			} else {
	...
	/**
	 * Parse a single line of text into a GPSPeak object New format Position IP
	 * Control Fold Q_-lg10 P_-lg10 IPvsEMP IPvsCTR Joint NearestGene Distance
	 * Alpha EM_Position 4:117003586 496.8 7.4 67.1 129.052 135.017 -0.772 4.490
	 * 1 NONE 2147483647 7.8 4:117003586
	 * 
	 * @param gpsLine
	 *            a line of text representing a hit
	...
	public static GPSPeak parseLine(Genome g, String gpsLine, int lineNumber) {
		...
		if (t.length == 7) {
			...
			peak = new GPSPeak(g, r.getChrom(), r.getStart(), Double.parseDouble(t[1]), Double.parseDouble(t[2]),
					Double.parseDouble(t[4]), Math.pow(10, -1 * Double.parseDouble(t[5])), Double.parseDouble(t[6]));
		} else if (t.length == 8) {
			...
			// Position IP Control Fold Q_-lg10 P_-lg10 IPvsEMP IPvsCTR Kmer
			// KmerCount KmerStrength BoundSequence
			Region r = Region.fromString(g, t[0]);
			peak = new GPSPeak(g, r.getChrom(), r.getStart(), Double.parseDouble(t[1]), Double.parseDouble(t[2]),
					Double.parseDouble(t[4]), Math.pow(10, -1 * Double.parseDouble(t[5])), Double.parseDouble(t[6]),
					Double.parseDouble(t[7]), t[8], (int) Double.parseDouble(t[9]), Double.parseDouble(t[10]),
					t.length >= 12 ? t[11] : "");
		} else if (t.length == 10) { // not with kmer info
			// GPS output format 2011-07-25
			// Position IP Control Fold Expectd Q_-lg10 P_-lg10 P_poiss IPvsEMP
			// IPvsCTR
			Region r = Region.fromString(g, t[0]);
			peak = new GPSPeak(g, r.getChrom(), r.getStart(), Double.parseDouble(t[1]), Double.parseDouble(t[2]),
					Double.parseDouble(t[4]), Double.parseDouble(t[5]), Math.pow(10, -1 * Double.parseDouble(t[6])),
					Double.parseDouble(t[7]), Double.parseDouble(t[8]), Double.parseDouble(t[9]));
		} else if (t.length == 14 || t.length == 15) { // with kmer info
			// GPS output format 2011-07-25
			// Position IP Control Fold Expectd Q_-lg10 P_-lg10 P_poiss IPvsEMP
			// IPvsCTR Kmer Count Strength BoundSequence EnrichedHGP
			Region r = Region.fromString(g, t[0]);
			peak = new GPSPeak(g, r.getChrom(), r.getStart(), Double.parseDouble(t[1]), Double.parseDouble(t[2]),
					Double.parseDouble(t[4]), Double.parseDouble(t[5]), Math.pow(10, -1 * Double.parseDouble(t[6])),
					Double.parseDouble(t[7]), Double.parseDouble(t[8]), Double.parseDouble(t[9]), t[10],
					(int) Double.parseDouble(t[11]), t[12].charAt(0), t[13]);
		} else if (t.length == 13) { // with kmer info
			// GPS output format 2012-03
			// Position IP Control Fold Expectd Q_-lg10 P_-lg10 P_poiss IPvsEMP
			// IPvsCTR Kmer Count Strength BoundSequence
			Region r = Region.fromString(g, t[0]);
			peak = new GPSPeak(g, r.getChrom(), r.getStart(), Double.parseDouble(t[1]), Double.parseDouble(t[2]),
					Double.parseDouble(t[4]), Double.parseDouble(t[5]), Math.pow(10, -1 * Double.parseDouble(t[6])),
					Double.parseDouble(t[7]), Double.parseDouble(t[8]), Double.parseDouble(t[9]), t[10],
					(int) Double.parseDouble(t[11]), t[12].charAt(0), "");
		} else {
			throw new RuntimeException(
					"Invalid number of fields (" + t.length + ") on line " + lineNumber + ": " + gpsLine);
		}
		return peak;
	}
}
