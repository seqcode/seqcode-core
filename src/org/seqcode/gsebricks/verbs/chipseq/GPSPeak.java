...
public class GPSPeak extends Point {
	...
	public void setKmer(String km) {
		kmer = km;
	}

	public int getKmerGroupCount() {
		return kmerGroupCount;
	}
	...
	public void setBoundSequence(String bs) {
		boundSequence = bs;
	}

	public GPSPeak(Genome g, String chr, int pos, double ipStrength, double controlStrength, double qvalue,
			double pvalue, double IPvsEMP, int joint, String nearestGene, int distance) {
	...
	public GPSPeak(Genome g, String chr, int pos, double ipStrength, double controlStrength, double qvalue,
			double pvalue, double IPvsEMP, double IPvsCTR, int joint, String nearestGene, int distance) {
		super(g, chr.replaceFirst("chr", ""), pos);
		this.strength = ipStrength;
		this.controlStrength = controlStrength;
		this.qvalue = qvalue;
	...
	public GPSPeak(Genome g, String chr, int pos, double ipStrength, double ctrlStrength, double qvalue, double pvalue,
			double IPvsEMP) {
		super(g, chr.replaceFirst("chr", ""), pos);
		this.strength = ipStrength;
	...
	// Position IP Control Fold Q_-lg10 P_-lg10 IPvsEMP IPvsCTR Kmer KmerCount
	// KmerStrength BoundSequence
	public GPSPeak(Genome g, String chr, int pos, double ipStrength, double ctrlStrength, double qvalue, double pvalue,
			double IPvsEMP, double IPvsCTR, String kmer, int kmerCount, double kmerStrength, String boundSequence) {
		super(g, chr.replaceFirst("chr", ""), pos);
		this.strength = ipStrength;
		this.controlStrength = ctrlStrength;
		this.qvalue = qvalue;
	...
	public GPSPeak(Genome g, String chr, int pos, double ipStrength, double expectedStrength, double ctrlStrength,
	...
	// GPS output format 2011-07-25
	// Position IP Control Fold Expectd Q_-lg10 P_-lg10 P_poiss IPvsEMP IPvsCTR
	// Kmer Count Strength BoundSequence EnrichedHGP
	public GPSPeak(Genome g, String chr, int pos, double ipStrength, double ctrlStrength, double expectedStrength,
			double qvalue, double pvalue, double pois_pvalue, double IPvsEMP, double IPvsCTR, String kmer,
			int kmerCount, char kmerStrand, String boundSequence) {
		super(g, chr.replaceFirst("chr", ""), pos);
		this.strength = ipStrength;
		this.controlStrength = ctrlStrength;
		this.expectedStrength = expectedStrength;
	...
	public String toGPS() {
		return toString() + "\t" + strength + "\t" + controlStrength + "\t" + qvalue + "\t" + pvalue + "\t" + IPvsEMP
				+ "\t" + jointEvent + "\t" + nearestGene + "\t" + distance;
	}
	...
	public String toGPS_short() {
		...
		String out = String.format("%s\t%.1f\t%.1f\t%.1f\t%.3f\t%.3f\t%.3f", toString(), strength, controlStrength,
				fold, qvalue, pvalue, IPvsEMP);
		return out;
	}
	...
	public String toGPS_motifShort() {
		return toString() + "\t" + nearestGene + "\t" + distance + "\t" + strength + "\t" + controlStrength;// +"\t"+qvalue+"\t"+shape+"\t"+shapeZ;
	}
}
