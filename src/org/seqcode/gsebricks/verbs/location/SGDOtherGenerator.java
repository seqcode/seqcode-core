...
/**
 * Generator that returns NamedTypedRegion objects from the specified table. The
 * table must have columns: chrom, chromStart, chromEnd, name, strand, and type
 * . This parses tables like the UCSC sgdOther table from the sacCer1 annotation
 * database.
 */
...
public class SGDOtherGenerator<X extends Region> implements Expander<X, NamedTypedRegion> {
	...
	public Iterator<NamedTypedRegion> byName(String name) {
		try {
			...
			PreparedStatement ps = cxn.prepareStatement(
					"select name, chrom, strand, type, chromStart, chromEnd from " + tablename + " where name = ?");
			ps.setString(1, name);
	...
	public Iterator<NamedTypedRegion> execute(X region) {
		try {
			java.sql.Connection cxn = genome.getAnnotationDBConnection();
			PreparedStatement ps = cxn.prepareStatement("select name, chrom, strand, type, chromStart, chromEnd from "
	...
	private Iterator<NamedTypedRegion> parseResults(PreparedStatement ps) throws SQLException {
		try {
			...
			while (rs.next()) {
				...
				results.add(new NamedTypedRegion(genome, chr, rs.getInt(5), rs.getInt(6), rs.getString(1),
						rs.getString(4), strand.length() > 0 ? strand.charAt(0) : ' '));
			}
			rs.close();
			ps.close();
