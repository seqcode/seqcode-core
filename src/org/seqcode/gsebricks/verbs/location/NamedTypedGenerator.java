...
public class NamedTypedGenerator<X extends Region> implements Expander<X, NamedTypedRegion> {
	...
	public Iterator<NamedTypedRegion> execute(X region) {
		try {
			...
			PreparedStatement ps = cxn.prepareStatement("select name, strand, type, chromStart, chromEnd from "
			...
			while (rs.next()) {
				results.add(new NamedTypedRegion(genome, region.getChrom(), rs.getInt(4), rs.getInt(5), rs.getString(1),
						rs.getString(3), rs.getString(2).charAt(0)));
