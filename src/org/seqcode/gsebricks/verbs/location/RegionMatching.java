...
public class RegionMatching<RBase extends Region, RCover extends Region> {
	...
	public Iterator<RBase> covered() {
		return cover.keySet().iterator();
	}

	public Iterator<RCover> coverage(RBase b) {
		return cover.containsKey(b) ? cover.get(b).iterator() : new EmptyIterator<RCover>();
	}

	public Iterator<RCover> allCoverage() {
		return new ExpanderIterator<RBase, RCover>(new Expander<RBase, RCover>() {
	...
	public Iterator<RCover> pointCoverage(RBase b, int offset) {
		...
		return new FilterIterator<RCover, RCover>(new ContainsFilter(p), coverage(b));
	}

	private class ContainsFilter implements Filter<RCover, RCover> {
		private Point p;

		public ContainsFilter(Point pp) {
			p = pp;
		}
