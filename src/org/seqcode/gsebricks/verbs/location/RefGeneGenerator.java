 ...
 * similar structure, eg sgdGene) in a UCSC annotation database.
 * RefGeneGenerator can work on two tables- the first contains the coordinates
 * for the genes and the second contains a mapping from gene name to aliases.
 * 
 * reads the properties file org.seqcode.gseutils.properties.gene_names which is
 * formatted as genomeversion,tablename=symboltablename,namecolumn,symbolcolumn
 *
 * namecolumn is the column in the symboltable that should match the name.
 * aliascolumn is the column in the alias table that contains the alias
 *
 * RefGeneGenerator can query in several modes: - genes whose orf overlaps the
 * query region - genes whose 5' IGR overlaps the query region - the closest n
 * genes to a region, regardless of distance
 * 
 * Most of the SQL generation code here could probably be generalized out and
 * then reused to query a larger number of table types
 * 
 * Notes from Shaun: 1) One quirk of the UCSC tables is that the starting
 * coordinates of genes, exons, etc is 0-based, while the end coordinate is
 * 1-based. Correcting for that here. 2) The execute method prepends "chr" to
 * the chromosome names by default. There are genomes where this is not what we
 * want, so I'm adding constructors in here to handle this case.
 * 
 *
 * @author Alex Rolfe
 */
...
public class RefGeneGenerator<X extends Region>
	 ...
	 * 
	 * @param prependChr:
	 *            should we prepend "chr" to the chromosome names?
	 */
	...
	/**
	 * @param t
	 *            the tablename from which gene information is retrieved
	 */
	...
	public void setGenome(Genome g, String t) {
		...
		tablename = t;
		/*
		 * if no table name was given, then we'll end up using the first table
		 * listed in the gene_names properties file
		 */
	...
	/**
	 * if retrieveCoding is true, then the start and stop will be of the coding
	 * region rather than the transcribed region
	 */
	...
	public void setWantSymbol(boolean b) {
		wantsymbol = b;
	}
	...
	public void setUpstreamDownstream(int up, int down) {
		if (upstream == 0 && downstream == 0 && (up != 0 || down != 0)) {
			close();
		}
		upstream = up;
		downstream = down;
	...
	public void setUpstream(int up) {
		if (upstream == 0 && downstream == 0 && (up != 0)) {
			close();
		}
		upstream = up;
	}
	...
	public void setDownstream(int down) {
		if (upstream == 0 && downstream == 0 && (down != 0)) {
			close();
		}
		downstream = down;
	}
	...
	/*
	 * close releases the database connection. This ought to be called when
	 * query parameters change that might change the structure of the SQL.
	...
	/*
	 * generate SQL PreparedStatements based on the current query parameters.
	 * Set ps and nameps.
	 * 
	 * Any changes made to ps here probably need to be reflected in execute() so
	 * the bind variables set up in the sql query here are bound in the proper
	 * order in execute().
	 */
	private void prepareStatements(Connection cxn) {
		...
		try {
			...
			if (symboltable == null) {
				...
				// this noop extra select is to fool the mysql query optimizer
				// so it
				// doesn't screw this query up
				namequery += String.format(
						" from %s g where g.name = ? or g.name in (select id from (select %s as id from %s a where a.%s = ? ) as x)",
						tablename, namecolumn, symboltable, symbolcolumn);
			}
			nameps = cxn.prepareStatement(namequery);
			if (symboltable != null && symbolcolumn != null && namecolumn != null) {
				getgenesym = cxn.prepareStatement(
						String.format("select %s from %s where %s=?", symbolcolumn, symboltable, namecolumn));
			} else {
				getgenesym = null;
			}
	...
	/*
	 * common method for execute() and byName(). The prepared statement should
	 * have all variables be bound and be ready to execute. IT should return
	 * fields from the genes table
	 */
	public synchronized Iterator<Gene> parseResults(PreparedStatement ps) throws SQLException {
		...
		while (rs.next()) {
			...
			if (wantsExons) {
				...
				ExonicGene exonicGene = new ExonicGene(genome, chr, rs.getInt(4) + 1, // Correction
																						// for
																						// 0-based
																						// starts
						rs.getInt(5), rs.getString(1), rs.getString(1), strand, "RefGene");
				g = exonicGene;
					...
					try {
						...
						for (int i = 0; i < startArray.length && i < endArray.length; i++) {
							if (startArray[i].length() > 0 && endArray[i].length() > 0) {
								int start = Integer.parseInt(startArray[i]) + 1; // Correction
																					// for
																					// 0-based
																					// starts
																					// in
				...
				g = new Gene(genome, chr, rs.getInt(4) + 1, // Correction for
															// 0-based starts
						rs.getInt(5), rs.getString(1), rs.getString(1), strand, "RefGene");
			}
	...
	private String getFields() {
		String query = String.format("select name, chrom, strand, %s, %s", getStartField(), getEndField());
		if (wantsExons) {
			query += ", exonCount, exonStarts, exonEnds";
		}
		return query;
	}

	/*
	 * returns an order clause to get the closest gene to the input region
	 */
	private String getClosestOrder() {
		if (toBoundary == TOSTART) {
			return String.format(
					" order by least(abs(? - if(strand = '+', %s, %s)),abs(? - if(strand = '+', %s, %s))) ",
			...
			return String.format(" order by least(abs(%s - ?), abs(? - %s),abs(%s - ?), abs(? - %s)) ", getStartField(),
					getEndField(), getStartField(), getEndField());
		}
	}

	private int bindClosestOrder(PreparedStatement ps, int offset, X region) throws SQLException {
		/*
		 * use floats here because otherwise MySQL does stupid things when
		 * subtracting from the unsigned ints in the database
		 */
	...
	private String getUpstreamOverlap() {
		String query = String.format(
				"((strand = '+' and ((%s <= ? and %s >= ?) or (%s >= ? and %s <= ?))) or "
						+ " (strand = '-' and ((%s <= ? and %s >= ?) or (%s >= ? and %s <= ?))))",
				getStartField(), getStartField(), getStartField(), getStartField(), getEndField(), getEndField(),
	...
	private String getOrfOverlap() {
		return String.format(" ((%s <= ? and %s >= ?) or (%s >= ? and %s <= ?))", getStartField(), getEndField(),
