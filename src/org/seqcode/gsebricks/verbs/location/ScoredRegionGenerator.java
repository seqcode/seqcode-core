 ...
 * into ScoredRegions. StrandedRegionGenerator provides a similar function for a
 * slightly different table schema (and has further useful documentation).
 */
public class ScoredRegionGenerator<X extends Region> implements Expander<X, ScoredRegion> {
	...
	public boolean useBins() {
		return useBins;
	}

	public String getColumnsSQL() {
		return "chromStart, chromEnd, score";
	}
	...
	public Iterator<ScoredRegion> execute(X region) {
		try {
			...
			while (rs.next()) {
				ScoredRegion r = new ScoredRegion(genome, region.getChrom(), rs.getInt(1), rs.getInt(2),
						rs.getFloat(3));
				results.add(r);
			}
			rs.close();
