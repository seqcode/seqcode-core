...
/**
 * maps a Region to a set of sub-regions that are tiled in a particular array
 * design *
 */
...
public class TiledRegionGenerator<X extends Region> implements Expander<X, Region> {
	 ...
	 * @param spacing
	 *            the maximum spacing in bp between adjacent probes for them to
	 *            be counted as part of the same tiled region
	 * @param mincount
	 *            the minimum number of probes in a region (with at most
	 *            <code>spacing</code> bp between adjacent probes) for a tiled
	 *            region to be included in the output
	 */
	public TiledRegionGenerator(String design, int spacing, int mincount) throws NotFoundException {
		this(design, spacing, mincount, 0);
	}
	...
	public TiledRegionGenerator(String design, int spacing, int mincount, int minlen) throws NotFoundException {
		...
		this.minlen = minlen;
		try {
			java.sql.Connection cxn = DatabaseConnectionManager.getConnection("chipchip");
	...
	public Iterator<Region> execute(X r) {
		try {
			...
			PreparedStatement ps = cxn
					.prepareStatement("select pl.startpos, pl.stoppos from probelocation pl, probedesign pd where"
							+ " pl.id = pd.id and pl.chromosome = ? and pl.stoppos >= ? and pl.startpos <= ? and pd.arraydesign = ? "
							+ " and pl.loccount = 1 order by pl.startpos");
			// System.err.println("Looking for probes from " + arrayDesign + ",
			// " + designID + " in " + r + ", " + chromid);
			// System.err.println("spacing=" + spacing + " and mincount="+
			// mincount);
			ps.setInt(1, chromid);
