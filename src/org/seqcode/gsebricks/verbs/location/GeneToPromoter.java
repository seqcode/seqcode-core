...
public class GeneToPromoter
	...
	/**
	 * will generate promoter regions this many bases upstream and downstream of
	 * the gene's start site and that do not overlap the features returned by
	 * otherfeature.
	 */
	...
	public NamedStrandedRegion execute(Gene a) {
		...
		// System.err.println("Gene is " + a + " -> " + a.getStart()
		// +","+a.getEnd()+"," + a.getStrand());
		// System.err.println("Intermediate output " + output + " -> " + start
		// +"," + stop);
		if (dontoverlap != null) {
			...
			for (Expander<Region, ? extends Region> expander : dontoverlap) {
				...
				while (iter.hasNext()) {
					...
					// other.toString(), other.getStart(),other.getEnd(),
					// start,stop));
				}
			}
			if (start != output.getStart() || stop != output.getEnd()) {
				output = new NamedStrandedRegion(output.getGenome(), output.getChrom(), start, stop, output.getName(),
						output.getStrand());
			}
		}
		return output;
