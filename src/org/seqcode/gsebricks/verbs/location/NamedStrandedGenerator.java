...
public class NamedStrandedGenerator<X extends Region> implements Expander<X, NamedStrandedRegion> {
	...
	public Iterator<NamedStrandedRegion> execute(X region) {
		try {
			...
			PreparedStatement ps = cxn.prepareStatement("select name, strand, chromStart, chromEnd from " + tablename
					+ " where chrom = ? and "
			...
			while (rs.next()) {
				results.add(new NamedStrandedRegion(genome, region.getChrom(), rs.getInt(3), rs.getInt(4),
						rs.getString(1), rs.getString(2).charAt(0)));
