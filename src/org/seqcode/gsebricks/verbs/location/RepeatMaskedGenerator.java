	...
	public Iterator<RepeatMaskedRegion> execute(X region) {
		try {
			...
			PreparedStatement ps = cxn.prepareStatement(
					"select genoStart, genoEnd, strand, repName, repClass, repFamily, swScore from " + tablename
			...
			while (rs.next()) {
				RepeatMaskedRegion r = new RepeatMaskedRegion(genome, region.getChrom(), rs.getInt(1), rs.getInt(2),
						rs.getString(4), rs.getString(5), rs.getString(6), (double) rs.getInt(7),
