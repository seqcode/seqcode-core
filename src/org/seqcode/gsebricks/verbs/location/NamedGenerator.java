...
public class NamedGenerator<X extends Region> implements Expander<X, NamedRegion> {
	...
	public Iterator<NamedRegion> execute(X region) {
		try {
			...
			PreparedStatement ps = cxn.prepareStatement("select name, chromStart, chromEnd from " + tablename
					+ " where chrom = ? and "
			...
			while (rs.next()) {
				results.add(new NamedRegion(genome, region.getChrom(), rs.getInt(2), rs.getInt(3), rs.getString(1)));
