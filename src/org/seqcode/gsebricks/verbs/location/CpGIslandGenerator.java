	...
	public Iterator<CpGIsland> execute(X region) {
		try {
			...
			PreparedStatement ps = cxn
					.prepareStatement("select chromStart, chromEnd, cpgNum, gcNum, perCpg, perGc, obsExp from "
			...
			while (rs.next()) {
				CpGIsland island = new CpGIsland(genome, region.getChrom(), rs.getInt(1), rs.getInt(2), rs.getInt(3),
