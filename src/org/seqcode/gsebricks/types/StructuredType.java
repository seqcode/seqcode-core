...
public class StructuredType implements EchoType {
	...
	public StructuredType(StructuredType base, String tn, EchoType t) {
		...
		if (subtypes.containsKey(tn)) {
			throw new IllegalArgumentException(tn);
		}
		subtypes.put(tn, t);
		buildTotalName();
	}
	...
	public EchoType getTypeValue(String k) {
		return subtypes.get(k);
	}

	public boolean isSubType(EchoType et) {
		if (et instanceof GeneralType) {
			return true;
		}
		if (!(et instanceof StructuredType)) {
			return false;
		}
		...
		for (String key : st.subtypes.keySet()) {
			...
			if (!subtypes.get(key).isSubType(st.subtypes.get(key))) {
				return false;
			}
		}
		return true;
	...
	public boolean equals(Object o) {
		...
		if (st.subtypes.size() != subtypes.size()) {
			return false;
		}

		for (String key : subtypes.keySet()) {
			if (!st.subtypes.containsKey(key)) {
				return false;
			}
			if (!subtypes.get(key).equals(st.subtypes.get(key))) {
				return false;
			}
		}
		return true;
