...
/**
 * GenericSelectPanel is a panel with two table areas - one to hold a filterable
 * list, and one to hold selected items from that list.
 * 
 * @author mahony
 *
 * @param <X>
...
public abstract class GenericSelectPanel<X> extends JSplitPane implements Closeable, Runnable {
	 ...
	 * clearing them, your changes will be lost. By sticking things in
	 * asyncSelected and then adding them at the end, your additions will be
	 * kept
	 */
	...
	/*
	 * sets the genome and starts a new thread to process that change. Database
	 * work should happen in that thread and the results should be cached. Swing
	 * components should be updated with a call to SwingUtilities.invokeLater().
	 * Since this also invokes run(), the run() method must determine the
	 * current operation by querying: handlingNewGenome() and dataReady()
	 * 
	 */
