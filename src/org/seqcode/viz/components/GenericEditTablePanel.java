...
public abstract class GenericEditTablePanel<X> extends JPanel implements Closeable, Runnable {
	...
	/**
	 * UpdateData Starts a new thread to update data. Database work should
	 * happen in that thread and the results should be cached. Swing components
	 * should be updated with a call to SwingUtilities.invokeLater(). Since this
	 * also invokes run(), the run() method must determine the current operation
	 * by querying: handlingNewGenome() and dataReady()
	 */
	...
	public boolean handlingUpdate() {
		return handlingUpdate;
	}
	...
	/*
	 * called from swing thread to update the swing components for the cached
	 * data
	 */
	public abstract void updateComponents();

	/* sub-class constructors must call one of the two init() methods */
	public void init(JTable filteredList, ObjectTableModel<X> filteredModel) {
	...
	public void init(ObjectTableModel<X> filteredModel) {
		init(new JTable(filteredModel), filteredModel);
	}
	...
	/*
	 * returns the elements of the filtered list that should be added when the
