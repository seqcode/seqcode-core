...
public class ObjectTableModel<X> implements TableModel {
	...
	public void clear() {
		if (objects.size() > 0) {
			TableModelEvent evt = new TableModelEvent(this, 0, objects.size() - 1, TableModelEvent.ALL_COLUMNS,
	...
	public void addObject(X o) {
		if (!contains(o)) {
			objects.add(o);
			sort();
			TableModelEvent evt = new TableModelEvent(this, objects.size() - 1, objects.size() - 1,
	...
	public Object getValueAt(int rowIndex, int c) {
		if (c == 0) {
			return objects.get(rowIndex).toString();
		}
		return null;
	}
