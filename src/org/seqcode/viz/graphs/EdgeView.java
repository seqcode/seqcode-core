...
public class EdgeView extends ObjectView {
	...
	public EdgeView(GraphView g, NodeView s, NodeView f) {
		...
		if (start.equals(finish)) {
			...
			setOption("selfAngle", new Double(Math.PI));
		}
	...
	public boolean isDirected() {
		...
		if (!value.equals("true") && !value.equals("false")) {
			throw new IllegalStateException(String.format("option 'directed' has illegal value '%s'", value));
		}
	...
	public void paintEdge(Graphics2D g2) {
		...
		if (isDirected() && (ex != sx || ey != sy)) {
			...
			/*
			 * Point p1 = (Point)ev.getOption("arrowPoint1"); Point p2 =
			 * (Point)ev.getOption("arrowPoint2"); Point p3 =
			 * (Point)ev.getOption("arrowPoint3"); xs[0] = p1.x; xs[1] = p2.x;
			 * xs[2] = p3.x; ys[0] = p1.y; ys[1] = p2.y; ys[2] = p3.y;
			 * g2.fillPolygon(xs, ys, 3); if(ev.containsOption("name")){ String
			 * edName = (String)ev.getOption("name"); g2.setColor(Color.black);
			...
			GraphPaintable.drawArrow(g2, (Integer) getOption("arrowSize"), new Point(c0, c1), omega + Math.PI);
	...
	public void paintCurvedEdge(Graphics2D g2) {
		...
		int curveBend = options.containsKey("curve-bend") ? (Integer) options.get("curve-bend") : 250;
		...
		QuadCurve2D.Double curve = new QuadCurve2D.Double(-(ex - sx) / 2, -(ey - sy) / 2,
				new Double(nP.getX()).intValue(), new Double(nP.getY()).intValue(), (ex - sx) / 2, (ey - sy) / 2);
		g2.draw(curve);
		if (isDirected()) {
	...
	public void paintSelfEdge(Graphics2D g2, double angle) {
		...
		CubicCurve2D.Double curve = new CubicCurve2D.Double(0, 0, new Double(cp1.getX()).intValue(),
				new Double(cp1.getY()).intValue(), new Double(cp2.getX()).intValue(), new Double(cp2.getY()).intValue(),
				0, 0);
		...
		if (isDirected()) {
			...
			do {
				...
				case PathIterator.SEG_CUBICTO:
					...
					GraphPaintable.drawArrow(g2, (Integer) getOption("arrowSize"), new Point(c2, c3), omega + Math.PI);
