...
public class SubEdgeView extends EdgeView {
	...
	public int getSubWidth(int x) {
		if (x >= 0 && x < numSubEdges) {
			return subEdgeWidths[x];
		} else {
			return -1;
		}
	}

	public Color getSubColor(int x) {
		if (x >= 0 && x < numSubEdges) {
			return subEdgeColors[x];
		} else {
			return Color.black;
		}
	}
	...
	public void paintCurvedSubEdges(Graphics2D g2) {
		...
		for (int i = 0; i < getNumSubEdges(); i++) {
			...
			Point nP = GraphPaintable.rotate(new Point(new Double(vx).intValue(), new Double(vy).intValue()), omega);

			double sPhi = Math.atan2(new Double(nP.getY()).intValue() + (ey - sy) / 2,
					new Double(nP.getX()).intValue() + (ex - sx) / 2) - Math.toRadians(90);
			double ePhi = Math.atan2(new Double(nP.getY()).intValue() - (ey - sy) / 2,
					new Double(nP.getX()).intValue() - (ex - sx) / 2) + Math.toRadians(90);
			...
			Point nPb = GraphPaintable.rotate(new Point(new Double(vx).intValue(), new Double(vyb).intValue()), omega);

			QuadCurve2D.Double curve = new QuadCurve2D.Double(-(ex - sx) / 2, -(ey - sy) / 2,
					new Double(nPb.getX()).intValue(), new Double(nPb.getY()).intValue(), (ex - sx) / 2, (ey - sy) / 2);
	...
	public void paintSelfSubEdges(Graphics2D g2, double angle) {
		...
		for (int i = 0; i < getNumSubEdges(); i++) {
			...
			int newapexx = (cpmidx * 3 / 4)
					- new Double((((totalWidth - ewidth) / 2) - cumul) * Math.sin(omega)).intValue();
			int newapexy = (cpmidy * 3 / 4)
					+ new Double((((totalWidth - ewidth) / 2) - cumul) * Math.cos(omega)).intValue();
			...
			int newDist = new Double(new Point(newcpmainX, newcpmainY).distance(basePoint.getX(), basePoint.getY()))
					.intValue();
			...
			CubicCurve2D.Double curve = new CubicCurve2D.Double(new Double(basePoint.getX()).intValue(),
					new Double(basePoint.getY()).intValue(), new Double(cp1.getX()).intValue(),
					new Double(cp1.getY()).intValue(), new Double(cp2.getX()).intValue(),
					new Double(cp2.getY()).intValue(), new Double(basePoint.getX()).intValue(),
					new Double(basePoint.getY()).intValue());
