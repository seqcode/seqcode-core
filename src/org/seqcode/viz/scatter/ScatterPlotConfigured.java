...
public class ScatterPlotConfigured extends ScatterPlot {
	...
	/**
	 * A mean-variance plot is log-log scaled, and a line can be drawn along the
	 * lowess fit
	 * 
	 * @param datapoints
	 *            : mean vs variance
	 * @param fitLine
	 *            : coordinates (dataspace) of line
	 * @param outFilename
	 * @param rasterImage
	 */
	public void saveMeanVarPlot(Matrix datapoints, Matrix fitLine, String outFilename, boolean rasterImage) {
		...
		if (fitLine != null) {
			...
			XYLineAnnotation lineAnnot = new XYLineAnnotation(xLower, xLower, xUpper, xUpper, new BasicStroke(2),
					Color.blue);
			this.plot.addAnnotation(lineAnnot);
