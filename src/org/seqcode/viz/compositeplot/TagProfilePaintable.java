...
public class TagProfilePaintable extends AbstractPaintable {
	...
	private List<Integer> pointsOfInterest = new ArrayList<Integer>(); // x-coordinates
																		// of
																		// points
																		// of
																		// interest
	...
	public void setWatsonColor(Color c) {
		watsonCol = transparent ? Coloring.clearer(c) : c;
	}

	public void setCrickColor(Color c) {
		crickCol = transparent ? Coloring.clearer(c) : c;
	}
	...
	public void setPointOfInterest(int coord) {
		pointsOfInterest.add(coord);
	}

	public void setHmargin(int hm) {
		hmargin = hm;
	}

	public void setWmargin(int wm) {
		wmargin = wm;
	}
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		if (drawAxis) {
			...
			// X-axis labels
			g2.drawString(String.format("%d", leftLimit),
					tX1 - (metrics.stringWidth(String.format("%d", leftLimit)) / 2), wPlotY2 + (metrics.getHeight()));
			g2.drawString(String.format("%d", (rightLimit + 1)),
					tX2 - (metrics.stringWidth(String.format("%d", (rightLimit + 1))) / 2),
					wPlotY2 + (metrics.getHeight()));
			...
			for (int x = tX1; x <= tX2; x += tickPixels) {
				if (coordX % bigTickSpacing == 0)
					g2.drawLine(x, wPlotY2 - 4, x, wPlotY2 + 4); // big tick
																	// marks
				else
					g2.drawLine(x, wPlotY2 - 2, x, wPlotY2 + 2); // little tick
																	// marks
				coordX += tickSpacing;
			}
