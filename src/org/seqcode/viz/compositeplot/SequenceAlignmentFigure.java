...
public class SequenceAlignmentFigure {
	 ...
	 * 
	 * @param seqs,
	 *            raw sequences or FASTA sequences
	 * @param width,
	 *            width of each base, in pixel
	 * @param height,
	 *            height of each base, in pixel
	 * @param f,
	 *            output file
	 */
	public static void visualizeSequences(List<String> seqs, int width, int height, File f) {
		...
		int maxLen = 0;
		for (String s : seqs) {
			if (s.length() != 0 && s.charAt(0) != '>') { // ignore header line
															// of FASTA file
				pixheight += height;
				if (maxLen < s.length())
					maxLen = s.length();
	...
	public static void main(String[] args) {
		...
		if (!ap.hasKey("species")) {
			System.err.println("Usage:\n " + "SequenceAlignmentFigure \n" + " Required: \n"
					...
					+ "  --peaks <file containing stranded coordinates> OR" + "  --seq <FASTA file>\n"
		...
		try {
			...
			if (ap.hasKey("peaks")) {
				...
				List<StrandedRegion> regions = RegionFileUtilities.loadStrandedRegionsFromMotifFile(currgen, peaksFile,
						win);
