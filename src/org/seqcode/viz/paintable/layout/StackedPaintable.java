...
public class StackedPaintable extends AbstractPaintable {
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		int stackedHeight = (int) Math.floor((double) (y2 - y1) / (double) fArray.size());
		int stackedWidth = (int) Math.floor((double) (x2 - x1) / (double) fArray.size());

		for (int i = 0; i < fArray.size(); i++) {
			Paintable p = fArray.get(i);

			if (fVertical) {
				p.paintItem(g, x1, y1 + (i * stackedHeight), x2, y1 + ((i + 1) * stackedHeight));
			} else {
				p.paintItem(g, x1 + (i * stackedWidth), y1, x1 + ((i + 1) * stackedWidth), y2);
			}
		}
	}
}
