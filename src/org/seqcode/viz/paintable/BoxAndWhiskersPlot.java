...
public class BoxAndWhiskersPlot extends AbstractPaintable implements PaintableScaleListener {
	...
	/**
	 * Testing whether the BoxAndWhiskersPlot works, along with the Scale
	 * painters.
	 */
	...
	public BoxAndWhiskersPlot(PaintableScale s, double min, double lower, double middle, double upper, double max) {
		scale = s;
		this.min = min;
		this.max = max;
		this.lowerBox = lower;
		...
		if (min > lower || lower > middle || middle > upper || upper > max) {
			throw new IllegalArgumentException(String.format("%f < %f < %f < %f < %f", min, lower, middle, upper, max));
		}
