...
public class ArrowPainter extends AbstractPaintable {
	...
	public void paintArrow(Graphics2D g2, int x1, int y1, int x2, int y2) {
		...
		int xc = direction ? x2 : x1; // xc is the x of the point of the arrow
										// head.
		...
		int[] hx = new int[] { xa, xc, xa }; // coordinates (hx and hy) for the
												// arrow head.
		int[] hy = new int[] { y1, yc, y2 };

		g2.setColor(color);
		g2.fillPolygon(hx, hy, 3); // fills in the head of the arrow
		g2.fillRect((direction ? xb : xa), ya, bw, bh); // fills in the base of
														// the arrow
