...
public class BorderPaintable extends AbstractPaintable {
	...
	public void setTop(Paintable p) {
		if (fTop != null) {
			fTop.removePaintableChangedListener(this);
		}
		fTop = p;
	...
	public void setBottom(Paintable p) {
		if (fBottom != null) {
			fBottom.removePaintableChangedListener(this);
		}
		fBottom = p;
		dispatchChangedEvent();
		if (p != null) {
	...
	public void setLeft(Paintable p) {
		if (fLeft != null) {
			fLeft.removePaintableChangedListener(this);
		}
		fLeft = p;
		dispatchChangedEvent();
		if (p != null) {
	...
	public void setRight(Paintable p) {
		if (fRight != null) {
			fRight.removePaintableChangedListener(this);
		}
		fRight = p;
		dispatchChangedEvent();
		if (p != null) {
	...
	public Collection<Action> getPaintableActions() {
		...
		if (fTop != null) {
			lst.addAll(fTop.getPaintableActions());
		}
		if (fBottom != null) {
			lst.addAll(fBottom.getPaintableActions());
		}
		if (fLeft != null) {
			lst.addAll(fLeft.getPaintableActions());
		}
		if (fRight != null) {
			lst.addAll(fRight.getPaintableActions());
		}
		return lst;
	}
	...
	public void registerClick(double xf, double yf) {
		...
		if (x1 <= xf && xf <= x2 && y1 <= yf && yf <= y2) {
			...
			if (fLeft != null && xf < x1 && y1 <= yf && yf <= y2) {
				fLeft.registerClick(xf / x1, (yf - y1) / (y2 - y1));
			}

			if (fRight != null && xf > x2 && y1 <= yf && yf <= y2) {
				fRight.registerClick((xf - x2) / (1.0 - x2), (yf - y1) / (y2 - y1));
			}

			if (fTop != null && yf < y1 && x1 <= xf && xf <= x2) {
				fTop.registerClick((xf - x1) / (x2 - x1), yf / y1);
			}

			if (fBottom != null && yf > y2 && x1 <= xf && xf <= x2) {
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		if (fLeft != null) {
			fLeft.paintItem(g, x1, yi1, xi1, yi2);
		}
		if (fRight != null) {
			fRight.paintItem(g, xi2, yi1, x2, yi2);
		}
		if (fTop != null) {
			fTop.paintItem(g, xi1, y1, xi2, yi1);
		}
		if (fBottom != null) {
			fBottom.paintItem(g, xi1, yi2, xi2, y2);
		}
	}
}
