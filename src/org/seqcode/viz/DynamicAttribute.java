 ...
 * @author Lindy Briggs
 * 
 *         (Again, another class by Lindy: this was, as with
 *         AttributeCalculator, originally copied over from psrg.lindy package.
 *         This is a default implementation of the AttributeCalculator class).
 * 
 *         The attributes in a DynamicAttribute object change depending on the
 *         size of the viewing window, unless the user specifically sets
 *         attribute values. If that happens, the specified attributes will
 *         remain fixed. They can be reset to their original dynamic version as
 *         well. Right now, the dynamic attribute values are kind of random on
 *         my part, for experiment's sake.
 * 
 *         I may consider adding a way for the user to set values as percentages
 *         later on.
 */
public class DynamicAttribute implements AttributeCalculator {

	private static DynamicAttribute global = null;
	public static final int SCREEN = 1, PRINT = 2, DISPLAY = 3;
	...
	public int getPointWidth(int w, int h) {
		return getPointWidth(w, h, 1.0);
	}

	public int getLineWidth(int w, int h) {
		return getLineWidth(w, h, 1.0);
	}

	public int getFontSize(int w, int h) {
		return getFontSize(w, h, 1.0);
	}
	...
	// get diameter of a circle (although the agilent painter is treating this
	// as a radius...)
