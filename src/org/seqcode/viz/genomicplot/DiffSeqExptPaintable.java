...
public class DiffSeqExptPaintable extends AbstractPaintable {
	...
	private Sample signalSamp = null;
	private Sample controlSamp = null;
	...
	private double[] diff;
	private int[] dataProfile; // y pixel values of where the data line is
								// drawn.

	// Load data from ReadDB sources
	public DiffSeqExptPaintable(GenomeConfig gcon, Region basereg, Sample sig, Sample ctrl, int win, int step,
			double scaling) {
		this.gconfig = gcon;
		region = basereg;
	...
	public DiffSeqExptPaintable(GenomeConfig gcon, Region basereg, File data, int win, int step, double scaling) {
		this.gconfig = gcon;
		region = basereg;
		binWidth = win;
		binStep = step;
	...
	public void setNegColor(Color c) {
		negColor = c;
	}

	public void setPosColor(Color c) {
		posColor = c;
	}

	public void setSigPerBaseMax(int pb) {
		sigPerBaseMax = pb;
	}

	public void setCtrlPerBaseMax(int pb) {
		ctrlPerBaseMax = pb;
	}
	...
	public void setMaxLogFold(int m) {
		maxLogFold = m;
	}

	public void setMinLogFold(int m) {
		minLogFold = m;
	}
	...
	public void loadData() {
		...
		List<StrandedBaseCount> AHits = new ArrayList<StrandedBaseCount>();
		List<StrandedBaseCount> BHits = new ArrayList<StrandedBaseCount>();

		if (signalSamp != null)
			AHits.addAll(signalSamp.getBases(region));
		if (controlSamp != null)
			BHits.addAll(controlSamp.getBases(region));
	...
	public void loadDataFromFile(File data) {
		...
		try {
			if (!data.isFile()) {
				System.err.println("Invalid file name: " + data.getAbsolutePath());
				System.exit(1);
			}
	...
	// Makes integer arrays corresponding to the read landscape over the current
	// region
	private double[] makeHitLandscape(List<StrandedBaseCount> hits, Region currReg, int perBaseMax, char strand) {
		...
		for (StrandedBaseCount r : hits) {
			if (strand == '.' || r.getStrand() == strand) {
				int rpos = r.getCoordinate();
				int offset = inBounds(rpos - currReg.getStart(), 0, currReg.getWidth());
				counts[offset]++;// small issue here... counts will not be
									// corrected for scalingFactor in control
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		for (int a = 0; a <= numBins; a++) {
			...
			double yf = foldDiff < 0 ? Math.abs(foldDiff / -(double) minLogFold)
					: Math.abs(foldDiff / (double) maxLogFold);
	...
	/**
	 * A one-shot (i.e., one use) expander, that automatically closes its inner
	 * expander when it's been called the first time. Only used in the
	 * constructor, above.
	 * 
	 * @author tdanford
	 *
	 * @param <R>
	 */
	private static class ClosingRegionExpanderWrapper<R extends Region> implements Expander<Region, Region> {
