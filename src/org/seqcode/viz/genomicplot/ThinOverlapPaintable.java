...
public class ThinOverlapPaintable extends AbstractPaintable {
	...
	public void setLoopColor(Color c) {
		loopColor = c;
	}

	public void setInterColor(Color c) {
		interColor = c;
	}
	...
	public void setBgThick(int t) {
		bgThick = t;
	}

	public void setHighlightThick(int t) {
		highlightThick = t;
	}

	public void setFilledColumns(boolean b) {
		filledColumns = b;
	}
	...
	public void loadData() {
		...
		for (StrandedBaseCount sbc : hits) {
			ScoredStrandedRegion hit = sbc.expandToScoredStrandedRegion(region.getGenome(), region.getChrom(), 0,
					threePrimeExt);
	...
	public void setMaxOverlap(int m) {
		maxOverlap = m;
	}
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		// Split the area into two sections: reads & pairs. The latter is
		// usually zero area.
		...
		if (drawPairedCurves) {
			...
			for (Pair<Point, Point> inter : inters) {
				if (region.contains(inter.car()) && region.contains(inter.cdr())) {
					...
					int xMid = (xA + xB) / 2;
					int yMid = (int) (((double) (inter.cdr().getLocation() - inter.car().getLocation())
							/ (double) region.getWidth()) * pairh * 2) + pairy1;
	...
	private void setDataProfile(int x, int y) {
		if (x >= 0 && x < dataProfile.length) {
			...
			// System.err.println(String.format("%d not in %d profile", x,
			// dataProfile.length));
		}
	}
	...
	private void strokeLine(Graphics2D g2, java.awt.Point p1, java.awt.Point p2, int thick1, int thick2, Color c1,
			Color c2) {
