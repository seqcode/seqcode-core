	...
	public ChipSeqFigurePaintable(FigureOptions opts){
		...
		for(String t : exptNames){
			...
			if(options.experiments.get(t).preFormattedDataFile != null){
				...
				DiffSeqExptPaintable dp = new DiffSeqExptPaintable(options.gconfig, options.gRegion, options.experiments.get(t).preFormattedDataFile, options.experiments.get(t).diffWinWidth, options.experiments.get(t).diffWinStep, options.experiments.get(t).scaling );
				...
				if(options.experiments.get(t).isDiff && options.experiments.get(t).baseExpt != null){
					...
					DiffSeqExptPaintable dp = new DiffSeqExptPaintable(options.gconfig, options.gRegion, sample, otherSample, options.experiments.get(t).diffWinWidth, options.experiments.get(t).diffWinStep, options.experiments.get(t).scaling );
					dp.setMaxLogFold(options.experiments.get(t).yMax);
					dp.setMinLogFold(options.experiments.get(t).yMax);
					dp.setReverse(options.reverseOrder);
					dp.setNegColor(options.diffExptNegColor);
					...
					ThinOverlapPaintable tp = new ThinOverlapPaintable(options.gRegion, sites, sample, options.readExt, options.experiments.get(t).pairedReads, options.experiments.get(t).inters);
