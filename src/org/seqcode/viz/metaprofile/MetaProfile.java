...
public class MetaProfile implements Profile, ProfileListener {
	...
	public synchronized void addProfile(Profile p) {
		...
		if (p.length() != params.getNumBins()) {
			throw new IllegalArgumentException(String.format("Profile length %d doesn't" + " match bin-length %d",
		...
		if (profiles.contains(p)) {
			 ...
			 * "Can't add same profile %s to MetaProfile", p.getName()));
			 */
		} else {
