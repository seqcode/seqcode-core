	...
	public void run() {
		try {
			if (mconfig.printHelp) {
				...
				System.out.println("Binding Parameters:\tWindow size: " + params.getWindowSize() + "\tBins: "
						+ params.getNumBins());
				...
				if (mconfig.profilerType.equals("simplechipseq")) {
					...
					profiler = new Stranded5PrimeProfiler(gconfig, params, manager, mconfig.strand,
							mconfig.fivePrimeShift, mconfig.baseLimit, mconfig.baseLimitRelPosition);
				...
				if (mconfig.batchRun) {
					...
					if (mconfig.peakFiles.size() == 1 || mconfig.peakFiles.size() == 0) {
						...
						MetaNonFrameMultiSet multinonframe = new MetaNonFrameMultiSet(mconfig.peakFiles, gen, params,
								profiler, true);
