...
/**
 * PointProfile associates a (String) "name" and a Point object with each array
 * of values.
 * 
 * @author mahony
 * @uthor: tdanford Date: Aug 12, 2008
 */
	...
	public PointProfile(Point p, BinningParameters ps, double[] vs, boolean str) {
		...
		if (values.length != params.getNumBins()) {
			throw new IllegalArgumentException(
					String.format("# Bins (%d) must match array length (%d)", params.getNumBins(), values.length));
		}
