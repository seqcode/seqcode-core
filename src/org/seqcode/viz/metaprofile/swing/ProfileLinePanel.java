	...
	protected void paintComponent(Graphics g) {
		...
		synchronized (this) {
			for (int i = 0; i < linePainters.size(); i++) {
				...
				linePainters.get(i).paintItem(g, 0, colorbarHeight + i * lineWeight, w,
		...
		if (addPercentages) {
			if (linePainters.size() > (numAxisTicks * 10)) {
				for (int i = 1; i <= numAxisTicks; i++) {
					...
					g.drawString(s, w - 5 - metrics.stringWidth(s),
							colorbarHeight + (i * ((linePainters.size() * lineWeight) / numAxisTicks)));
				}
			}
		}
