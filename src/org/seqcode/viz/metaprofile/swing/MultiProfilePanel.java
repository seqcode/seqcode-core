...
public class MultiProfilePanel extends JPanel implements PaintableChangedListener {
	...
	private Color[] peakColors = { Color.blue, Color.red, Color.gray, Color.green, Color.cyan, Color.orange,
			Color.magenta };
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (v == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						// System.out.println("Saved Image [" + sImageWidth + "
						// by " + sImageHeight + "]");
	...
	public void saveImage(File f, int w, int h) throws IOException {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int p = 0; p < profiles.size(); p++) {
			String counter = String.format("%s: %d datapoints", profiles.get(p).getName(),
					profiles.get(p).getNumProfiles());
		...
		if (isStranded) {
			...
			int[] a = new int[7];
			int[] b = new int[7];
			arrangeArrow(a, b, lineHeight, border + (binPix * (profileLength / 2)),
					border + (binPix * (profileLength / 2)) + 150, h - border);
			g2.drawPolyline(a, b, 7);
		} else {
			g2.fillRect(border + (binPix * (profileLength / 2)) - lineWidth / 2, h - lineHeight - (border / 2),
