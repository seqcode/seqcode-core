...
public class ProfilePanel extends JPanel implements PaintableChangedListener {
	...
	protected void paintComponent(Graphics g) {
		...
		if (profile.isStranded()) {
			...
			int[] a = new int[7];
			int[] b = new int[7];
			arrangeArrow(a, b, lineHeight, border + (binPix * (profile.length() / 2)),
					border + (binPix * (profile.length() / 2)) + 150, h - border);
			g2.drawPolyline(a, b, 7);
		} else {
			g2.fillRect(border + (binPix * (profile.length() / 2)) - lineWidth / 2, h - lineHeight - (border / 2),
