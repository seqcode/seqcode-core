...
/**
 * MetaFrame is a Swing component that provides a default interface to the
 * metaprofile system.
 * 
 * Once it's created, it provides UI elements that allow the user to load new
 * points into the given profile, and displays the results.
 * 
 * It also (through calls to the getHandler() method) provides a programmatic
 * hook for adding new Point objects automatically.
 * 
...
public class MetaFrame extends JFrame {
	...
	public MetaFrame(Genome g, BinningParameters bps, PointProfiler pp, boolean normalizedMeta) {
		...
		c.add(new JScrollPane(linePanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
	...
	public void startup() {
		// We want to call setVisible() and *then* pack() -- but once
		// setVisible() has
		// been called on a Swing component, you shouldn't call any other
		// methods of that
