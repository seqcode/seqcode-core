...
/**
 * Stranded5PrimeProfiler profiles the occurrence of sequencing reads around
 * points
 * 
 * @author: tdanford Date: Aug 19, 2008
 */
...
public class Stranded5PrimeProfiler implements PointProfiler<Point, PointProfile> {
	...
	private char strand;
	private char base = '.'; // Only plot tags that have this base at
								// baseRelPosition
	private int baseRelPosition = 0; // Only plot tags that have this base at
										// baseRelPosition
	private SequenceGenerator seqgen = null;
	private int fivePrimeShift = 0;

	public Stranded5PrimeProfiler(GenomeConfig genConfig, BinningParameters ps, ExperimentManager man, char strand,
			int fivePrimeShift, char base, int baseRelPosition) {
	...
	public PointProfile execute(Point a) {
		...
		int start = pointStrand == '+' ? Math.max(1, a.getLocation() - upstream)
				: Math.max(1, a.getLocation() - downstream);
		int end = pointStrand == '+'
				? Math.min(a.getLocation() + downstream, a.getGenome().getChromLength(a.getChrom()))
				: Math.min(a.getLocation() + upstream, a.getGenome().getChromLength(a.getChrom()));
		Region query = new Region(a.getGenome(), a.getChrom(), start, end);
		int ext = 200;
		Region extQuery = new Region(a.getGenome(), a.getChrom(), start - ext > 0 ? start - ext : 1,
				end + ext < a.getGenome().getChromLength(a.getChrom()) ? end + ext
						: a.getGenome().getChromLength(a.getChrom()));
		...
		for (ControlledExperiment expt : manager.getReplicates()) {
			...
			for (StrandedBaseCount sbc : sbcs) {
				if (base == '.' || (seq != null && getBaseAtPosition(sbc, baseRelPosition, extQuery, seq) == base)) {
					...
					if (this.strand == '.' || hit.getStrand() == wantedStrand) { // only
																					// count
																					// one
																					// strand
						if (start <= hit.getFivePrime() && end >= hit.getFivePrime()) {
							int hitPos = hit.getStrand() == '+' ? hit.getFivePrime() + fivePrimeShift
									: hit.getFivePrime() - fivePrimeShift;
	...
	private char getBaseAtPosition(StrandedBaseCount a, int position, Region queryReg, char[] seq) {
		char b = '.';
		int wantedPos = a.getStrand() == '+' ? a.getCoordinate() + position : a.getCoordinate() - position;
		if (wantedPos >= queryReg.getStart() && wantedPos < queryReg.getEnd()) {
			b = a.getStrand() == '+' ? seq[wantedPos - queryReg.getStart()]
					: SequenceUtils.complementChar(seq[wantedPos - queryReg.getStart()]);
		}
		return b;
	}
