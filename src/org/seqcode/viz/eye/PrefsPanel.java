...
public class PrefsPanel<M extends Model> extends JPanel {
	...
	public int numFields() {
		return stringFields.size() + booleanFields.size() + doubleFields.size() + integerFields.size()
				+ colorFields.size();
	}
	...
	private void init() {
		...
		for (Field f : ifs) {
			...
			JTextField component = new JTextField();
			if (fvalue != null) {
				component.setText(String.valueOf(fvalue));
			}
		...
		for (Field f : bfs) {
			...
			JRadioButton[] array = new JRadioButton[] { new JRadioButton("True"), new JRadioButton("False") };
			group.add(array[0]);
			group.add(array[1]);
