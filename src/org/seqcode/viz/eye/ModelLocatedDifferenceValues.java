...
public class ModelLocatedDifferenceValues extends AbstractModelPaintable {
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		// Basically, we don't want to draw the lines so thick that they overlap
		// too much.
		...
		for (DiffModel p : points) {
			...
			// System.out.println(String.format("=> %d, %.2f", location,
			// value));
			...
			/*
			 * g2.setColor(Color.white); g2.fillOval(px-radius, py-radius, diam,
			 * diam); g2.setColor(color); g2.drawOval(px-radius, py-radius,
			 * diam, diam);
			 */
