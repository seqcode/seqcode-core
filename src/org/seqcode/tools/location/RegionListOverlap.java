...
/**
 * Reads regions from two tab delimited files (you specify the columns, default
 * is first column) and determines how many from the first file are present in
 * the second given some mismatch window.
 *
 * java RegionListOverlap --species "$MM;mm9" --one fileone.txt --two
 * filetwo.txt --colone 0 --coltwo 3 --window 50 [--stats]
 *
 * --stats says to use --one as a test set and --two as a gold standard set to
 * report the TP and FP rates for one
 */
...
public class RegionListOverlap {
	...
	public static void main(String args[]) throws Exception {
		...
		if (stats) {
			...
			System.out.println(String.format("n=%d  tp=%d  fp=%d  fn=%d   tpr=%.2f   fnr=%.2f", onecount, tp, fp, fn,
					tprate, fnrate));
		}
	}
	...
	public static void readFile(Genome genome, String fname, int column, Map<String, List<Region>> map)
			throws IOException {
		if (map == null) {
