...
public class DumpGeneGFF {
	...
	public static void main(String args[]) throws Exception {
		...
		for (String id : genes.keySet()) {
			...
			System.out.println(String.format("%s\tprotein_coding\tgene\t%d\t%d\t.\t%s\t.\tID=%s", chrom, minpos, maxpos,
					Character.toString(strand), id));
				...
				if (g instanceof ExonicGene) {
					...
					while (iter.hasNext()) {
						...
						System.out.println(String.format(
								"%s\tprotein_coding\texon\t%d\t%d\t.\t%s\t.\tID=%s.%d;Parent=%s", chrom, e.getStart(),
								e.getEnd(), Character.toString(g.getStrand()), g.getID(), count, g.getID()));
						count++;
					}
				}
			}
