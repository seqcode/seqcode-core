 ...
 * 
 * cat gene_names.txt | java GenePromoters --species "$SC;sacCer3" --genes
 * sgdGene --upstream 500 --downstream 100 cat gene_names.txt | java
 * GenePromoters --species "$SC;sacCer3" --genes sgdGene --upstream 500
 * --downstream 100 --fasta java GenePromoters --species "$SC;sacCer3" --genes
 * sgdGene --upstream 500 --downstream 100 --fasta --allgenes
 *
 * [--fasta] fasta formatted output [--dontoverlaporfs] don't overlap annotated
 * genes or some other features like refBase and sgdOther [--phastmax] don't
 * include regions with a phastcons score greater than this value [--phastmin]
 * only include regions with a phastcons score greater than this value
 * [--maskkeep] provide "name;score" for a ScoredRegionGenerator and minimum
 * score to specify regions to keep [--maskout] provide "name;score" for a
 * ScoredRegionGenerator and maximum score to specify regions to keep [--minlen]
 * specify a minimum length for the output regions
 *
 *
 */
...
public class GenePromoters {
	...
	public void parseArgs(String args[]) throws NotFoundException {
		...
		if (dontOverlapOrfs) {
			Collection<Expander<Region, ? extends Region>> dontoverlap = new ArrayList<Expander<Region, ? extends Region>>();
			for (RefGeneGenerator g : geneGenerators) {
				dontoverlap.add(g);
			}
			try {
				RegionExpanderFactoryLoader<NamedTypedRegion> annotLoader = new RegionExpanderFactoryLoader<NamedTypedRegion>(
						"annots");
				RegionExpanderFactory factory = annotLoader.getFactory(genome, "repBase");
			...
			try {
				RegionExpanderFactoryLoader<NamedTypedRegion> annotLoader = new RegionExpanderFactoryLoader<NamedTypedRegion>(
						"annots");
				RegionExpanderFactory factory = annotLoader.getFactory(genome, "sgdOther");
				dontoverlap.add(factory.getExpander(genome));
		...
		if (maskOut.size() == 0) {
			...
			for (Expander<Region, ? extends ScoredRegion> exp : maskOut.keySet()) {
				...
				while (iter.hasNext()) {
					...
					if (sr.getScore() >= maskOut.get(exp)) {
						maskedout.add(sr);
					}
				}
			}
					...
					for (Region old : maskedout) {
						if (old.overlaps(sr)) {
							if (old.contains(sr)) {
								...
								newmo.add(new Region(old.getGenome(), old.getChrom(),
										Math.min(sr.getEnd(), old.getEnd()), Math.max(sr.getEnd(), old.getEnd())));
							}
						} else {
							newmo.add(old);
						}
	...
	public void run() throws IOException {
		if (allGenes) {
			...
			while (chroms.hasNext()) {
				...
				/*
				 * we'll use all the gene generators provided but don't want to
				 * output duplicate regions. seen is keyed on 5' and contains a
				 * list of 3' ends that have already been output.
				 */
	...
	public void output(NamedStrandedRegion r) {
		...
		if (toFasta) {
			...
			System.out.println(String.format("%s\t%s:%d-%d:%s", r.toString(), r.getChrom(), r.getStart(), r.getEnd(),
					r.getStrand()));
		}
	}
