...
public class RandomReadGenerator {
	...
	public static void main(String args[]) throws IOException {
		...
		/*
		 * in solexa model, error probability is base + multiplier * ((exp) ^
		 * length)
		 * 
		 * Based on the data in Dohm et al (2008), base = .005, multiplier =
		 * .0001, and exp = 1.2 their data only went to 32bp, so this may not be
		 * accurate for long reads
		 */
		...
		/*
		 * can specify either absolute number of reads per sequence in the FASTA
		 * file or an amount of coverage
		 */
				...
				if (errorP > 0) {
					...
					for (int j = 0; j < read.length; j++) {
						...
						if (uniform.nextDouble() < p) {
							// System.err.println("Mutating base " + j + " of "
							// + i + " from " + read[j]);
							char old = read[j];
							while (old == (read[j] = letters[uniform.nextIntFromTo(0, 3)])) {
							}
