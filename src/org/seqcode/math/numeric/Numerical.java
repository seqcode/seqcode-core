...
public abstract class Numerical {
	...
	public static class NumericalException extends RuntimeException {
		...
		public NumericalException(String m) {
			super(m);
		}
	}
	...
	public static double log_sum_exp(DoubleMatrix1D exponents) {
		...
		if (maxExp == Double.NEGATIVE_INFINITY) {
			...
			double expSum = exponents.aggregate(Functions.plus,
					Functions.chain(Functions.exp, Functions.minus(maxExp)));
	...
	private static double gammln_cof[] = { 76.18009172947146, -86.50532032941677, 24.01409824083091, -1.231739572450155,
	...
	public static double erfcc(double x) {
		...
		z = Math.abs(x);
		t = 1.0 / (1.0 + 0.5 * z);
		ans = t * Math.exp(-z * z - 1.26551223 + t * (1.00002368 + t * (0.37409196 + t * (0.09678418 + t * (-0.18628806
	...
	public static class SpearmanResult {
		...
		public SpearmanResult(double _d, double _zd, double _probd, double _rs, double _probrs) {
			d = _d;
			zd = _zd;
			probd = _probd;
			rs = _rs;
	...
	/**
	 * Computes the log-space PDF of the Poisson distribution with the specified
	 * mean. Code lifted from the COLT Poisson class so that it can be
	 * implemented as a static method instead of having to instantiate a COLT
	 * Poisson object
	 * 
	 * @param mean
	 * @param k
	...
	/**
	 * LambertW function default value for <tt>maxIters = 100</tt>
	 * 
	 * @see org.seqcode.math.numeric.Numerical#lambertW(int, double, double,
	 *      int)
	 */
	...
	/**
	 * LambertW function default value for <tt>thres = 1e-12</tt>
	 * 
	 * @see org.seqcode.math.numeric.Numerical#lambertW(int, double, double,
	 *      int)
	 */
	...
	/**
	 * LambertW function default values for
	 * <tt>thres = 1e-12, maxIters = 100</tt>
	 * 
	 * @see org.seqcode.math.numeric.Numerical#lambertW(int, double, double,
	 *      int)
	 */
	 ...
	 * for only the W_{0} and W_{-1} branches. This function is known as the
	 ...
	 * Prior and Parameter Extinction </a> <br>
	 * <a href="http://www.cs.uwaterloo.ca/research/tr/1993/03/W.pdf"> On the
	 * Lambert W Function </a> <br>
	 * <a href="http://en.wikipedia.org/wiki/Lambert_function"> Lambert W
	 * function </a>
	 * 
	 * @param branch_index
	 *            Valid values 0 or -1. <br>
	 *            0 for principal (W_{0}), -1 for W_{-1} branch
	 * @param z
	 *            The value whose we wish to determine the W s.t. W * exp(W) = z
	 *            <br>
	 *            Valid values for z: z > -exp(-1)
	 * @param thres
	 *            precision threshold
	 * @param maxIters
	 *            maximum number of iterations for searching for W
	 * @return
	 */
	public static double lambertW(int branch_index, double z, double thres, int maxIters) {
		...
		while (numIters < maxIters) {
			...
			if (Math.abs(-delta / (Math.exp(W) * (W + 1))) < thres) {
				break;
			}
	...
	private static double initW(int branch_index, double z) {
		...
		if (z >= 0) {
			...
			if (branch_index == 0) {
				if (z <= 500.0) {
					W_init = 0.665 * (1 + 0.0195 * Math.log(z + 1.0)) * Math.log(z + 1.0) + 0.04;
				} else {
					W_init = Math.log(z - 4.0) - (1.0 - 1.0 / Math.log(z)) * Math.log(Math.log(z));
				}
			}
		...
		// Negative z
			...
			if (Math.abs(z + Math.exp(-1)) > 0.01) {
				...
				// branch W_{-1}
					...
					W_init = Math.log(-z) - Math.log(-Math.log(-z));
				}
			}
			...
			// z too near -Math.exp(-1)
				...
				if (z == -Math.exp(-1)) {
					...
					// branch W_{0}
					if (branch_index == 0) {
						W_init = -1 + Math.sqrt(2 * (Math.exp(1) * z + 1));
					}
