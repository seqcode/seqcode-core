...
/**
 * TMMNormalization: a Normalization class that implements the "trimmed mean of
 * M values" (TMM) normalization as proposed by Robinson & Oshlack, Genome
 * Biology, 2010
 * 
 * @author Shaun Mahony
 * @version %I%, %G%
...
public class TMMNormalization extends Normalization {
	...
	/**
	 * Constructor: provide the number of samples, the fraction of extreme M
	 * values to trim, and the fraction of lowest A values to trim
	 * 
	 * @param numSamples
	 * @param Mtrim
	 * @param Atrim
	 */
	public TMMNormalization(int numSamples) {
		this(numSamples, 0.3, 0.05);
	}
			...
			if (x == ref) {
				...
				for (int d = 0; d < data.numUnits; d++)
					if (data.getCount(d, x) > 0 && data.getCount(d, ref) > 0)
						MA.add(new MAval(data.getCount(d, x), data.getCount(d, ref), data.getTotal(x),
								data.getTotal(ref)));
				...
				// System.err.println("logTMM: "+logTMM+"\tTMM:
				// "+TMM+"\tTotalRatio:"+(data.totals[x] /
				// data.totals[ref])+"\tScaling:"+scalingFactors[x]);
			}
		}
	...
	public void savePairwiseMAPlots(CountsDataset data, boolean rasterImage) {
		...
		for (int x = 0; x < data.numSamples; x++) {
			if (x != ref) {
				...
				String fileName = refName.car() + "-" + refName.cdr() + "_vs_" + currName.car() + "-" + currName.cdr()
						+ ".ma";
				if (rasterImage)
				...
				plotter.saveMAplot(maMatrixOther, maMatrixHighlight,
						Math.log(propScaling[x] / propScaling[ref]) / LOG_2, fileName, rasterImage);
			}
		}
	}

	/**
	 * trimMA: trim the MA values list according to the Mtrim and Atrim
	 * fractions
	 * 
	 * @param input
	 * @return
