...
import org.seqcode.viz.scatter.ScatterPlotMaker;

import Jama.Matrix;

/**
 * CountsDataset is a class used to hold a matrix of counts associated with a
 * set of experiments. The class is used as the basis for pairwise statistical
 * testing. As such, the class also maintains scaling factors (for each pair of
 * experiments), and p-value matrices (for each pair of conditions).
 * 
 * @author Shaun Mahony
 * @version %I%, %G%
 */
public class CountsDataset {
	...
	protected Matrix DEpval; // P-values of differential expression. Indexed by
								// unit and condition
	protected Matrix condFold;// Condition Fold (logFold from EdgeR). Indexed by
								// unit and condition
	protected Matrix condMean;// Condition means (logCPM from EdgeR). Indexed by
								// unit and condition
	protected Matrix condRawVar; // Condition variances. Indexed by unit and
									// condition
	 ...
	 *            : count matrix
	 * @param units
	 *            : Names of each unit (i.e. gene name or peak coordinate)
	 * @param design
	 *            : design array (which index in counts corresponds to which
	 *            condition, indexed by integer)
	 * @param exptNameMap:
	 *            translation between sample index and condition/replicate names
	 * @param condNameMap:
	 *            translation between condition index and name
	 * @param focalCond:
	 *            focal condition (sample index of)
	 */
	public CountsDataset(Matrix counts, String[] units, int[] design,
			HashMap<Integer, Pair<String, String>> exptNameMap, HashMap<Integer, String> condNameMap) {
		this(counts, units, design, exptNameMap, condNameMap, 0);
	}

	public CountsDataset(Matrix counts, String[] units, int[] design,
			HashMap<Integer, Pair<String, String>> exptNameMap, HashMap<Integer, String> condNameMap, int focalCond) {
		this.counts = counts;
	...
	/**
	 * Calculcate the scaled mean and fold for all points in reference to the
	 * focal condition
	 */
		...
		for (int i = 0; i < numUnits; i++) {
			...
			for (int c = 0; c < numConds; c++) {
				if (c != focalCondition) {
					...
					double fold = focalScMean > 0 ? (otherScMean > 0 ? (otherScMean / focalScMean) : 1 / focalScMean)
							: otherScMean;
	...
	/**
	 * Update the provided events with differential expression values in the
	 * current dataset.
	 * 
	 * @param events
	 * @return
	...
	public List<BindingEvent> updateEvents(List<BindingEvent> events, ExperimentManager manager) {
		...
		for (int e = 0; e < events.size(); e++) {
			...
			for (ExperimentCondition c : manager.getConditions()) {
				if (c != ref) {
					// Only update the p-value since this should only be called
					// after EdgeR as EdgeR's fold and mean are weird.
	 ...
	 * 
	 * @param u
	 *            Unit index (i.e. gene or peak)
	 * @param s
	 *            Sample index
	 * @return
	 */
	public double getCount(int u, int s) {
		return counts.get(u, s);
	}
	 ...
	 * @return
	 */
	public String getUnitName(int u) {
		return unitNames[u];
	}
	 ...
	 * @return
	 */
	public Pair<String, String> getExptName(int i) {
		return sampleToExptName.get(i);
	}
	 ...
	 * @return
	 */
	public String getCondName(int i) {
		return condToName.get(i);
	}
	...
	public void setScalingFactors(double[] s) {
		if (s.length == scaling.length)
			...
			System.err.println("Error: scaling array has incompatible length.");
			System.exit(1);
		}
	}
	 ...
	 * @return
	 */
	public double getScalingFactor(int s) {
		return scaling[s];
	}
	 ...
	 * @param x
	 */
	public void setFocalCondition(int x) {
		focalCondition = x;
	}
	 ...
	 * @return
	 */
	public int getFocalCondition() {
		return focalCondition;
	}
	 ...
	 * @return
	 */
	public int getSampleCondition(int s) {
		return design[s];
	}
	 ...
	 * @return
	 */
	public double getDEpval(int u, int c) {
		return DEpval.get(u, c);
	}
	 ...
	 * @param p
	 */
	public void setDEpvals(Matrix p) {
		DEpval = p;
	}
	 ...
	 * @param p
	 */
	public void setDEpval(int u, int c, double p) {
		DEpval.set(u, c, p);
	}
	 ...
	 * @return
	 */
	public double getCondMean(int u, int c) {
		return condMean.get(u, c);
	}
	 ...
	 * @return
	 */
	public double getCondFold(int u, int c) {
		return condFold.get(u, c);
	}
	 ...
	 * @param c
	 */
	public void setCondMeans(Matrix c) {
		condMean = c;
	}
	 ...
	 * @param p
	 */
	public void setCondMean(int u, int c, double p) {
		condMean.set(u, c, p);
	}
	 ...
	 * @param c
	 */
	public void setCondFolds(Matrix c) {
		condFold = c;
	}
	 ...
	 * @param p
	 */
	public void setCondFold(int u, int c, double f) {
		condFold.set(u, c, f);
	}
	 ...
	 * @return
	 */
	public double getCondRawVar(int u, int c) {
		return condRawVar.get(u, c);
	}
	 ...
	 * @param c
	 */
	public void setCondRawVars(Matrix c) {
		condRawVar = c;
	}
	 ...
	 * @param p
	 */
	public void setCondRawVar(int u, int c, double p) {
		condRawVar.set(u, c, p);
	}

	/**
	 * Return the scaling factors If tableFormat is true, print on a single line
	 * 
	 * @param tableFormat
	 * @return
	 */
	...
	/**
	 * Return the experiment names (condition:replicate) If tableFormat is true,
	...
	public void savePairwiseFocalSampleMAPlots(String directory, boolean rasterImage) {
		double A_min = 1;
		int ref = 0;
		// Set one sample as the reference (the deepest sequenced sample in the
		// focal condition)
		double maxTotal = 0;
		for (int s = 0; s < numSamples; s++)
			if (design[s] == focalCondition)
		...
		for (int x = 0; x < numSamples; x++) {
			if (design[x] == focalCondition && x != ref) {
				...
				String fileName = currName.car() + "-" + currName.cdr() + "_vs_" + refName.car() + "-" + refName.cdr()
						+ ".MA";
				if (rasterImage)
				...
				// Generate image
				ScatterPlotMaker plotter = new ScatterPlotMaker(currName.car() + ":" + currName.cdr() + " vs "
						+ refName.car() + ":" + refName.cdr() + " MA plot");
	...
	public void savePairwiseFocalSampleXYPlots(String directory, boolean rasterImage) {
		...
		for (int x = 0; x < numSamples; x++) {
			if (design[x] == focalCondition && x != ref) {
				...
				// Generate image
				ScatterPlotMaker plotter = new ScatterPlotMaker(currName.car() + ":" + currName.cdr() + " vs "
						+ refName.car() + ":" + refName.cdr() + " XY plot");
				plotter.saveXYplot(maMatrixOther, maMatrixHighlight, currName.car() + "-" + currName.cdr(),
						refName.car() + "-" + refName.cdr(), fileName, rasterImage);
			}
		}
	}
	 ...
	 * @param rasterImage
	 */
	public void savePairwiseConditionXYPlots(ExperimentManager man, BindingManager bindingMan, double pValThreshold,
			String directory, boolean rasterImage) {
