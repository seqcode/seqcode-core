...
import org.seqcode.math.stats.Lowess;
import org.seqcode.math.stats.NegativeBinomialDistrib;

import Jama.Matrix;

/**
 * DESeqDifferentialEnrichment: implements the differential enrichment method
...
public class DESeqDifferentialEnrichment extends DifferentialEnrichment {
	...
	protected Normalization normalizer; // DESeq requires a separate
	...
	// DESeq parameters
	protected double[][] q; // Strength parameters (i.e. estimates on condition
							// means). Indexed by unit & condition
	protected double[][] w; // Sample variances. Indexed by unit & condition
	protected double[][] z; // Variance correction parameters. Indexed by unit &
							// condition
	protected double[][] v; // Variance estimates. Indexed by unit & condition
	protected double[] m; // Number of replicates for each condition. Indexed by
							// condition
	...
	public CountsDataset execute(CountsDataset data) {
		...
		for (int u = 0; u < numUnits; u++) {
			for (int c = 0; c < numConds; c++) {
				if (m[c] > 1) {
					...
					for (int samp = 0; samp < numSamples; samp++) {
						if (design[samp] == c) {
							currW += ((k[u][samp] / scaling[samp]) - q[u][c])
									* ((k[u][samp] / scaling[samp]) - q[u][c]);
		...
		// If the reference condition has replicates, and other conditions
		// don't, use the reference function for those.
		if (m[ref] > 1) {
			for (int c = 0; c < numConds; c++) {
				if (m[c] == 1) {
					lowess[c] = lowess[ref];
		...
		// If reference condition has only a single replicate, pool all counts
		// and build a single shared reference function.
		if (m[ref] == 1) {
			ArrayList<QWItem> vals = new ArrayList<QWItem>();
			for (int c = 0; c < numConds; c++)
				for (int u = 0; u < numUnits; u++)
		...
		double[] Kx = new double[numUnits]; // Total counts for conditions REF
											// and X
		double[] Qzero = new double[numUnits]; // Pooled mean (under null
												// hypothesis)
		double[] MUref = new double[numUnits];
		double[] MUx = new double[numUnits]; // Mean for conditions REF and X
		double[] SIGMAref;
		double[] SIGMAx; // Variance for conditions REF and X
		double[] rawSCVref = new double[numUnits];
		double[] rawSCVx = new double[numUnits]; // raw SCV
		double[] Pref = new double[numUnits];
		double[] Px = new double[numUnits];
		double[] Rref = new double[numUnits];
		double[] Rx = new double[numUnits];
		...
		for (int x = 0; x < numConds; x++) {
			if (x != ref) {
				for (int u = 0; u < numUnits; u++) {
					...
					Qzero[u] = 0;
					for (int samp = 0; samp < numSamples; samp++) {
						if (design[samp] == x || design[samp] == ref) {
							Qzero[u] += k[u][samp] / scaling[samp];
						}
					}
					Qzero[u] /= (m[ref] + m[x]);
				...
				for (int u = 0; u < numUnits; u++) {
					...
					fullVarRef[u] = Math.max(
							MUref[u] + (rawSCVref[u] * Qzero[u] * Qzero[u] * (scaling[ref] * scaling[ref] * m[ref])),
							MUref[u] * (1 + 1e-8));
					fullVarX[u] = Math.max(
							MUx[u] + (rawSCVx[u] * Qzero[u] * Qzero[u] * (scaling[x] * scaling[x] * m[x])),
							MUx[u] * (1 + 1e-8));
					...
					if (Pref[u] > 0 && Px[u] > 0 && Rref[u] > 0 && Rx[u] > 0) {
						...
						if (pObs.isInfinite() || pObs.isNaN()) {
							...
							Pair<Double, Double> totalAbove = addFromMiddle((int) (Kref[u] + Kx[u]), pObs, MUref[u],
									fullVarRef[u], MUx[u], fullVarX[u], true, 1e-4);
							Pair<Double, Double> totalBelow = addFromMiddle((int) (Kref[u] + Kx[u]), pObs, MUref[u],
									fullVarRef[u], MUx[u], fullVarX[u], false, 1e-4);
	...
	/**
	 * Method used to evaluate the upper or lower half of the p(a,b)
	 * distribution. The P value of a pair of observed count sums (kA, kB) is
	 * the sum of all probabilities less or equal to p(kA, kB), given that the
	 * overall sum is kS.
	 * 
	 ...
	 * @return
	 */
	private Pair<Double, Double> addFromMiddle(int kS, double pobs, double muA, double vA, double muB, double vB,
			boolean upwards, double eps) {
		int k = (int) ((double) kS * muA / (muA + muB));
		if (!upwards)
			k++;
		double total = 0;
		double esttotal = NegativeBinomialDistrib.pdf(kS, (muA + muB) * (muA + muB) / (vA + vB - muA - muB),
				(muA + muB) / (vA + vB));
		double obstotal = 0;
		int step = 1;
		double val = 0;
		...
		while (upwards ? (k < kS) : (k > 0)) {
			...
			while (true) {
				...
				val = NegativeBinomialDistrib.pdf(knew, sizeA, probA)
						* NegativeBinomialDistrib.pdf(kS - knew, sizeB, probB);
	...
	/**
	 * QWItem: separate class is necessary because the Lowess class requires a
	 * sorted list of points, and we need a way to map back to the original data
	 * structure.
	 * 
	 * @author Shaun Mahony
	 * @version %I%, %G%
