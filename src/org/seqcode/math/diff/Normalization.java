...
/**
 * Normalization: parent class for normalization methods that calculate scaling
 * factors.
 * 
 * @author Shaun Mahony
 * @version %I%, %G%
...
public abstract class Normalization {
	 ...
	 * @return
	 */
	public double getScalingFactor(int x) {
		return scalingFactors[x];
	}
	...
	public void savePairwiseMAPlots(CountsDataset data, boolean rasterImage) {
		...
		for (int x = 0; x < data.numSamples; x++) {
			if (x != ref) {
				...
				plotter.saveMAplot(maMatrix, null, Math.log(propScaling[x] / propScaling[ref]) / LOG_2, fileName,
						rasterImage);
			}
		}
	}
