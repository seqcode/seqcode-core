...
public class Sampler {
	...
	public static double[][] gauss_rnd(int[] data_size, double[] mu) {
		return gauss_rnd(data_size, mu, new double[data_size.length]);
	}

	public static double[][] gauss_rnd(int[] data_size, double mu) {
		return gauss_rnd(data_size, mu);
	}

	public static double[][] gauss_rnd(int num_rows, int num_cols, double mu) {
		return gauss_rnd(num_rows, num_cols, mu, 1.0);
	}

	public static double[] gauss_rnd(int num_cols, double mu) {
		return gauss_rnd(num_cols, mu, 1.0);
	}

	/**
	 * It generates gaussian samples for <tt>data_size.length</tt> data sets.
	 * <br>
	 * Each data set has data size <tt>data_size[t]</tt> with mean
	 * <tt>mu[t]</tt> and standard deviation <tt>std[t]</tt>. (where t in {0,
	 * ..., data_size.length-1}
	 * 
	 * @param data_size
	...
	/**
	 * It generates gaussian samples of mean <tt>mu</tt> and standard deviation
	 * <tt>std</tt> for <tt>data_size.length</tt> data sets, each having
	 * data_size[t] points. (where t in {0, ..., data_size.length-1}
	 * 
	 * @param data_size
	 *            the size of each data set
	 * @param mu
	...
	public static double gauss_rnd(double mu) {
		return gauss_rnd(mu, 1.0);
	}

	/**
	 * It generates a gaussian sample of mean <tt>mu</tt> and standard deviation
	 * <tt>std</tt>.
	 * 
	 * @param mu
	 *            the mean of the gaussian
	...
	/**
	 * Gives <tt>num_rows</tt> data sets of continuous uniform samples, each
	 * having lower bound <tt>lower_bound[t]</tt> (inclusive) and upper bound
	 * <tt>upper_bound</tt> (exclusive).
	 * 
	 * @param num_rows
	 * @param num_cols
	 * @param lower_bound
	...
	/**
	 * Gives a continuous uniform sample in the interval between
	 * <tt>lower_bound</tt> (inclusive) and <tt>upper_bound</tt> (exclusive).
	 * 
	 * @param lower_bound
	 * @param upper_bound
	 * @return
	...
	/**
	 * Gives <tt>num_rows</tt> data sets of continuous uniform samples, each
	 * having lower bound <tt>lower_bound[t]</tt> and upper bound
	 * <tt>upper_bound</tt>.
	 * 
	 * @param num_rows
	 * @param num_cols
	...
	/**
	 * Gives a discrete uniform sample in the interval between
	 * <tt>lower_bound</tt> (inclusive) and <tt>upper_bound</tt> (exclusive).
	 * 
	 * @param lower_bound
	 * @param upper_bound
	...
	/**
	 * This method creates a sample from 0 to N-1 following a Binomial
	 * distribution, Bin(N, p), using the algorithm by Devroye taken from: <br>
	 * <a href=
	 * "http://delivery.acm.org/10.1145/50000/42381/p216-kachitvichyanukul.pdf?key1=42381&key2=4588540521&coll=GUIDE&dl=GUIDE&CFID=48844090&CFTOKEN=30501264">
	 * Binomial Random Variate Generation</a> <br>
	...
	/**
	 * Creates <tt>data_size.length</tt> data sets, each of size
	 * <tt>data_size[t]</tt> following a multinomial distribution
	 * <tt>prob[t]</tt>.
	 */
	 ...
	 * 
	 * @param prob
	 *            multinomial probability. Has to sum up to 1.
	 * @return
	 */
	...
	/**
	 * Creates samples from a Markov Chain for <tt>data_size.length</tt> data
	 * sets, each having <tt>data_size[t]</tt> samples and having a
	 * <tt>init_prob[t]</tt> initial probability and a transition matrix
	 * <tt>trans_mat[t]</tt>.
	 */
	...
	/**
	 * Creates samples from a Markov Chain for <tt>num_rows</tt> data sets, each
	 * having <tt>num_cols</tt> samples and having a <tt>init_prob</tt> initial
	 * probability and a transition matrix <tt>trans_mat</tt>.
	 * 
	 * @param num_rows
	 * @param num_cols
	 * @param init_prob
	...
	public static int[][] mc_rnd(int[] data_size, double[] init_prob, double[][] trans_mat_homog_part,
			double[][] trans_mat_inhomog_part) {
		int[][] samples = new int[data_size.length][];
		for (int t = 0; t < samples.length; t++)
	...
	public static int[][] mc_rnd(int num_rows, int num_cols, double[] init_prob, double[][] trans_mat_homog_part,
			double[][] trans_mat_inhomog_part) {
		int[][] samples = new int[num_rows][];
		for (int t = 0; t < samples.length; t++)
	 ...
	 *            the constant (homogeneous) part of the transition matrix
	 * @param trans_mat_inhomog_part
	 *            the non-constant (inhomogeneous) part of the transition matrix
	 *            (last row)
	 * @return
	 */
	public static int[] mc_rnd(int num_cols, double[] init_prob, double[][] trans_mat_homog_part,
			double[][] trans_mat_inhomog_part) {
		int[] samples = new int[num_cols];
	...
	public static int[][] mc_rnd(int[] data_size, int[][] close_read_inds, double[] init_prob,
			double[][] trans_mat1_homog_part, double[][] trans_mat1_inhomog_part, double[][] trans_mat2) {
		int[][] samples = new int[data_size.length][];
		for (int t = 0; t < data_size.length; t++)
			samples[t] = mc_rnd(data_size[t], close_read_inds[t], init_prob, trans_mat1_homog_part,
					trans_mat1_inhomog_part, trans_mat2);

		return samples;
	}// end of mc_rnd method

	public static int[][] mc_rnd(int num_rows, int num_cols, int[][] close_read_inds, double[] init_prob,
			double[][] trans_mat1_homog_part, double[][] trans_mat1_inhomog_part, double[][] trans_mat2) {
		int[][] samples = new int[num_rows][];
		for (int t = 0; t < num_rows; t++)
			samples[t] = mc_rnd(num_cols, close_read_inds[t], init_prob, trans_mat1_homog_part, trans_mat1_inhomog_part,
					trans_mat2);
	 ...
	 * @param num_cols
	 *            N
	 * @param close_read_inds
	 *            1xK (K<N) indices for reads that are within distance
	 *            <tt>d</tt> from their previous read
	 * @param init_prob
	 *            1xM
	 * @param trans_mat1_homog_part
	 *            (M-1)xM Homogeneous transition matrix part when adjacent reads
	 *            are very close together (<=d)
	 * @param trans_mat1_inhomog_part
	 *            KxM Inomogeneous transition matrix part when adjacent reads
	 *            are very close together (<=d)
	 * @param trans_mat2
	 *            Homogeneous transition matrix when adjacent reads are far away
	 *            (>d)
	 * @return
	 */
	public static int[] mc_rnd(int num_cols, int[] close_read_inds, double[] init_prob,
			double[][] trans_mat1_homog_part, double[][] trans_mat1_inhomog_part, double[][] trans_mat2) {
		int[] samples = new int[num_cols];
		samples[0] = multinomial_rnd(init_prob);
		...
		for (int i = 0; i < close_read_inds.length; i++) {
			...
			if (n < num_cols) {
				// When n stops being less than close_read_inds[i], it becomes
				// certainly
	...
	/*
	 * Creates <tt>num_cols<tt> samples of an inhomogeneous Markov Chain, with
	 * an initial probability <tt>init_prob</tt> and a transition matrix
	 * <tt>trans_mat</tt>.
	 */
	public static int[] mc_rnd(int num_cols, double[] init_prob, double[][][] trans_mat) {
		...
		samples[0] = multinomial_rnd(init_prob);
		for (int n = 1; n < samples.length; n++) {
			samples[n] = multinomial_rnd(trans_mat[n][samples[n - 1]]);
		}
	...
	/*
	 * Creates <tt>num_cols<tt> samples of a Markov Chain, with an initial
	 * probability <tt>init_prob</tt> and a transition matrix
	 * <tt>trans_mat</tt>.
	 */
	public static int[] mc_rnd(int num_cols, double[] init_prob, double[][] trans_mat) {
		int[] samples = new int[num_cols];
	...
	public static void main(String[] args) {
		...
		/*
		 * double[] prob = {.2, .5, .3}; int[] samples =
		 * Sampler.multinomial_sample(prob, 1000); for(int i = 0; i <
		 * samples.length; i++) System.out.print(samples[i]);
		 * 
		 * double count0 = 0; double count1 = 0; double count2 = 0; for(int i =
		 * 0; i < samples.length; i++) { if(samples[i] == 0) {count0++;} else
		 * if(samples[i] == 1) {count1++;} else {count2++;} }
		 * System.out.println();
		...
		/*
		 * double[] init_prob = {.8, .2}; double[][] trans_mat = {{.6, .4}, {.3,
		 * .7}};
		 * 
		 * int num_rows = 1000; int num_cols = 1000; int[][] samples =
		 * Sampler.mc_sample(num_rows, num_cols, init_prob, trans_mat); double
		 * count_init_0 = 0; double count00 = 0; double count01 = 0; double
		 * count10=0; double count11 = 0; for(int t = 0; t < num_rows; t++) {
		 * if(samples[t][0] == 0) { count_init_0++; } for(int n = 1; n <
		 * num_cols; n++) { if(samples[t][n-1] == 0) { if(samples[t][n] == 0)
		 * count00++; else count01++; } else { if(samples[t][n] == 0) count10++;
		 * else count11++; } } }
		 * 
		 * double sum0 = count00+count01; double sum1 = count10+count11;
		 * 
		 * System.out.printf("init_0: %.2f, init_1: %.2f\n\n",
		 * count_init_0/num_rows, (1-count_init_0/num_rows));
		 * 
		 * System.out.printf("a0->0: %.2f, a0->1: %.2f\n" +
		 * "a1->0: %.2f, a1->1: %.2f\n", count00/sum0, count01/sum0,
		 * count10/sum1, count11/sum1);
		 */

		/*
		 * double mu = 10.0; double std = 4.0; double[] samples =
		 * Sampler.gauss_rnd(20, mu, std);
		 */

		/*
		 * double[] mu = {5, 50, 1000}; double[] std = {1, 10, 200}; int[]
		 * num_samples = {5, 10, 50};
		 * 
		 * //double[][] samples = Sampler.gauss_rnd(num_samples, mu, std);
		 * double[][] prob = { {.1, .9}, {.7, .3}, {.5, .5} }; // int[][]
		 * sampler = Sampler.multinomial_rnd(num_samples, prob);
		 * 
		 * 
		 * double[][] init_prob = { {.2, .8}, {.5, .5}, {.35, .65} };
		 * double[][][] trans_mat = { {{.8, .2}, {.6, .4}} , {{.6, .4}, {.5,
		 * .5}} , {{.1, .9}, {.2, .8}} };
		 * 
		 * 
		...
		double[][] trans_mat1_homog_part = { { .4, .3, .3 }, { .2, .2, .6 } };

		double[][] trans_mat1_inhomog_part = { { .3, .6, .1 }, { .8, .1, .1 }, { .2, .1, .8 } };

		double[][] trans_mat2 = { { .1, .7, .2 }, { .1, .6, .3 }, { .7, .2, .1 } };

		int[] samples1 = mc_rnd(num_cols, close_read_inds, init_prob, trans_mat1_homog_part, trans_mat1_inhomog_part,
				trans_mat2);
