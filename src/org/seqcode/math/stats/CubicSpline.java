...
public class CubicSpline {
	...
	// if any poit lies outside the interpolation bounds
			...
			while (test2) {
				if (this.x[ii] == this.x[jj]) {
					if (this.y[ii] == this.y[jj]) {
						System.out.print(subMatrixIndices + "CubicSpline: Two identical points, " + this.x[ii] + ", "
								+ this.y[ii]);
	...
	public double interpolate(double xx) {
		...
		if (xx < this.x[0]) {
			...
			if (CubicSpline.roundingCheck
				...
				throw new IllegalArgumentException("x (" + xx + ") is outside the range of data points (" + x[0]
						+ " to " + x[this.nPoints - 1] + ")");
			}
		}
		if (xx > this.x[this.nPoints - 1]) {
			if (CubicSpline.roundingCheck && Math.abs(xx - this.x[this.nPoints - 1]) <= Math.pow(10,
					Math.floor(Math.log10(Math.abs(this.x[this.nPoints - 1])))) * CubicSpline.potentialRoundingError) {
