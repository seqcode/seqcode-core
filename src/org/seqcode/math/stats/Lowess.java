...
/**
 * Lowess: This implementation of Lowess interpolation is taken from MIDAS
 * (v2.22). http://www.tm4.org/midas.html
 * 
 * References: Quackenbush, J. Microarray data normalization and transformation.
 * Nature Genetics. Vol.32 supplement pp496-501 (2002). Yang, I.V. et al. Within
 * the fold: assessing differential expression measures and reproducibility in
 * microarray assays. Genome Biol. 3, research0062.1-0062.12 (2002). Cleveland,
 * W.S. Robust locally weighted regression and smoothing scatterplots. J. Amer.
 * Stat. Assoc. 74, 829-836 (1979).
 * 
 * Copyright @ 2001-2003, The Institute for Genomic Research (TIGR). All rights
 * reserved.
 *
 * Lowess.java
 *
...
public class Lowess {
	...
	/**
	 * Constructor: Note that xInput,yInput pairs should be sorted along the
	 * x-axis (increasing).
	 * 
	 * @param xInput
	 * @param yInput
	 * @param bandwidthPct
	 *            : Reasonable values are 0.25 to 0.5
	 */
	...
	private void getEstWindowSubArrayAndWeights(int queryPointIndex, int window) {
		...
		// if ((queryPointIndex >= windowSize) && (queryPointIndex <= fullLength
		// - windowSize)){//middle region data
		if ((queryPointIndex >= windowSize) && (queryPointIndex < fullLength - windowSize)) {// middle
																								// region
																								// data
			for (int i = 0; i < windowSize; i++) {
				if ((xArray[queryPointIndex]
						- (xArray[queryPointIndex - windowSize + 1 + i])) <= (xArray[queryPointIndex + 1 + i]
								- xArray[queryPointIndex])) {
					subStartIndex = queryPointIndex - windowSize + 1 + i;
					subQueryPointIndex = queryPointIndex - subStartIndex;
					maxDistance = (((xArray[queryPointIndex]
							- xArray[subStartIndex]) > (xArray[subStartIndex + windowSize - 1])
	...
	private void getEstWindowSubArrayAndWeightsEV(double queryX, int window) {
		...
		int closestXPoint = 0;
		double closestXDist = Double.MAX_VALUE;
		...
		if ((closestXPoint >= windowSize) && (closestXPoint < fullLength - windowSize)) {// middle
																							// region
																							// data
			for (int i = 0; i < windowSize; i++) {
				if ((xArray[closestXPoint]
						- (xArray[closestXPoint - windowSize + 1 + i])) <= (xArray[closestXPoint + 1 + i]
								- xArray[closestXPoint])) {
					subStartIndex = closestXPoint - windowSize + 1 + i;
					subQueryPointIndex = closestXPoint - subStartIndex;
					maxDistance = (((queryX - xArray[subStartIndex]) > (xArray[subStartIndex + windowSize - 1])
							- queryX) ? (queryX - xArray[subStartIndex])
									: (xArray[subStartIndex + windowSize - 1] - queryX));
					break;
				}
			}
			...
			for (int i = 0; i < windowSize; i++) {
				if ((xArray[closestXPoint] - xArray[i]) <= (xArray[windowSize + i] - xArray[closestXPoint])) {
					subStartIndex = i;
					subQueryPointIndex = closestXPoint - subStartIndex;
					maxDistance = ((queryX - xArray[subStartIndex]) > (xArray[subStartIndex + windowSize - 1] - queryX))
							? (queryX - xArray[subStartIndex]) : (xArray[subStartIndex + windowSize - 1] - queryX);
					break;
				}
			}
		} else {// right margin data
			for (int i = 0; i < windowSize; i++) {
				if ((xArray[fullLength - 1 - i] - xArray[closestXPoint]) <= (xArray[closestXPoint]
						- xArray[fullLength - windowSize - 1 - i])) {
					subStartIndex = fullLength - windowSize - i;
					subQueryPointIndex = closestXPoint - subStartIndex;
					maxDistance = ((queryX - xArray[subStartIndex]) > (xArray[subStartIndex + windowSize - 1] - queryX))
