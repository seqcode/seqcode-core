...
public class WeightMatrix {
	...
	/**
	 ** the input matrix must follow the definition of WeightMatrix (see comments
	 * at the beginning of the java class)
	 **/
	...
	public static Collection<WeightMatrix> getAllWeightMatrices() {
		try {
			...
			PreparedStatement ps = cxn.prepareStatement(
					"select m.id, m.species, m.name, m.version, m.type, m.bg_model_map_id, c.position, c.letter, c.weight from "
	...
	/**
	 * Gets all the matrices specified in the result set which is the result of
	 * joining weightmatrix and weightmatrixcols, eg select m.id, m.species,
	 * m.name, m.version, m.type, m.bg_model_map_id, c.position, c.letter,
	 * c.weight from weightmatrix m, weightmatrixcols c where m.id =
	 * c.weightmatrix order by c.weightmatrix, c.position desc
	 *
	 * sorting by descending position is critical so that the first row of a new
	 * matrix gives the largest index (ie, the length of the matrix)
	 */
	public static Collection<WeightMatrix> getWeightMatrices(ResultSet rs) throws SQLException {
		...
		long end = System.currentTimeMillis();
		System.err.println("Returning " + output.size() + " from WeightMatrix.getWeightMatrices(ResultSet).  Took "
				+ (end - start));
	...
	/*
	 * creates a new weightMatrixObject based on the provided database
	 * identifier
	 */
	public static WeightMatrix getWeightMatrix(int dbid) throws NotFoundException {
		...
		try {
			...
			PreparedStatement ps = cxn.prepareStatement(
					"select position, letter, weight from weightmatrixcols where weightmatrix = ? order by position desc");
			ps.setInt(1, dbid);
			ResultSet rs = ps.executeQuery();
			matrix = null;
			while (rs.next()) {
				...
				if (weight < 0.0) {
					matrix.islogodds = true;
				}
			}
			rs.close();
			ps.close();
			ps = cxn.prepareStatement(
					"select name, version, type, species, bg_model_map_id from weightmatrix where id = ?");
			ps.setInt(1, dbid);
			rs = ps.executeQuery();
			rs.next();
	...
	/*
	 * returns the maximum possible score that a sequence could have against the
	 * specified matrix
	 */
	...
	public boolean equals(Object other) {
		if (other instanceof WeightMatrix) {
			...
			if (o.hasdbid && this.hasdbid) {
				...
				return (o.name != null && this.name != null && this.name.equals(o.name) && o.version != null
						&& this.version != null && this.version.equals(o.version));
			}
		} else {
			return false;
		}
	 ...
	 * model. You might want to do this if you're scanning for a matrix in a
	 * different genome than the matrix's default background model
	 */
	public void toLogOdds(MarkovBackgroundModel bgModel) {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < allLetters.length; j++) {
				matrix[i][allLetters[j]] = (float) (Math.log(Math.max(matrix[i][allLetters[j]], .000001)
						/ bgModel.getMarkovProb(("" + allLetters[j]).toUpperCase())) / LOG2);
			}
		}
	}
	...
	public void toLogOdds(Map<String, Double> freqs) {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < allLetters.length; j++) {
				matrix[i][allLetters[j]] = (float) (Math.log(
						Math.max(matrix[i][allLetters[j]], .000001) / freqs.get(("" + allLetters[j]).toUpperCase()))
						/ LOG2);
			}
		}
	}
	...
	/*
	 * converts this matrix to log-odds form. Uses either the matrix's default
	 * background model or a uniform .25 model
	 */
	...
	/*
	 * converts this matrix to frequencies. This conversion is in-exact because
	 * it uses a default background model
	 */
	...
	public void toFrequency(MarkovBackgroundModel bgModel) {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < allLetters.length; j++) {
				matrix[i][allLetters[j]] = (float) (Math.pow(2, (double) (matrix[i][allLetters[j]]))
						* bgModel.getMarkovProb(("" + allLetters[j]).toUpperCase()));
			}
		}
	}

	/**
	 * Modifies this WeightMatrix by normalizing the entries. The matrix must be
	 * a frequency matrix. The result is that the frequencies at each position
	 * in the matrix sum to 1. This method is useful if you've read in a matrix
	 * of counts and need to convert to frequencies.
	 */
	...
	public static int getWeightMatrixID(int speciesid, String wmname, String wmversion) throws NotFoundException {
		int wmid = -1;
		try {
			java.sql.Connection cxn = DatabaseConnectionManager.getConnection("annotations");
			PreparedStatement ps = cxn.prepareStatement(
					"select id from weightmatrix where species = ? " + "and name = ? and version = ?");
			...
			if (rs.next()) {
				...
				throw new NotFoundException("Can't find WM " + wmname + ", " + wmversion + " in species " + speciesid);
			}
			rs.close();
			ps.close();
			if (cxn != null)
			...
			throw new NotFoundException("Can't find WM " + wmname + ", " + wmversion + " in species " + speciesid, ex);
		} catch (UnknownRoleException ex) {
			throw new DatabaseException("Can't connect to annotations datasource", ex);
		}
		return wmid;
	...
	/* looks up the database id for the specified weight matrix */
	public static int getWeightMatrixID(String wmname, String wmversion) throws NotFoundException {
		int wmid = -1;
		try {
			java.sql.Connection cxn = DatabaseConnectionManager.getConnection("annotations");
			...
			if (rs.next()) {
				...
				if (rs.next())
					System.err.println(
							String.format("getWeightMatrixID(%s, %s) returns more than 1 motif!", wmname, wmversion));
			} else {
				rs.close();
				ps.close();
				if (cxn != null)
	...
	public static String printTransfacMatrix(WeightMatrix matrix, String name) {
		...
		out.append("DE\t" + name + "\n");
		for (int i = 0; i < length; i++) {
			out.append(i + 1 + "\t" + format.format(matrix.matrix[i]['A']) + "\t" + format.format(matrix.matrix[i]['C'])
					+ "\t" + format.format(matrix.matrix[i]['G']) + "\t" + format.format(matrix.matrix[i]['T']) + "\t"
	...
	public static List<String> getConsensusKmerList(WeightMatrix matrix, int Kmin, int Kmax) {
		...
		if (consensus.length() < Kmin) { // Return an empty set if Kmin is gt
											// than motif length
			return ret;
		} else {
			...
			for (int k = Kmin; k <= KCap; k++) {
				for (int i = 0; i < (consensus.length() - k + 1); i++) {
					...
					if (sub.contains("N"))
						continue;
					if (sub.contains("R") || sub.contains("Y") || sub.contains("S") || sub.contains("W")
							|| sub.contains("K") || sub.contains("M")) {
	...
	public static String getConsensus(WeightMatrix matrix) {
		...
		for (int i = 0; i < matrix.length(); i++) {
			if (out[i] == 'N') {
				...
				if (matrix.matrix[i]['A'] + matrix.matrix[i]['G'] > maxval) {
					out[i] = 'R';
					maxval = matrix.matrix[i]['A'] + matrix.matrix[i]['G'];
				}
				if (matrix.matrix[i]['C'] + matrix.matrix[i]['T'] > maxval) {
					out[i] = 'Y';
					maxval = matrix.matrix[i]['C'] + matrix.matrix[i]['T'];
				}
				if (matrix.matrix[i]['C'] + matrix.matrix[i]['G'] > maxval) {
					out[i] = 'S';
					maxval = matrix.matrix[i]['C'] + matrix.matrix[i]['G'];
				}
				if (matrix.matrix[i]['A'] + matrix.matrix[i]['T'] > maxval) {
					out[i] = 'W';
					maxval = matrix.matrix[i]['A'] + matrix.matrix[i]['T'];
				}
				if (matrix.matrix[i]['T'] + matrix.matrix[i]['G'] > maxval) {
					out[i] = 'K';
					maxval = matrix.matrix[i]['T'] + matrix.matrix[i]['G'];
				}
				if (matrix.matrix[i]['A'] + matrix.matrix[i]['C'] > maxval) {
					out[i] = 'M';
					maxval = matrix.matrix[i]['A'] + matrix.matrix[i]['C'];
				}
			}
		}
	...
	public static char getConsensusLetter(WeightMatrix matrix, int pos) {
		...
		if (out == 'N') {
			...
			if (matrix.matrix[pos]['A'] + matrix.matrix[pos]['G'] > maxval) {
				out = 'R';
				maxval = matrix.matrix[pos]['A'] + matrix.matrix[pos]['G'];
			}
			if (matrix.matrix[pos]['C'] + matrix.matrix[pos]['T'] > maxval) {
				out = 'Y';
				maxval = matrix.matrix[pos]['C'] + matrix.matrix[pos]['T'];
			}
			if (matrix.matrix[pos]['C'] + matrix.matrix[pos]['G'] > maxval) {
				out = 'S';
				maxval = matrix.matrix[pos]['C'] + matrix.matrix[pos]['G'];
			}
			if (matrix.matrix[pos]['A'] + matrix.matrix[pos]['T'] > maxval) {
				out = 'W';
				maxval = matrix.matrix[pos]['A'] + matrix.matrix[pos]['T'];
			}
			if (matrix.matrix[pos]['T'] + matrix.matrix[pos]['G'] > maxval) {
				out = 'K';
				maxval = matrix.matrix[pos]['T'] + matrix.matrix[pos]['G'];
			}
			if (matrix.matrix[pos]['A'] + matrix.matrix[pos]['C'] > maxval) {
				out = 'M';
				maxval = matrix.matrix[pos]['A'] + matrix.matrix[pos]['C'];
			}
		}
		return out;
	}
	...
	public static WeightMatrix getLogOddsVersion(WeightMatrix input, Map<String, Double> back) {
		...
		if (!input.islogodds) {
			...
			for (int i = 0; i < output.matrix.length; i++) {
				for (int j = 0; j < allLetters.length; j++) {
					output.matrix[i][allLetters[j]] = (float) (Math
							.log(Math.max(output.matrix[i][allLetters[j]], .000001)
									/ back.get(("" + allLetters[j]).toUpperCase()))
							/ LOG2);
				}
			}
		}
	...
	public void setZeroOffset(int z) {
		zeroOffset = z;
	}
...
class WMLetterCmp implements Comparator<Character> {
	...
	public WMLetterCmp(WeightMatrix m) {
		matrix = m;
	}
