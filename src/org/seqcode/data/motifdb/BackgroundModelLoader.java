...
/**
 * @author rca Code for database interaction involving background models
 */

/*
 * ~/psrg/software/meme-4.3.0/bin/fasta-get-markov -m 0 < hg19.fa | awk '{print
 * $2 FS $1}' | grep -v order > hg19.zero
 * 
 * java org.seqcode.data.motifdb.BackgroundModelLoader --species "$HS;hg19"
 * --bgname 'whole genome zero order' --bgtype FREQUENCY --bgfile hg19.zero
 * where hg19.zero is .2 A .3 C .2 T .3 G
 */
public class BackgroundModelLoader {
	...
	private static final String SQL_GET_METADATA_CORE = "select map.id, map.genome_id, map.bg_model_id, bm.name, bm.max_kmer_len, bm.model_type, map.has_counts"
			+ " from background_model bm, background_genome_map map" + " where bm.id = map.bg_model_id";
	...
	private static final String SQL_GET_MODEL_CORE = "select map.id, map.genome_id, bm.name, bm.max_kmer_len, bm.id, bgmc.kmer, bgmc.probability, bgmc.count"
	...
	/**
	 * @see getBackgroundModelID(String, int, String, Connection) Creates a
	 *      database connection for the query
	 */
		...
		try {
			...
			if (cxn != null)
				try {
					...
					throw new DatabaseException("Couldn't close connection with role annotations", ex);
				}
		}
	}
	 ...
	 * @param kmerLen
	 *            the length of the longest kmer in the model
	 * @param dbModelType
	 *            the type of the model (FREQUENCY or MARKOV)
	 * @param cxn
	 *            an open database connection
	 * @return the ID of the model, or null if there's no match
	 * @throws SQLException
	 */
	public static Integer getBackgroundModelID(String name, int kmerLen, String modelType, Connection cxn)
			throws SQLException {
	...
	/**
	 * @see getBackgroundGenomeMapID(int, int, Connection) Creates a database
	 ...
	 * @param genomeID
	 *            a genome for which there may be an instance of the model
	 * @param cxn
	 *            an open database connection
	 * @return the background genome map ID of the model, or null if there's no
	 *         match
	 * @throws SQLException
	 */
	 ...
	 * @param cxn
	 *            an open db connection to the annotations schema
	 ...
	 *            the model ID to look up
	 * @param genomeID
	 *            the genome ID to look up
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return A metadata object, or null if there's no match
	 * @throws SQLException
	 */
	public static BackgroundModelMetadata getBackgroundModel(int modelID, int genomeID, Connection cxn)
			throws SQLException {
		PreparedStatement getModel = null;
		ResultSet rs = null;
		try {
			...
			if (rs.next()) {
				return new BackgroundModelMetadata(rs.getInt(SQL_GET_METADATA_CORE_MAP_ID_INDEX), genomeID, modelID,
						...
						rs.getString(SQL_GET_METADATA_CORE_MODEL_TYPE_INDEX),
	...
	/**
	 * @see getBackgroundModel(String name, int maxKmerLen, String modelType,
	 *      int genomeID, Connection cxn)
	 */
	public static BackgroundModelMetadata getBackgroundModel(String name, int maxKmerLen, String modelType,
	 ...
	 *            the name to look up
	 * @param maxKmerLen
	 *            the max kmer length to look up
	 * @param modelType
	 *            the model type to look up
	 * @param genomeID
	 *            the genome ID to look up
	 * @param cxn
	 *            an open db connection to the annotations schema
	 ...
	 */
	public static BackgroundModelMetadata getBackgroundModel(String name, int maxKmerLen, String modelType,
			int genomeID, Connection cxn) throws SQLException {
		PreparedStatement getModel = null;
		ResultSet rs = null;
		try {
			StringBuffer sql = new StringBuffer(SQL_GET_METADATA_CORE);
			...
			rs = getModel.executeQuery();
			if (rs.next()) {
				return new BackgroundModelMetadata(rs.getInt(SQL_GET_METADATA_CORE_MAP_ID_INDEX), genomeID,
						rs.getInt(SQL_GET_METADATA_CORE_MODEL_ID_INDEX), name, maxKmerLen, modelType,
						rs.getBoolean(SQL_GET_METADATA_CORE_HAS_COUNTS_INDEX));
			} else {
				return null;
			}
	 ...
	 * 
	 * @param mapID
	 *            the background genome map ID to look up
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return A metadata object, or null if there's no match
	 * @throws SQLException
	...
	public static BackgroundModelMetadata getBackgroundModelByMapID(int mapID, Connection cxn) throws SQLException {
		...
		try {
			...
			rs = getModel.executeQuery();
			if (rs.next()) {
				return new BackgroundModelMetadata(mapID, rs.getInt(SQL_GET_METADATA_CORE_GENOME_ID_INDEX),
						rs.getInt(SQL_GET_METADATA_CORE_MODEL_ID_INDEX), rs.getString(SQL_GET_METADATA_CORE_NAME_INDEX),
						rs.getInt(SQL_GET_METADATA_CORE_KMER_LEN_INDEX),
						rs.getString(SQL_GET_METADATA_CORE_MODEL_TYPE_INDEX),
						rs.getBoolean(SQL_GET_METADATA_CORE_HAS_COUNTS_INDEX));
			} else {
				return null;
	 ...
	 * 
	 * @param ignoreGenome
	 *            if false return metadata for each entry in the
	 *            background_genome_map table, if true return metadata for each
	 *            entry in the background_model table
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return A list of metadata objects
	 * @throws SQLException
	 */
	public static List<BackgroundModelMetadata> getAllBackgroundModels(boolean ignoreGenome, Connection cxn)
			throws SQLException {
		...
		try {
			if (ignoreGenome) {
				...
				while (rs.next()) {
					results.add(
							new BackgroundModelMetadata(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getString(4)));
				}
				return results;
			} else {
				StringBuffer sql = new StringBuffer(SQL_GET_METADATA_CORE);
				...
				List<BackgroundModelMetadata> results = new ArrayList<BackgroundModelMetadata>();
				while (rs.next()) {
					results.add(new BackgroundModelMetadata(rs.getInt(SQL_GET_METADATA_CORE_MAP_ID_INDEX),
							rs.getInt(SQL_GET_METADATA_CORE_GENOME_ID_INDEX),
							rs.getInt(SQL_GET_METADATA_CORE_MODEL_ID_INDEX),
							rs.getString(SQL_GET_METADATA_CORE_NAME_INDEX),
							rs.getInt(SQL_GET_METADATA_CORE_KMER_LEN_INDEX),
							rs.getString(SQL_GET_METADATA_CORE_MODEL_TYPE_INDEX),
							rs.getBoolean(SQL_GET_METADATA_CORE_HAS_COUNTS_INDEX)));
				}
	...
	/**
	 * Get a list of metadata objects for all background models for the
	 * specified genome
	 * 
	 * @param genomeID
	 *            the genome ID to look up
	 ...
	 * @throws SQLException
	 */
	public static List<BackgroundModelMetadata> getBackgroundModelsForGenome(int genomeID, Connection cxn)
			throws SQLException {
	...
	/**
	 * Given the parameters that describe one or more background models
	 * construct an appropriate SQL query and return the Background Models that
	 * are found
	 * 
	 * @param mapID
	 *            an ID from the background genome map (this will limit to a
	 *            single model)
	 * @param genomeID
	 *            a genome ID for the models
	 * @param modelID
	 *            a model ID from the background_model table
	 * @param name
	 *            a model name
	 * @param maxKmerLen
	 *            a maximum kmer length
	 * @param cxn
	 *            an open DB connection to the annotations schema
	 * @return A list of matching Counts Background Models
	 * @throws SQLException
	 */
	private static PreparedStatement assembleGetModelQuery(Integer mapID, Integer genomeID, Integer modelID,
			String name, Integer maxKmerLen, String modelType, Boolean hasCounts, Connection cxn) throws SQLException {
	 ...
	 * kmer probabilities (and perhaps other data), parse those rows and set the
	 * model probabilities accordingly
	 * 
	 * @param fbm
	 *            A FrequencyBackgroundModel object to initialize
	 * @param rs
	 *            a ResultSet of kmer probability data, already on the first row
	 *            for the specified background model
	 * @param queryCore
	 *            the query core used to obtain the result set. This is used to
	 *            determine which indices to check for the kmer probabilities
	 * @return true if there are more rows in the result set for another
	 *         background model, false otherwise
	 * @throws SQLException
	 */
	private static boolean initFrequencyModelProbs(FrequencyBackgroundModel fbm, ResultSet rs, String queryCore)
			throws SQLException {
		int idIndex;
		int kmerIndex;
		int probIndex;
		...
		do {
			...
			// if this row is a new kmer length then set the probabilities for
			// the
			// previous length
	 ...
	 */
	public static FrequencyBackgroundModel getFrequencyModel(BackgroundModelMetadata md)
	 ...
	 * 
	 * @param md
	 *            A complete metadata object specifying a background model.
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return A FrequencyBackgroundModel, or null if none exists (and can't be
	 *         generated)
	 * @throws SQLException
	 * @throws NotFoundException
	 *             if the genomeID is invalid, most likely because the metadata
	 *             object wasn't loaded from the database or was modified
	 *             afterwards
	 */
	public static FrequencyBackgroundModel getFrequencyModel(BackgroundModelMetadata md, Connection cxn)
			throws SQLException, NotFoundException {
		...
		try {
			...
			// otherwise load the counts if available and convert from the
			// counts model
	 ...
	 * 
	 * @param mapID
	 *            the mapID of the model to look up
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return
	 * @throws SQLException
	 * @throws NotFoundException
	 */
	public static FrequencyBackgroundModel getFrequencyModel(int mapID, Connection cxn)
			throws SQLException, NotFoundException {
	...
	/**
	 * Returns a list of Frequency Background Models parsed out of a single
	 * result set
	 * 
	 * @param rs
	 *            the result set containing the data for the background model(s)
	 * @param queryCore
	 *            the SQL query core that was used to generate the specified
	 *            result set
	 * @return A list of FrequencyBackgroundModels
	 * @throws SQLException
	 */
	private static List<FrequencyBackgroundModel> createFrequencyModels(ResultSet rs, String queryCore)
			throws SQLException {
		int mapIDIndex;
		int nameIndex;
		int kmerLenIndex;
		...
		try {
			...
			if (rs.next()) {
				...
				while (hasNext) {
					...
					FrequencyBackgroundModel mbm = new FrequencyBackgroundModel(rs.getString(nameIndex),
							Genome.findGenome(rs.getInt(genomeIDIndex)), rs.getInt(kmerLenIndex));
					mbm.setMapID(mapID);
					mbm.setModelID(rs.getInt(modelIDIndex));
					models.add(mbm);
					// call the subroutine to parse all the probabilities from
					// the result set
	...
	/**
	 * @see getFrequencyModels(Integer mapID, Integer genomeID, Integer modelID,
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	private static List<FrequencyBackgroundModel> getFrequencyModels(Integer mapID, Integer genomeID, Integer modelID,
			String name, Integer maxKmerLen) throws SQLException {
		java.sql.Connection cxn = null;
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
	 ...
	 */
	private static List<FrequencyBackgroundModel> getFrequencyModels(Integer mapID, Integer genomeID, Integer modelID,
			String name, Integer maxKmerLen, Connection cxn) throws SQLException {
		PreparedStatement getModels = null;
		ResultSet rs = null;
		try {
			getModels = BackgroundModelLoader.assembleGetModelQuery(mapID, genomeID, modelID, name, maxKmerLen,
					FREQUENCY_TYPE_STRING, null, cxn);
			rs = getModels.executeQuery();
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModels(String name, int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModels(BackgroundModelMetadata md, Connection cxn)
			throws SQLException {
		Integer mapID = null;
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModelsByLength(int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModelsByGenome(int genomeID, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModelsByGenome(int genomeID, String name)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModelsByGenome(int genomeID, String name, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModelsByGenome(int genomeID, int maxKmerLen)
			throws SQLException {
	 ...
	 */
	public static List<FrequencyBackgroundModel> getFrequencyModelsByGenome(int genomeID, int maxKmerLen,
			Connection cxn) throws SQLException {
	...
	/**
	 * Given a MarkovBackgroundModel object and a result set whose rows are kmer
	 * probabilities (and perhaps other data), parse those rows and set the
	 * model probabilities accordingly
	 * 
	 * @param mbm
	 *            A MarkovBackgroundModel object to initialize
	 * @param rs
	 *            a ResultSet of kmer probability data, already on the first row
	 *            for the specified background model
	 * @param queryCore
	 ...
	 * @throws SQLException
	 */
	private static boolean initMarkovModelProbs(MarkovBackgroundModel mbm, ResultSet rs, String queryCore)
			throws SQLException {
		int idIndex;
		int kmerIndex;
		int probIndex;
		...
		do {
			...
			// if this row is a new value of prev bases then set the
			// probabilities for
			// the old value
	 ...
	 */
	public static MarkovBackgroundModel getMarkovModel(BackgroundModelMetadata md)
			throws SQLException, NotFoundException {
		java.sql.Connection cxn = null;
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
	 ...
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return A MarkovBackgroundModel, or null if none exists (and can't be
	 *         generated)
	 * @throws SQLException
	 * @throws NotFoundException
	 *             if the genomeID is invalid, most likely because the metadata
	 ...
	 *             afterwards
	 */
	public static MarkovBackgroundModel getMarkovModel(BackgroundModelMetadata md, Connection cxn)
			throws SQLException, NotFoundException {
		PreparedStatement getProbs = null;
		ResultSet rs = null;
		try {
			...
			// otherwise, if it's frequency, then load it as a frequency model
			// and
	 ...
	 * @throws NotFoundException
	 */
	public static MarkovBackgroundModel getMarkovModel(int mapID, Connection cxn)
			throws SQLException, NotFoundException {
		BackgroundModelMetadata md = BackgroundModelLoader.getBackgroundModelByMapID(mapID, cxn);
		if (md != null) {
			return BackgroundModelLoader.getMarkovModel(md, cxn);
	...
	private static List<MarkovBackgroundModel> createMarkovModels(ResultSet rs, String queryCore) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				while (hasNext) {
					int mapID = rs.getInt(mapIDIndex);
					MarkovBackgroundModel mbm = new MarkovBackgroundModel(rs.getString(nameIndex),
							Genome.findGenome(rs.getInt(genomeIDIndex)), rs.getInt(kmerLenIndex));
					mbm.setMapID(mapID);
					mbm.setModelID(rs.getInt(modelIDIndex));
					models.add(mbm);
	...
	/**
	 * @see getMarkovModels(Integer mapID, Integer genomeID, Integer modelID,
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	private static List<MarkovBackgroundModel> getMarkovModels(Integer mapID, Integer genomeID, Integer modelID,
			String name, Integer maxKmerLen) throws SQLException {
		java.sql.Connection cxn = null;
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModels(String name, int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModels(BackgroundModelMetadata md, Connection cxn)
			throws SQLException {
		Integer mapID = null;
		Integer genomeID = null;
		Integer modelID = null;
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModelsByLength(int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModelsByGenome(int genomeID, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModelsByGenome(int genomeID, String name, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModelsByGenome(int genomeID, int maxKmerLen)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<MarkovBackgroundModel> getMarkovModelsByGenome(int genomeID, int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 * 
	 * @param mapID
	 *            the backrgound genome map id of the model to check
	 * @param cxn
	 *            an open db connection to the annotations schema
	...
	/**
	 * Given a CountsBackgroundModel object and a result set whose rows are kmer
	 * counts (and perhaps other data), parse those rows and set the model
	 * counts accordingly
	 * 
	 * @param fbm
	 *            A CountsBackgroundModel object to initialize
	 * @param rs
	 *            a ResultSet of kmer probability data, already on the first row
	 *            for the specified background model
	 * @param queryCore
	 ...
	 * @throws SQLException
	 */
	private static boolean initCountsModel(CountsBackgroundModel cbm, ResultSet rs, String queryCore)
			throws SQLException {
		int idIndex;
		int kmerIndex;
		int countIndex;
	 ...
	 */
	public static CountsBackgroundModel getCountsModel(BackgroundModelMetadata md)
			throws SQLException, NotFoundException {
		java.sql.Connection cxn = null;
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
	 ...
	 * @param cxn
	 *            an open db connection to the annotations schema
	 * @return A CountsBackgroundModel, or null if none exists (and can't be
	 *         generated)
	 * @throws SQLException
	 * @throws NotFoundException
	 *             if the genomeID is invalid, most likely because the metadata
	 ...
	 *             afterwards
	 */
	public static CountsBackgroundModel getCountsModel(BackgroundModelMetadata md, Connection cxn)
			throws SQLException, NotFoundException {
	 ...
	 * @throws NotFoundException
	 */
	public static CountsBackgroundModel getCountsModel(int mapID, Connection cxn)
			throws SQLException, NotFoundException {
		BackgroundModelMetadata md = BackgroundModelLoader.getBackgroundModelByMapID(mapID, cxn);
		if (md != null) {
			return BackgroundModelLoader.getCountsModel(md, cxn);
	...
	private static List<CountsBackgroundModel> createCountsModels(ResultSet rs, String queryCore) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				while (hasNext) {
					int mapID = rs.getInt(mapIDIndex);
					CountsBackgroundModel mbm = new CountsBackgroundModel(rs.getString(nameIndex),
							Genome.findGenome(rs.getInt(genomeIDIndex)), rs.getInt(kmerLenIndex));
					mbm.setMapID(mapID);
					mbm.setModelID(rs.getInt(modelIDIndex));
					models.add(mbm);
					// call the subroutine to parse all the counts from the
					// result set
	...
	/**
	 * @see getCountsModels(Integer mapID, Integer genomeID, Integer modelID,
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	private static List<CountsBackgroundModel> getCountsModels(Integer mapID, Integer genomeID, Integer modelID,
			String name, Integer maxKmerLen) throws SQLException {
		java.sql.Connection cxn = null;
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
	...
	private static List<CountsBackgroundModel> getCountsModels(Integer mapID, Integer genomeID, Integer modelID,
		...
		ResultSet rs = null;
		try {
			getModels = BackgroundModelLoader.assembleGetModelQuery(mapID, genomeID, modelID, name, maxKmerLen, null,
					true, cxn);
			rs = getModels.executeQuery();
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModels(String name, int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModels(BackgroundModelMetadata md, Connection cxn)
			throws SQLException {
		Integer mapID = null;
		Integer genomeID = null;
		Integer modelID = null;
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModelsByLength(int maxKmerLen, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModelsByGenome(int genomeID, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModelsByGenome(int genomeID, String name, Connection cxn)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModelsByGenome(int genomeID, int maxKmerLen)
			throws SQLException {
	 ...
	 *      String name, Integer maxKmerLen, Connection cxn)
	 */
	public static List<CountsBackgroundModel> getCountsModelsByGenome(int genomeID, int maxKmerLen, Connection cxn)
			throws SQLException {
	...
	public static Pair<Integer, Integer> insertMarkovModel(MarkovBackgroundModel model) throws SQLException, Exception {
		...
		if ((model.getName() == null) || (model.getName().isEmpty()) || (model.getGenome() == null)) {
			throw new IllegalArgumentException(
					"Model must have a name and genome specified to be imported to database.");
		}
		java.sql.Connection cxn = null;
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
			...
			Pair<Integer, Integer> ids = BackgroundModelLoader.insertBackgroundModelAndMap(model.getName(),
					model.getMaxKmerLen(), MARKOV_TYPE_STRING, model.getGenome().getDBID(), false, cxn);
			int mapID = ids.car();
			...
			// If any runtime exceptions come up rollback the transaction and
			// then
	 ...
	 * @throws SQLException
	 */
	public static Pair<Integer, Integer> insertFrequencyModel(FrequencyBackgroundModel model)
		...
		try {
			...
			// insert into the background model and background
			Pair<Integer, Integer> ids = BackgroundModelLoader.insertBackgroundModelAndMap(model.getName(),
					model.getMaxKmerLen(), FREQUENCY_TYPE_STRING, model.getGenome().getDBID(), false, cxn);
			int mapID = ids.car();
	 ...
	 * @param model
	 *            the model to insert
	 * @param insertAsMarkov
	 *            indicates whether to insert the markov probabilities or the
	 *            frequencies corresponding to the counts
	 * @return the DBID of the model
	 * @throws SQLException
	 */
	public static Pair<Integer, Integer> insertCountsModel(CountsBackgroundModel model, boolean insertAsMarkov)
			throws SQLException, Exception {
		// make sure the model has a name and genome
		if ((model.getName() == null) || (model.getName().isEmpty()) || (model.getGenome() == null)) {
			throw new IllegalArgumentException(
		...
		try {
			...
			// insert into the background model and background
			Pair<Integer, Integer> ids = BackgroundModelLoader.insertBackgroundModelAndMap(model.getName(),
					model.getMaxKmerLen(), modelType, model.getGenome().getDBID(), true, cxn);
			int mapID = ids.car();
	...
	public static void updateMarkovModel(MarkovBackgroundModel model) throws SQLException, Exception {
		...
		try {
			...
			// remove from the database all the existing entries for the model
			// columns
	...
	public static void updateCountsModel(CountsBackgroundModel model) throws SQLException, Exception {
		...
		try {
			...
			// determine whether this model exists in the database as a markov
			// model or
			// a frequency model, so that it can be updated in the same format
			boolean isMarkov;
			getModelType = cxn.prepareStatement(
					"select model_type from background_model bm, background_genome_map map where map.id = ? and map.bg_model_id = bm.id");
	 ...
	 *            the length of the longest kmer in the model
	 * @param dbModelType
	 *            the type of the model ("MARKOV" or "FREQUENCY")
	 * @param genomeID
	 *            the DBID of the genome the model is for
	 * @param cxn
	 *            an open database connection
	 * @return the background genome map ID of the model
	 * @throws SQLException
	 */
	private static Pair<Integer, Integer> insertBackgroundModelAndMap(String name, int kmerLen, String modelType,
			int genomeID, boolean hasCounts, Connection cxn) throws SQLException, Exception {
		/**
		 * Check whether there is already an entry for a model with this name,
		 * maxKmerLen, and type. If so, reuse the model ID, otherwise create
		 * one.
		 */
		...
		/**
		 * Check whether there is already an entry for this model's genome in
		 * the background model genome map. If not then create one, otherwise
		 * throw an exception indicating that the update method should be called
		 * to update an existing model.
		 */
		Integer mapID = BackgroundModelLoader.getBackgroundGenomeMapID(modelID, genomeID, cxn);
		if (mapID != null) {
			throw new Exception(
					"Model already exists. Select a different name or use updateMarkovModel() to update the existing model.");
		} else {
	 ...
	 * @throws SQLException
	 */
	private static Integer insertBackgroundModel(String name, int kmerLen, String modelType, Connection cxn)
			throws SQLException {
		PreparedStatement insertBG = cxn.prepareStatement(
				"insert into background_model(id,name,max_kmer_len,model_type) values (weightmatrix_id.nextval,?,?,?)");
	 ...
	 * 
	 * @param modelID
	 *            the background model table ID of the model
	 * @param genomeID
	 *            the DBID of the genome the model is for
	 * @param cxn
	 *            an open database connection
	 ...
	 * @throws SQLException
	 */
	private static Integer insertBackgroundGenomeMap(int modelID, int genomeID, boolean hasCounts, Connection cxn)
			throws SQLException {
		PreparedStatement insertMap = cxn.prepareStatement(
				"insert into background_genome_map(id, genome_id, bg_model_id, has_counts) values (weightmatrix_id.nextval, ?, ?, ?)");
		...
		try {
			if (rs.next()) {
				...
				throw new SQLException(
						"Failed to get Background Model Genome Map ID following insert into Background_Genome_Map table.");
			}
		} finally {
			rs.close();
			getMapID.close();
	 ...
	 * 
	 * @param mapID
	 *            the model's ID in the background genome map table
	 * @param cxn
	 *            an open database connection
	 * @throws SQLException
	 */
	 ...
	 * @throws SQLException
	 */
	private static void insertMarkovModelColumns(MarkovBackgroundModel model, int mapID, Connection cxn)
			throws SQLException {
		PreparedStatement insertCol = cxn
				.prepareStatement("insert into background_model_cols(bggm_id,kmer,probability) values(?,?,?)");
		try {
	 ...
	 * @throws SQLException
	 */
	private static void insertFrequencyModelColumns(FrequencyBackgroundModel model, int mapID, Connection cxn)
			throws SQLException {
		PreparedStatement insertCol = cxn
				.prepareStatement("insert into background_model_cols(bggm_id,kmer,probability) values(?,?,?)");
		try {
	 ...
	 *            the model's ID in the background genome map table
	 * @param insertAsMarkov
	 *            if true then insert the markov probabilities, if false then
	 *            insert the kmer frequencies
	 * @param cxn
	 *            an open database connection
	 * @throws SQLException
	 */
	private static void insertCountsModelColumns(CountsBackgroundModel model, int mapID, boolean insertAsMarkov,
			Connection cxn) throws SQLException {
		PreparedStatement insertCol = cxn
				.prepareStatement("insert into background_model_cols(bggm_id,kmer,probability,count) values(?,?,?,?)");
		try {
			for (int i = 1; i <= model.getMaxKmerLen(); i++) {
				for (String kmer : model.getKmers(i)) {
					double prob;
	...
	/**
	 * Imports a background model from a file into the DB. File format is 1 A .2
	 * 2 C .3 3 G .3 4 T .2 5 AA ....
	 *
	 * Usage: java org.seqcode.data.motifdb.BackgroundModelLoader --genome "Mus
	 * musculus;mm8" --bgname "whole genome" --bgtype MARKOV --bgfile foo.back
	 */
	public static void importModel(String args[]) {
		try {
			...
			// args = new String[] {"--species", "Mus musculus;mm5", "--bgname",
			// "test1", "--bgtype", MARKOV_TYPE_STRING, "--bgfile",
			// "mm8_2.back"};
			// args = new String[] {"--species", "Saccharomyces
			// cerevisiae;sacCer1", "--bgname", "test2", "--bgtype",
			// MARKOV_TYPE_STRING, "--bgfile", "yeast1.back"};
			// args = new String[] {"--species", "Homo sapiens;hg17",
			// "--bgname", "test1", "--bgtype", MARKOV_TYPE_STRING, "--bgfile",
			// "human_1.back"};
			// args = new String[] {"--species", "Mus musculus;mm5", "--bgname",
			// "test1", "--bgtype", FREQUENCY_TYPE_STRING, "--bgfile",
			// "testfreq3.back"};
			// args = new String[] {"--species", "Mus musculus;mm5", "--bgname",
			// "test4", "--bgtype", "COUNTS;FREQUENCY", "--bgfile",
			// "testcount2.back"};
	...
	private static void testDBLoading() {
		...
		try {
			...
			testValuesYeast_3 = BackgroundModelIO.parseMarkovBackgroundModel("yeast2.back",
					Genome.findGenome("sacCer1"));
			testValuesYeast_2 = BackgroundModelIO.parseMarkovBackgroundModel("yeast1.back",
					Genome.findGenome("sacCer1"));
			testValuesHuman_2 = BackgroundModelIO.parseMarkovBackgroundModel("human_1.back", Genome.findGenome("hg17"));

			testFreqValuesMM8_3 = BackgroundModelIO.parseFrequencyBackgroundModel("testfreq3.back",
					Genome.findGenome("mm8"));
			testFreqValuesMM8_2 = BackgroundModelIO.parseFrequencyBackgroundModel("testfreq2.back",
					Genome.findGenome("mm8"));

			testCountValuesMM8_3 = BackgroundModelIO.parseCountsBackgroundModel("testcount3.back",
					Genome.findGenome("mm8"));
			testCountValuesMM8_2 = BackgroundModelIO.parseCountsBackgroundModel("testcount2.back",
					Genome.findGenome("mm8"));
		...
		try {
			...
			BackgroundModelMetadata expectedMDModelOnly_test1_2 = new BackgroundModelMetadata(expectedModelID_test1_2,
					expectedName_test1_2, expectedKmerLen_test1_2, expectedType_test1_2);
			BackgroundModelMetadata expectedMD_test1_2 = new BackgroundModelMetadata(expectedMapID_test1_2,
					expectedGenomeID_test1_2, expectedModelID_test1_2, expectedName_test1_2, expectedKmerLen_test1_2,
					expectedType_test1_2, false);

			BackgroundModelMetadata expectedFreqMDModelOnly_test1_2 = new BackgroundModelMetadata(
					expectedFreqModelID_test1_2, expectedName_test1_2, expectedKmerLen_test1_2, FREQUENCY_TYPE_STRING);
			BackgroundModelMetadata expectedFreqMD_test1_2 = new BackgroundModelMetadata(expectedFreqMapID_test1_2,
					expectedGenomeID_test1_2, expectedFreqModelID_test1_2, expectedName_test1_2,
					expectedKmerLen_test1_2, FREQUENCY_TYPE_STRING, false);

			BackgroundModelMetadata expectedCountMDModelOnly_test3_2 = new BackgroundModelMetadata(
					expectedCountModelID_test3_2, expectedCountName_test3_2, expectedKmerLen_test1_2,
					FREQUENCY_TYPE_STRING);
			BackgroundModelMetadata expectedCountMD_test3_2 = new BackgroundModelMetadata(expectedCountMapID_test3_2,
					expectedGenomeID_test1_2, expectedCountModelID_test3_2, expectedCountName_test3_2,
					expectedKmerLen_test1_2, FREQUENCY_TYPE_STRING, false);

			BackgroundModelMetadata expectedCountMD_test4_2 = new BackgroundModelMetadata(expectedCountMapID_test3_2,
					expectedGenomeID_test1_2, expectedCountModelID_test3_2, expectedCountName_test4_2,
					expectedKmerLen_test1_2, FREQUENCY_TYPE_STRING, false);
			...
			BackgroundModelMetadata expectedMDModelOnly_test1_3 = new BackgroundModelMetadata(expectedModelID_test1_3,
					expectedName_test1_3, expectedKmerLen_test1_3, expectedType_test1_3);
			BackgroundModelMetadata expectedMD_test1_3 = new BackgroundModelMetadata(expectedMapID_test1_3,
					expectedGenomeID_test1_3, expectedModelID_test1_3, expectedName_test1_3, expectedKmerLen_test1_3,
					expectedType_test1_3, false);
			BackgroundModelMetadata expectedFreqMD_test1_3 = new BackgroundModelMetadata(expectedFreqMapID_test1_3,
					expectedGenomeID_test1_3, expectedFreqModelID_test1_3, expectedName_test1_3,
					expectedKmerLen_test1_3, FREQUENCY_TYPE_STRING, false);
			BackgroundModelMetadata expectedCountMD_test3_3 = new BackgroundModelMetadata(expectedCountMapID_test3_3,
					expectedGenomeID_test1_3, expectedCountModelID_test3_3, expectedCountName_test3_3,
					expectedKmerLen_test1_3, FREQUENCY_TYPE_STRING, false);
			...
			BackgroundModelMetadata expectedMDModelOnly_test2_3 = new BackgroundModelMetadata(expectedModelID_test2_3,
					expectedName_test2_3, expectedKmerLen_test2_3, expectedType_test2_3);
			BackgroundModelMetadata expectedMD_test2_3 = new BackgroundModelMetadata(expectedMapID_test2_3,
					expectedGenomeID_test2_3, expectedModelID_test2_3, expectedName_test2_3, expectedKmerLen_test2_3,
					expectedType_test2_3, false);

			// test getBackgroundModelID(name, kmerlen, type)
			BackgroundModelLoader.getBackgroundModelID(expectedName_test1_3, expectedKmerLen_test1_3,
					expectedType_test1_3);
			if (BackgroundModelLoader.getBackgroundModelID(expectedName_test1_3, expectedKmerLen_test1_3,
					expectedType_test1_3) == expectedModelID_test1_3) {
			...
			// test getBackgroundModelID(name, kmerlen, type)
			if (BackgroundModelLoader.getBackgroundGenomeMapID(expectedModelID_test1_3,
					expectedGenomeID_test1_3) == expectedMapID_test1_3) {
			...
			if (expectedMDModelOnly_test1_3
					.equals(BackgroundModelLoader.getBackgroundModelByModelID(expectedModelID_test1_3))) {
			...
			if (expectedMD_test1_3.equals(
					BackgroundModelLoader.getBackgroundModel(expectedModelID_test1_3, expectedGenomeID_test1_3))) {
			...
			if (expectedMD_test1_3.equals(BackgroundModelLoader.getBackgroundModel(expectedName_test1_3,
					expectedKmerLen_test1_3, expectedType_test1_3, expectedGenomeID_test1_3))) {
			...
			if ((mbmList.size() == 2) && mbmList.get(0).equalValues(testValuesHuman_2)
					&& mbmList.get(1).equalValues(testValuesMM8_2)) {
			...
			if ((mbmList.size() == 3) && mbmList.get(1).equalValues(testValuesMM8_2)
					&& mbmList.get(2).equalValues(testValuesMM8_3) && mbmList.get(0).equalValues(testValuesHuman_2)) {
			...
			if ((mbmList.size() == 3) && mbmList.get(1).equalValues(testValuesMM8_2)
					&& mbmList.get(2).equalValues(testValuesYeast_2) && mbmList.get(0).equalValues(testValuesHuman_2)) {
			...
			if ((mbmList.size() == 2) && mbmList.get(0).equalValues(testValuesMM8_2)
					&& mbmList.get(1).equalValues(testValuesMM8_3)) {
			...
			if ((fbmList.size() == 2) && fbmList.get(0).equalValues(testFreqValuesMM8_2)
					&& fbmList.get(1).equalValues(testFreqValuesMM8_3)) {
			...
			// if ((fbmList.size() == 3) &&
			// fbmList.get(0).equalValues(testFreqValuesMM8_2) &&
			// fbmList.get(1).equalValues(testFreqValuesMM8_2)) {
			...
			if ((fbmList.size() == 5) && fbmList.get(0).equalValues(testFreqValuesMM8_2)
					&& fbmList.get(1).equalValues(testFreqValuesMM8_3)
					&& fbmList.get(2).equalValues(testFreqValuesMM8_2)
					&& fbmList.get(3).equalValues(testFreqValuesMM8_3)
			...
			fbmList = BackgroundModelLoader.getFrequencyModelsByGenome(expectedGenomeID_test1_2,
					expectedKmerLen_test1_2);
			if ((fbmList.size() == 3) && fbmList.get(0).equalValues(testFreqValuesMM8_2)
					&& fbmList.get(1).equalValues(testFreqValuesMM8_2)
					&& fbmList.get(2).equalValues(testFreqValuesMM8_2)) {
			...
			if ((cbmList.size() == 2) && cbmList.get(0).equalValues(testCountValuesMM8_2)
					&& cbmList.get(1).equalValues(testCountValuesMM8_3)) {
			...
			// if ((cbmList.size() == 1) &&
			// cbmList.get(0).equalValues(testCountValuesMM8_2)) {
			...
			if ((cbmList.size() == 3) && cbmList.get(0).equalValues(testCountValuesMM8_2)
					&& cbmList.get(1).equalValues(testCountValuesMM8_3)
			...
			if ((cbmList.size() == 2) && cbmList.get(0).equalValues(testCountValuesMM8_2)
					&& cbmList.get(1).equalValues(testCountValuesMM8_2)) {
