...
public class MarkovBackgroundModel extends BackgroundModel {
	 ...
	 * @param gen
	 * @param maxKmerLen
	 *            the max Kmer Length for the model
	 */
	 ...
	 * 
	 * @param fbg
	 *            the existing frequency background model
	 */
	 ...
	 * 
	 * @param prevBases
	 *            The bases that the probabilities are conditionally dependent
	 *            upon
	 * @param aProb
	 *            P( A | prevBases)
	 * @param cProb
	...
	public void setMarkovProb(String prevBases, double aProb, double cProb, double gProb, double tProb) {
		...
		if (BackgroundModel.isKmerValid(prevBases)) {
			...
			// && (Fmath.isEqualWithinLimits(total, 1.0,
			// BackgroundModel.EPSILON)
			// || (total == 0))) {
			...
			// or must all be 0, but instead sum to " + total + " for prevBases
			// " + prevBases);
			// }
			...
			throw new IllegalArgumentException(
					"Previous Bases must consist of zero or more DNA bases, but is: " + prevBases);
		}
	}
	...
	public boolean checkAndSetIsStranded() {
		...
		while (currKmerLen <= modelProbs.length) {
			// set up and solve the log-space linear system for the current
			// kmers
			...
			// now check that the rev. comp components of the solution are
			// matched
			for (Pair<Integer, Integer> rcPair : revCompPairs) {
				if (!Fmath.isEqualWithinLimits(sol.getQuick(rcPair.car()), sol.getQuick(rcPair.cdr()),
						BackgroundModel.EPSILON)) {
					isStranded = true;
					return isStranded;
				}
	 ...
	 * @return
	 */
	private static DoubleMatrix2D createStrandednessLinearSystem(int numKmers,
			List<Pair<Integer, Integer>> revCompPairs) {
