...
/**
 * @author rca This class is a lightweight object that contains the basic fields
 *         describing a background model. It is primarily used by the
 *         BackgroundModelLoader for returning the metadata for background
 *         models that are in the database. It is also the superclass for the
 *         BackgroundModel class. It intentially doesn't constrain what values
 *         can be set for any of the fields except dbModelType - GIGO.
 * 
 *         Note: -1 is used as a value for numeric fields that are unknown or
 *         not set.
 */
public class BackgroundModelMetadata {
	...
	/**
	 * these fields may be -1 for parsed background models or metadata instances
	 * that just represent a row from the background model table
	 */
	protected int mapID;
	...
	public void setDBModelType(String dbModelType) {
		if ((dbModelType == null) || dbModelType.equals(BackgroundModelLoader.MARKOV_TYPE_STRING)
				|| dbModelType.equals(BackgroundModelLoader.FREQUENCY_TYPE_STRING)) {
	...
	public boolean equals(BackgroundModelMetadata other) {
		if ((other != null) && this.name.equals(other.getName()) && (this.maxKmerLen == other.getMaxKmerLen())
				&& (((this.dbModelType == null) && (other.getDBModelType() == null))
						|| this.dbModelType.equals(other.dbModelType))
				&& (this.modelID == other.getModelID() && this.mapID == other.getMapID())
				&& (this.genomeID == other.getGenomeID()) && (this.hasCounts == other.hasCounts())) {
			return true;
		} else {
			return false;
		}
	...
	public String toString() {
		return mapID + "\t" + genomeID + "\t" + modelID + "\t" + name + "\t" + maxKmerLen + "\t" + dbModelType + "\t"
				+ hasCounts;
	}
}
