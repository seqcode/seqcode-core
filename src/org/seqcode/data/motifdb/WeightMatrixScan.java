...
public class WeightMatrixScan {
	...
	public static WeightMatrixScan getWeightMatrixScan(int dbid) throws NotFoundException {
		...
		try {
			...
			PreparedStatement ps = cxn
					.prepareStatement("select weightmatrix,name,cutoff from weightmatrixscan where " + " id = ?");
			ps.setInt(1, dbid);
	...
	public static WeightMatrixScan getScanForMatrix(int matrixid, String scanname) {
		...
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
			PreparedStatement ps = cxn
					.prepareStatement("select id from weightmatrixscan where weightmatrix = ? and name = ?");
			ps.setInt(1, matrixid);
	...
	public static Collection<WeightMatrixScan> getScansForSpecies(int speciesid, String name, String version,
			String type, String scanname) {
		java.sql.Connection cxn = null;
		ArrayList<WeightMatrixScan> results = new ArrayList<WeightMatrixScan>();
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
			...
			if (version != null) {
				ps.setString(count++, version);
			}
			if (scanname != null) {
				ps.setString(count++, scanname);
			}
			if (type != null) {
				ps.setString(count++, type);
			}
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				results.add(getWeightMatrixScan(rs.getInt(1)));
	...
	public static Collection<WeightMatrixScan> getScansForGenome(int genomeid, String name, String version, String type,
			String scanname) {
		java.sql.Connection cxn = null;
		ArrayList<WeightMatrixScan> results = new ArrayList<WeightMatrixScan>();
		try {
	...
	private static Collection<String> getField(int genomeid, String field) {
		...
		try {
			cxn = DatabaseConnectionManager.getConnection("annotations");
			String sql = "select unique(" + field
					+ ") from weightmatrix wm, weightmatrixscan wms, wms_scanned_genomes wsg "
			...
			while (rs.next()) {
				if (rs.getString(1) == null) {
					// throw new NullPointerException("NULL getting " + field +
					// " for " + speciesid);
	...
	public static Collection<String> getNames(int genomeid) {
		return getField(genomeid, "wm.name");
	}

	public static Collection<String> getVersions(int genomeid) {
		return getField(genomeid, "wm.version");
	}

	public static Collection<String> getTypes(int genomeid) {
		return getField(genomeid, "wm.type");
	}

	public static Collection<String> getScanNames(int genomeid) {
		return getField(genomeid, "wms.name");
	}
	...
	public boolean equals(Object other) {
		if (other instanceof WeightMatrixScan) {
			...
			return (o.matrix != null && matrix != null && o.matrix.equals(matrix) && scanname != null
					&& o.scanname != null && scanname.equals(o.scanname) && cutoff == o.cutoff);
		} else {
			return false;
		}
	}
