...
public class CountsBackgroundModel extends BackgroundModel implements BackgroundModelFrequencySupport {
	...
	/**
	 * Map for holding the kmer counts for the model. Each element of the array
	 * holds kmers whose length is the index of that element. i.e. model[2]
	 * holds 2-mers. Accordingly, model[0] should always be empty.
	 * 
	 ...
	 * 
	 * @param md
	 *            the metadata describing this model
	 * @throws NotFoundException
	 *             if a Genome can't be found for the metadata's genome ID
	 */
	...
	public CountsBackgroundModel(String name, Genome gen, int maxKmerLen) {
		// Note: typically Count based models will go into the DB as frequency
		// models
	...
	/**
	 * Initialize the kmer counts hash and set the hasCounts field of the
	 * metadata
	 */
	protected void init() {
		this.hasCounts = true;
	...
	/**
	 * @see BackgroundModel On the fly compute the markov probability from the
	 *      known frequencies
	 */
	...
	public void setKmerCount(String kmer, long count) {
		if (BackgroundModel.isKmerValid(kmer)) {
			...
			// reset the isStranded variable to null to indicate unknown
			// strandedness
			isStranded = null;
		} else {
	 ...
	 * @param sequence
	 * @param addRevComp
	 *            if true also add counts for the reverse complement
	 */
	...
	/**
	 * @see BackgroundModel Check whether the reverse complement kmers have
	 *      equal counts for all the kmer lengths
	 * 
	 */
	public boolean checkAndSetIsStranded() {
		...
		/**
		 * If all kmerlens have been checked and the method hasn't returned then
		 * the model is not stranded
		 */
		isStranded = false;
		return isStranded;
	}
	...
	/**
	 * @see BackgroundModelFrequencySupport For a count based model this is
	 *      accomplished by setting reverse complements to have the sum of their
	 *      counts.
	 */
	public void degenerateStrands() {
		...
		// only destrand the model if its stranded, otherwise the counts will
		// blow up
		if (isStranded) {
	...
	public static CountsBackgroundModel modelFromWholeGenome(Genome gen) {
		return modelFromWholeGenome(gen, DEFAULT_MAX_KMER_LEN);
	}
	...
	public static CountsBackgroundModel modelFromRegionList(Genome gen, List<Region> regionList) {
		return modelFromRegionList(gen, regionList, DEFAULT_MAX_KMER_LEN);
	}
	...
	public static CountsBackgroundModel modelFromSeqList(Genome gen, List<String> seqList) {
		return modelFromSeqList(gen, seqList, DEFAULT_MAX_KMER_LEN);
	}
	...
	public static CountsBackgroundModel modelFromFASTAStream(FASTAStream stream) {
		return modelFromFASTAStream(stream, DEFAULT_MAX_KMER_LEN);
	}
