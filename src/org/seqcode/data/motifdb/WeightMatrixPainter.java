...
public class WeightMatrixPainter {
	...
	public void paint(WeightMatrix wm, Graphics g, int x1, int y1, int x2, int y2) {
		this.paint(wm, g, x1, y1, x2, y2, null, false);
	}

	public void paint(WeightMatrix wm, Graphics g, int x1, int y1, int x2, int y2, String name) {
		this.paint(wm, g, x1, y1, x2, y2, name, false);
	}

	/*
	 * paints a representation of a weight matrix wherein the height of the
	 * letters roughly indicates their relative probability at each base. The
	 * image is painted in g in the bounding box defined by the upper left
	...
	public void paint(WeightMatrix wm, Graphics g, int x1, int y1, int x2, int y2, String name, boolean drawAxes) {
		...
		g2d.drawString(label,
				x1 + X_MARGIN + w / 2 - fontmetrics.charsWidth(label.toCharArray(), 0, label.length()) / 2,
				y2 - Y_MARGIN);
		...
		// System.err.println("Xfactor is " + xfactor + ", base size is " +
		// baseFont.getSize()+", pixels per letter is "+pixelsPerLetter);
		...
		for (int pos = 0; pos < wm.length(); pos++) {
			...
			for (int j = 3; j >= 0; j--) {
				...
				if (letterHeight > 1) {
					...
					int offset = 0;
					/*
					 * if (letters[j] == 'G') offset =
					 * -(int)(pixelsPerLetter*0.05); else if (letters[j] == 'T')
					...
					g2d.fillRect(x1 + X_MARGIN + pos * pixelsPerLetter, ypos, (int) (pixelsPerLetter * 0.9),
							letterHeight);
				}
				ypos -= letterHeight;
			}
