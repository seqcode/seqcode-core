...
/**
 * Usage: java org.seqcode.data.motifdb.WeightMatrixImport --species
 * "Saccharomyces cerevisiae" --wmname HSF1 --wmversion MacIsaac06 --wmtype TAMO
 * --wmfile v1.HSF1.wm
 */
...
public class WeightMatrixImport {
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].equals("--") || args[i].equals("--wmfile")) {
				wmfile = args[++i];
			}
		}
	...
	public static int insertMatrixIntoDB(WeightMatrix matrix) throws SQLException, NotFoundException {

		java.sql.Connection cxn = DatabaseConnectionManager.getConnection("annotations");
		int wmid = -1;
		PreparedStatement exists = cxn.prepareStatement(
				"select id from weightmatrix where species = ? and name = ? and version = ? and type = ?");
		...
		if (!rs.next()) {
			...
			if (matrix.bgMapID != -1) {
				insertwm = cxn.prepareStatement(
						"insert into weightmatrix(id,species,name,version,type,bg_model_map_id) values (weightmatrix_id.nextval,?,?,?,?,?)");
				insertwm.setInt(5, matrix.bgMapID);
			} else {
				insertwm = cxn.prepareStatement(
						"insert into weightmatrix(id,species,name,version,type) values (weightmatrix_id.nextval,?,?,?,?)");
			}
			...
			// System.err.println(String.format("Inserting %s %s %s",
			// matrix.name, matrix.version, matrix.type));
			insertwm.execute();
			insertwm.close();
			...
			PreparedStatement insertWMSM = cxn.prepareStatement(
					"insert into weightmatrix_species_map(id, species_id, wm_id) values (weightmatrix_id.nextval, ?, ?)");
	...
	/*
	 * reads a weight matrix from the specified file, inserts it into the
	 * database with the specified name and version, and returns its dbid. This
	 * means that the file may contain only a single weight matrix
	 */
	public static int insertWMFromFile(String species, String wmname, String wmversion, String wmtype, String wmfile)
			throws SQLException, NotFoundException, UnknownRoleException, FileNotFoundException, ParseException,
			IOException {
		WeightMatrix matrix;
	 ...
	 */
	public static int insertWMFromFile(String species, String wmname, String wmversion, String wmtype, String wmfile,
			String bgFreq) throws SQLException, NotFoundException, UnknownRoleException, FileNotFoundException,
	...
	/**
	 * Constructs a matrix from a set of strings. The strings must all have the
	 * same length. The WeightMatrix returned has the frequencies of the bases
	 * at each position given.
	 * 
	 * @param strings
	 * @return
	 * @throws IOException
		...
		for (String line : strings) {
			...
			if (line.length() > 0) {
				...
				if (line.length() != wm.length()) {
					throw new ParseException("Line \"" + line + "\" was of uneven length (" + wm.length() + ")", 0);
				}
