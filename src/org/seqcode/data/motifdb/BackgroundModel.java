...
public abstract class BackgroundModel extends BackgroundModelMetadata {
	...
	/**
	 * Map for holding the kmer probability values for the model. Each element
	 * of the array holds kmers whose length is the index of that element. i.e.
	...
	/**
	 * Construct a model with the specified name, genome, and model type, and
	 * the specified max kmer len
	 * 
	 * @param maxKmerLen
	 */
	 ...
	 * should only set the dbid if the type of the subclass model matches the
	 * type of the source model.
	 * 
	 * @param source
	 *            the Background Model on which to base the one being
	 *            constructed
	 */
	...
	/**
	 * A subclass specific init method to be called at the end of the
	 * constructor to handle tasks common among the constructors
	 */
	protected abstract void init();

	/**
	 * overrides setHasCounts in BackgroundModelMetadata. It's not possible to
	 * set whether an instance of a background model has counts - it's implicit
	 * based on the subclass type.
	 */
	...
	/**
	 * Return the markov probability for the last base of the kmer represented
	 * by the specified int, conditioned upon the preceding bases.
	 * 
	 * @param intVal
	 * @param kmerLen
	 * @return
	 ...
	 * terms of the kmer values. The class must be the same so they can be
	 * compared properly.
	 * 
	 * @param other
	 *            the model to check
	...
	public boolean equalValues(BackgroundModel other) {
		if (this.getClass().equals(other.getClass())) {
			...
			for (int k = maxKmerLen; k > 0; k--) {
				...
				// check all the kmers values as appropriate for the type of
				// model
	...
	protected static List<Pair<Integer, Integer>> computeDistinctRevCompPairs(int kmerLen) {
		...
		for (int i = 0; i < revCompMap.length; i++) {
			if (revCompMap[i] == -1) {
				int revComp = BackgroundModel
						.seq2int(SequenceUtils.reverseComplement(BackgroundModel.int2seq(i, kmerLen)));
	...
	/**
	 * Return true if the specified kmer only of (zero or more) DNA letters
	 * (ACGT)
	 * 
	 * @param kmer
	 * @return
