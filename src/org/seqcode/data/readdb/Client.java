 ...
 *
 * <p>
 * Calls throw IOException on network errors. Calls throw ClientException on
 * other errors (authentication, authorization, invalid request ,etc.
 *
 * <p>
 * Client generally assumes that the hit positions are the 5' end of the hit.
 *
 * <p>
 * Client IS NOT REENTRANT. Do not overlap calls to a single Client object.
 *
 * <p>
 * The current version of Client keeps a separate thread that would close the
 * connection if it is idle too long.
 * 
 * <p>
 * Most method parameters that are object types (eg Integer, Boolean) are
 * optional. If a null value is passed then no filtering is done based on that
 * parameter.
 * 
 * <p>
 ...
 * <li>isType2 specifies whether to work on ordinary single-ended reads (false)
 * or the "type 2" reads (true). An example of type 2 reads are the R2 reads in
 * paired-end ChIP-exo.
 * <li>isPaired specifies whether to work on single-ended reads (false) or
 * paired-end reads (true)
 * <li>isLeft if isPaired is true, then isLeft specifies whether to work on the
 * left read (true) or right read (false) of the pair
 * <li>plusStrand specifies whether to return only reads on the plus strand
 * (true) or minus strand (false). null means that reads on both strands should
 * be returned.
 * <li>minWeight specifies the minimum weight of reads (or read pairs) to be
 * returned or included in the histogram
 * <li>start, stop specify the lowest (inclusive) and highest (exclusive)
 * coordinates of reads that should be included in the results
 * </ul>
 *
 * 
 */
public class Client implements ReadOnlyClient {
	...
	Socket socket; // the socket to talk to the server. outstream and instream
					// are from the socket
	OutputStream outstream;
	BufferedInputStream instream;
	Thread closeTimerThread = null; // checks time of last activity - closes
									// connection if idle for too long
	byte[] buffer; // temporary space for receiving data; contents not
					// persistent between method calls
	private static final int BUFFERLEN = 8192 * 20;
	private final int socketLoadDataReadTimeout = 1000 * 60 * 8; // socket
																	// timeout
																	// in ms:
																	// set to 8
																	// minutes
																	...
																	// the
																	// Server.
	private final int socketQueryReadTimeout = 60000; // socket timeout in ms
														// for queries
	private final int threadSleepTime = 30000; // check time of last activity
												// thread sleep time in ms
	private final int connectionIdleTimeLimit = 1000 * 60 * 10; // close idle
																// connection
																// after this
																// time (10
																// minutes)
	private long lastActivityTime; // Time of last activity is updated by each
									// query
	...
	/**
	 * Connects to a ReadDB server on the specified host and port using the
	 * specified username and password.
	 * 
	 * @throws IOException
	 *             on network errors
	 * @throws ClientException
	 *             if the client cannot authenticate to the server
	 */
	...
	/**
	 * Creates the default connection as specified by ~/.readdb_passwd or a
	 * readdb_passwd found in the classpath Must have keys hostname, port,
	 * username, and passwd in a format that java.util.PropertyResourceBundle
	 * can read
	 *
	 * @throws IOException
	 *             on network errors
	...
	private void init(String hostname, int portnum, String username, String passwd)
		...
		synchronized (this) {
			...
			/*
			 * linger = true, time = 0 means that the other side gets a reset if
			 * the socket is closed on our end (eg, java exits). We turn this
			 * off just before sending a "bye" to allow for a graceful shutdown.
			 * But the RST in other cases lets the server figure out that we've
			 * disappeared
			 */
	...
	/**
	 * Determines whether the client will print error messages to STDERR. Useful
	 * for debugging but may produce unwanted screen output.
	 */
	...
	/**
	 * performs the SASL authentication exchange with the server. currently
	...
	private boolean authenticate(String hostname, String username, String password) throws IOException {
		...
		try {
			...
			sasl = Sasl.createSaslClient(SaslMechanisms, username, "readdb", hostname, props,
			...
			while (continueByte != 0) {
				...
				/*
				 * the continueByte tells us whether the server expects to do
				 * another round. Necessary because sometimes isComplete()
				 * returned true here but the server wasn't done
				 */
	...
	/**
	 * Tells the server to shut itself down. Use this to stop the server
	 * process.
	 * 
	 * @throws IOException
	 *             on network errors
	 * @throws ClientException
	 *             if the user isn't authorized to shut the server down.
	 */
	...
	/**
	 * this was to fix a bug in the server. You shouldn't need it for general
	 * use. Regenerate the index for this alignment and chromosome
	 */
	...
	/**
	 * This was to fix a bug in the server. You shouldn't need it for general
	 * use. Resort the hits for a single-ended alignment and regenerate the
	 * index.
	 */
	...
	/**
	 * Stores a set of SingleHit objects (representing an un-paired or
	 * single-ended read aligned to a genome) in the specified alignment. The
	 * hits are appended to any hits that have already been stored in the
	 * alignment.
	 * 
	 */
	public void storeSingle(String alignid, List<SingleHit> allhits, boolean isType2)
			throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
				...
				synchronized (this) {
					...
					for (int startindex = 0; startindex < hits.size(); startindex += chunk) {
						...
						try {
							...
							// I think it's best to kill the process at this
							// point, since we won't know if the sent reads
							// actually got loaded.
	 ...
	 * aligned to a genome) in the specified alignment. The hits are appended to
	 * any hits that have already been stored in the alignment
	 */
			...
			synchronized (this) {
				...
				for (int startindex = 0; startindex < hits.size(); startindex += chunk) {
					...
					try {
						...
						// IOException here is probably a socket time-out.
						// I think it's best to kill the process at this point,
						// since we won't know if the sent reads actually got
						// loaded.
						System.err.println(ioe);
						System.exit(1);
					}
	...
	/**
	 * Returns true if the alignment and chromosome exist and are accessible to
	 * the user. Returns false if they don't exist or if they aren't accessible
	 */
	...
	/**
	 * Deletes an alignment (all chromosomes). isPaired specifies whether to
	 * delete the paired or single ended reads.
	 */
	 ...
	 */
	public Set<Integer> getChroms(String alignid, boolean isType2, boolean isPaired, Boolean isLeft)
			throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
	 ...
	 */
	public int getCount(String alignid, boolean isType2, boolean isPaired, Boolean isLeft, Boolean plusStrand)
			throws IOException, ClientException {
		int count = 0;
	 ...
	 */
	public double getWeight(String alignid, boolean isType2, boolean isPaired, Boolean isLeft, Boolean plusStrand)
			throws IOException, ClientException {
		double total = 0;
		for (int c : getChroms(alignid, isType2, isPaired, isLeft)) {
	...
	/**
	 * returns the total number of hits on the specified chromosome in the
	 * alignment. Any of the object parameters can be set to null to specify "no
	 * value"
	 */
	public int getCount(String alignid, int chromid, boolean isType2, boolean paired, Integer start, Integer stop,
			Float minWeight, Boolean isLeft, Boolean plusStrand) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
			request.clear();
	 ...
	 */
	public double getWeight(String alignid, int chromid, boolean isType2, boolean paired, Integer start, Integer stop,
			Float minWeight, Boolean isLeft, Boolean plusStrand) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
	...
	/**
	 * returns the sorted (ascending order) hit positions in the specified range
	 * of a chromosome,alignment pair.
	 */
	public int[] getPositions(String alignid, int chromid, boolean isType2, boolean paired, Integer start, Integer stop,
			Float minWeight, Boolean isLeft, Boolean plusStrand) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
	...
	/**
	 * returns the hit weights in the specified range of a chromosome,alignment
	 * pair. The weights will be in the same order as the sorted positions
	 * returned by getPositions()
	 */
	public float[] getWeightsRange(String alignid, int chromid, boolean isType2, boolean paired, Integer start,
			Integer stop, Float minWeight, Boolean isLeft, Boolean plusStrand) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
			request.clear();
	...
	public List<SingleHit> getSingleHits(String alignid, int chromid, boolean isType2, Integer start, Integer stop,
			Float minWeight, Boolean plusStrand) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
			request.clear();
	...
	public List<PairedHit> getPairedHits(String alignid, int chromid, boolean isLeft, Integer start, Integer stop,
			Float minWeight, Boolean plusStrand) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
			...
			int numhits = Integer.parseInt(readLine());
			for (int i = 0; i < numhits; i++) {
				output.add(new PairedHit(chromid, 0, false, (short) 0, chromid, 0, false, (short) 0, (float) 0, 0));
			}
			IntBP ints = new IntBP(numhits);
			ReadableByteChannel rbc = Channels.newChannel(instream);
			Bits.readBytes(ints.bb, rbc);
	...
	/**
	 * returns a TreeMap from positions to counts representing a histogram of
	 * the hits in a range with the specified binsize. Bins with a count of zero
	 * are not included in the output.
	 * 
	 * Ex: getHistgram("foo","1+",1,100,10) 6: 5 16: 0 36: 30
	 *
	 * minweight is the minimum weight for reads to be included in the
	 * histogram.
	 *
	 * dedup is the limit on how many times reads with any given 5' position
	 * will be counted. A value of zero means no limit. A limit of, eg, 2, means
	 * that at most two reads at any 5' position will be included in the output.
	 * For weighted histograms, the choice of reads included is unspecified. For
	 * methods that operate on a set of alignments, this many reads from each
	 * alignment will be included.
	 *
	 * Normally, a read is only counted in a single bin as defined by its
	 * position (generally the 5' end of the read). A non-zero read-Extension
	 * counts the read in any bin that you cross within that many bases of the
	 * read's position. A negative value goes backwards (smaller coordinates)
	 * and a larger value goes forward. You need to get the sign right depending
	 * on the strandedness of the chromosome that you're working with.
	 */
	public TreeMap<Integer, Integer> getHistogram(String alignid, int chromid, boolean isType2, boolean paired,
			int extension, int binsize, Integer start, Integer stop, Float minWeight, Boolean plusStrand)
			throws IOException, ClientException {
		return getHistogram(alignid, chromid, isType2, paired, extension, binsize, 0, start, stop, minWeight,
				plusStrand, true);
	}

	public TreeMap<Integer, Integer> getHistogram(String alignid, int chromid, boolean isType2, boolean paired,
			int extension, int binsize, int dedup, Integer start, Integer stop, Float minWeight, Boolean plusStrand,
			boolean isLeft) throws IOException, ClientException {
		synchronized (this) {
			if (!connectionOpen)
				reConnect();
			request.clear();
	...
	public TreeMap<Integer, Float> getWeightHistogram(String alignid, int chromid, boolean isType2, boolean paired,
			int extension, int binsize, Integer start, Integer stop, Float minWeight, Boolean plusStrand)
			throws IOException, ClientException {
		return getWeightHistogram(alignid, chromid, isType2, paired, extension, binsize, 0, start, stop, minWeight,
				plusStrand, true);
	}
	...
	public TreeMap<Integer, Integer> getHistogram(Collection<String> alignids, int chromid, boolean isType2,
			boolean paired, int extension, int binsize, Integer start, Integer stop, Float minWeight,
			Boolean plusStrand) throws IOException, ClientException {
		return getHistogram(alignids, chromid, isType2, paired, extension, binsize, 0, start, stop, minWeight,
				plusStrand);
	}

	public TreeMap<Integer, Integer> getHistogram(Collection<String> alignids, int chromid, boolean isType2,
			boolean paired, int extension, int binsize, int dedup, Integer start, Integer stop, Float minWeight,
			Boolean plusStrand) throws IOException, ClientException {
		TreeMap<Integer, Integer> output = null;
		for (String alignid : alignids) {
			TreeMap<Integer, Integer> o = getHistogram(alignid, chromid, isType2, paired, extension, binsize, dedup,
					start, stop, minWeight, plusStrand, true);
			if (paired) // run for isLeft =true & false
				o.putAll(getHistogram(alignid, chromid, isType2, paired, extension, binsize, dedup, start, stop,
						minWeight, plusStrand, false));
			for (int k : o.keySet()) {
				if ((k - start - binsize / 2) % binsize != 0) {
					System.err.println(String.format("Bad key %d for binsize %d and start %d in %s,%d", k, binsize,
							start, alignid, chromid));
				}
			}
	...
	public TreeMap<Integer, Float> getWeightHistogram(Collection<String> alignids, int chromid, boolean isType2,
			boolean paired, int extension, int binsize, Integer start, Integer stop, Float minWeight,
			Boolean plusStrand) throws IOException, ClientException {
		return getWeightHistogram(alignids, chromid, isType2, paired, extension, binsize, 0, start, stop, minWeight,
				plusStrand);
	}
	...
	public TreeMap<Integer, Float> getWeightHistogram(Collection<String> alignids, int chromid, boolean isType2,
		...
		for (String alignid : alignids) {
			TreeMap<Integer, Float> o = getWeightHistogram(alignid, chromid, isType2, paired, extension, binsize, dedup,
					start, stop, minWeight, plusStrand, true);
			if (paired) // run for isLeft =true & false
				o.putAll(getWeightHistogram(alignid, chromid, isType2, paired, extension, binsize, dedup, start, stop,
						minWeight, plusStrand, false));
			if (output == null) {
				output = o;
			} else {
	...
	/**
	 * Returns a Map from READ, WRITE, and ADMIN to lists of principals that
	 * have those privileges on the specified alignment.
	 */
	...
	/*
	 * fills one section of the acl output data structure. A section is either
	 * read, write, or admin.
	 */
	...
	/**
	 * Applies the specified ACLChangeEntry objects to the acl for this
	...
	/**
	 * ClientConnectionTimerThread closes the connection if it is idle for too
	 * long
	 * 
	 * @author mahony
	 *
		...
		public void run() {
			while (true) {
				try {
					...
					// lastActivityTime)+")\tconnectionOpen="+connectionOpen);
					if (connectionOpen && System.currentTimeMillis() - lastActivityTime > connectionIdleTimeLimit) {
						parent.closeConnection();
						// System.out.println("ClientConnectionTimerThread:
						// connection closed");
					}
...
class ClientCallbackHandler implements CallbackHandler {
	...
	public ClientCallbackHandler(String n, String p) {
		name = n;
		pass = p;
	}
