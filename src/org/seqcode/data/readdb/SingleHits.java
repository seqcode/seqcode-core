...
/**
 * Represents the list of sorted reads on disk. Some alignments store the
 * standard SingleHits and "type 2" SingleHits separately. Type 2 SingleHits are
 * used in alignments where the read 2 hits represent something different from
 * read 1 hits. For example, in paired-end ChIP-exo, where only read 1 is
 * affected by exonuclease.
 */
public class SingleHits extends Hits {
	/**
	 * Initializes a Hits object from a file
	 */
	public SingleHits(String prefix, int chrom, boolean type2)
			throws FileNotFoundException, SecurityException, IOException {
		super(chrom, getPositionsFname(prefix, chrom, type2), getWeightsFname(prefix, chrom, type2),
				getLaSFname(prefix, chrom, type2));
	}

	public static void writeSingleHits(IntBP positions, FloatBP weights, IntBP las, String prefix, int chrom,
	...
	public static void writeSingleHits(SingleHit[] hits, String prefix, int chrom, boolean type2) throws IOException {
	...
	public void appendSingleHits(SingleHit[] hits, String prefix, int chrom, boolean type2) throws IOException {
		if (hits.length == 0) {
			return;
		}
		int s = getPositionsBuffer().limit() - 1;
		SingleHit last = new SingleHit(chrom, getPositionsBuffer().get(s), getWeightsBuffer().get(s),
				Hits.getStrandOne(getLASBuffer().get(s)), Hits.getLengthOne(getLASBuffer().get(s)));
	...
	private void append(SingleHit[] hits, String prefix, int chrom, boolean type2) throws IOException {
	...
	private void merge(SingleHit[] hits, String prefix, int chrom, boolean type2) throws IOException {
		String postmp = getPositionsFname(prefix, chrom, type2) + ".tmp";
		String weightstmp = getWeightsFname(prefix, chrom, type2) + ".tmp";
		String lastmp = getLaSFname(prefix, chrom, type2) + ".tmp";
		RandomAccessFile positionsRAF = new RandomAccessFile(postmp, "rw");
		...
		IntBP posfile = new IntBP(positionsRAF.getChannel().map(FileChannel.MapMode.READ_WRITE, 0, newsize * 4));
		FloatBP weightfile = new FloatBP(weightsRAF.getChannel().map(FileChannel.MapMode.READ_WRITE, 0, newsize * 4));
		IntBP lasfile = new IntBP(lasRAF.getChannel().map(FileChannel.MapMode.READ_WRITE, 0, newsize * 4));
