...
/**
 * <p>
 * Generic command line client for the readdb Usage:
 * 
 * <pre>
 * ReadDB -h nanog.csail.mit.edu -P 5200 -u arolfe -p SECRET <command>
 * .....</apre> omitting connection info uses default connection info in
 * ~/.readdb_passwd
 *
 * <p>
 * command can be:
 ...
 * 
 * <p>
 * The --paired flag can be provided to make getweight, getcount, and getchroms
 * work on paired-end rather than single-end alignments
 * 
 * <p>
 * The --type2 flag can be provided to make getweight, getcount, and getchroms
 * work on type2 single-end alignments
 *
 */
public class ReadDB {
			...
			if (cmd.equals("exists")) {
				...
				if (cmd.equals("getchroms")) {
					...
					if (otherargs.length == 3) {
						count = client.getCount(align, Integer.parseInt(otherargs[2]), isType2, paired, null, null,
								null, isleft, null);
					} else {
					...
					double weight = 0;
					if (otherargs.length == 3) {
						weight = client.getWeight(align, Integer.parseInt(otherargs[2]), isType2, paired, null, null,
								null, isleft, null);
					} else {
