...
public class PairedHits extends Hits {
	...
	/*
	 * true if the positions (and chrom) for these PairedHits are the left side,
	 * meaning that the other chrom and position are for the right side. If
	 * false, then the positions are for the right side and the other chrom and
	...
	public PairedHits(String prefix, int chrom, boolean isLeft)
			throws FileNotFoundException, SecurityException, IOException {
		super(chrom, getPositionsFname(prefix, chrom, isLeft), getWeightsFname(prefix, chrom, isLeft),
	...
	public IntBP getPairCodeBuffer() {
		return pairCode;
	}

	public IntBP getOtherChromsBetween(int firstindex, int lastindex, int start, int stop, Float minweight,
			Boolean isPlus) throws IOException {
		return getIntsBetween(chroms, firstindex, lastindex, start, stop, minweight, isPlus);
	}

	public IntBP getPairCodesBetween(int firstindex, int lastindex, int start, int stop, Float minweight,
			Boolean isPlus) throws IOException {
		return getIntsBetween(pairCode, firstindex, lastindex, start, stop, minweight, isPlus);
	}

	public IntBP getOtherPositionsBetween(int firstindex, int lastindex, int start, int stop, Float minweight,
			Boolean isPlus) throws IOException {
	...
	/**
	 * hits is a sorted list. prefix is file name prefix chrom is chromosome
	 * number (must match left if isLeft is true and right otherwise)
	 */
	public static void writePairedHits(List<PairedHit> hits, String prefix, int chrom, boolean isLeft)
			throws IOException {
	...
	public void appendPairedHits(List<PairedHit> hits, String prefix, int chrom, boolean isLeft) throws IOException {
		if (hits.size() == 0) {
			return;
		}
		...
		PairedHit last = new PairedHit(isLeft ? chrom : chroms.get(s),
				...
				!isLeft ? chrom : chroms.get(s), !isLeft ? getPositionsBuffer().get(s) : otherPositions.get(s),
				!isLeft ? getStrandOne(getLASBuffer().get(s)) : getStrandTwo(getLASBuffer().get(s)),
				!isLeft ? getLengthOne(getLASBuffer().get(s)) : getLengthTwo(getLASBuffer().get(s)),

				getWeightsBuffer().get(s), getPairCodeBuffer().get(s));
	...
	private void append(List<PairedHit> hits, String prefix, int chrom, boolean isLeft) throws IOException {
	...
	private void merge(List<PairedHit> hits, String prefix, int chrom, boolean isLeft) throws IOException {
		String postmp = getPositionsFname(prefix, chrom, isLeft) + ".tmp";
		String weightstmp = getWeightsFname(prefix, chrom, isLeft) + ".tmp";
		String paircodetmp = getPairCodeFname(prefix, chrom, isLeft) + ".tmp";
		String lastmp = getLaSFname(prefix, chrom, isLeft) + ".tmp";
		...
		if (isLeft) {
			for (int i = 0; i < hits.size(); i++) {
				...
				while (oldpos < positions.limit()
						&& (positions.get(oldpos) < h.leftPos || (positions.get(oldpos) == h.leftPos
			...
			for (int i = 0; i < hits.size(); i++) {
				PairedHit h = hits.get(i);
				while (oldpos < positions.limit()
						&& (positions.get(oldpos) < h.rightPos || (positions.get(oldpos) == h.rightPos
