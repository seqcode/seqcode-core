...
public abstract class Hits implements Closeable {
	...
	/**
	 * returns just the length from an int representing a length and strand
	 */
	...
	public static int makeLAS(short lengthOne, boolean strandOne, short lengthTwo, boolean strandTwo) {
		return (lengthOne & lenOneMask) | ((strandOne ? 1 : 0) << 15) | ((lengthTwo & lenOneMask) << 16)
				| ((strandTwo ? 1 : 0) << 31);
	}
	...
	public Hits(int chrom, String positionsFname, String weightsFname, String lasFname)
	...
	/**
	 * returns indices = int[2] such that indices[0] is the first element of
	 * positions >= startpos and indices[1] is the first element of positions >
	 * lastpos. indices[0] == indices[1] -> no hits between startpos and lastpos
	...
	public int[] getIndices(int firstindex, int lastindex, int startpos, int lastpos) {
		...
		int indices[] = new int[2]; // will be the output
		/*
		 * this is a binary search in two steps. In the first step, we establish
		 * the upper bound for the binary search by looking out in windows of
		 * doubling size until we've bounded the spot we're looking for. We do
		 * this because a binary search over the entire array of hits would
		 * defeat the purpose of the index in Header. The second step is the
		 * actual binary search using firstindex as the lower bound and the
		...
		while (firstindex + step < limit && positions.getib().get(firstindex + step) < startpos) {
			firstindex += step;
			step *= 2;
		}
		while (firstindex < limit && positions.getib().get(firstindex) < startpos) {
			step = (step >> 2) | 1;
			if (step == 1 || (firstindex + step < limit && positions.getib().get(firstindex + step - 1) < startpos)) {
				firstindex += step;
			}
		}

		step = 256;
		while (lastindex - step > firstindex && positions.getib().get(lastindex - step) > lastpos) {
			lastindex -= step;
			step *= 2;
		}
		while (lastindex > firstindex && positions.getib().get(lastindex - 1) > lastpos) {
			step = (step >> 2) | 1;
			if ((step == 1 || lastindex - step > firstindex) && positions.getib().get(lastindex - step) > lastpos) {
				lastindex -= step;
			}
		}
	...
	/**
	 * Returns the number of elements between start and stop where firstindex
	 * and lastindex are the lower and upper bounds to search
	 */
	public int getCountBetween(int firstindex, int lastindex, int start, int stop, Float minweight, Boolean isPlus)
			throws IOException {
		assert (firstindex >= 0);
		assert (lastindex >= firstindex);
		assert (lastindex <= positions.getib().limit());
	...
	public double getWeightBetween(int firstindex, int lastindex, int start, int stop, Float minweight, Boolean isPlus)
			throws IOException {
		assert (firstindex >= 0);
		assert (lastindex >= firstindex);
		assert (lastindex <= positions.getib().limit());
	...
	public IntBP getIntsBetween(IntBP buffer, int firstindex, int lastindex, int start, int stop, Float minweight,
	...
	/**
	 * returns the hits between firstindex and last index and between start and
	 * stop, inclusive. firstindex and lastindex come from Header.getFirstIndex
	 * and Header.getLastIndex
	 */
	public IntBP getHitsBetween(int firstindex, int lastindex, int start, int stop, Float minweight, Boolean isPlus)
			throws IOException {
		return getIntsBetween(positions, firstindex, lastindex, start, stop, minweight, isPlus);
	}

	public IntBP getLASBetween(int firstindex, int lastindex, int start, int stop, Float minweight, Boolean isPlus)
			throws IOException {
		return getIntsBetween(lenAndStrand, firstindex, lastindex, start, stop, minweight, isPlus);
	}

	/**
	 * returns the weights between firstindex and last index and between start
	 * and stop, inclusive. firstindex and lastindex come from
	 * Header.getFirstIndex and Header.getLastIndex
	 */
	public FloatBP getWeightsBetween(int firstindex, int lastindex, int start, int stop, Float minweight,
			Boolean isPlus) throws IOException {
		assert (firstindex >= 0);
		assert (lastindex >= firstindex);
		assert (lastindex <= positions.getib().limit());
	...
	/**
	 * return a histogram from positions start to stop in units of stepsize.
	 * firstindex and lastindex come from Header.getFirstIndex(start) and
	 * Header.getLastIndex(stop)
	 *
	 * If extension is zero, histograms are based on 5' positions. If extension
	 * is negative, histograms are based on read lengths If extension is
	 * positive, histograms are based on 5' position + extension
	 */
	public int[] histogram(int firstindex, int lastindex, int start, int stop, int stepsize, int dedup, Float minweight,
			Boolean isPlus, int extension) throws IOException {
			...
			for (int i = p[0]; i < p[1]; i++) {
				...
				if (pos < start || pos > stop) {
					System.err.println(String.format(
							"firstindex %d lastindex %d start %d stop %d p[0] %d p[1] %d positions[p[0]] %d i %d pos %d",
	...
	public float[] weightHistogram(int firstindex, int lastindex, int start, int stop, int stepsize, int dedup,
			Float minweight, Boolean isPlus, int extension) throws IOException {
