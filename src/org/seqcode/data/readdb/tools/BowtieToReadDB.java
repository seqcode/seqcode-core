...
/**
 * Reads Bowtie output on stdin. Produces a file on stdout in the format
...
public class BowtieToReadDB {
	...
	/*
	 * ImportHits (where the printed lines are eventually used) takes the
	 * average between the start and stop positions in the coordinate. I think
	 * it is most correct to have this value reflect the 5' coordinate for each
	 * hit. This is why I do the calculation gymnastics for the "coord" String
	 * below (even though this will slow imports down a bit) and also why the
	...
	public static void printLines(ArrayList<String[]> lines, boolean uniqueOnly, boolean filterSubOpt) {
		...
		if (!uniqueOnly || linesToPrint.size() == 1) {
			...
			for (String[] pieces : linesToPrint) {
				...
				Integer coord = pieces[1].equals("+") ? Integer.valueOf(pieces[3])
						: Integer.valueOf(pieces[3]) + readLength - 1;
				coord += 1; // Bowtie default output is 0-based - we want
							// 1-based.
				System.out.println(
