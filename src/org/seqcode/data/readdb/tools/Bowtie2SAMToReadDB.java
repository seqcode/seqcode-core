 ...
 * expected by ImportHits. Ignores supplementary (i.e. chimeric) alignments.
 * 
 ...
 * reads as pairs) --concordantonly (flag to print only concordant pairs)
 * --read1 (flag to print only read 1 hits) --read2 (flag to print only read 2
 * hits)
 */
...
public class Bowtie2SAMToReadDB {
	...
	public static void main(String args[]) throws IOException, ParseException {
		...
		String lastName = "";
		while (iter.hasNext()) { // Group neighboring reads by name (note this
									// will group read pair hits too)
	...
	public static void processRecord(List<SAMRecord> records) {
		...
		for (SAMRecord record : records) {
			...
			if (inclPairedEnd || inclJunction) {
				...
				/*
				 * Only accept proper pairs, optionally only concordant. It also
				 * assumes that the left and right mates have the same length,
				 * and that there are no gaps in the second mate alignment (SAM
				 * doesn't store the paired read's end) Note: if you change
				 * this, you may have to change the SAMStats output also
				 */
				if (inclPairedEnd) {
					// Check this if using bowtie2 to produce multiple mappings
					// for each read pair. could be problematic
					if (record.getReadPairedFlag() && record.getFirstOfPairFlag() && record.getProperPairFlag()
							&& (!concordantOnly || record.getStringAttribute("YT").equals("CP"))) {
