...
public class SAMStats {
	...
	public void processSAMRecord(SAMRecord r) {
		...
		if (!r.getReadUnmappedFlag()) {
			totalHits++;
			int count = 1; // Have to figure out something for BWA when
							// reporting multiple alignments
			if (r.getIntegerAttribute("NH") != null)
				count = r.getIntegerAttribute("NH");
			if (count == 1 && r.getMappingQuality() != 0) // Second clause for
															// BWA
				uniquelyMapped++;
	...
	public void processBT1SAMRecord(SAMRecord r) {
		...
		if (!r.getReadUnmappedFlag())
			if (r.getIntegerAttribute("XM") != null) {
				...
				totalHits++;
				int count = 1; // TODO: Fix this if using bowtie for
								// multi-mapping reads
	...
	public void processBT2SAMRecord(SAMRecord r) {
		...
		if (!r.getReadUnmappedFlag()) {
			totalHits++;
			int count = 1; // TODO: Fix this if using bowtie2 for multi-mapping
							// reads
