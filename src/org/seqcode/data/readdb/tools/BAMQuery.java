 ...
 *
 * Usage: Query --data http://nanog.csail.mit.edu/readdb-test/foo.bam --index
 * foo.index [--quiet]
 *
 * --quiet means don't print any output. This is useful for testing the query
 * performance without worrying about the time it takes to print the output.
 *
...
public class BAMQuery {
	...
	public void parseArgs(String args[]) throws IllegalArgumentException, ParseException {
		...
		options.addOption("H", "histogram", true,
		...
		while ((line = reader.readLine()) != null) {
			...
			if (histogram > 0) {
				...
				while (iter.hasNext()) {
					...
					int pos = record.getReadNegativeStrandFlag() ? record.getAlignmentEnd()
				...
				while (iter.hasNext()) {
					...
					if (!quiet) {
						System.out.println(String.format("chrom %s, pos %d, %s, len %d", record.getReferenceName(),
								record.getReadNegativeStrandFlag() ? record.getAlignmentEnd()
										: record.getAlignmentStart(),
								record.getReadNegativeStrandFlag() ? "-" : "+", record.getReadLength()));
					}
