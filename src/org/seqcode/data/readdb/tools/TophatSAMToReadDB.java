...
public class TophatSAMToReadDB {
	...
	public static void processRecord(SAMRecord record) {
		...
		// Grouping hits will not work, as Tophat BAM output is sorted by
		// genomic location
		...
		if (inclPairedEnd || inclJunction) {
			/*
			 * Okay, so the paired-end part is just hacked together for now. It
			 * only accepts true pairs. It also assumes that the left and right
			 * mates have the same length, and that there are no gaps in the
			 * second mate alignment (SAM doesn't store the paired read's end)
			 * Note: if you change this, you may have to change the SAMStats
			 * output also
			 */
			if (inclPairedEnd) {
				if (record.getReadPairedFlag() && !record.getNotPrimaryAlignmentFlag() && record.getFirstOfPairFlag()
						&& record.getProperPairFlag() && record.getReadPairedFlag()) {
			...
			/*
			 * Outputs as paired alignments those reads that are aligned in >2
			 * blocks Note: if you change this, you may have to change the
			 * SAMStats output also
			 */
			if (inclJunction) {
				...
				if (blocks.size() >= 2) {
					for (int ab = 0; ab < blocks.size() - 1; ab++) {
						...
						System.out.println(refname + (neg ? lEnd : lStart) + "\t" + (neg ? "-\t" : "+\t") + lLen + "\t"
								+ refname + (neg ? rEnd : rStart) + "\t" + (neg ? "-\t" : "+\t") + rLen + "\t" + weight
								+ "\t" + 0);
					}
				}
			}
			...
			List<AlignmentBlock> blocks = record.getAlignmentBlocks();
			for (int a = 0; a < blocks.size(); a++) { // Iterate over alignment
														// blocks
				...
				while (nearbyBlocks && a < blocks.size() - 1) {
					if (blocks.get(a + 1).getReferenceStart() - currBlock.getReferenceStart() < record
							.getReadLength()) {
				...
				boolean neg = record.getReadNegativeStrandFlag();
				System.out.println(record.getReferenceName() + "\t" + (neg ? aEnd : aStart) + "\t"
						+ (neg ? "-\t" : "+\t") + aLen + "\t" + weight);
			}
		}
