 ...
 * 
 * The matching of reads between files is done by stripping "/\d" from the end
 * of the read name, as reads usually end in /1 or /2.
 *
 * Usage: java PairedSAMToReadDB --left leftreads.bam --right rightreads.bam
 *
 *
 * Options: --nosuboptimal (flag to only take the hits with the minimum number
 * of mismatches) --uniquehits (flag to only print 1:1 read to hit mappings)
 * 
 * nosuboptimal is applied before uniquehits
 *
 * Output columns are 1) left chromname 2) left position 3) left strand 4) left
 * readlen 5) right chromname 6) right position 7) right strand 8) right length
 * 9) weight 10) paircode
 */
...
public class PairedSAMToReadDB {
	...
	public static void dumpRecords(Collection<SAMRecord> lefts, Collection<SAMRecord> rights) {
		if (filterSubOpt) {
		...
		for (SAMRecord left : lefts) {
			for (SAMRecord right : rights) {
				System.out.println(String.format("%s\t%d\t%s\t%d\t%s\t%d\t%s\t%d\t%f\t%d", left.getReferenceName(),
						left.getReadNegativeStrandFlag() ? left.getAlignmentEnd() : left.getAlignmentStart(),
						left.getReadNegativeStrandFlag() ? "-" : "+", left.getReadLength(),

						right.getReferenceName(),
						right.getReadNegativeStrandFlag() ? right.getAlignmentEnd() : right.getAlignmentStart(),
						right.getReadNegativeStrandFlag() ? "-" : "+", right.getReadLength(),
			...
			for (int i = 0; i < lbs; i++) {
				...
				while (j < rightbuffer.size()) {
					if (readname.equals(rightbuffer.get(j).getReadName())) {
						/*
						 * having found a match, find the rest of the reads with
						 * that ID and output
						 */
						int k = i;
						int l = j;
						/*
						 * make sure we'll be able to find all reads with this
						 * name
						 */
	...
	public static void main(String args[]) throws IOException, ParseException {
		...
		options.addOption("s", "nosuboptimal", false,
