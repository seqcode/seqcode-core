...
/**
 * ServerTask represents a client connection. Server creates ServerTasks when it
 * receives a connection and passes them to Dispatch. Dispatch manages a pool of
 * WorkerThreads and assigns them to ServerTasks as the tasks appear to be
 * available.
 * 
 * The current ServerTasks shut themselves down if they haven't been run in a
 * given time period.
 * 
 */
...
public class ServerTask {
	...
	/*
	 * true if the client sent a close command. If the client sent a close
	 * command or the thread has detected an error from the client, then
	 * shouldClose is true and the server will close the connection.
	 */
	...
	/*
	 * other variables maintained across calls to Run but reset between
	 * connections
	 */
	...
	/*
	 * The shouldClose method checks the last activity time (set in run method)
	 * and returns true if past the time limit. Other methods set shouldClose
	 * after exceptions or once the task is finished.
	 */
	public boolean shouldClose() {
		...
		if (limitInactivity && System.currentTimeMillis() - lastActivity > taskInactivityLimit) {
			shouldClose = true;
			server.getLogger().log(Level.INFO, "serverTask",
					"connection idle more than " + taskInactivityLimit + "ms: closing connection");
		}
		...
		try {
			...
			if (avail || haventTriedRead++ > 1000) {
				...
				if (r == -1) {
					...
					// System.err.println("iA has read through " + new
					// String(buffer,0,bufferpos));
				}
			}
		} catch (SocketTimeoutException e) {
			// timeout means the socket is still open but there's no data.
			// That's ok.
			try {
				socket.setSoTimeout(1000000);
	...
	public String readLine() throws IOException {
		...
		while (instream.available() > 0 && (i = instream.read()) != -1) {
			if (i == '\n') {
				done = true;
				break;
			} else {
				if (bufferpos >= buffer.length) {
					server.getLogger().logp(Level.WARNING, "ServerTask", "readLine " + toString(),
							"readline would overflow.  quitting");
					shouldClose = true;
					return null;
				}
	...
	/**
	 * main method for the task. This method is asynchronous- it shouldn't block
	 * too long on the client. It does block on disk reads and such and it does
	 * block on the client while waiting, eg, for more hits to store. It does
	 * not block while reading parameters or waiting for the next command.
	 *
	 * The asynchronous behavior is achieved through readLine(), which returns
	 * null if it doesn't have a complete line. Typically run then returns and
	 * waits until it's called again, at which point there is hopefully a
	 * complete line to deal with. aside from authenticate(), the basic
	 * procedure is - read the type of request - read the list of request
	 * parameters - call processRequest()
	 *
	 * Run CANNOT throw any exceptions in the current model. If the outer block
	 * gets an exception, it swallows it and returns, setting shouldClose = true
	 * to indicate that this connection to a client should be closed.
	 */
	public void run() {
		try {
			lastActivity = System.currentTimeMillis();
			if (username == null) {
				...
				server.getLogger().logp(Level.INFO, "ServerTask", "run " + toString(),
						" authenticated " + username + " from " + socket.getInetAddress() + ":" + socket.getPort());
	...
	/**
	 * performs authentication exchange over the socket and sets the username
	 * field if successful. Returns true if authenticate should continue or is
	 * successful. Returns false if authenticate has failed.
	 */
	public boolean authenticate() throws IOException {
		/*
		 * The SASL client and server give you back bytes to send to the other
		 * side. We achieve this by sending a length line first (ascii encoded
		 * integer followed by '\n') and then the raw bytes of the SASL
		 * exchange. Two complexities: 1) input.read() doesn't necessarily read
		 * the expected number of bytes all at once, so we have to loop around
		 * it until it does. 2) I had problems with isComplete() returning true
		 * at different times in the client and server. The server sends an
		 * isComplete() byte at the end of the loop to tell the client when it's
		 * done.
		 */
		if (uname == null) {
			uname = readLine();
		...
		if (sasl == null) {
			sasl = Sasl.createSaslServer(Server.SaslMechanisms[0], "readdb",
					socket.getInetAddress().getCanonicalHostName(), saslprops,
		...
		if (sasl == null || sasl.isComplete()) {
			...
			outstream.flush();
			server.getLogger().logp(Level.INFO, "ServerTask", "authenticate " + toString(),
					"Failed Authentication for " + uname);
			return false;
		}
	...
	public void processRequest() {
		try {
			...
			if (request.type.equals("ping")) {
				...
				server.getLogger().logp(Level.INFO, "ServerTask", "processRequest " + toString(),
						"Received shutdown from " + username);
			...
			server.getLogger().logp(Level.INFO, "ServerTask", "processRequest " + toString(),
					"Error in request " + request.toString());
			server.getLogger().logp(Level.INFO, "ServerTask", "processRequest " + toString(),
	...
	/**
	 * Handles the subset of requests that deal with a particular file that we
	 * expect to exist
	 */
	public void processFileRequest() throws IOException {
		...
		try {
			...
			// happens if the file doesn't exist or if we can't read it at the
			// OS level
			server.getLogger().logp(Level.INFO, "ServerTask", "processFileRequest " + toString(),
					String.format("read error on acl for %s : %s", request.alignid, e.toString()));
			printInvalid(e.toString());
			return;
		}
		if (!authorizeRead(acl)) {
			server.getLogger().logp(Level.INFO, "ServerTask", "processFileRequest " + toString(),
					String.format("%s can't read %s", username, request.alignid));
			printAuthError();
			return;
		}
		Header header = null;
		...
		try {
			...
			// We already know the alignment exists and we have permissions at
			// this point.
			// This exception may happen if the file doesn't exist or if we
			// can't read it at the OS level
			// Let's choose to ignore the exception for now, and to have the
			// methods below behave like there are 0 hits in this case
			header = null;
			hits = null;

			/*
			 * server.getLogger().logp(Level.INFO,
			 * "ServerTask","processFileRequest " + toString(),
			 * String.format("read error on header or hits for %s, %d, %s : %s",
			 * request.alignid, request.chromid, request.isLeft, e.toString()));
			 * printInvalid(e.toString()); return;
			 */
		}
	...
	/**
	 * reads two lines from socket: alignment id and chromosome id. returns
	 * "exists" or "unknown" to indicate whether the server knows about that
	 * pair
	 */
	...
	public void processGetChroms() throws IOException {
		...
		Set<Integer> chroms = server.getChroms(request.alignid, request.isType2, request.isPaired, request.isLeft);
		if (chroms == null) {
			printString("No Such Alignment\n");
			return;
		}
	...
	/**
	 * Deletes an alignment: the header and hits files, acl file, and the
	...
	public void processDeleteAlignment() throws IOException {
		...
		/*
		 * list of files to delete: datafiles first, then the directory itself
		 */
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (request.isPaired == null) {
				...
				boolean pairedfile = name.indexOf(".prleft.") > 0 || name.indexOf(".prright.") > 0
						|| name.indexOf(".pairedleftindex") > 0 || name.indexOf(".pairedrightindex") > 0
				...
				boolean singlefile = name.indexOf("singleindex") > 0 || name.indexOf("singlet2index") > 0
						...
						|| name.indexOf("sweights") > 0 || name.indexOf("st2weights") > 0 || name.indexOf("slas") > 0
						|| name.indexOf("st2las") > 0;
	 ...
	 *
	 * If the chromosome file doesn't exist yet, then create a new one and dump
	 * in positions and weights. If it does exist, then create a new file and
	 * merge the old file with the new set of hits.
	 */
	public void processSingleStore() throws IOException {
		...
		for (int i = 0; i < numHits; i++) {
			newhits[i] = new SingleHit(request.chromid, positions.get(i), weights.get(i), Hits.getStrandOne(las.get(i)),
					Hits.getLengthOne(las.get(i)));
		}
		for (int i = 1; i < newhits.length; i++) {
			if (newhits[i - 1].compareTo(newhits[i]) > 0) {
				throw new RuntimeException(String.format("at %d : %d vs %d", i, newhits[i - 1].pos, newhits[i].pos));
			}
		}
		...
		try {
			if (chroms != null && chroms.contains(request.chromid)) {
				try {
					...
					server.getLogger().logp(Level.INFO, "serverTask", "processSingleStore " + toString(), e.toString(),
							e);
					printInvalid(e.toString());
					return;
				}
				try {
					server.getSingleHits(request.alignid, request.chromid, request.isType2).appendSingleHits(newhits,
							server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"),
							request.chromid, request.isType2);
				} catch (Exception e) {
					server.getLogger().logp(Level.INFO, "ServerTask", "processSingleStore " + toString(),
							"error writing hits", e);
					printInvalid(e.toString());
					return;
				}
			} else {
				...
				if (!dir.exists() && !dir.mkdirs()) {
					server.getLogger().logp(Level.INFO, "ServerTask", "processSingleStore " + toString(),
							"Can't create directories for " + request.alignid + ":"
									+ server.getAlignmentDir(request.alignid));
					printAuthError();
					return;
				}
				...
				acl.writeToFile(server.getACLFileName(request.alignid));
				server.removeACL(request.alignid); // make sure the server
													// doesn't have this ACL
													// cached
				SingleHits.writeSingleHits(newhits,
						server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"), request.chromid,
						request.isType2);
			}
			SingleHits singlehits = new SingleHits(
					server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"), request.chromid,
					request.isType2);
			Header header = new Header(singlehits.getPositionsBuffer().getib());
			header.writeIndexFile(server.getSingleHeaderFileName(request.alignid, request.chromid, request.isType2));
		} catch (IOException e) {
			server.getLogger().logp(Level.INFO, "ServerTask", "processSingleStore " + toString(),
					"IOException trying to save files : " + e.toString(), e);
			return;
		}
		printOK();
	...
	public void processPairedStore() throws IOException {
		...
		try {
			...
			server.getLogger().logp(Level.INFO, "ServerTask", "processPairedStore " + toString(),
					"Invalid numhits " + request.map.get("numhits"), e);
			printString("Invalid numhits value : " + request.map.get("numhits") + "\n");
			return;
		}
		...
		if (!f.exists()) {
			...
			acl.getReadACL().add(username);
			acl.writeToFile(server.getACLFileName(request.alignid));
			server.removeACL(request.alignid); // make sure the server doesn't
												// have this ACL cached
		} else {
			try {
				/* sure we're allowed to write here */
				AlignmentACL acl = server.getACL(request.alignid);
				...
				server.getLogger().logp(Level.INFO, "ServerTask", "processPairedStore " + toString(),
						"IOException reading acls : " + e.toString(), e);
				printInvalid(e.toString());
				return;
			}
		}
		...
		for (int i = 0; i < positions.limit(); i++) {
			hits[i] = new PairedHit(request.chromid, positions.get(i), Hits.getStrandOne(las.get(i)),
					Hits.getLengthOne(las.get(i)), otherchrom.get(i), otherpos.get(i), Hits.getStrandTwo(las.get(i)),
					Hits.getLengthTwo(las.get(i)), weights.get(i), paircodes.get(i));
		}
		positions = null;
		weights = null;
		paircodes = null;
	...
	private void appendPairedHits(PairedHit[] newhits, boolean isLeft) throws IOException {
		/*
		 * instead of the map, could have a list passed, sort, and then use
		 * subList to save some memory
		 */
		...
		for (int chromid : map.keySet()) {
			...
			try {
				PairedHits oldhits = server.getPairedHits(request.alignid, chromid, isLeft);
				oldhits.appendPairedHits(nhlist,
						server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"), chromid,
						isLeft);
			} catch (FileNotFoundException e) {
				PairedHits.writePairedHits(nhlist,
						server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"), chromid,
						isLeft);
			}
			PairedHits pairedhits = new PairedHits(
					server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"), chromid, isLeft);
			Header header = new Header(pairedhits.getPositionsBuffer().getib());
			header.writeIndexFile(server.getPairedHeaderFileName(request.alignid, chromid, isLeft));
	...
	public void processReindex() throws IOException {
		...
		if (request.isPaired) {
			...
			header.writeIndexFile(server.getPairedHeaderFileName(request.alignid, request.chromid, true));
			...
			header.writeIndexFile(server.getPairedHeaderFileName(request.alignid, request.chromid, false));
	...
	public void processCount(Header header, Hits hits) throws IOException {
		if (header == null || hits == null) {
			...
			printOK();
			if (request.start == null && request.end == null && request.minWeight == null
			...
			printString(Integer.toString(hits.getCountBetween(first, last, request.start, request.end,
					request.minWeight, request.isPlusStrand)) + "\n");
		}
	}
	...
	public void processWeight(Header header, Hits hits) throws IOException {
		if (header == null || hits == null) {
			...
			printString(Double.toString(hits.getWeightBetween(first, last, request.start, request.end,
					request.minWeight, request.isPlusStrand)) + "\n");
		}
	}
	...
	public void processGetHits(Header header, Hits hits) throws IOException {
		if (header == null || hits == null) {
			...
			if (!(hits instanceof PairedHits)) {
				if (request.map.containsKey("wantotherchroms") || request.map.containsKey("wantotherpositions")
			...
			int first = header.getFirstIndex(request.start);
			int last = header.getLastIndex(request.end);
			if (request.start == 0 && request.end == Integer.MAX_VALUE && request.minWeight == null
					&& request.isPlusStrand == null) {
				count = header.getNumHits();
			} else {
				count = hits.getCountBetween(first, last, request.start, request.end, request.minWeight,
						request.isPlusStrand);
			}
			printOK();
			printString(Integer.toString(count) + "\n");
			if (request.map.containsKey("wantpositions")) {
				IntBP p = hits.getHitsBetween(first, last, request.start, request.end, request.minWeight,
						request.isPlusStrand);
				Bits.sendBytes(p.bb, outchannel);
			}
			if (request.map.containsKey("wantweights")) {
				FloatBP p = hits.getWeightsBetween(first, last, request.start, request.end, request.minWeight,
						request.isPlusStrand);
				Bits.sendBytes(p.bb, outchannel);
			}
			if (request.map.containsKey("wantpaircodes")) {
				IntBP p = ((PairedHits) hits).getPairCodesBetween(first, last, request.start, request.end,
						request.minWeight, request.isPlusStrand);
				Bits.sendBytes(p.bb, outchannel);
			}
			if (request.map.containsKey("wantlengthsandstrands")) {
				IntBP p = hits.getLASBetween(first, last, request.start, request.end, request.minWeight,
						request.isPlusStrand);
				Bits.sendBytes(p.bb, outchannel);
			}
			if (request.map.containsKey("wantotherchroms")) {
				IntBP p = ((PairedHits) hits).getOtherChromsBetween(first, last, request.start, request.end,
						request.minWeight, request.isPlusStrand);
				Bits.sendBytes(p.bb, outchannel);
			}
			if (request.map.containsKey("wantotherpositions")) {
				IntBP p = ((PairedHits) hits).getOtherPositionsBetween(first, last, request.start, request.end,
						request.minWeight, request.isPlusStrand);
				Bits.sendBytes(p.bb, outchannel);
			}
		}
	...
	public void processHistogram(Header header, Hits hits) throws IOException {
		if (header == null || hits == null) {
			...
			try {
				...
				server.getLogger().logp(Level.INFO, "ServerTask", "processHistogram " + toString(),
						"Exception parsing binsize : " + request.map.get("binsize"), e);
			...
			int first = header.getFirstIndex(request.start);
			int last = header.getLastIndex(request.end);
			int[] raw = hits.histogram(first, last, request.start, request.end, binsize, dedup, request.minWeight,
					request.isPlusStrand, extension);
			int n = 0;
	...
	/*
	 * returns a histogram of hit weights in a region. Inputs startposition,
	 * stopposition, binsize. Each bin's value is the total amount of weight in
	 * the bin.
	 *
	...
	public void processWeightHistogram(Header header, Hits hits) throws IOException {
		if (header == null || hits == null) {
			...
			try {
				...
				server.getLogger().logp(Level.INFO, "ServerTask", "processWeightHistogram " + toString(),
						"Exception parsing binsize : " + request.map.get("binsize"), e);
				printString("missing or invalid bin size : " + request.map.get("binsize") + "\n");
				return;
			}
			...
			int first = header.getFirstIndex(request.start);
			int last = header.getLastIndex(request.end);
			float[] raw = hits.weightHistogram(first, last, request.start, request.end, binsize, dedup,
					request.minWeight, request.isPlusStrand, extension);
			int n = 0;
			for (int i = 0; i < raw.length; i++) {
				if (raw[i] > 0) {
					n++;
		...
		if (header == null || hits == null) {
			...
			if (needsort) {
				if (hits instanceof SingleHits) {
					server.getLogger().logp(Level.INFO, "ServerTask", "processCheckSort",
							String.format("Resorting %s %d", request.alignid, request.chromid));
					((SingleHits) hits).resort(
							server.getAlignmentDir(request.alignid) + System.getProperty("file.separator"),
							request.chromid, request.isType2);
					...
					header = new Header(hits.getPositionsBuffer().getib());
					header.writeIndexFile(
							server.getSingleHeaderFileName(request.alignid, request.chromid, request.isType2));
	...
	public String toString() {
		return String.format("thread %s, user %s, remote %s:%d", Thread.currentThread().toString(), username,
				socket.getInetAddress(), socket.getPort());
	}
}

/**
 * SASL callback handler for the authenticate() method. Provides a username and
 * server name to the CallBack
 */
