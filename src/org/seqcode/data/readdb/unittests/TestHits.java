...
public class TestHits {
	...
	public TestHits() throws IOException {
		...
		for (int i = 0; i < hits.limit(); i++) {
			...
			las.put(i, Hits.makeLAS((short) (hits.get(i) % 500), hits.get(i) % 2 == 1, (short) (hits.get(i) % 1000),
					hits.get(i) % 5 == 1));
		}
	...
	public void testCornerCases() {
		...
		p = hitsfile.getIndices(firstindex, lastindex, start, end);
		assertTrue(p[0] <= p[1]);
		assertTrue(String.format("cornercase : %d,%d, %d,%d -> %d,%d", start, end, firstindex, lastindex, p[0], p[1]),
				p[1] == header.getNumHits());
	...
	public void testGetIndices() {
		...
		for (int q = 1; q < 5000; q++) {
			...
			if (!ok) {
				System.err.println(String.format("Not OK : getIndices(%d,%d,%d,%d) -> %d,%d", firstindex, lastindex,
						start, end, p[0], p[1]));
				System.err.println(String.format("      %d,%d,%d .. %d, %d, %d", (p[0] == 0 ? -10 : hits.get(p[0] - 1)),
						hits.get(p[0]), hits.get(p[0] + 1), hits.get(p[1] - 1),
						(p[1] == hits.size() ? -10 : hits.get(p[1])),
						(p[1] == hits.size() - 1 ? -10 : hits.get(p[1] + 1))));
			}
			allok = allok && ok;
		}
		assertTrue(allok);
	...
	public void testCountRange() throws IOException {
		for (int q = 0; q < 300; q++) {
			...
			int reported = hitsfile.getCountBetween(header.getFirstIndex(start), header.getLastIndex(end), start, end,
					null, null);
	...
	public void testGetHits() throws IOException {
		for (int q = 0; q < 300; q++) {
			...
			IntBP reported = hitsfile.getHitsBetween(header.getFirstIndex(start), header.getLastIndex(end), start, end,
					null, null);
			FloatBP wreported = hitsfile.getWeightsBetween(header.getFirstIndex(start), header.getLastIndex(end), start,
					end, null, null);
			int k = 0;
			int j = 0;
			while (k < hits.size() && hits.get(k) < start) {
				k++;
			}
			while (k < hits.size() && hits.get(k) <= end) {
				if (hits.get(k) != reported.get(j)) {
					System.err
							.println(String.format("Mismatch %d (%d) != %d (%d)", hits.get(k), k, reported.get(j), j));
				}
	...
	public void testHistogram() throws IOException {
		for (int q = 0; q < 300; q++) {
			...
			int[] histogram = hitsfile.histogram(header.getFirstIndex(start), header.getLastIndex(end), start, end,
					binsize, 0, null, null, 0);
	...
	public void testDeDupHistogram() throws IOException {
		for (int q = 0; q < 300; q++) {
			...
			histogram = hitsfile.histogram(header.getFirstIndex(start), header.getLastIndex(end), start, end, binsize,
					2, null, null, 0);

			myhist = new int[(end - start) / binsize + 1];
			for (int i = 0; i < hits.size(); i++) {
				if (hits.get(i) >= start && hits.get(i) <= end) {
					if (i < 2 || (hits.get(i - 1) != hits.get(i) || hits.get(i - 2) != hits.get(i))) {
						myhist[(hits.get(i) - start) / binsize]++;
					}
				}
			}
	...
	public void testWeightHistogram() throws IOException {
		for (int q = 0; q < 300; q++) {
			...
			int binsize = 10 + (int) Math.round(Math.random() * 40);
			float[] histogram = hitsfile.weightHistogram(header.getFirstIndex(start), header.getLastIndex(end), start,
					end, binsize, 0, null, null, 0);
