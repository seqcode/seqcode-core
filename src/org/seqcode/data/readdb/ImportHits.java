...
/**
 * <p>
 * Imports hits to the db. Usage:
 * 
 * <pre>
 * cat hits.txt | ImportHits -H nanog.csail.mit.edu -P 5200 -a "Gcn4 ChipSeq" -u arolfe -p SECRET</code>
 *
 * <p>Lines in the input must be of the form<br>
 * <pre>chromosome\tstart\tstrand\tlength\tweight
 * </pre>
 * 
 * <br>
 * or
 * 
 * <pre>
 * chromosomeone\tstartone\tstrandone\tlengthone\tchromosometwo\tstarttwo\tstrandtwo\tlengthtwo\tweight\tpairCode
 * </pre>
 *
 * <p>
 * where start is the position of the 5' end of the read.
 * <p>
 * and where pairCode is 1 if the pair represents a link between two mated
 * reads, and 0 otherwise (e.g. if the pair represents two sections of the same
 * gapped read alignment).
 *
 * <p>
 * For Mahony lab use:
 * <ul>
 * <li>The chromosome must be numeric and should be the chromosome id from core.
 * <li>The alignment should be numeric and should the alignment identifier from
 * the seqdata schema
 * </ul>
 *
 */
public class ImportHits {
	...
	public ImportHits(String hostname, int port, String alignname, String username, String password) {
	...
	public void printHelp() {
		...
		System.out.println(
				"usage: cat foo.sam | java org.seqcode.data.readdb.SAMToReadDB | java org.seqcode.data.readdb.ImportHits \\");
		System.out.println(" --align alignmentname");
		System.out.println(
				" --type2 the imported hits are type 2 single end hits (e.g. read 2 hits in certain circumstances)");
		...
		System.out.println(
				"  (5) R chromosome (6) position of 5' end of R read (7) R strand (8) R length (9) weight (10) pair between mates indicator");
	}
	...
	public void run(InputStream instream) throws IOException, ClientException {
		...
		while ((line = reader.readLine()) != null) {
			...
			if (pieces.length == 5) {
				hits.add(new SingleHit(Integer.parseInt(pieces[0]), Integer.parseInt(pieces[1]),
						Float.parseFloat(pieces[4]), pieces[2].equals("+"), Short.parseShort(pieces[3])));
			} else if (pieces.length == 10) {
				paired.add(new PairedHit(Integer.parseInt(pieces[0]), Integer.parseInt(pieces[1]),
						pieces[2].equals("+"), Short.parseShort(pieces[3]), Integer.parseInt(pieces[4]),
						Integer.parseInt(pieces[5]), pieces[6].equals("+"), Short.parseShort(pieces[7]),
						Float.parseFloat(pieces[8]), Integer.parseInt(pieces[9])));
			} else {
