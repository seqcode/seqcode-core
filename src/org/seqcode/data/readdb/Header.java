...
public class Header implements Closeable {
	...
	private int numHits;
	/*
	 * indexPositions and indexPointers are paired arrays. indexPositions
	 * contains values from the hits file; the corresponding entry in
	 * indexPointers is the offset (in sizeof(int) units) of the first position
	 * in the hits file that contains that value. indexPositions may not contain
	 * duplicate values.
	 */
	...
	/*
	 * returns the first element in the file of hits that should be read when
	 * looking for a hit at a given position. Note that the value returned is a
	 * lower bound on the index of that position in the list of hits.
	 */
	...
	/*
	 * returns the last element in the file of hits that should be read when
	 * looking for a hit at a given position. This is an upper bound on the
	 * index of that position in the list of hits.
	 */
	...
	/**
	 * Creates a header file to index to provided set of hits. The hits must be
	 * sorted.
	 */
	...
	private void init(IntBuffer hits, int pagesize) {
		...
		for (int i = 0; i < numHits; i++) {
			if (i > lastpointer + hitstoskip && lastposition != hits.get(i)) {
