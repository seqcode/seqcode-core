...
/**
 * Dispatch is a thread that manages the WorkerThreads and assigns WorkerThreads
 * to ServerTasks.
 */
...
public class Dispatch implements Runnable {
	...
	/**
	 * Add a new ServerTask to the set of tasks that will be run. Called by
	 * Server when it accepts a new connection.
	 */
	...
	/**
	 * called by WorkerThread when it's finished with a ServerTask. WorkerThread
	 * has called the run method(), so getting to this point means that the run
	 * method returned. The run() method should not throw any exceptions.
	 *
	 */
	...
	/**
	 * our main loop. work through the tasks, seeing who appears to have input
	 */
	public void run() {
				...
				for (int i = 0; i < threads.size(); i++) {
					if (!threads.get(i).isAlive()) {
						...
						try {
							...
							server.getLogger().logp(Level.INFO, "Dispatch", "run: trying to stop old thread",
									e.toString(), e);
						}
