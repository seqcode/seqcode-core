 ...
 * 
 * <pre>
 *  echo "1:100-6000" | java org.seqcode.data.readdb.Query [--hostname nanog.csail.mit.edu --port 52000 --user foo --passwd bar] [--quiet] --align 100
 * </pre>
 * <ul>
 * <li>--quiet means don't print any output. This is useful for testing the
 * query performance without worrying about the time it takes to print the
 * output.
...
public class Query {
	...
	public void parseArgs(String args[]) throws IllegalArgumentException, ParseException {
		...
		options.addOption("g", "histogram", true,
				"produce a histogram with this binsize instead of printing all read positions");
	...
	public void printHelp() {
		...
		System.out.println("usage: java org.seqcode.data.readdb.Query --align alignmentname < regions.txt");
		System.out.println(
				" [--quiet]  don't print any output.  Useful for performance testing without worrying about output IO time");
	...
	public void run(InputStream instream) throws IOException, ClientException {
		...
		while ((line = reader.readLine()) != null) {
			try {
				...
				if (histogram > 0) {
					TreeMap<Integer, Integer> hits = client.getHistogram(alignname, chr, isType2, paired, 0, histogram,
							start, stop, null, strand);
					TreeMap<Integer, Float> weightsmap = null;
					if (weights) {
						weightsmap = client.getWeightHistogram(alignname, chr, isType2, paired, 0, histogram, start,
								stop, null, strand);
					}
					if (!quiet) {
					...
					if (paired) {
						List<PairedHit> hits = client.getPairedHits(alignname, chr, isleft, start, stop, null, strand);
						if (!quiet) {
							if (!noheader) {
						...
						List<SingleHit> hits = client.getSingleHits(alignname, chr, isType2, start, stop, null, strand);
						if (!quiet) {
							if (!noheader) {
								System.out.println(line);
							}
							for (SingleHit h : hits) {
								if (bed) {// BED is 0-based and half-open by
											// definition
									System.out.println(String.format("chr%d\t%d\t%d\thit\t%f\t%s\n", h.chrom,
											h.strand ? h.pos - 1 : h.pos - h.length - 1,
											h.strand ? h.pos + h.length : h.pos, h.weight, h.strand ? "+" : "-"));
