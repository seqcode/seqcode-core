...
public class PairedHit implements Comparable<PairedHit> {
	...
	public PairedHit(int leftchrom, int leftpos, boolean leftstrand, short leftlen, int rightchrom, int rightpos,
			boolean rightstrand, short rightlen, float weight, int paircode) {
		leftChrom = leftchrom;
	...
	public boolean equals(Object o) {
		if (o instanceof PairedHit) {
			...
			return (leftChrom == other.leftChrom && rightChrom == other.rightChrom && leftPos == other.leftPos
					&& rightPos == other.rightPos && leftStrand == other.leftStrand && rightStrand == other.rightStrand
					&& pairCode == other.pairCode && Math.abs(weight - other.weight) < .001);
		} else {
			return false;
		}
	}
	...
	public PairedHit flippedCopy() {
		return new PairedHit(rightChrom, rightPos, rightStrand, rightLength, leftChrom, leftPos, leftStrand, leftLength,
				weight, pairCode);
	}
	...
	public String toString() {
		return String.format("%d:%d,%d:%c and %d:%d,%d:%c weight %.2f paircode %d", leftChrom, leftPos, leftLength,
				leftStrand ? '+' : '-', rightChrom, rightPos, rightLength, rightStrand ? '+' : '-', weight, pairCode);
	}
