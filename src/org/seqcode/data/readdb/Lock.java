...
/**
 * Static class to maintain locks on objects within the JVM. We need this
 * because... - multiple threads can have a read lock on an object at once;
 * synchronized doesn't do this - the java.io locking stuff only locks between
 * processes, not within the JVM
 */
public class Lock {

	private static Map<String, ReentrantReadWriteLock> locks = Collections
			.synchronizedMap(new HashMap<String, ReentrantReadWriteLock>());
	private static Map<Thread, Set<java.util.concurrent.locks.Lock>> threadlocks = Collections
			.synchronizedMap(new HashMap<Thread, Set<java.util.concurrent.locks.Lock>>());
	private static int rlcount = 0;

	/**
	 * blocks to acquire a shared lock to the specified file. The locking is
	 * implemented in java, so it's only good for keeping out other threads.
	 * It's keyed off the file name you provide, so make sure you always
	 * generate the filename in the same way.
	 */
	protected static java.util.concurrent.locks.Lock readLock(String fname) {
		...
		// System.err.println("READLOCK by " + t + " of " + fname + " as " +
		// lock);
		return lock;
	}
	...
	protected static java.util.concurrent.locks.Lock writeLock(String fname) {
		...
		threadlocks.get(t).add(lock);
		// System.err.println("WRITELOCK by " + t + " of " + fname + " as " +
		// lock);
		return lock;
	}
	...
	protected static void releaseLocks() {
		...
		if (rlcount++ > 1000) {
			...
			if (locks.size() > 10000) {
				synchronized (locks) {
					/*
					 * cleanup loop so that we don't have an ever-expanding data
					 * structure
					 */
