...
/**
 * <p>
 * ReadDB server class. Provides configuration-type information, but pushes all
 * the request handling off onto ServerTask. Usage:
 * 
 * <pre>
 * java org.seqcode.data.readdb.Server --datadir /path/to/data
 * </pre>
 *
 * <p>
 * Optional parameters
 ...
 * <li>--sleepiness 2 how sleepy the server should be waiting for input. Lower
 * values use more CPU but improve responsiveness
 * <li>--idlelimit 24 number of hours after which idle task connections are
 * closed
...
public class Server {
	...
	/*
	 * topdir is the top-level directory for our data files. pwfile is
	 * "${topdir}/users.txt" and groupfile is "${topdir}/groups.txt"
	 */
	private String topdir, pwfile, groupfile;
	private boolean keepRunning;
	private Dispatch dispatch;
	private Map<String, Set<String>> groups;
	// BUFFERLEN should be a multiple of 8 to avoid problems with partial ints,
	// floats, or doubles
	...
	public void parseArgs(String[] args) throws ParseException {
		...
		options.addOption("L", "idlelimit", true,
				"number of hours after which to close idle connections. Negative sets no limit.");
		...
		logger.log(Level.INFO,
				String.format("Server parsed args: port %d, threads %d, directory %s", port, numThreads, topdir));
	...
	public void printHelp() {
		...
		System.out.println("usage: java org.seqcode.data.readdb.Server --datadir /path/to/datadir --port 52000");
		...
		System.out.println(
				" [--sleepiness 4]  (1-100) higher values use less CPU when idle but may incur more delay in processing requests");
	}
	...
	public String getAlignmentDir(String alignID) {
		...
		return getTopDir() + System.getProperty("file.separator") + alignID;
	}
	...
	public String getSingleHeaderFileName(String alignID, int chromID, boolean isType2) {
		return getAlignmentDir(alignID) + System.getProperty("file.separator") + chromID
				+ (isType2 ? ".singlet2index" : ".singleindex");
	}

	public String getPairedHeaderFileName(String alignID, int chromID, boolean isLeft) {
		return getAlignmentDir(alignID) + System.getProperty("file.separator") + chromID + ".paired"
				+ (isLeft ? "left" : "right") + "index";
	}
	...
	public Set<Integer> getChroms(String alignID, boolean isType2, boolean isPaired, boolean isLeft) {
		...
		String suffix = isPaired ? (isLeft ? ".pairedleftindex" : ".pairedrightindex")
				: (isType2 ? ".singlet2index" : ".singleindex");
	 ...
	 */
	public SingleHits getSingleHits(String alignID, int chrom, boolean isType2)
	...
	public PairedHits getPairedHits(String alignID, int chrom, boolean isLeft)
			throws IOException, SecurityException, FileNotFoundException {
	...
	public void removeACL(String alignID) {
		acls.remove(alignID);
	}
	...
	/**
	 * Returns true iff this princ is allowed to create alignments.
	 *
	 * Currently implemented as members of the cancreate group
	 */
	 ...
	 * server down and have read/write/admin access to any data.
	 *
	 * Currently implemented as members of the admin group.
	 */
	...
	/**
	 * The groups file has lines of the form groupname:user1 user2
	 * user3 @othergroup user4
	 *
	 * Group names that are also usernames are removed since ACLs can contain
	 * either users or groups and we don't want confusion.
	 *
	 */
	 ...
	 */
	private void expand(Map<String, Set<String>> toexpand, Map<String, Set<String>> expanded) {
	...
	/**
	 * returns the password for the specified user, or null if the user is
	 * unknown
	 *
