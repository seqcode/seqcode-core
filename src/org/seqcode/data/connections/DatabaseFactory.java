 ...
 * database connections (don't use CxnPool directly). DatabaseFactory provides
 * database connections for *roles* and manages the connection pool for you. A
 * role is a database resource that you want to access, eg, <b>ucsc_SGDv1</b>
 * for annotations for SGDv1 or <b>core</b> for the core db.
 *
...
public abstract class DatabaseFactory {
	...
	public static Connection getConnection(String role) throws SQLException, UnknownRoleException {
		...
		try {
			...
			if (!usedRoles.contains(role))
				usedRoles.add(role);
			String user, schema;
			if (defaultUsers.containsKey(role) && defaultSchemas.containsKey(role)) {
	...
	/**
	 * for a given role name, eg chipchip, look up CHIPCHIPROLE in the
	 * environment. If it's set, use its value as the return value which is the
	 * actual role to use.
	 *
	 * For example, you might set CHIPCHIPROLE=finkchipchip to connect to a
	 * chipchip schema that isn't the one in your .chipchip_passwd
	 */
	...
	/**
	 * Returns the Connection to the appropriate pool or frees it if it didn't
	 * come from any CxnPool.
	 */
	...
	private static Properties getPropertiesForRole(String role) throws UnknownRoleException, IOException {
		...
		if (propfile.exists() && propfile.canRead()) {
			// System.err.println(String.format("Reading HOME/. properties file
			// %s: \"%s\"", propfile.getName(), propfile.getAbsolutePath()));
		...
		if (propfile.exists() && propfile.canRead()) {
			...
			// System.err.println("Looking in classpath for properties
			// file...");
			try {
				ClassLoader cl = ClassLoader.getSystemClassLoader();
				URL url = cl.getResource(basename);
				if (url != null) {
					// System.err.println(String.format("Found properties URL:
					// %s", url.toString()));
					if (System.getenv("DEBUGPW") != null) {
	...
	/**
	 * returns the type (ORACLE, MYSQL, UNKNOWN) of this Connection
	 */
	...
	/**
	 * Reestabish all connections. May be buggy and error-prone, but this is for
	 * a drastic case where db connections are down.
	 */
