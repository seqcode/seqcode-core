...
public class TimeSeriesLoader implements Closeable {
	...
	public void close() {
		...
		try {
			loadSeriesByID.close();
			loadSeriesByID = null;
			loadPointByID.close();
			loadPointByID = null;
			loadPointsBySeries.close();
			loadPointsBySeries = null;
	...
	public TimeSeries loadTimeSeries(int dbid) throws SQLException {
		if (cachedSeries.containsKey(dbid)) {
			return cachedSeries.get(dbid);
		}
