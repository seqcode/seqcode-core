...
/**
 * Reads a FASTA file and creates the chromosomes and their sequence in the
 * database. Leading "chr" is stripped off names in the FASTA file:
 *
 * AddChromosomes --species "$MM;mm8" --fasta mm8.fa AddChromosomes --species
 * "$MM;mm8" < mm8.fa
 *
 * --incremental does writes in little chunks if your
 * PreparedStatement.setString() can't handle a full chromosome sequence at
 * once.
 */
...
public class AddChromosomes {
	...
	public static void incremental_load(String[] args) throws IOException, NotFoundException, SQLException {
		...
		PreparedStatement insertChrom = cxn.prepareStatement("insert into chromosome(id, genome, name) values ("
				+ DatabaseSequence.getInsertSQL(cxn, "chromosome_id") + "," + genome.getDBID() + ",?)");

		PreparedStatement getChromID = cxn.prepareStatement("select id from chromosome where genome = ? and name = ?");

		PreparedStatement insertEmptySequence = cxn
				.prepareStatement("insert into chromsequence(id,len,sequence) values (?,0,'')");

		PreparedStatement appendSequence = cxn
				.prepareStatement("update chromsequence " + "set sequence=CONCAT(sequence,?) where id=?");
		PreparedStatement updateLength = cxn
				.prepareStatement("update chromsequence " + "set len=length(sequence) where id=?");
	...
	public static void bulk_load(String args[]) throws IOException, NotFoundException, SQLException {
		...
		PreparedStatement insertChrom = cxn.prepareStatement("insert into chromosome(id, genome, name) values ("
				+ DatabaseSequence.getInsertSQL(cxn, "chromosome_id") + "," + genome.getDBID() + ",?)");
		PreparedStatement getChromID = cxn.prepareStatement("select id from chromosome where genome = ? and name = ?");
		PreparedStatement insertSequence = cxn
				.prepareStatement("insert into chromsequence(id,len,sequence) values (?,?,?)");
