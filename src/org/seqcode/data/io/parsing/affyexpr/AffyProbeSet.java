 ...
 */
package org.seqcode.data.io.parsing.affyexpr;
...
public class AffyProbeSet {
	...
	private void addProbe(AffyProbe ap) {
		if (probes.containsKey(ap.getProbeName())) {
			throw new IllegalArgumentException(ap.getProbeName());
		}
	...
	public AffyProbe getAffyProbe(String pn) {
		return probes.get(pn);
	}
	...
	public Set<AffyProbe> getAllAffyProbes() {
		return new HashSet<AffyProbe>(probes.values());
	}

	public Set<AffyProbe> lookupGeneSymbol(String v) {
		return symbol.get(v);
	}

	public Set<AffyProbe> lookupUnigene(String v) {
		return unigene.get(v);
	}

	public Set<AffyProbe> lookupLocusID(String v) {
		return locus.get(v);
	}

	public Set<AffyProbe> lookupGeneName(String v) {
		return name.get(v);
	}
}
