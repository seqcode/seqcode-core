...
import org.seqcode.data.motifdb.BackgroundModel;
...
public class BackgroundModelIO {
	 ...
	 * based model TODO make integer value header column optional
	 * 
	 * @param model
	 * @param lines
			...
			for (int j = 0; j < Math.pow(4, i + 1); j++) {
				...
				if (bgLineMatcher.matches()) {
					...
					if ((intVal != j) || !mer.equals(BackgroundModel.int2seq(j, (i + 1)))) {
						throw new ParseException("Expected index " + j + " and kmer "
								+ BackgroundModel.int2seq(j, (i + 1)) + ", but got " + lines[lineIndex], 0);
					}
	 ...
	 * model TODO make integer value header column optional
	 * 
	 * @param model
	 * @param lines
	 * @throws ParseException
	...
	private static void initMarkovBackgroundModel(MarkovBackgroundModel model, String[] lines) throws ParseException {
		...
		for (int i = 0; i < model.getMaxKmerLen(); i++) {
			for (int j = 0; j < Math.pow(4, i + 1); j += 4) {
				...
				for (int k = 0; k < 4; k++) {
					...
					if (bgLineMatcher.matches()) {
						...
						if ((intVal != (j + k)) || !mer.equals(BackgroundModel.int2seq((j + k), (i + 1)))) {
							throw new ParseException("Expected index " + (j + k) + " and kmer "
									+ BackgroundModel.int2seq((j + k), (i + 1)) + ", but got " + lines[lineIndex], 0);
						}
				...
				model.setMarkovProb(BackgroundModel.int2seq(j, i + 1).substring(0, i), probs[0], probs[1], probs[2],
						probs[3]);
			}
		}
	}
	...
	private static int checkModelMaxKmerLen(String[] lines) throws ParseException {
		// determine the max kmer length for model and check that the number of
		// lines is valid
		int maxKmerLen = 1;
		int numLines = 4;
	...
	/**
	 * @see parseCountsBackgroundModel(String modelName, String filename, Genome
	 *      gen) This version uses the filename as the name for the model
	 * @param filename
	 * @return
	 * @throws IOException
	 * @throws ParseException
	 */
	public static CountsBackgroundModel parseCountsBackgroundModel(String filename, Genome gen)
	 ...
	 * @throws ParseException
	 */
	public static CountsBackgroundModel parseCountsBackgroundModel(String modelName, String filename, Genome gen)
			throws IOException, ParseException {
	...
	/**
	 * @see parseMarkovBackgroundModel(String modelName, String filename, Genome
	 *      gen) This version uses the filename as the name for the model
	 * @param filename
	 * @return
	 * @throws IOException
	 * @throws ParseException
	 */
	public static MarkovBackgroundModel parseMarkovBackgroundModel(String filename, Genome gen)
			throws IOException, ParseException {
	 ...
	 * @throws ParseException
	 */
	public static MarkovBackgroundModel parseMarkovBackgroundModel(String modelName, String filename, Genome gen)
			throws IOException, ParseException {
		String[] lines = LineByLineFileReader.readFile(filename, LineByLineFileReader.DEFAULT_COMMENT_PREFIXES, true);
	 ...
	 *      Genome gen) This version uses the filename as the name for the model
	 * @param filename
	 * @return
	 * @throws IOException
	 * @throws ParseException
	 */
	public static FrequencyBackgroundModel parseFrequencyBackgroundModel(String filename, Genome gen)
			throws IOException, ParseException {
	 ...
	 * @throws ParseException
	 */
	public static FrequencyBackgroundModel parseFrequencyBackgroundModel(String modelName, String filename, Genome gen)
			throws IOException, ParseException {
		String[] lines = LineByLineFileReader.readFile(filename, LineByLineFileReader.DEFAULT_COMMENT_PREFIXES, true);
			...
			for (int i = 1; i <= bgModel.getMaxKmerLen(); i++) {
				for (int j = 0; j < Math.pow(4, i); j++) {
					...
					if (bgModel instanceof MarkovBackgroundModel) {
						...
						lblfw.writeLine(
								j + "\t" + currKmer + "\t" + ((FrequencyBackgroundModel) bgModel).getFrequency(j, i));
					} else if (bgModel instanceof CountsBackgroundModel) {
						lblfw.writeLine(
								j + "\t" + currKmer + "\t" + ((CountsBackgroundModel) bgModel).getKmerCount(j, i));
					}
				}
			}
			lblfw.writeLine("");
