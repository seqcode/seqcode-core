...
/**
 * This class will hold IO operations for various types of variables: arrays,
 * matrices, lists etc
 * 
 * @author geopapa
 *
...
public class IOUtil {
	 ...
	 * 
	 * @param f
	 *            file from where the array will be read
	 * @param delim
	 *            delimiter used
	 * @param num_cols
	...
	public static String[][] readFile(String f, String delim, int num_cols) {
		...
		try {
			a = readStream(new FileInputStream(new File(f)), delim, num_cols);
	 ...
	 * 
	 * @param os
	 *            The output stream where the array will be stored
	 * @param a
	 *            array to be stored
	 * @param delim
	 *            delimiter used
	 * @param num_cols
	 *            number of array elements in each line (row) of the stream
	 */
			...
			if (num_cols > 0) {
				...
				while ((str = br.readLine()) != null) {
					...
					for (int i = 0; i < tokens.length; i++) {
						temp_list.add(tokens[i]);
					}
				}
				al.add(temp_list.toArray(new String[0]));
			} else {
				throw new IllegalArgumentException(
						"Invalid value for num_cols. The valid values are -1 (one row), 0 (as they are on the file), k (columns in each row).");
			}
	...
	public static <T> void write2file(String f, T[][][] a, String delim) {
		try {
			write2stream(new FileOutputStream(new File(f)), a, delim);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	}// end of write2file method
	...
	public static <T> void write2file(String f, T[] a, String delim, int num_cols) {
		try {
			write2stream(new FileOutputStream(new File(f)), a, delim, num_cols);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	}// end of write2file method
	...
	public static <T> void write2stream(OutputStream os, T[] a, String delim, int num_cols) {
		...
		try {
			...
			if (a.length > 0) {
				bw.write(a[a.length - 1].toString() + "\n");
			} else {
				bw.write("\n");
			}
		} catch (IOException e) {
	...
	public static void main(String[] args) {
		...
		int[][][] a = { { { 3, 2, 1, 5, 3, 6, 2, 7 }, { 1, 6, 3, 7, 2, 1 } },
				{ { -2, -4, 2, 1, 5 }, { 0, 4, 2, -1 } }, };
		String delim = ",";
