...
/**
 * SeqDataLoader serves as a clearinghouse for query interactions with the
 * seqdata database and associated metadata from tables in the core database
 * (via the MetadataLoader).
 * 
 * Implements a simple access control by checking if the username is in the
 * permissions field in each SeqAlignment. Note that while this is
 * access-control-lite for experiment metadata, access to the data stored in the
 * underlying readdb entries has more robust access-control.
 * 
 * @author tdanford
 * @author mahony
 * 
...
public class SeqDataLoader implements org.seqcode.gseutils.Closeable {
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			for (SeqAlignment align : aligns) {
				...
				System.out.println(expt.getDBID() + "\t" + expt.getName() + ";" + expt.getReplicate() + "\t"
						+ align.getName() + "\t" + align.getDBID() + "\t" + align.getGenome());
			}
	...
	// Constructors
	public SeqDataLoader() throws SQLException, IOException {
		this(true, true);
	}
	...
	/**
	 * Find a SeqExpt by name and replicate (returns null if not found)
	 * 
	 * @param name
	 * @param rep
	 * @return
	...
	/**
	 * Load a SeqExpt by name and replicate (throws exception if not found)
	 * 
	 * @param name
	 * @param rep
	 * @return
	...
	public SeqExpt loadExperiment(String name, String rep) throws NotFoundException, SQLException {
		...
		try {
			...
			if (expt == null) {
				throw new NotFoundException(name + ";" + rep);
			}
		} finally {
			if (rs != null) {
				try {
	...
	public Collection<SeqAlignment> loadAllAlignments() throws SQLException {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				SeqAlignment align = new SeqAlignment(rs, exptmap.get(rs.getInt(2)),
						mloader.loadAlignType(rs.getInt(6), false));
				aligns.add(align);
			}
		} finally {
			if (rs != null) {
	...
	public SeqAlignment loadAlignment(SeqExpt expt, String n, Genome g) throws SQLException {
		...
		try {
			...
			while (align == null && rs.next()) {
				...
				if (!align.getGenome().equals(g))
					align = null;
				if (align != null && !align.getPermissions().contains("public")
						&& !align.getPermissions().contains(myusername))
					align = null;
			}
		} finally {
			if (rs != null) {
		...
		if (align == null) {
			// Don't throw exception because sometimes we have replicates that
			// don't match all alignment names
			// throw new NotFoundException("Couldn't find alignment " + n + "
			// for " + expt + " in genome " + g);
		}
		return align;
	 ...
	 * @throws NotFoundException
	 */
	public Collection<SeqAlignment> loadAlignments(SeqLocator locator, Genome genome)
			throws SQLException, NotFoundException {
		List<SeqAlignment> output = new ArrayList<SeqAlignment>();
		if (locator.getAlignName() != null) { // Alignment name provided
			if (locator.getReplicates().size() == 0) { // No replicate names
														// provided
				...
				for (String rep : locator.getReplicates()) { // Replicate names
																// provided
					try {
			...
			if (locator.getReplicates().size() == 0) { // No alignment or
														// replicate names
														// provided
				for (SeqExpt expt : loadExperiments(locator.getExptName())) {
		...
		if (filtered.size() == 0)
			throw new NotFoundException(
					"Locators were " + toString() + " but didn't get any alignments that you have permission to see.");
	 ...
	 * @throws SQLException
	 */
	public Collection<SeqAlignment> loadAlignments(String name, String replicate, String align, Integer exptType,
			Integer lab, Integer condition, Integer target, Integer cells, Integer readType, Genome genome)
			throws SQLException {
		...
		try {
			...
			if (name != null || replicate != null || align != null || exptType != null || lab != null
					|| condition != null || target != null || cells != null || readType != null || genome != null) {
				query += " where ";
			}
			boolean and = false;
			if (name != null || replicate != null || exptType != null || lab != null || condition != null
					|| target != null || cells != null || readType != null) {
				...
				if (replicate != null) {
					query += (and ? " and " : " ") + " replicate = ? ";
					and = true;
				}
				if (exptType != null) {
					query += (and ? " and " : " ") + " expttype = " + exptType;
					and = true;
				}
				if (lab != null) {
					query += (and ? " and " : " ") + " lab = " + lab;
					and = true;
				}
				if (condition != null) {
					query += (and ? " and " : " ") + " exptcondition = " + condition;
					and = true;
				}
				if (target != null) {
					query += (and ? " and " : " ") + " expttarget = " + target;
					and = true;
				}
				if (cells != null) {
					query += (and ? " and " : " ") + " cellline = " + cells;
					and = true;
				}
				if (readType != null) {
					query += (and ? " and " : " ") + " readtype = " + readType;
					and = true;
				}
				query += ")";
				and = true;
			...
			if (genome != null) {
				query += (and ? " and " : " ") + " genome = " + genome.getDBID();
				and = true;
			}
			if (align != null) {
				query += (and ? " and " : " ") + " name = ? ";
			...
			if (name != null || replicate != null) {
				...
				if (replicate != null) {
					ps.setString(index++, replicate);
				}
			}
			if (align != null) {
				ps.setString(index++, align);
			}
			// System.out.println(ps.toString());
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public Map<String, String> getAlignmentParameters(SeqAlignment align) throws SQLException {
		...
		try {
			...
			// System.out.println("select name, value from alignmentparameters
			// where alignment = " + align.getDBID());
			while (rs.next()) {
	...
	public List<SeqHit> loadByRegion(SeqAlignment align, Region r, boolean loadR2) throws IOException {
		try {
			return convert(client.getSingleHits(Integer.toString(align.getDBID()),
					r.getGenome().getChromID(r.getChrom()), loadR2, r.getStart(), r.getEnd(), null, null), align);
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	}
	...
	public List<SeqHitPair> loadPairsByRegion(SeqAlignment align, Region r) throws IOException {
		try {
			return convertPairs(client.getPairedHits(Integer.toString(align.getDBID()),
					r.getGenome().getChromID(r.getChrom()), true, r.getStart(), r.getEnd(), null, null), align);
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	}
	...
	/**
	 * Load integer positions of single hits by region. If Region is a
	 * StrandedRegion, then the positions returned are only for that strand
	 * 
	 * @param alignments
	 * @param r
	 * @return
	 ...
	 */
	public List<Integer> positionsByRegion(List<SeqAlignment> alignments, Region r, boolean loadR2)
		...
		for (SeqAlignment a : alignments) {
			int[] pos = client.getPositions(Integer.toString(a.getDBID()), r.getGenome().getChromID(r.getChrom()),
					loadR2, false, r.getStart(), r.getEnd(), null, null,
	...
	public List<Integer> positionsByRegion(SeqAlignment alignment, Region r, boolean loadR2) throws IOException {
		...
		try {
			int[] pos = client.getPositions(Integer.toString(alignment.getDBID()),
					r.getGenome().getChromID(r.getChrom()), loadR2, false, r.getStart(), r.getEnd(), null, null,
					r instanceof StrandedRegion ? (((StrandedRegion) r).getStrand() == '+') : null);
			for (int i = 0; i < pos.length; i++) {
				output.add(pos[i]);
			}
	...
	/**
	 * Count single hits in a region. If Region is a StrandedRegion, then the
	 * counts returned are only for that strand
	 * 
	 * @param align
	 * @param r
	 * @return
	...
	public int countByRegion(SeqAlignment align, Region r, boolean loadR2) throws IOException {
		try {
			return client.getCount(Integer.toString(align.getDBID()), r.getGenome().getChromID(r.getChrom()), loadR2,
					false, r.getStart(), r.getEnd(), null, null,
					r instanceof StrandedRegion ? (((StrandedRegion) r).getStrand() == '+') : null);
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	...
	/**
	 * Weigh single hits in a region. If Region is a StrandedRegion, then the
	 * counts returned are only for that strand
	 * 
	 * @param alignments
	 * @param r
	...
	public double weightByRegion(List<SeqAlignment> alignments, Region r, boolean loadR2) throws IOException {
		...
		for (SeqAlignment a : alignments) {
			try {
				total += client.getWeight(Integer.toString(a.getDBID()), r.getGenome().getChromID(r.getChrom()), loadR2,
						false, r.getStart(), r.getEnd(), null, null,
						r instanceof StrandedRegion ? (((StrandedRegion) r).getStrand() == '+') : null);
			} catch (ClientException e) {
				throw new IllegalArgumentException(e);
	...
	public int countAllHits(SeqAlignment align, boolean loadR2) throws IOException {
		try {
			return client.getCount(Integer.toString(align.getDBID()), loadR2, false, false, null);
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	}
	...
	public double weighAllHits(SeqAlignment align, boolean loadR2) throws IOException {
		try {
			return client.getWeight(Integer.toString(align.getDBID()), loadR2, false, false, null);
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	}
	 ...
	 * Output maps bin center to weight centered around that bin. Each read is
	 * summarized by its start position.
	 */
	public Map<Integer, Float> histogramWeight(SeqAlignment align, char strand, Region r, int binWidth, boolean loadR2)
			throws IOException {
		try {
			return client.getWeightHistogram(Integer.toString(align.getDBID()), r.getGenome().getChromID(r.getChrom()),
					loadR2, false, 0, binWidth, r.getStart(), r.getEnd(), null, strand == '+');
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	}
	 ...
	 * summarized by its start position.
	 */
	public Map<Integer, Integer> histogramCount(SeqAlignment align, char strand, Region r, int binWidth, boolean loadR2)
			throws IOException {
		try {
			return client.getHistogram(Integer.toString(align.getDBID()), r.getGenome().getChromID(r.getChrom()),
					loadR2, false, 0, binWidth, r.getStart(), r.getEnd(), null, strand == '+');
		} catch (ClientException e) {
			throw new IllegalArgumentException(e);
		}
	...
	/**
	 * Get the total # of hits and weight for an alignment but only include
	 * reads on the specified strand.
	 */
	public Pair<Long, Double> getAlignmentStrandedCountWeight(SeqAlignment align, char strand, boolean loadR2)
			throws IOException {
		try {
