 ...
 *         seqalignment table) from a single SeqExpt, which satisfy the
 *         following conditions:
 * 
 *         (1) all SeqAlignments share the same name (i.e. are from the same
 *         aligner) (2) if the locator's 'reps' field is non-empty, then only
 *         those replicates whose names appear in that set are indicated.
 * 
...
public class SeqLocator implements Comparable<SeqLocator> {
	...
	// The reps field has a particular semantics -- if it contains any entries,
	// then
	...
	public boolean isSubset(SeqLocator loc) {
		if (!exptName.equals(loc.exptName)) {
			return false;
		}
		if (!alignName.equals(loc.alignName)) {
			return false;
		}
		for (String rep : reps) {
			if (!loc.reps.contains(rep)) {
				return false;
			}
		}
		return true;
	...
	public int hashCode() {
		int code = 17;
		code += exptName.hashCode();
		code *= 37;
		code += alignName.hashCode();
		code *= 37;
