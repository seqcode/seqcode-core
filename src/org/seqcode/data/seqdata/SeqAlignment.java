 ...
 * @author mahony
 *
 *         Represents a row from the seqalignment table, in the *seqdata
 *         schemas.
 * 
 *         create table seqalignment ( id int(10) not null auto_increment, expt
 *         int(10) not null, name varchar(200) not null, genome int(10) not
 *         null, permissions varchar(500) not null, aligntype int(10) not null,
 *         numhits int(15), totalweight float(17,2), numtype2hits int(15),
 *         totaltype2weight float(17,2), numpairs int(15), totalpairweight
 *         float(17,2), aligndir varchar(400), alignfile varchar(500), idxfile
 *         varchar(400), collabalignid varchar(200), foreign key
 *         fk_seqalignment_expt (expt) references seqexpt(id), index(name,
 *         genome), unique(id), primary key(id) )Type=InnoDB;
 */
public class SeqAlignment {
	...
	public int getNumType2Hits() {
		return numType2Hits;
	}

	public double getTotalType2Weight() {
		return totalType2Weight;
	}
	...
	public double getTotalPairWeight() {
		return totalPairWeight;
	}
	...
	public String getCollabAlignID() {
		return collabAlignID;
	}
	...
	public static PreparedStatement createInsertStatement(java.sql.Connection c) throws SQLException {
		String query = String
				.format("insert into seqalignment (expt, name, genome, permissions, aligntype, numhits, totalweight, numtype2hits, totaltype2weight, numpairs, totalpairweight, aligndir, alignfile, idxfile, collabalignid) "
						+ "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		return c.prepareStatement(query);
	}
	...
	public static PreparedStatement createInsertStatementWithID(java.sql.Connection c) throws SQLException {
		String query = String
				.format("insert into seqalignment (id, expt, name, genome, permissions, aligntype, numhits, totalweight, numtype2hits, totaltype2weight, numpairs, totalpairweight, aligndir, alignfile, idxfile, collabalignid) "
						+ "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		return c.prepareStatement(query);
	}
	...
	public static PreparedStatement createUpdateStatementWithID(java.sql.Connection c) throws SQLException {
		String query = String
				.format("update seqalignment set expt=?, name=?, genome=?, permissions=?, aligntype=?, numhits=?, totalweight=?, numpairs=?,  numtype2hits=?, totaltype2weight=?, totalpairweight=?, aligndir=?, alignfile=?, idxfile=?, collabalignid=? "
						+ " where id=?");
		return c.prepareStatement(query);
	}
