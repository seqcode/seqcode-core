...
public class SeqLocatorMatchedAligns implements Comparable<SeqLocatorMatchedAligns> {
			...
			for (SeqExpt expt : expts) {
				if (!expts.get(0).isReplicateOf(expt))
					throw new SeqLMENotReplicatesException(
							"SeqLocatorMatchedAligns: input experiments are not replicates: " + locator.toString());
			}
		}
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof SeqLocatorMatchedAligns)) {
			return false;
		}
		SeqLocatorMatchedAligns le = (SeqLocatorMatchedAligns) o;
		if (!locator.getExptName().equals(le.locator.getExptName())) {
			return false;
		}
		if (locator.getReplicates().size() != le.locator.getReplicates().size()) {
			return false;
		}
		for (String rep : locator.getReplicates()) {
			if (!le.locator.getReplicates().contains(rep)) {
				return false;
			}
		}
		if (!locator.getAlignName().equals(le.locator.getAlignName())) {
			return false;
		}
