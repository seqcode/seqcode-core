...
/**
 * Creates an alignment (if necessary) in the database and prints the DBID on
 ...
 * creation.
 * 
 * Usage: CreateAlignment --species "$SC;SGDv1" --align
 * "name;replicate;alignment version" --lab "Pugh" --expttype "CHIPSEQ"
 * --expttarget "Gcn4" --cellline "FY4" --exptcondition "YPD" --readtype
 * "SINGLE" --aligntype "SINGLE" --paramsfile params.txt --readlength 36
 */
			...
			try {
				...
				if (alignment == null) {
					try {
						...
						System.err.println(
								"Creating alignment " + alignpieces[0] + ";" + alignpieces[1] + ";" + alignpieces[2]);
					...
					// Check if the alignment information is the same before
					...
					if (!(alignment.getName().equals(alignpieces[2]) && alignment.getPermissions().equals(permissions)
							...
							&& alignment.getTotalWeight() == totalweight && alignment.getNumPairs() == numpairs
							&& alignment.getTotalPairWeight() == totalpairweight
							&& alignment.getAlignDir().equals(aligndir) && alignment.getAlignFile().equals(alignfile)
							&& alignment.getIDXFile().equals(idxfile)
							&& alignment.getCollabAlignID().equals(collabalignid))) {
						try {
							int aID = alignment.getDBID();
							update = SeqAlignment.createUpdateStatementWithID(cxn);
							System.err.println("Updating alignment " + aID + " " + alignpieces[0] + ";" + alignpieces[1]
									+ ";" + alignpieces[2]);
				...
				if (alignment == null) {
					...
					throw new DatabaseException(
							"Couldn't create/update alignment " + alignpieces[2] + " for " + alignpieces[0]);
				}
