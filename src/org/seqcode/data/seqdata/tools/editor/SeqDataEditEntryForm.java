...
public class SeqDataEditEntryForm extends JFrame implements ActionListener {
	...
	private boolean sharedExptType = true, sharedLab = true, sharedCond = true, sharedTarget = true, sharedCell = true,
			sharedRep = true, sharedPubSrc = true, sharedPubID = true, sharedCollabExptID = true;
	...
	public void init() {
		...
		for (SeqExpt e : editExpts) {
			if (!e.getExptType().getName().equals(exptType)) {
				sharedExptType = false;
				exptType = "";
			}
			if (!e.getLab().getName().equals(lab)) {
				sharedLab = false;
				lab = "";
			}
			if (!e.getExptCondition().getName().equals(cond)) {
				sharedCond = false;
				cond = "";
			}
			if (!e.getExptTarget().getName().equals(target)) {
				sharedTarget = false;
				target = "";
			}
			if (!e.getCellLine().getName().equals(cell)) {
				sharedCell = false;
				cell = "";
			}
			if (!e.getReplicate().equals(rep)) {
				sharedRep = false;
				rep = "";
			}
			if (!e.getPublicSource().equals(pubSrc)) {
				sharedPubSrc = false;
				pubSrc = "";
			}
			if (!e.getPublicDBID().equals(pubID)) {
				sharedPubID = false;
				pubID = "";
			}
			if (!e.getCollabID().equals(collabExptID)) {
				sharedCollabExptID = false;
				collabExptID = "";
			}
		}
		...
		if (!sharedExptType && !sharedLab && !sharedCond && !sharedTarget && !sharedCell && !sharedRep && !sharedPubSrc
				&& !sharedPubID && !sharedCollabExptID) {
			JOptionPane.showMessageDialog(null,
					"Cannot update this selection: no information is shared by all experiments", "Error",
			...
			allInputsPanel.setLayout(new BoxLayout(allInputsPanel, BoxLayout.PAGE_AXIS));
	...
	public JPanel getInputsPanel() {
		...
		try {
			...
			if (!sharedLab) {
				...
				editLab.setOpaque(true);
			} else {
				editLab.setText(lab);
				editLab.setCaretPosition(0);
			}

			editCond = new JTextField();
			if (!sharedCond) {
				editCond.setEnabled(false);
				editCond.setOpaque(true);
			} else {
				editCond.setText(cond);
				editCond.setCaretPosition(0);
			}

			editTarget = new JTextField();
			if (!sharedTarget) {
				editTarget.setEnabled(false);
				editTarget.setOpaque(true);
			} else {
				editTarget.setText(target);
				editTarget.setCaretPosition(0);
			}

			editCell = new JTextField();
			if (!sharedCell) {
				editCell.setEnabled(false);
				editCell.setOpaque(true);
			} else {
				editCell.setText(cell);
				editCell.setCaretPosition(0);
			}

			editRep = new JTextField();
			if (!sharedRep) {
				editRep.setEnabled(false);
				editRep.setOpaque(true);
			} else {
				editRep.setText(rep);
				editRep.setCaretPosition(0);
			}

			editPubSrc = new JTextField();
			if (!sharedPubSrc) {
				editPubSrc.setEnabled(false);
				editPubSrc.setOpaque(true);
			} else {
				editPubSrc.setText(pubSrc);
				editPubSrc.setCaretPosition(0);
			}

			editPubID = new JTextField();
			if (!sharedPubID) {
				editPubID.setEnabled(false);
				editPubID.setOpaque(true);
			} else {
				editPubID.setText(pubID);
				editPubID.setCaretPosition(0);
			}

			editCollabExptID = new JTextField();
			if (!sharedCollabExptID) {
				editCollabExptID.setEnabled(false);
				editCollabExptID.setOpaque(true);
			} else {
				editCollabExptID.setText(collabExptID);
				editCollabExptID.setCaretPosition(0);
			}

			Font labelFont = new Font("SansSerif", Font.BOLD, 12);
			Border paddingBorder = BorderFactory.createEmptyBorder(5, 5, 5, 5);
			JLabel labelExpt = new JLabel("ExptType");
			labelExpt.setFont(labelFont);
			labelExpt.setBorder(paddingBorder);
			JLabel labelLab = new JLabel("Lab");
			labelLab.setFont(labelFont);
			labelLab.setBorder(paddingBorder);
			JLabel labelCond = new JLabel("ExptCondition");
			labelCond.setFont(labelFont);
			labelCond.setBorder(paddingBorder);
			JLabel labelTarget = new JLabel("ExptTarget");
			labelTarget.setFont(labelFont);
			labelTarget.setBorder(paddingBorder);
			JLabel labelCell = new JLabel("CellLine");
			labelCell.setFont(labelFont);
			labelCell.setBorder(paddingBorder);
			JLabel labelRep = new JLabel("Replicate");
			labelRep.setFont(labelFont);
			labelRep.setBorder(paddingBorder);
			JLabel labelPubSrc = new JLabel("PublicSource");
			labelPubSrc.setFont(labelFont);
			labelPubSrc.setBorder(paddingBorder);
			JLabel labelPubID = new JLabel("PublicDBID");
			labelPubID.setFont(labelFont);
			labelPubID.setBorder(paddingBorder);
			JLabel labelCollabExptID = new JLabel("CollabID");
			labelCollabExptID.setFont(labelFont);
			labelCollabExptID.setBorder(paddingBorder);
	...
	public JPanel getPermissionsPanel() {
		...
		Font labelFont = new Font("SansSerif", Font.BOLD, 12);
		Border paddingBorder = BorderFactory.createEmptyBorder(5, 5, 5, 5);
		JLabel labelPerm = new JLabel("Permissions");
		labelPerm.setFont(labelFont);
		labelPerm.setBorder(paddingBorder);
		labelPerm.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		for (SeqDataUser u : users) {
			Checkbox cb = new Checkbox(u.getName(),
					sharedUsers.containsKey(u.getName()) && sharedUsers.get(u.getName()));
			if (u.isAdmin())
				cb.setEnabled(false);
	...
	public void updateSeqExpts() throws SQLException {
		...
		for (SeqExpt expt : editExpts) {
			...
			System.err.println("UPDATING:\t" + updateName + "\t" + updateExptType + "\t" + updateLab + "\t" + updateCond
					+ "\t" + updateTarget + "\t" + updateCell + "\t" + updateRep + "\t" + updatePubSrc + "\t"
					+ updatePubID + "\t" + updateCollabExptID + "\t" + expt.getDBID());
			try {
				seqModifier.updateSeqExpt(seqLoader.getMetadataLoader(), expt, updateExptType, updateLab, updateCond,
						updateTarget, updateCell, updateRep, updatePubSrc, updatePubID, updateCollabExptID);
			} catch (DuplicateDatabaseEntryException e) {
				System.err.println("Cannot update " + expt.getName() + ";" + expt.getReplicate() + " to " + updateName
						+ ";" + updateRep + " as this will result in a duplicate SeqExpt key.");
				JOptionPane.showMessageDialog(null,
						"Cannot update " + expt.getName() + ";" + expt.getReplicate() + " to " + updateName + ";"
								+ updateRep + " as this will result in a duplicate SeqExpt key.",
						"Error: attempted duplicate", JOptionPane.INFORMATION_MESSAGE);
			}
