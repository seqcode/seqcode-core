...
/**
 * Tries to determine one or more control experiments to use for one or more
 * chipseq experiments.
 *
 * Usage: java org.seqcode.gse.tools.chipseq.GuessControlExpt --species 'Mus
 * musculus;mm9' --chipseq 'Sing ES CTCF E14;bowtie_unique'
 */
public class GuessControlExpt {
	...
	public static void main(String args[]) throws Exception {
		...
		for (SeqLocator l : locators) {
			...
			if (l.getReplicates().size() == 0) {
				alignments.addAll(loader.loadAlignments(l.getExptName(), null, l.getAlignName(), null, null, null, null,
						null, null, genome));
			} else {
	...
	public static Collection<SeqAlignment> controlsForAlignment(SeqAlignment a) throws Exception {
		...
		for (Integer i : controlTypeIDs) {
			output.addAll(loader.loadAlignments(null, null, null, i, null, cond.getDBID(), null, cells.getDBID(), null,
					genome));
		}
		return output;
	}
