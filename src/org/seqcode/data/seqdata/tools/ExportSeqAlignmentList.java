...
/**
 * Export existing SeqExpt & SeqAlignment descriptions to a file. This exports a
 * file like deepseq.list for backing up the seqdata mysql database. Since a
 * couple of deepseq.list fields are not stored in the database, we
 * unfortunately must provide the old deepseq.list file as input.
 * 
 * @author mahony
 *
 *         Usage: ExportSeqAlignmentList --list "deepseq.list" --out "outname"
 * 
 *         The assumed input & output files are in the deepseq.list format, with
	...
	public void execute() {
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (!fields[0].equals("ReadDBID") && !fields[0].startsWith("#")) {// skip
																					// the
																					// first
																					// line
																					// in
			...
			for (SeqAlignment align : loader.loadAllAlignments()) {
				...
				if (!oldList.containsKey(align.getDBID()))
					System.out.println("NOTFOUND\t" + align.getDBID() + "\t" + expt.getName() + ";"
							+ expt.getReplicate() + ";" + align.getName() + ";" + align.getGenome().getVersion());
				else {
					fw.write(align.getDBID() + "\t" + expt.getExptType().getName() + "\t" + expt.getLab().getName()
							+ "\t" + expt.getExptCondition().getName() + "\t" + expt.getExptTarget().getName() + "\t"
							+ expt.getCellLine().getName() + "\t" + expt.getReplicate() + "\t" + align.getName() + "\t"
							+ align.getGenome().getVersion() + "\t" + permissions + "\t" + expt.getPublicSource() + "\t"
							+ expt.getPublicDBID() + "\t" + expt.getCollabID() + "\t" + align.getCollabAlignID() + "\t"
							+ expt.getReadType().getName() + "\t" + align.getAlignType().getName() + "\t"
							+ expt.getReadLength() + "\t" + expt.getNumRead() + "\t"
							+ oldList.get(align.getDBID()).alignedhitsStr + "\t"
							+ oldList.get(align.getDBID()).uniqalignedhitsStr + "\t" + align.getNumHits() + "\t"
							+ align.getTotalWeight() + "\t" + align.getNumType2Hits() + "\t"
							+ align.getTotalType2Weight() + "\t" + align.getNumPairs() + "\t"
							+ align.getTotalPairWeight() + "\t"
							+ (expt.getFQFile().equals("") ? oldList.get(align.getDBID()).fqfile : expt.getFQFile())
							+ "\t" + align.getAlignDir() + "\t" + align.getAlignFile() + "\t" + align.getIDXFile()
							+ "\t" + oldList.get(align.getDBID()).paramsfname + "\t" + expt.getExptNote() + "\t"
							+ oldList.get(align.getDBID()).loaddate + "\t" + expt.getName() + ";" + expt.getReplicate()
							+ ";" + align.getName() + "\n");
					count++;
				}
			}
