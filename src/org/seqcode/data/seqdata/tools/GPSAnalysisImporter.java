...
/**
 * See AnalysisImporter docs. Command line options are the same; the only
 * difference is that GPSAnalysisImporter parses the GPS native output format.
 */

public class GPSAnalysisImporter extends AnalysisImporter {

	/*
	 * oracle complains about underflow if we don't limit the pvalues. the
	 * actual min value is somewhere between E-100 and E-200, but I didn't
	 * bother tracking it down more closely since I don't think the difference
	 * really matters
	 */
	...
	public SeqAnalysisResult parseLine(String line) {
		...
		GPSPeak p = GPSParser.parseLine(getGenome(), line, ++lineno);
		...
		return new SeqAnalysisResult(getGenome(), p.getChrom(), p.getLocation(), p.getLocation() + 1, p.getLocation(),
				p.getStrength(), p.getControlStrength(), p.getStrength(), p.getShape(),
				Math.max(p.getPvalue(), minpval), p.getStrength() / p.getControlStrength());
	}
