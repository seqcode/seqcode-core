...
/**
 * Returns the ids of the alignments specified on the command line. All fields
 * are optional. expttype, lab, expttarget, cellline, exptcondition, readtype
 * are ANDed together. align parameters are ORed together and then applied to
 * the expttype/lab/expttarget/cellline/exptcondition/readtype. The fields
 * include --align name;replicate;version --expttype --lab --expttarget
 * --cellline --exptcondition --readtype
 *
 * GetAlignments --species "$SC;SGDv1" --align "name;replicate;alignment
 * version" --expttarget "Gcn4" --exptcondition "YPD"
 */
	...
	public static void main(String args[]) throws SQLException, NotFoundException, IOException {
		...
		for (String an : alignnames) {
			...
			for (SeqAlignment a : loader.loadAlignments(pieces[0],
					...
					(pieces.length > 2 && pieces[2] != "") ? pieces[2] : null, expttype, lab, condition, target, cells,
					readtype, genome)) {
