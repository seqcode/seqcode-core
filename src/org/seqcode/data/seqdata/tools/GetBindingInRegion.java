...
/**
 * Reads regions on STDIN, from a file, or uses a list such as all genes and
 * produces a line on STDOUT for each that lists either the analyses and binding
 * events for each.
 * 
 * java org.seqcode.data.seqdata.tools.GetBindingInRegion --species "$MM;mm9"
 * --genes refGene java org.seqcode.data.seqdata.tools.GetBindingInRegion
 * --species "$MM;mm9" --region 10:0-1000000 java
 * org.seqcode.data.seqdata.tools.GetBindingInRegion --species "$MM;mm9"
 * --regionfile foo.txt java org.seqcode.data.seqdata.tools.GetBindingInRegion
 * --species "$MM;mm9" this form above reads regions from STDIN
 * 
...
public class GetBindingInRegion {
	...
	public void setGenes(Collection<RefGeneGenerator> g) {
		geneGenerators.clear();
		geneGenerators.addAll(g);
	}

	public void setRegions(Collection<Region> r) {
		regions.clear();
		regions.addAll(r);
	}
	...
	public void plot(Region r) throws SQLException, NotFoundException, IOException {
		...
		for (SeqAnalysis analysis : analyses) {
			...
			for (SeqAlignment align : analysis.getForeground()) {
				Pair<String, String> namever = new Pair<String, String>(align.getExpt().getName(), align.getName());
