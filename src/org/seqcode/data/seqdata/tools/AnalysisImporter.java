...
/**
 * Imports the results from a chipseq analysis (ie, peak calling) to the
 * database. Command line args are --species "species;genome" --name "iCdx2 at
 * Day4 in P2A, bowtie_unique" --version "GPS stringent threshold" --program
 * "GPS, git commit abc123" [--inactive] [--paramfile params.txt] [--foreground
 * "exptname;exprreplicate;alignname"] (can specify multiple --foreground)
 * [--background "exptname;exprreplicate;alignname"] (can specify multiple
 * --background)
 * 
 * Peak calls are read on stdin as TSV. Columns are mandatory: 1) chromosome
 * name 2) startpos 3) stoppos optional: 4) position (eg, center of peak) 5)
 * fgcount (double, number of foreground reads for the event) 6) bgcount
 * (double) 7) strength (double) 8) shape (double) 9) pvalue (double) 10 fold
 * enrichment (double)
 *
 * Paramsfile is in key=value format
 *
 * specify --inactive if you want to save the results but don't want them to
 * show up by default in lists of analyses. You might do this, eg, for early
 * rounds of GPS output.
 *
 */
...
public class AnalysisImporter {
	...
	public void parseArgs(String args[]) throws NotFoundException, SQLException, DatabaseException, IOException {
		...
		for (String s : Args.parseStrings(args, "foreground")) {
			...
			if (pieces.length == 2) {
				...
				fg.addAll(
						loader.loadAlignments(pieces[0], null, pieces[1], null, null, null, null, null, null, genome));
	...
	public SeqAnalysisResult parseLine(String line) {
		...
		return new SeqAnalysisResult(getGenome(), pieces[0], Integer.parseInt(pieces[1]), Integer.parseInt(pieces[2]),
