...
public class GeneBasedReport {
	 ...
	 * --genes refGene [--proxup 5000] [--proxdown200] [--up 10000] [--intronlen
	 * 10000] [--noalias] --regions foo.txt
	 *
	 * --regions - to read from STDIN
	 *
	 * Output columns are 0) gene name 1) positions of distal binding events 2)
	 * positions of proximal binding events 3) positions of intronic or exonic
	 * binding events
	 */
	...
	public void report() throws Exception {
		ArrayList<Region> proxevents = new ArrayList<Region>(), distalevents = new ArrayList<Region>(),
				intronevents = new ArrayList<Region>();
		for (RefGeneGenerator generator : geneGenerators) {
			...
			while (all.hasNext()) {
				...
				Region distalPromoter = g.getStrand() == '+'
						? new Region(g.getGenome(), g.getChrom(), g.getStart() - up, g.getStart() - proxup)
						: new Region(g.getGenome(), g.getChrom(), g.getEnd() + proxup, g.getEnd() + up);

				Region proximalPromoter = g.getStrand() == '+'
						? new Region(g.getGenome(), g.getChrom(), g.getStart() - proxup, g.getStart() + proxdown)
				...
				if (firstIntron) {
					...
					while (iter.hasNext()) {
						...
						if (intronicRegion == null
								|| (intron.distance(proximalPromoter) < intronicRegion.distance(proximalPromoter))) {
					...
					intronicRegion = g.getStrand() == '+'
							? new Region(g.getGenome(), g.getChrom(), g.getStart() + proxdown,
									g.getStart() + proxdown + thisintronlen)
							: new Region(g.getGenome(), g.getChrom(), g.getEnd() - proxdown - thisintronlen,
									g.getEnd() - proxdown);
				}
				...
				System.out.println(String.format("%s\t%s\t%s\t%s", g.getName(), distalevents.toString(),
						proxevents.toString(), intronevents.toString()));
			}
		}
	}
}