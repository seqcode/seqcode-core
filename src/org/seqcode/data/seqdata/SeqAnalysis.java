...
/**
 * A SeqAnalysis represents the results of running some binding-call or peak
 * finding program on a set of SeqAlignments. The name and version of the
 * analysis are independent of the name and version of the alignments, though in
 * practice they should be related; the name and version are used as they DB
 * key.
 *
 * The analysis stores a set of parameters (Map<String,String>) that can be any
 * relevant data about how the binding calls were generated.
 */
...
public class SeqAnalysis implements Comparable<SeqAnalysis> {
	...
	public void setInputs(Set<SeqAlignment> foreground, Set<SeqAlignment> background) {
	...
	private void storeinputs(PreparedStatement ps, String type, int analysis, int alignment) throws SQLException {
	...
	public void store() throws SQLException {
		...
		PreparedStatement ps = cxn.prepareStatement(String.format(q,
				org.seqcode.data.connections.DatabaseSequence.getInsertSQL(cxn, "chipseqanalysis_id")));
		ps.setString(1, name);
		...
		if (results != null && results.size() > 0) {
			ps = cxn.prepareStatement(
					"insert into analysisresults(analysis,chromosome,startpos,stoppos,position,fgcount,bgcount,strength,peak_shape,pvalue,fold_enrichment) "
							+ " values (?,?,?,?,?,?,?,?,?,?,?)");
			ps.setInt(1, dbid);
		...
		if (cxn != null)
			try {
				...
				throw new DatabaseException("Couldn't close connection with role " + SeqDataLoader.role, ex);
			}
	}

	/*
	 * stores the active flag to the database. Must be done on an object that
	 * has already been stored such that it has a DBID
	 */
	...
	/*
	 * these methods are primarily for querying an object that you've gotten
	...
	private void loadInputs() throws SQLException {
		java.sql.Connection cxn = DatabaseConnectionManager.getConnection(SeqDataLoader.role);
		PreparedStatement ps = cxn
				.prepareStatement("select alignment, inputtype from analysisinputs where analysis = ?");
		ps.setInt(1, dbid);
		...
		try {
			 ...
			 * but we told it not to do that. So this shouldn't happen
			 */
			throw new RuntimeException(e.toString(), e);
		} catch (NotFoundException e) {
			/*
			 * the loader throws a NotFoundException if it can't find the
			 * alignment. But the database constraints should have prevented us
			 * from getting an invalid alignment id back.
			 */
			throw new DatabaseException(e.toString(), e);
		}
		if (cxn != null)
	...
	public List<SeqAnalysisResult> getResults(Genome genome, Region queryRegion) throws SQLException {
		...
		while (rs.next()) {
			SeqAnalysisResult r = new SeqAnalysisResult(genome, genome.getChromName(rs.getInt(1)), rs.getInt(2),
					rs.getInt(3), isnullint(rs, 4), isnulldouble(rs, 5), isnulldouble(rs, 6), isnulldouble(rs, 7),
					isnulldouble(rs, 8), isnulldouble(rs, 9), isnulldouble(rs, 10));
	...
	public static Collection<SeqAnalysis> getAll(Boolean active) throws DatabaseException, SQLException {
		...
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			SeqAnalysis a = new SeqAnalysis(rs.getString(2), rs.getString(3), rs.getString(4),
	...
	public static SeqAnalysis get(SeqDataLoader loader, String name, String version)
			throws NotFoundException, DatabaseException, SQLException {
		return get(loader, name, version, true);
	}

	public static SeqAnalysis get(SeqDataLoader loader, String name, String version, Boolean active)
			throws NotFoundException, DatabaseException, SQLException {
		java.sql.Connection cxn = DatabaseConnectionManager.getConnection(SeqDataLoader.role);
	...
	/**
	 * returns the collection of active analyses that have a result in the
	 * specified region
	 */
