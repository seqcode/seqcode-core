...
public class SeqHit extends StrandedRegion {
	...
	public SeqHit(Genome g, String chrom, StrandedBaseCount sbc) {
		super(g, chrom, sbc.getStrand() == '+' ? sbc.getCoordinate() : sbc.getCoordinate() - 1,
				sbc.getStrand() == '+' ? sbc.getCoordinate() + 1 : sbc.getCoordinate(), sbc.getStrand());
	...
	public SeqHit shiftExtendHit(int ext, int shift) {
		if (getStrand() == '+') {
			return new SeqHit(getGenome(), getChrom(), getStart() + shift - (ext / 2), getEnd() + shift + (ext / 2),
					getStrand(), weight);
		} else {
			return new SeqHit(getGenome(), getChrom(), getStart() - shift - (ext / 2), getEnd() - shift + (ext / 2),
					getStrand(), weight);
