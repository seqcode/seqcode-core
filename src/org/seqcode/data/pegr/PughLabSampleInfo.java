 ...
 *
 *
 *         CREATE TABLE `PughLabSampleInfo` ( `SAMPLE_ID` int(11) unsigned NOT
 *         NULL AUTO_INCREMENT, `PROJECT_ID` char(40) DEFAULT NULL, `RUN_ID`
 *         char(10) DEFAULT NULL, `NAME` char(255) DEFAULT NULL, `TARGET`
 *         char(40) DEFAULT NULL, `ANTIBODY` char(40) DEFAULT NULL,
 *         `ANTIBODY_VOLUME` char(10) DEFAULT NULL, `STRAIN` char(40) DEFAULT
 *         NULL, `MUTATION` char(40) DEFAULT NULL, `MEDIA` char(40) DEFAULT
 *         NULL, `PERTURB` char(40) DEFAULT NULL, `ASSAY_CODE` char(10) DEFAULT
 *         NULL, `REP_CODE` char(10) DEFAULT NULL, `USERID` char(10) DEFAULT
 *         NULL, `SEQ_DATE` char(10) DEFAULT NULL, `UNIQ_ID` char(40) NOT NULL
 *         DEFAULT '', `PRI_GENOME` char(20) DEFAULT NULL, `SEQ_BY` char(20)
 *         DEFAULT NULL, `COMMENT` blob, `REF_GENOME` char(80) DEFAULT NULL,
 *         `SUCCESS` enum('Good','Failed','Pending') DEFAULT 'Pending',
 *         `INDEX_COUNT` int(11) unsigned NOT NULL DEFAULT '0', `PUBLIC_FLAG`
 *         enum('Y','N') DEFAULT 'N', `SRA_ID` char(15) DEFAULT NULL, PRIMARY
 *         KEY (`SAMPLE_ID`,`UNIQ_ID`) ) ENGINE=MyISAM AUTO_INCREMENT=7106
...
public class PughLabSampleInfo implements Comparable<PughLabSampleInfo> {
	...
	public String getAntibodyVol() {
		return antibody_volume;
	}
	...
	/**
	 * Remake name here - as I'm not sure if database entries always reflect
	 * updates
	 */
	public String buildName() {
		String ab = antibody_volume == null ? antibody : antibody + "-" + antibody_volume;
		String repID = assay_code + rep_code;
		return target + "_" + ab + "_" + strain + "_" + mutation + "_" + media + "_" + perturb + "_" + repID + "_"
				+ userid + "-" + seq_date + "_" + uniq_id;
	}
	...
	public String getSeqDataTargetName() {
		...
		while (tname.endsWith("_-")) {
			tname = tname.substring(0, tname.lastIndexOf("_-"));
		}
		while (tname.startsWith("-_")) {
			tname = tname.replaceFirst("-_", "");
		}
		return tname;
	}
	...
	public String getSeqDataConditionName() {
		...
		while (cname.endsWith("_-")) {
			cname = cname.substring(0, cname.lastIndexOf("_-"));
		}
		while (cname.startsWith("-_")) {
			cname = cname.replaceFirst("-_", "");
		}
		return cname;
	}
	...
	public String convertAssayCode() {
		...
		if (assay_code.equals("XO")) {
			assay = "CHIPEXO";
		} else if (assay_code.equals("CS")) {
			assay = "CHIPSEQ";
		} else if (assay_code.equals("IN") || assay_code.equals("NA")) {
			assay = "INPUT";
		} else if (assay_code.equals("KS")) {
			assay = "KWIKSEQ";
		} else if (assay_code.equals("BS")) {
			assay = "BISSEQ";
		} else if (assay_code.equals("CT")) {
			assay = "CHIPTOPO";
		} else if (assay_code.equals("PS")) {
			assay = "PERMSEQ";
		} else if (assay_code.equals("MN")) {
			assay = "MNASECHIPSEQ";
		}
		if (target.toLowerCase().matches("no-tag") || target.toLowerCase().matches("no-ab")
				|| target.toLowerCase().matches("None") || target.toLowerCase().matches("IgG")) {
			assay = "CONTROL";
		}
		return assay;
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof PughLabSampleInfo)) {
			return false;
		}
		PughLabSampleInfo c = (PughLabSampleInfo) o;
		if (sample_id != c.sample_id) {
			return false;
		}
		return true;
	}
	...
	public static PreparedStatement createLoadAll(java.sql.Connection c) throws SQLException {
		return c.prepareStatement(
				"select sample_id, project_id, run_id, name, target, antibody, antibody_volume, strain, mutation, media, perturb, assay_code, rep_code, userid, seq_date, uniq_id, pri_genome, seq_by, comment, ref_genome, success, index_count, public_flag, sra_id "
