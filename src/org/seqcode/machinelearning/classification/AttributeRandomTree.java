package org.seqcode.machinelearning.classification;
 ...
 */
public class AttributeRandomTree extends AbstractClassifier implements OptionHandler,
  WeightedInstancesHandler, Randomizable, Drawable, PartitionGenerator {
	...
	public int[] nodeNumInstances()
	{
		...
		//			if (weights[i] != 0)
		//				System.out.println(i + " " + m_Info.attribute(i).name() + " " + weights[i]);
		return weights;
	}
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (data.classAttribute().isNumeric()) {
      trainVariance = AttributeRandomTree.singleVariance(classProbs[0], totalSumSquared,
  ...
  protected class Tree implements Serializable {
    ...
    public void backfitData(Instances data) throws Exception {
      ...
      double trainVariance = 0;
      if (data.classAttribute().isNumeric()) {
        trainVariance = AttributeRandomTree.singleVariance(classProbs[0],
    ...
    protected void backfitData(Instances data, double[] classProbs,
      ...
      if (data.classAttribute().isNumeric()) {
        ...
        priorVar = AttributeRandomTree.singleVariance(totalSum, totalSumSquared,
          totalSumOfWeights);
      }
      ...
      if (attribute.isNominal()) {
        ...
        for (int i = 0; i < indexOfFirstMissingValue; i++) {
          ...
          if (inst.value(att) > currSplit) {
            currVal = AttributeRandomTree.variance(currSums, currSumSquared,
              currSumOfWeights);
            if (currVal < bestVal) {
              bestVal = currVal;
