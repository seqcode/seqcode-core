		...
		for (int queryoffset = 0; queryoffset <= query.matrix.length - complen; queryoffset++) {
			for (int targetoffset = 0; targetoffset <= target.matrix.length - complen; targetoffset++) {
				...
				for (int i = 0; i < complen; i++) {
					score += Math.sqrt(Math
							.pow(query.matrix[queryoffset + i]['A'] - target.matrix[targetoffset + i]['A'], 2)
				...
				score = 0;
				for (int i = 0; i < complen; i++) {
					score += Math.sqrt(Math.pow(
							query.matrix[queryoffset + complen - i - 1]['T'] - target.matrix[targetoffset + i]['A'], 2)
							+ Math.pow(query.matrix[queryoffset + complen - i - 1]['G']
									- target.matrix[targetoffset + i]['C'], 2)
							+ Math.pow(query.matrix[queryoffset + complen - i - 1]['C']
									- target.matrix[targetoffset + i]['G'], 2)
							+ Math.pow(query.matrix[queryoffset + complen - i - 1]['A']
									- target.matrix[targetoffset + i]['T'], 2));
				}
				if (score < minscore) {
					minscore = score;
				}
