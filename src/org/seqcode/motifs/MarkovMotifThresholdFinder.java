...
public class MarkovMotifThresholdFinder {
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		if (!ap.hasKey("species") || !ap.hasKey("genome") || (!ap.hasKey("motifname") && !ap.hasKey("motiffile"))) {
			System.err.println("Usage:\n " + "MarkovMotifThresholdFinder " + "--species <organism name> "
					+ "--genome <genome version> " + "--motifname <weightmatrix name> "
					+ "--motifversion <weightmatrix version> " + "--motiffile <file containing motifs> "
					+ "--back <background Markov model> "
					+ "--win <window of sequence around positive/negative points> "
					+ "--num <number of sequences to sample> " + "--printroc ");
			return;
		}
		...
		try {
			...
			if (backFile == null) {
				backMod = new MarkovBackgroundModel(
						CountsBackgroundModel.modelFromWholeGenome(Genome.findGenome(genome)));
			} else {
			...
			if (printROC) {
				...
				for (WeightMatrix matrix : motifList) {
					...
					System.out.println(matrix.getName() + "\t" + min + "\t" + max + "\t" + thres_1 + "\t" + thres_05
							+ "\t" + thres_01 + "\t" + thres_005 + "\t" + thres_001);
					// System.out.println("Threshold for
					// Sp=0.005:\t"+thres_005);
	...
	public MarkovMotifThresholdFinder(WeightMatrix wm, MarkovBackgroundModel markov, int numSim) {
		...
		motif = wm;
		if (wm == null) {
			System.err.println("No motif specified");
			System.exit(1);
		}
		back = markov;
	}

	public void setNumTest(int nt) {
		numTest = nt;
	}
	...
	public void setROC(boolean r) {
		ROC = r;
	}

	public void setRandomSeq(ArrayList<String> rand) {
		if (rand.size() > 0) {
			seqSet = rand;
			seqGenerated = true;
		}
	...
	public double execute(double Sp) {
		if (Sp < 0 || Sp > 1) {
			System.err.println("Invalid Sp value in MarkovMotifThreshold");
			System.exit(1);
		}
