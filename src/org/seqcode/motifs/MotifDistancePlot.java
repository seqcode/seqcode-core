 ...
 * a motif and a maximum distance from the point (or center of region) to look
 * for the motif. The y-value of the plot is the average distance to a motif
 * over a sliding window of n regions/points. The x-y values for the plot are
 * produced on stdout.
 *
 * java MotifDistancePlot --species "$MM;mm9" --wm "Cdx2;Jaspar" [--maxdistance
 * 500] [--smooth 100] [--minpercent .75] < regions.txt > plot.txt
 */
...
public class MotifDistancePlot {
	...
	public static void main(String args[]) throws Exception {
		...
		int sumdistances = 0;
		int distances[] = new int[smooth]; // circular buffer for whether or not
		...
		while ((line = reader.readLine()) != null) {
			...
			if (smooth != 0 && lineno >= smooth && lineno % smooth == 0) {
				...
				System.out.println(String.format("%d\t%d\t%.2f", (lineno - smooth), median, mean));
			}
			if (r == null) {
			...
			for (WeightMatrix wm : matrices) {
				...
				List<WMHit> hits = WeightMatrixScanner.scanSequence(wm, minscore, string);
				for (WMHit h : hits) {
