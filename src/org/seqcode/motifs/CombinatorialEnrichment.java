...
/**
 * Scans for significant pairs of motifs. Specify the genome and set of motifs
 * in the same way as for CompareEnrichment.
 *
 * Output fields are 1) logfoldchange 2) count in set one (fg) 3) size of set
 * one 4) freq in set one 5) count in set two (bg) 6) size of set two 7)
 * frequency in set two 8) pvalue Then for each matrix, the name, version,
 * cutoff percent, cutoff value
 */
...
public class CombinatorialEnrichment extends CompareEnrichment {
	...
	public static Map<String, WMHit[]> doScan(int threads, double cutoffpercent, List<WeightMatrix> matrices,
		...
		for (int i = 0; i < threads; i++) {
			Thread t = new Thread(new Scanner(v, seqs, matrices, output, cutoffpercent));
			t.start();
			threadlist.add(t);
		}
	...
	class Counter implements Runnable {
		...
		public int count(int matrixone, int matrixtwo, double threshone, double threshtwo, Map<String, WMHit[]> hits) {
			int count = 0;
			for (String s : hits.keySet()) {
				WMHit[] hitlist = hits.get(s);
				WMHit one = hitlist[matrixone];
				WMHit two = hitlist[matrixtwo];
				if (one != null && two != null && one.score >= threshone && two.score >= threshtwo) {
				...
				for (int j = i + 1; j < matrices.size(); j++) {
					...
					while (percenti <= 1) {
						...
						while (percentj <= 1) {
							...
							if (thetatwo < minfrac || thetatwo <= 0) {
								continue;
							}
...
class CombResult {
	...
	public String toString() {
		String out = String.format("%2.1f\t%d\t%d\t%.2f\t%d\t%d\t%.2f\t%.4e\t", logfoldchange, countone, sizeone,
				freqone, counttwo, sizetwo, freqtwo, pval);
		for (int i = 0; i < matrices.size(); i++) {
			out += String.format("\t%s\t%s\t%.2f\t%.1f", matrices.get(i).name, matrices.get(i).version, percents.get(i),
					cutoffs.get(i));
		}
		return out;
	}
...
class Scanner implements Runnable {
	...
	public Scanner(Vector<String> k, Map<String, char[]> s, List<WeightMatrix> m, Map<String, WMHit[]> o, double c) {
		if (k == null) {
		...
		try {
			while (true) {
				...
				for (int i = 0; i < hitlist.length; i++) {
					...
					hitlist[i] = WeightMatrixScanner.scanSequenceBestHit(matrix, (float) (maxscore * cutoffpercent),
							seqs.get(s));
				}
				synchronized (output) {
