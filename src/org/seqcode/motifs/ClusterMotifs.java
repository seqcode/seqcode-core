...
public class ClusterMotifs {
	...
	public ClusterMotifs(WMComparator comp, ClusterRepresentative<WeightMatrix> rep,
	...
	public static void drawCluster(Collection<WeightMatrix> cluster, String fname, int pixwidth, int pixheight,
			int maxcols) {
		...
		for (cols = 1; cols <= maxcols; cols++) {
			...
			// System.err.println("size " + cluster.size() + " cols=" + cols + "
			// => " + sq);
		...
		cols = minind + 1;
		rows = (int) Math.ceil(cluster.size() / (float) cols);
		// System.err.println("Trying to use cols " + cols + ", rows " + rows +
		// " for size " + cluster.size());
