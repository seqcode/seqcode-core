...
public class MotifOccurrenceMatrix {
	...
	/**
	 * reads the list of genes from STDIN. Each line is one elemen of genes and
	 * contains the tab-separated gene names
	 */
	...
	/**
	 * Finds a WeightMatrix for each gene by searching through the aliases in
	 * order.
	 *
	 * It might eventually make sense to store a set of matrices and then score
	 * them all and take the highest score for each target gene.
	 */
	public void getMatrices() {
		...
		for (int i = 0; i < genes.size(); i++) {
			...
			if (matrix == null) {
				// now see what aliases we have in our database for the gene and
				// try all of those
	...
	/**
	 * Returns the genomic region to search for a given set of aliases. Looks
	 * through the aliases in order, trying all elements of geneGenerators on
	 * the first before going on to the second.
	 *
	...
	public Region getPromoterForGene(List<String> aliases) {
		for (String a : aliases) {
			for (RefGeneGenerator g : geneGenerators) {
				...
				if (iter.hasNext()) {
					Gene gene = iter.next();
					StrandedRegion sr = new StrandedRegion(gene.getGenome(), gene.getChrom(), gene.getFivePrime(),
							gene.getFivePrime(), gene.getStrand());
	...
	public void computeMatrix() throws SQLException {
		...
		for (int i = 0; i < genes.size(); i++) {
			...
			for (int j = 0; j < genes.size(); j++) {
				...
				List<WMHit> hits = WeightMatrixScanner.scanSequence(m, (float) (m.getMaxScore() * .75),
						seq.toCharArray());
