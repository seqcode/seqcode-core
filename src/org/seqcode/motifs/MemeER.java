...
/**
 * Utility to run MEME within a java class and from a command line. It also
 * evaluates significance of the MEME discovered motifs against randomly picked
 * genomic sequences and calculates ROC score.
 * 
 * Input: - Genome - MEME path and MEME arguments - Window around peaks in which
 * to get sequences Output: - MEME motif files - ROC score for each motif
 * 
...
public class MemeER {
	...
	public void setMotifMinROC(double minroc) {
		MOTIF_MIN_ROC = minroc;
	}

	public Pair<List<WeightMatrix>, List<WeightMatrix>> execute(List<String> sequences, File memeOutDirFullName,
			boolean bestOnly) {
		...
		try {
			...
			// Test if meme directory exists. If it does, recursively delete
			// contents
			...
			Process proc = Runtime.getRuntime()
					.exec(MEMEcmd + " " + seqFilename + " " + MEMEargs + " -o " + memeOutDir);
			// any error message?
			StreamGobbler errorGobbler = new StreamGobbler(proc.getErrorStream(), "MEME_ERR", true);
			// any output?
			StreamGobbler outputGobbler = new StreamGobbler(proc.getInputStream(), "MEME_OUT", true);
			// kick them off
			errorGobbler.start();
			outputGobbler.start();
			// any error???
	...
	public static void main(String[] args) {
		try {
			...
			for (int p = 0; p < points.length; p++) {
				...
				// Pair<List<WeightMatrix>,List<WeightMatrix>> matrices =
				// meme.execute(seqs, null, false);
				// allowing to specify output directory
				Pair<List<WeightMatrix>, List<WeightMatrix>> matrices = meme.execute(seqs,
						new File(outFolderName + File.separator + "meme_out"), false);
				...
				if (wm.size() > 0) {
					...
					for (int w = 0; w < fm.size(); w++) {
						if (fm.get(w) != null) {
							System.err.println("\t" + fm.get(w).getName() + "\t" + WeightMatrix.getConsensus(fm.get(w))
									+ "\tROC:" + String.format("%.2f", rocScores[w]));
						}
			...
			for (int m = 0; m < selectedMotifs.size(); m++) {
				...
				// String out =
				// WeightMatrix.printTransfacMatrix(selectedMotifs.get(m),"Motif_"+Integer.toString(m));
				System.err.println(out);

				// optional : print selected motif ROC and PWM to a file
				if (writer != null) {
					writer.println("\t" + selectedMotifs.get(m).getName() + "\t"
							+ WeightMatrix.getConsensus(selectedMotifs.get(m)) + "\tROC:"
							+ String.format("%.2f", selectedMotifsRocs.get(m)));
					writer.println(out);
				}
			}
	...
	public class LabeledDouble implements Comparable<LabeledDouble> {
		...
		public LabeledDouble(Double d, Integer i) {
			dat = d;
			label = i;
		}
	...
	public static List<Region> randomRegionPick(Genome gen, List<Region> blackList, int numSamples, int sampleSize) {
		...
		// valid and not overlapping repeats.
			...
			for (int c = 0; c < numChroms && !found; c++) {
				if (randPos < total + chromoSize[c]) {
					...
					if (randPos + sampleSize < total + chromoSize[c]) {
						potential = new Region(gen, chromoNames[c], (int) (randPos - total),
								(int) (randPos + sampleSize - total));
