...
/**
 * Scans sequence to determine the overal density of motifs (hits / window) in
 * windows of some size.
 *
 * --cutoff .7 minimum percent (specify between 0 and 1) match to maximum motif
 * score that counts as a match. --species "$SC;SGDv2" --window 100 specifies
 * the window size to scan. The windows are non-overlapping
 *
 * can also specify --accept to give a regex which the motif name must match of
 * --reject to specify a regex that the motif name must not match. Remember the
 * regex must match the *entire* name, so use something like Hnf.*
 *
 * 
 * Output format is chrom\tstart\tstop\tcount
 * 
 */
	...
	public static void main(String args[]) throws Exception {
		...
		for (Region region : regions) {
			...
			for (WeightMatrix m : matrices) {
				...
				List<WMHit> hits = WeightMatrixScanner.scanSequence(m, (float) (m.getMaxScore() * cutoffpercent),
						aschars);
				if (multicount) {
					for (WMHit h : hits) {
			...
			for (int i = 0; i < counts.length; i++) {
				if (counts[i] > 0) {
					System.out.println(String.format("%s\t%d\t%d\t%d", region.getChrom(),
							region.getStart() + i * windowsize, region.getStart() + (i + 1) * windowsize, counts[i]));
				}
			}
		}
	}
