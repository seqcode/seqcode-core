...
public class ConsensusSequence {
	...
	private boolean[][] consensusMatrix; // Represents whether the consensus
											// allows each character at each
											// position
	private String consensus;
	private int length;

	public ConsensusSequence(String cons) {
		this(cons.toCharArray());
	}
	...
	public ConsensusSequence(char[] cons) {
		...
		for (int i = 0; i < cons.length; i++) {
			if (Character.toUpperCase(cons[i]) == 'A' || Character.toUpperCase(cons[i]) == 'C'
					|| Character.toUpperCase(cons[i]) == 'G' || Character.toUpperCase(cons[i]) == 'T')
				consensusMatrix[i][Character.toUpperCase(cons[i])] = true;
			else if (Character.toUpperCase(cons[i]) == 'N') {
				consensusMatrix[i]['A'] = true;
				consensusMatrix[i]['C'] = true;
				consensusMatrix[i]['C'] = true;
				consensusMatrix[i]['T'] = true;
			} else if (Character.toUpperCase(cons[i]) == 'R') {
				consensusMatrix[i]['A'] = true;
				consensusMatrix[i]['G'] = true;
