...
/**
 * Scans a genome for a previously loaded weight matrix. Can read either from
 * FASTA files or from the database. Can put results into the database or print
 * to the screen. Can also load scan results from a file. All matrices are
 * converted to log-odds before scanning.
 *
 * --species "$MM;mm9" --scanname "90%" [required only when storing to db]
 *
 * --wm, --acceptwm, --rejectwm, --acceptwmver, --rejectwmver, --acceptwmtype,
 * --rejectwmtype used to specify set of matrices [--print] print results rather
 * than storing to db [--loadfile foo.txt] load results from file rather than
 * doing a new scan [--cutoff .9] as a fraction of maximum log-odds score
 */
...
public class WeightMatrixScanner {
	...
	private PreparedStatement getScan, insertScan, getscanid, insertScanned, getScanned, getScannedGenome,
			insertScannedGenome, insertHit;
		...
		for (WeightMatrix matrix : matrices) {
			...
			if (print) {
				...
				insertHit = cxn.prepareStatement("insert into wms_hits(scan,chromosome,startpos,stoppos,strand,score) "
						+ " values (" + scanid + ",?,?,?,?,?)");
				consumer = new StoreConsumer(cxn, insertHit);
			...
			if (fastafiles.size() != 0) {
				...
				for (String fastafile : fastafiles) {
					scanFasta(genome, matrix, consumer, cutoffscore, fastafile, regions);

				}
			} else {
				try {
					scanFromDB(genome, matrix, consumer, (float) cutoff, regions);
	...
	public void setup() throws SQLException {
		...
		insertScanned = cxn
				.prepareStatement("insert into wms_scanned_regions(scan,chromosome,startpos,stoppos) values(?,?,?,?)");
		getScanned = cxn.prepareStatement(
				"select count(*) from wms_scanned_regions where scan = ? and chromosome = ? and startpos = ? and stoppos = ?");
		getScannedGenome = cxn
				.prepareStatement("select count(*) from wms_scanned_genomes where scan = ? and genome = ?");
		insertScannedGenome = cxn.prepareStatement("insert into wms_scanned_genomes (scan,genome) values (?,?)");
		cxn.setAutoCommit(false);
	}

	/*
	 * retrieves the DBID of the scan with the specified name and cutoff for a
	 * weight matrix
	 */
	public int getScanID(int wmid, String scanname, float cutoffscore) throws SQLException {
		int scanid;
	...
	/*
	 * adds a record to the wms_scanned_regions table. This needs to be done for
	 * all weight matrix scans that are storing results in the database
	 */
	...
	public void storeRegionList(Genome genome, int scanid, List<Region> regions) {
	...
	public void loadFile(Genome genome, String fname, float cutoffscore, WMConsumer consumer)
			...
			while ((line = reader.readLine()) != null) {
				...
				if (pieces.length >= 5) {
					...
					if (score >= cutoffscore) {
						hits.add(new WMHit(-1, genome.getChromID(pieces[0]), Integer.parseInt(pieces[1]),
								Integer.parseInt(pieces[2]), pieces[3], score));
					}
				}
			}
	...
	public List<WMHit> scanFasta(Genome genome, WeightMatrix matrix, WMConsumer consumer, float cutoffscore,
			String fastafile, List<Region> regions) {
		try {
			...
			while (stream.hasNext()) {
				...
				if (chromid >= 0) {
					if (end == -1) {
						regions.add(new Region(genome, name, 0, aschars.length));
					} else {
						regions.add(new Region(genome, name, offset, end));
					}
				}
				...
				List<WMHit> hits = scanSequence(matrix, cutoffscore, aschars);
	...
	public void scanFromDB(Genome genome, WeightMatrix matrix, WMConsumer consumer, float scorecutoff,
		...
		for (Region region : regions) {
			...
			while (rstart < region.getEnd()) {
				...
				char[] bytes = seqgen.execute(new Region(region.getGenome(), region.getChrom(), rstart, rend))
						.toCharArray();
				for (WMHit hit : scanSequence(matrix, scorecutoff, bytes)) {
					hit.chromid = chromid;
					hit.start += rstart;
					hit.end += rstart;
					results.add(hit);
	...
	/*
	 * Scans a list of regions for a weight matrix and returns the best hit in
	 * each Region (no cutoff).
	 */
	public static ArrayList<WMHit> scanRegionsReturnBest(Genome genome, WeightMatrix matrix, ArrayList<Region> regions)
		...
		for (Region region : regions) {
			...
			// This method is meant for getting the best hit in small regions; I
			// am not working the region in chunks
			if (rend - rstart < length) {
				break;
			}
			...
			for (int i = 0; i < scoreScape.length; i++) {
				if (scoreScape[i] > maxScore) {
					maxScore = scoreScape[i];
					maxI = i;
				}
				// I'm introducing a slight bias towards the center of the
				// sequence for my own selfish reasons
				if (scoreScape[i] == maxScore
						&& Math.abs(i - (region.getWidth() / 2)) < Math.abs(maxI - (region.getWidth() / 2))) {
					maxScore = scoreScape[i];
					maxI = i;
				}
			}
			WMHit hit = new WMHit(-1, chromid, maxI + rstart, maxI + rstart + matrix.length(),
					String.format("%c", hitStrands[maxI]), maxScore.floatValue());
			results.add(hit);
		}
		return results;
	}
	 ...
	 */
	public static WMHit scanSequenceBestHit(WeightMatrix matrix, float scorecutoff, char[] sequence) {
		List<WMHit> hits = scanSequence(matrix, scorecutoff, sequence);
		if (hits.size() == 0) {
			return null;
		} else {
	 ...
	 */
	public static List<WMHit> scanSequence(WeightMatrix matrix, float scorecutoff, char[] sequence) {
		ArrayList<WMHit> results = new ArrayList<WMHit>();
		...
		/*
		 * now reverse complement the sequence and scan that. basically the same
		 * as before but with slightly different computation of the hit
		 * coordinates
		 */
		...
		SequenceUtils.reverseComplement(sequence); // put the sequence back the
													// way we found it
	...
	/*
	 * returns an array of scores that describe how well the matrix matches each
	 * base of the input sequence. Indices in the output array correspond ot the
	 * score of the match *starting* at that position. This means that the last
	 * n entries are meaningless (where n = matrix length)
	 */
	public static Double[] scanSequence(WeightMatrix matrix, char[] sequence) {
	...
	class PrintConsumer extends WMConsumer {
		...
		public void consume(WMHit hit) {
			...
			out.println(matrix.toString() + "\t" + chromname + "\t" + hit.start + "\t" + hit.end + "\t" + hit.score
					+ "\t" + hit.strand);
		}
	...
	class StoreConsumer extends WMConsumer {
		...
		public StoreConsumer(java.sql.Connection c, PreparedStatement i) {
			cxn = c;
			insertHit = i;
			done = 0;
		}
