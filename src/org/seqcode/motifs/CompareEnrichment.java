...
/**
 * Compare the frequencies of a set of motifs between two FASTA files usage:
 * java org.seqcode.motifs.CompareEnrichment --species "$SC;SGDv2" [--first
 * foo.fasta] [--second bar.fasta]
 *
 * can also specify --acceptwm to give a regex which the motif name must match
 * of --rejectwm to specify a regex that the motif name must not match. Remember
 * the regex must match the *entire* name, so use something like Hnf.*
 *
 * If --first is not supplied, then regions are read from STDIN. If --second is
 * not supplied, then random regions are chosen according to --randombgcount and
 * --randombgsize.
 *
 * Input files ending in .fasta or .fa are parsed as fasta. Otherwise, they're
 * parsed as a list of regions.
 *
 * --cutoff .5 minimum percent (specify between 0 and 1) match to maximum motif
 * score that counts as a match. --filtersig .001 maximum pvalue for reporting
 * an enrichment between the two files --minfoldchange 1 --minfrac 0 minimum
 * fraction of the sequences that must contain the motif (can be in either file)
 * --savedata motif_presence_file.txt --savedatahits --outfg output_fg.fasta
 * --outbg output_bg.fasta --mask name;version;cutoff --threads 4 to control
 * number of parallel threads
 *
 * The comparison code will check all percent cutoffs between the value you
 * specify as --cutoff and 1 (in increments of .05) to find the most significant
 * threshold that also meets the other criteria.
 *
 * Output columns are 1) log foldchange in frequency between the two files 2)
 * motif count in first set 3) size of first set 4) frequency in first set 5)
 * motif count in second set 6) size of second set 7) frequency in second set 8)
 * pvalue of first count given second frequency 9) motif name 10) motif version
...
public class CompareEnrichment {
	...
	public static final double step = .05; // when looking for most significant
											// threshold, increment % identity
											// by this much each try
	int randombgcount = 1000, // number of random background regions to pic
			randombgsize = 100, // size of random background regions
			parsedregionexpand; // expand input regions by this much on either
								// side
	Genome genome;
	...
	Map<String, char[]> foreground, background; // foreground and background
												// sequences
	Map<WeightMatrix, Double> maskingMatrices; // matrices to mask out of
												// foreground and background
	ArrayList<String> fgkeys, bgkeys; // order in which to scan; also used when
										// saving region list to a matrix of
										// motif-sequence presence
	...
	/**
	 * reads region strings, eg "3:100-5000" fromreader returns corresponding
	 * sequence as output. If matchedRegions is not null, then fills it in with
	 * the flanking regions for each output region. You can use matchedRegion as
	 * background sequence since it came from the same approximate loci as the
	 * foreground that was read
	 */
	public static Map<String, char[]> readRegions(Genome g, BufferedReader reader, int parsedregionexpand,
		...
		while ((line = reader.readLine()) != null) {
			...
			if (matchedRegions != null) {
				Region before = new Region(region.getGenome(), region.getChrom(), region.getStart() - region.getWidth(),
						region.getStart());
				Region after = new Region(region.getGenome(), region.getChrom(), region.getEnd(),
				...
				matchedRegions.put(before.toString(), seqgen.execute(before).toCharArray());
				matchedRegions.put(after.toString(), seqgen.execute(after).toCharArray());
			}
		}
		return output;
	}
	...
	public static Map<String, char[]> randomRegions(Genome genome, int count, int size) {
		...
		while (count-- > 0) {
			...
			for (int i = 0; i < chromlengths.size(); i++) {
				if (i == chromlengths.size() - 1 || target + size < chromlengths.get(i)) {
					Region r = new Region(genome, chromnames.get(i), (int) target, (int) target + size);
	...
	public static int countMeetsThreshold(Map<String, List<WMHit>> hits, double t) {
		int count = 0;
	...
	public void maskSequence(WeightMatrix wm, double threshold, char[] seq) {
		List<WMHit> hits = WeightMatrixScanner.scanSequence(wm, (float) threshold, seq);
		for (WMHit hit : hits) {
	...
	public static CEResult doScan(WeightMatrix matrix, Map<String, char[]> fg, Map<String, char[]> bg,
			List<String> fgkeys, List<String> bgkeys, double cutoffpercent, double filtersig, double minfoldchange,
			double minfrac, double maxbackfrac, PrintWriter savedatafg, PrintWriter savedatabg, boolean savedatahits) {
		...
		for (String s : fgkeys) {
			List<WMHit> hits = WeightMatrixScanner.scanSequence(matrix, (float) (maxscore * cutoffpercent), fg.get(s));
			fghits.put(s, hits);
		}
		for (String s : bgkeys) {
			List<WMHit> hits = WeightMatrixScanner.scanSequence(matrix, (float) (maxscore * cutoffpercent), bg.get(s));
			bghits.put(s, hits);
		}
		...
		while (percent <= 1.0) {
			...
			if (pval <= filtersig &&
			// pval <= result.pval &&
					Math.abs(fc) >= Math.abs(result.logfoldchange) && Math.abs(fc) >= Math.abs(Math.log(minfoldchange))
					&& (thetatwo < maxbackfrac) && (thetaone >= minfrac || thetatwo >= minfrac)) {
				bestthresh = t;
				result.pval = pval;
	...
	public void parseArgs(String args[]) throws Exception {
		...
		if (firstfname == null) {
			...
			if (firstfname.matches(".*\\.fasta") || firstfname.matches(".*\\.fa")) {
		...
		if (!matchedbg) {
			if (secondfname == null) {
				System.err.println(
						"No background file given.  Generating " + randombgcount + " regions of size " + randombgsize);
				background = randomRegions(genome, randombgcount, randombgsize);
			} else {
				if (secondfname.matches(".*\\.fasta") || secondfname.matches(".*\\.fa")) {
					background = readFasta(new BufferedReader(new FileReader(secondfname)));
				} else {
					background = readRegions(genome, new BufferedReader(new FileReader(secondfname)),
							parsedregionexpand, null);
				}
			}
		}
	}
	...
	public void doScan() {
		...
		for (WeightMatrix matrix : matrices) {
			...
			CEResult result = doScan(matrix, foreground, background, fgkeys, bgkeys, cutoffpercent, filtersig,
					minfoldchange, minfrac, maxbackfrac, savedatafg, savedatabg, savedatahits);
			if (savedatafg != null) {
				savedatafg.println();
				savedatabg.println();
			}
			if (result.pval <= filtersig && Math.abs(result.logfoldchange) >= Math.abs(Math.log(minfoldchange))
					&& (result.freqtwo <= maxbackfrac) && (result.freqone >= minfrac || result.freqtwo >= minfrac)) {
