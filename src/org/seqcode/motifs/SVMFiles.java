...
/**
 * Generate input files for libsvm or liblinear classification. Uses both motifs
 * (see parameters for CompareEnrichment) and short kmers. The short kmers
 * assume the regions are centered around a motif of size --motifwidth
 * 
 * Specify the kmers with --k 4 --maxoffset 3
 *
 * java SVMFiles --species "$MM;mm9" --first foo.txt --second bar.txt --k 4
 * --maxoffset 2 --motifwidth 12 --expand 6
 *
 * Where foo.fasta and bar.txt are files of genomic points (single base
 * positions). Or you could use width 0 where the files are genomic regions of
 * width 12.
 *
 * TODO: - right now the kmer stuff doesn't look for reverse complements
 *
 */
...
class KmerFeature {
	...
	public String toString() {
		return String.format("%s at %d", new String(kmer), offset);
	}
}