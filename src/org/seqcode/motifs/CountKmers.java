...
/**
 * Use: java org.seqcode.motifs.CountKmers --species "$SC;sacCer3" --mink 4
 * --maxk 6 [--outputcounts] [--includerc] [--topn 100]
 *
 * outputcounts: output counts instead of frequencies includerc: include counts
 * from reverse complement strand too topn: only output the top N kmers instead
 * of all of them. table: output frequencies per region
 *
 */
...
public class CountKmers {
	...
	public void parseArgs(String args[]) throws NotFoundException {
		...
		init(Args.parseInteger(args, "mink", 1), Args.parseInteger(args, "maxk", 4));
	}
	...
	public Map<String, Integer> getCounts(int k) {
		...
		for (int i = 0; i < l.length; i++) {
			if (l[i] > 0) {
				output.put(indexToString(i, k), l[i]);
			}
		}
		return output;
	}
