...
public class WeightMatrixSearch {
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].equals("--name") || args[i].equals("--matrix")) {
				name = args[++i];
			}
		...
		try {
			...
			int wmid = WeightMatrix.getWeightMatrixID(o.getDBID(), name, version);
			...
			for (int i = 0; (i < keys.length && i < maxresults); i++) {
				...
				if (quotable) {
					...
					srs.next();
					System.out.println("--species \"" + srs.getString(1) + "\" --name \"" + hit.name + "\" --version \""
							+ hit.version + "\"\t\t" + totalresults.get(hit));
					srs.close();
				} else {
					System.out.println(hit.name + ", " + hit.version + ", " + hit.type + " : " + totalresults.get(hit));
				}
				if (showmatrix) {
