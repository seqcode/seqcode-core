...
public class MotifAnalysisMultiMotif {
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		double globalThreshold = ap.hasKey("globalthres") ? new Double(ap.getKeyValue("globalthres")).doubleValue()
				: Double.MIN_VALUE;
		double fractionThreshold = ap.hasKey("fracthres") ? new Double(ap.getKeyValue("fracthres")).doubleValue()
				: Double.MIN_VALUE;
	...
	// Print best hits in regions for each motif (only prints if the region
	// contains ANY motif)
	public void printBestMotifHits(boolean printBestOnly) {
		...
		for (int i = 0; i < posSet.size(); i++) {
			if (contains[i]) {
				...
				if (printBestOnly)
					System.out.print(
							"\t" + motifs.get(best).getName() + "\t" + bestScores[best][i] + "\t" + bestHits[best][i]);
				System.out.print("\n");
			}
		}
	}
	...
	public void printHitStats() {
		...
		boolean[] anyHitPos = new boolean[posSeq.size()];
		for (int i = 0; i < posSeq.size(); i++) {
			anyHitPos[i] = false;
		}
		boolean[] anyHitNeg = new boolean[negSeq.size()];
		...
		// ArrayList<IndexedDouble> n_hscores =
		// benjaminiHochbergCorrection(hscores);
		...
		// }
		for (IndexedDouble x : n_pscores) {
			stats.get(x.id).pvalPeaks = x.value;
		}

		for (MotifStats m : stats) {
			m.print();
		}
		...
		System.out.println("ANY\t" + posTotal + "\t\t\t" + anyPosHit + "\t" + anyPosRate + "\t" + negTotal + "\t\t\t"
				+ anyNegHit + "\t" + anyNegRate + "\t\t\t\t" + anyOverRep + "\n");
	}

	private double calcROCAUC(ArrayList<Double> posMaxScores, ArrayList<Double> negMaxScores, boolean printROC,
			WeightMatrix motif) {
		double auc = 0;
	...
	public void printBitPattern() {
		...
		for (int j = 0; j < motifs.size(); j++) {
			System.out.print("\t" + motifs.get(j).getName());
		}
		System.out.print("\n");
		for (int i = 0; i < posSet.size(); i++) {
	...
	public void setPrintROC(boolean pr) {
		printROCCurve = pr;
	}
	...
	// Multiple hypothesis testing correction -- assumes peaks ordered according
	// to p-value
	...
	protected class MotifStats {
		...
		public MotifStats(String name, double posTotal, double posHits, double posPeaks, double negTotal,
				double negHits, double negPeaks, double rocauc) {
			this.name = name;
			...
			// pvalHits = binomialSampleEquality(posHits, negHits, posTotal,
			// negTotal);
			pvalPeaks = binomialSampleEquality(posPeaks, negPeaks, posTotal, negTotal);
		}

		public void print() {
			System.out.println(name + "\t" + posTotal + "\t" + posHits + "\t" + posHitRate + "\t" + posPeaks + "\t"
					+ posPeaksRate + "\t" + negTotal + "\t" + negHits + "\t" + negHitRate + "\t" + negPeaks + "\t"
					+ negPeaksRate + "\t" + hitOverRep + "\t" + String.format("%.5e", pvalHits) + "\t" + peaksOverRep
					+ "\t" + String.format("%.5e", pvalPeaks) + "\t" + ROCAUC);
		}
	}
	...
	protected class IndexedDouble implements Comparable<IndexedDouble> {
		...
		public IndexedDouble(Integer i, Double v) {
			id = i;
			value = v;
		}
