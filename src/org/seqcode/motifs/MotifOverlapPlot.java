 ...
 * be presented along the x-axis of the plot. The command line arguments specify
 * a motif and a distance from the point (or center of region) within which an
 * occurence of the motif must fall. The y-value of the plot is the fraction of
 * regions near a motif; the fraction is computed over a sliding window of n
 * regions/points. The x-y values for the plot are produced on stdout.
 *
 * java MotifOverlapPlot --species "$MM;mm9" --wm "Cdx2;Jaspar" --distance 50
 * --smooth 100 < regions.txt > plot.txt
 */
...
public class MotifOverlapPlot {
	...
	public static void main(String args[]) throws Exception {
		...
		int numFound = 0;
		int found[] = new int[smooth]; // circular buffer for whether or not
										// there was a motif under a region
		...
		while ((line = reader.readLine()) != null) {
			...
			if (smooth != 0 && lineno >= smooth && lineno % smooth == 0) {
				System.out.println(String.format("%d\t%.2f", (lineno - smooth), (numFound / (double) smooth)));
			}
			if (r == null) {
			...
			if (end > genome.getChromLength(r.getChrom())) {
				end = genome.getChromLength(r.getChrom());
			}
			...
			for (WeightMatrix wm : matrices) {
				...
				WMHit hit = WeightMatrixScanner.scanSequenceBestHit(wm, minscore, string);
				if (hit != null) {
					anyhits = true;
					break;
				}
