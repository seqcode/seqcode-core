...
public class DiscriminativeKmers {
	...
	private int randombgcount = 1000, // number of random background regions to
										// pic
			randombgsize = 100, // size of random background regions
			parsedregionexpand; // expand input regions by this much on either
								// side
	private Map<String, char[]> foreground, background; // foreground and
	...
	/**
	 * returns a map from kmers to their frequency in the string.
	 */
	public static Map<Long, Integer> count(char[] chars, int k, long mask, Map<Long, Integer> map) {
		if (map == null) {
	...
	/**
	 * returns a long describing the best match between a and b. The upper int
	 * is the shift- the offset at which a best matched b. Positive shift means
	 * to shift a to the right such that the last characters of a and first
	 * characters of b are overhanging.The lower int is the number of bases in
	 * common.
	 */
	...
	/**
	 * first short is zero second short is one if reverse-complement was best
	 * third short is offset fourth short (LSB) is score
	 */
	...
	public static void paintMotif(WeightMatrix wm, String fname) throws IOException {
		...
		BufferedImage im = new BufferedImage(800, 200, BufferedImage.TYPE_INT_RGB);
	...
	public void printClusters(List<KmerCluster> clusters) {
		...
		for (KmerCluster cluster : clusters) {
			...
			wm.version = String.format("mfc %.2f expand %d size %d count %d", minfoldchange, parsedregionexpand,
					minclustersize, minclustercount);
			wm.type = "DiscriminativeKmers";

			CEResult scanresult = CompareEnrichment.doScan(wm, foreground, background, null, null, .1, .01, 2, .05, 1,
					null, null, false);
			if (scanresult.freqone > .1) {
				System.out.println("Cluster centroid " + longToString(cluster.centroid, k) + " and total count "
						+ cluster.totalCount());
				for (int i = 0; i < 4; i++) {
	...
	public void parseArgs(String args[]) throws NotFoundException, IOException, FileNotFoundException {
		...
		if (firstfname == null) {
			...
			foreground = CompareEnrichment.readRegions(genome, new BufferedReader(new InputStreamReader(System.in)),
			...
			if (firstfname.matches(".*\\.fasta") || firstfname.matches(".*\\.fa")) {
				...
				foreground = CompareEnrichment.readRegions(genome, new BufferedReader(new FileReader(firstfname)),
						parsedregionexpand, null);
			}
		}
		if (secondfname == null) {
			...
			background = CompareEnrichment.randomRegions(genome, randombgcount, randombgsize);
		} else {
			if (secondfname.matches(".*\\.fasta") || secondfname.matches(".*\\.fa")) {
				...
				background = CompareEnrichment.readRegions(genome, new BufferedReader(new FileReader(secondfname)),
						parsedregionexpand, null);
			}
		}
	}
	...
	public void run() {
		...
		for (long l : fgcounts.keySet()) {
			...
			if (fgprob > bgprob * minfoldchange) {
				...
				// before clustering, change count from absolute counts to count
				// above background
