...
public class MotifAnalysisSandbox {
	...
	// Added by akshay, to read and store more than one motif. The above motif
	// variable stores the first matrix in this list (not removed for
	// compatibility)
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		boolean printHits = false, printBestHits = false, printHitInfo = false, printPeakClosestHits = false,
				printSeqs = false, printSeqsNoMotifs = false, printclosesthitorientation = false;
		boolean printSeqsWithMotifs = false, printPeakInfo = false, printPeaksWithMotifs = false,
				printPeaksNoMotifs = false, printHitPoints = false;
		boolean printMotifScore = false, printMotifPerc = false, printRankMotifPerc = false, motifDistHist = false,
				screenMotifs = false, scoreSeqs = false;
		...
		// Added by akshay. Tools list for multiple motifs. The above tools
		// variable stores the first tool in this list (not removed for
		// compatibility)
		List<MotifAnalysisSandbox> toolsList = new ArrayList<MotifAnalysisSandbox>();

		ArgParser ap = new ArgParser(args);
		if ((!ap.hasKey("species") && !ap.hasKey("geninfo"))
				|| (!ap.hasKey("motiffile") && !ap.hasKey("weightMatfile") && !ap.hasKey("motifname"))) {
			System.err.println("Usage:\n " + "MotifAnalysisSandbox " + "--species <organism;genome> OR\n"
					...
					+ "--motifname <weightmatrix name> AND" + "--motifversion <weightmatrix version> \n"
					+ "--peaks <file containing coordinates of peaks> \n"
					+ "--wholegenome [motif analysis in whole genome]\n"
					+ "--win <window of sequence to take around peaks> " + "--minscore <minimum motif score> "
					+ "--motifthres <file with thresholds> " + "--threslevel <threshold level> "
					+ "--baseMotifScore <Min score to calculate summed LL>\n " + "\nOPTIONS:\n" + "--printhits "
					+ "--printbesthits " + "--printhitinfo " + "--printpeakclosesthits "
					+ "--printclosesthitorientation" + "--printHitPoints" + "--printseqs " + "--printseqsnomotifs "
					+ "--printseqswithmotifs " + "--screenmotifs " + "--printpeaksnomotifs " + "--printpeakswithmotifs "
					+ "--printpeakinfo " + "--printmotifscore " + "--printmotifperc " + "--printrankmotifperc "
					+ "--motifdensity " + "--motifdisthist \n" + "--motifhisto \n" + "--scoreseqs <seqFile> "
					+ "--pocc <seqFile>\n"
		...
		int bins = ap.hasKey("bins") ? new Integer(ap.getKeyValue("bins")).intValue() : 1; // Used
																							// by
																							// profile
																							// printing
		int binsize = ap.hasKey("binsize") ? new Integer(ap.getKeyValue("binsize")).intValue() : 10; // Used
																										// by
																										// motif
																										// histogram
		double minscore = (ap.hasKey("minscore")) ? new Double(ap.getKeyValue("minscore")).doubleValue() : -10000;
		double basemotifscore = (ap.hasKey("baseMotifScore"))
				? new Double(ap.getKeyValue("baseMotifScore")).doubleValue() : 0;
		...
		try {
			...
			if (havePeaks) {
				...
				while ((line = reader.readLine()) != null) {
					...
					String coord = curr[0];
					if (!curr[0].contains("#")) {
						if (curr.length >= 3 && curr[2].contains(":")) {
							coord = curr[2];
						}
			...
			for (WeightMatrix wm : matrixList) {
				toolsList.add(new MotifAnalysisSandbox(currgen, wm, matrixList, posLines, posPeaks, posRegs, minscore,
						basemotifscore, genomeSequencePath));
			}
			tools = new MotifAnalysisSandbox(currgen, matrix, matrixList, posLines, posPeaks, posRegs, minscore,
					basemotifscore, genomeSequencePath);
	...
	// Constructor
	public MotifAnalysisSandbox(Genome g, WeightMatrix wm, List<WeightMatrix> wmList, ArrayList<String> inL,
			ArrayList<Point> pospeak, ArrayList<Region> posreg, double minscore, double basescore,
			String genomeSequencePath) {
		gen = g;
		motif = wm;
		motifList = wmList;
		inFileLines = inL;
	...
	// Load freq matrix
	public static List<WeightMatrix> loadMotifFromFile(String filename, String backname, Genome gen)
	...
	// Find the best motif matches in the regions and report the distances to
	// the peaks
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			for (int z = 0; z < r.getWidth(); z++) {
				...
				if (currScore >= motifThres
						&& Math.abs((p.getLocation() - r.getStart()) - mpos) < Math.abs(closeDist)) {
	...
	public void printMotifHits() {
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			for (int z = 0; z < r.getWidth() - motif.length() + 1; z++) {
				...
				if (currScore >= motifThres) {
					...
					Region hitreg = new Region(gen, r.getChrom(), r.getStart() + z,
							r.getStart() + z + motif.length() - 1);
					hits.add(new ScoredStrandedRegion(gen, r.getChrom(), hitreg.getStart(), hitreg.getEnd(), currScore,
							profiler.getMaxStrand(z)));
					if (profiler.getMaxStrand(z) == '+') {
						hitseqs.add(subseq);
					} else {
						hitseqs.add(SequenceUtils.reverseComplement(subseq));
					}
				}
			}
			if (goodMotif) {
		...
		System.out.println(motif.name + " hits: " + numHits + " hits in " + peaksWithHits + " regions from "
				+ totalPeaks + " total peaks (" + perc + ").");
	...
	public void printBestMotifHits() {
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			if (bestMotifScore >= motifThres) {
				...
				Region hitreg = new Region(gen, r.getChrom(), r.getStart() + bestMotifIndex,
						r.getStart() + bestMotifIndex + motif.length() - 1);
				hits.add(new ScoredStrandedRegion(gen, r.getChrom(), hitreg.getStart(), hitreg.getEnd(), bestMotifScore,
						profiler.getMaxStrand(bestMotifIndex)));
				if (profiler.getMaxStrand(bestMotifIndex) == '+') {
					hitseqs.add(subseq);
				} else {
					hitseqs.add(SequenceUtils.reverseComplement(subseq));
				}
		...
		double perc = (double) peaksWithHits / (double) totalPeaks;
		System.err.println(motif.name + " hits: " + numHits + " hits in " + peaksWithHits + " regions from "
				+ totalPeaks + " total peaks (" + perc + ").");
		for (int h = 0; h < hits.size(); h++) {
			// System.out.println(hits.get(h)+"\t"+hitseqs.get(h)+"\t"+SequenceUtils.reverseComplement(hitseqs.get(h)));
			System.out.println(peaksWithHitsList.get(h).getLocationString() + "\t" + hits.get(h).toTabString() + "\t"
					+ hitseqs.get(h));
		}
	}
			...
			for (int m = 0; m < motifList.size(); m++) {
				...
				for (int z = 0; z < r.getWidth() - motifList.get(m).length() + 1; z++) {
					...
					if (currScore >= motifThres) {
						...
						Region hitreg = new Region(gen, r.getChrom(), r.getStart() + z,
								r.getStart() + z + motifList.get(m).length() - 1);
						hits.add(new ScoredStrandedRegion(gen, r.getChrom(), hitreg.getStart(), hitreg.getEnd(),
								currScore, profiler.getMaxStrand(z)));
						if (profiler.getMaxStrand(z) == '+') {
							hitseqs.add(subseq);
						} else {
							hitseqs.add(SequenceUtils.reverseComplement(subseq));
	...
	public void printPeakClosestMotifHits() {
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			if (bestMotifScore >= motifThres) {
				...
				Region hitreg = new Region(gen, r.getChrom(), r.getStart() + closestIndex,
						r.getStart() + closestIndex + motif.length() - 1);
				hits.add(new ScoredStrandedRegion(gen, r.getChrom(), hitreg.getStart(), hitreg.getEnd(), closestScore,
						profiler.getMaxStrand(closestIndex)));
				if (profiler.getMaxStrand(closestIndex) == '+') {
					hitseqs.add(subseq);
				} else {
					hitseqs.add(SequenceUtils.reverseComplement(subseq));
				}
			...
			while (rstart < r.getEnd()) {
				...
				for (int z = 0; z < cr.getWidth(); z++) {
					...
					if (currScore >= motifThres) {
						...
						goodMotif = true;
						String subseq = seq.substring(z, z + motif.length());
						Region hitreg = new Region(gen, r.getChrom(), cr.getStart() + z,
								cr.getStart() + z + motif.length() - 1);
						ScoredStrandedRegion ssr = new ScoredStrandedRegion(gen, r.getChrom(), hitreg.getStart(),
								hitreg.getEnd(), currScore, profiler.getMaxStrand(z));
						String currseq = null;
						if (profiler.getMaxStrand(z) == '+') {
							currseq = subseq;
						} else {
							currseq = SequenceUtils.reverseComplement(subseq);
						}
	...
	public void printMotifHitsAndPoints() {
		...
		double perc = (double) peaksWithHits / (double) totalPeaks;
		System.out.println(motif.name + " hits: " + numHits + " hits in " + peaksWithHits + " peaks from " + totalPeaks
				+ " total peaks (" + perc + ").");
		for (int h = 0; h < hits.size(); h++) {
			System.out.println(hitPoints.get(h) + "\t" + hits.get(h).toTabString() + "\t" + hitseqs.get(h) + "\t"
					+ SequenceUtils.reverseComplement(hitseqs.get(h)));
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			if (!goodMotif)
				System.out.println(words[0] + "\t" + words[1] + "\t" + words[2] + "\t" + words[3] + "\t"
						+ motif.getMinScore() + "\t" + bestSeq);
			else
				System.out.println(words[0] + "\t" + words[1] + "\t" + words[2] + "\t" + words[3] + "\t" + maxScore
						+ "\t" + bestSeq);
		}
	}
	...
	// Watch out for the minimum motif score in this; if the motif threshold is
	// below zero, the default will look funky
	public void printMotifScoreLandscape() {
		for (int i = 0; i < regions.size(); i++) {
			Region r = regions.get(i);
			double[] scores = new double[r.getWidth()];
			for (int j = 0; i < r.getWidth(); j++) {
				scores[j] = 0;
			}
	...
	public void scoreFastaFile(String inFile) {
		...
		System.out.println(motif.name + " hits: " + numHits + " hits in " + seqsWithHits + " peaks from " + totalSeqs
				+ " total peaks (" + perc + ").");
	}

	// Same as above except returning the probability of occupancy for each
	// sequence
	...
	public void printPeakInfo() {
		...
		for (int i = 0; i < peaks.size(); i++) {
			...
			int dist2Gene = closestGene.getName().equals("NONE") ? maxGeneDistance
					: Math.abs(p.getLocation() - closestGene.getFivePrime());
			String[] words = inFileLines.get(i).split("\\s+");
			System.out.println(words[0] + "\t" + words[1] + "\t" + words[2] + "\t" + words[3] + "\t"
					+ closestGene.getName() + "\t" + closestGene.getID() + "\t" + dist2Gene);
			// System.out.println(words[0]+"\t"+words[1]+"\t"+words[2]+"\t"+words[3]+"\t"+seq);
		}

	}

	// Arguments: a list of enriched peaks and an array of annotation source
	// names
	...
	private void printMotifDensity() {
		...
		while (chroms.hasNext()) {
			...
			for (int x = currentChrom.getStart(); x <= currentChrom.getEnd(); x += motifDensityStepSize) {
				...
				if (y >= currentChrom.getEnd()) {
					y = currentChrom.getEnd() - 1;
				}
	...
	public static HashMap<String, Double> loadThresholdsFromFile(String filename, double level) {
		...
		if (filename == null) {
			System.err.println("No threshold file specified");
		} else {
			int thresIndex = 3;
			try {
	...
	// Print motif-profiler style vectors -- percent-scores at binwidth
	// resolution
