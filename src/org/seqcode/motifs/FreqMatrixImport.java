...
java org.seqcode.gse.shaun.FreqMatrixImport --species 'Saccharomyces cerevisiae' --fmname HSF1 --fmversion TRANSFAC11.3 --fmtype TRANFSAC --fmfile HSF1.motif --backfile yeast.back
...
public class FreqMatrixImport {
	...
	public static void main(String args[]) throws IOException, ParseException, NotFoundException {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].equals("--") || args[i].equals("--fmfile")) {
				fmfile = args[++i];
			}
		}

		if (species == null) {
			System.err.println("Must supply a --species");
			System.exit(1);
		}
		if (fmfile == null) {
		...
		if (backfile == null && genome != null) {
			...
			System.err.println("Must supply --backfile or --genome");
			System.exit(1);
		} else {
	...
	public void setBackground(MarkovBackgroundModel b) {
		back = b;
	}

	// Does not convert the freq matrix into a weight matrix; loads multiple
	// motifs
	public static List<WeightMatrix> readTransfacMatricesAsFreqMatrices(String freqFile)
	...
	// Unlike the WeigthMatrixImport version, this one actually loads more than
	// one matrix!
	public static LinkedList<WeightMatrix> readTransfacMatrices(String wmfile, String version) throws IOException {
			...
			if (line.length() > 0) {
				...
				if (pieces[0].equals("DE")) {
					...
					if (pieces.length >= 3) {
						...
						if (version != null) {
							versions.add(new String(v_string + "," + version));
						} else {
							versions.add(v_string);
						}
					} else {
		...
		for (int m = 0; m < motifCount; m++) {
			...
			for (int i = 0; i < arrayLens.get(m); i++) {
				float ttl = 0;
				for (int j = 0; j < 4; j++) {
					ttl += arrays.get(m)[i][j];
				}
				float currScale = SCALE_FACTOR * ttl;
				for (int j = 0; j < 4; j++) {
					matrix.matrix[i][indices[j]] = (float) (Math
							.log((((arrays.get(m)[i][j] + (currScale * back.getMarkovProb(j, 1))) / (ttl + currScale))
									/ back.getMarkovProb(j, 1)))
							/ Math.log(2));
				}
			}
			matrices.add(matrix);
	...
	/*
	 * reads a freq matrix from the specified file, inserts it into the database
	 * with the specified name and version, and returns its dbid. This means
	 * that the file may contain only a single weight matrix
	 */
	public static int insertFMFromFile(String species, String fmname, String fmversion, String fmtype, String fmfile)
	 ...
	 * @throws ParseException
	 */
	public static LinkedList<WeightMatrix> readAlignedSequenceMatrices(String fname)
